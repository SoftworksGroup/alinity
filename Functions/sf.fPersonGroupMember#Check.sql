...
GO
CREATE FUNCTION [sf].[fPersonGroupMember#Check]
	(
	...
	,@ExpiryTime                     datetime
	,@IsReplacementRequiredAfterTerm bit
	,@ReplacementClearedDate         date
	,@PersonGroupMemberXID           varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : sf.fPersonGroupMember#Check
...
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.PersonGroupSID The %1 is marked inactive. Assign an active %1.
$AutoRules | Tim Edlund        ValueIsRequired.ReplacementClearedDate A value for "%1" is required.
...
begin
	...
	begin
		if (select x.IsActive from sf.PersonGroup x where x.PersonGroupSID = @PersonGroupSID) = @OFF		-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.PersonGroupSID'
			set @columnNames        = N'PersonGroupSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'person group'
		end
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonGroupMember','ValueIsRequired.ReplacementClearedDate') = @ON and @ReplacementClearedDate is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ReplacementClearedDate'
		set @columnNames        = N'ReplacementClearedDate'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Replacement Cleared Date'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonGroupMember','ValueIsRequired.Title') = @ON and @Title is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonGroupMember','BackDatingLimit.EffectiveTime') = @ON
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonGroupMember','FutureDatingLimit.ExpiryTime') = @ON
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'PersonGroupMember','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
