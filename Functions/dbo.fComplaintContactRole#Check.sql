...
GO
CREATE FUNCTION [dbo].[fComplaintContactRole#Check]
	(
	 @ComplaintContactRoleSID  int
	,@ComplaintContactRoleSCD  varchar(20)
	,@ComplaintContactRoleName nvarchar(50)
	,@IsDefault                bit
	,@ComplaintContactRoleXID  varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fComplaintContactRole#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.ComplaintContactRoleSCD The "%1" must be uppercase and not contain spaces or an
...
begin
	...
	if @errorMessageSCD is null and @ComplaintContactRoleSCD is not null and (sf.fIsStringContentValid(@ComplaintContactRoleSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@ComplaintContactRoleSCD) <> @ComplaintContactRoleSCD collate Latin1_General_CS_AI)			-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.ComplaintContactRoleSCD'
		set @columnNames        = N'ComplaintContactRoleSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Complaint Contact Role System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ComplaintContactRole','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fComplaintContactRole#Check'
		)
		begin
		
			select @errorText = ext.fComplaintContactRole#Check
				(
				 @ComplaintContactRoleSID
				,@ComplaintContactRoleSCD
				,@ComplaintContactRoleName
