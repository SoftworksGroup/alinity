...
GO
CREATE FUNCTION [dbo].[fGLAccount#Check]
	(
	 @GLAccountSID              int
	,@GLAccountCode             varchar(50)
	,@GLAccountLabel            nvarchar(35)
	,@IsRevenueAccount          bit
	,@IsBankAccount             bit
	,@IsTaxAccount              bit
	,@IsPAPAccount              bit
	,@IsUnappliedPaymentAccount bit
	,@DeferredGLAccountCode     varchar(50)
	,@IsActive                  bit
	,@GLAccountXID              varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        ValueIsRequired.DeferredGLAccountCode A value for "%1" is required.
Tim Edlund | Sep 2017          MBR.PAPGLAccount (see source for message text)
Tim Edlund | Sep 2017          MBR.PAPGLAccount (see source for message text)
Tim Edlund | Nov 2017          DuplicateGLAccount.IsPAPAccount'; (see source for message text)
Tim Edlund | Nov 2017          DuplicateGLAccount.IsUnappliedPaymentAccount'; (see source for message text)
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'GLAccount','ValueIsRequired.DeferredGLAccountCode') = @ON and @DeferredGLAccountCode is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.DeferredGLAccountCode'
		set @columnNames        = N'DeferredGLAccountCode'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Deferred GLAccount Code'
	end
	--!</Rule>
	
	--!<Rule Author="Tim Edlund | Sep 2017" Updates="None">
	if @errorMessageSCD is null and @IsPAPAccount = @ON and @IsBankAccount = @OFF
	begin
		set @errorMessageSCD      = 'MBR.PAPGLAccount'
		set @columnNames          = N'IsPAPAccount'
		set @defaultMessageText   = N'The GL Account set for Pre-Authorized Payment must also be a bank account.'
	end
	--!</Rule>
	
	--!<Rule Author="Tim Edlund | Sep 2017" Updates="None">
	if @errorMessageSCD is null and @IsUnappliedPaymentAccount = @ON and (@IsBankAccount = @ON or @IsRevenueAccount = @ON)
	begin
		set @errorMessageSCD      = 'MBR.PAPGLAccount'
		set @columnNames          = N'IsUnappliedPaymentAccount'
		set @defaultMessageText   = N'The GL Account set for Unapplied Payments should not be a bank account or revenue account. This is normally a liability account defined in your General Ledger (credit balance).'
	end
	--!</Rule>
	
	--!<Rule Author="Tim Edlund | Nov 2017" Updates="None">
	if @errorMessageSCD is null and @IsPAPAccount = @ON
	begin
		if(select
					count(1)
				from
					dbo.GLAccount ga
				where
					ga.IsPAPAccount = @ON) > 1
		begin
			set @errorMessageSCD = 'DuplicateGLAccount.IsPAPAccount';
			set @columnNames = N'IsPAPAccount';
			set @defaultMessageText = N'Only 1 account can be marked as the %1 account.';
			set @arg1 = N'pre-authorized payment';
		end;
	end;
	--!</Rule>
	
	--!<Rule Author="Tim Edlund | Nov 2017" Updates="None">
	if @errorMessageSCD is null and @IsUnappliedPaymentAccount = @ON
	begin
		if( select
					count(1)
				from
					dbo.GLAccount ga
				where
					ga.IsUnappliedPaymentAccount = @ON) > 1
		begin
			set @errorMessageSCD = 'DuplicateGLAccount.IsUnappliedPaymentAccount';
			set @columnNames = N'IsUnappliedPaymentAccount';
			set @defaultMessageText = N'Only 1 account can be marked as the %1 account.';
			set @arg1 = N'unapplied payment';
		end;
	end;
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'GLAccount','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fGLAccount#Check'
		)
		begin
		
			select @errorText = ext.fGLAccount#Check
				(
				 @GLAccountSID
				,@GLAccountCode
				,@GLAccountLabel
