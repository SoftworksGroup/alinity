...
GO
CREATE FUNCTION [dbo].[fExamQuestionChoice#Check]
	(
	 @ExamQuestionChoiceSID int
	,@ExamQuestionSID       int
	,@Sequence              tinyint
	,@IsCorrectAnswer       bit
	,@IsActive              bit
	,@ExamQuestionChoiceXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fExamQuestionChoice#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ExamQuestionChoice','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fExamQuestionChoice#Check'
		)
		begin
		
			select @errorText = ext.fExamQuestionChoice#Check
				(
				 @ExamQuestionChoiceSID
				,@ExamQuestionSID
				,@Sequence
