...
GO
CREATE FUNCTION [dbo].[fReinstatement#Check]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fReinstatement#Check
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Reinstatement','ValueIsRequired.InvoiceSID') = @ON and @InvoiceSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Reinstatement','ValueIsRequired.LastValidateTime') = @ON and @LastValidateTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Reinstatement','ValueIsRequired.NextFollowUp') = @ON and @NextFollowUp is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Reinstatement','ValueIsRequired.ReasonSID') = @ON and @ReasonSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Reinstatement','ValueIsRequired.RegistrationEffective') = @ON and @RegistrationEffective is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Reinstatement','BackDatingLimit.RegistrationEffective') = @ON
	...
	begin
		...
		(
			select
				1
			from
				dbo.fReinstatement#CurrentStatus(@ReinstatementSID, -1) cs
			where
...
-- SQL Prompt formatting on
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'Reinstatement','?SomeMessageCode.ColumnName') = 1							-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fReinstatement#Check'
		)
		begin
		
			select @errorText = ext.fReinstatement#Check
				(
				 @ReinstatementSID
				,@RegistrationSID
				,@PracticeRegisterSectionSID
