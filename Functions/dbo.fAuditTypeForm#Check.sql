...
GO
CREATE FUNCTION [dbo].[fAuditTypeForm#Check]
	(
	 @AuditTypeFormSID   int
	,@AuditTypeSID       int
	,@FormSID            int
	,@IsReviewForm       bit
	,@AuditTypeFormXID   varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fAuditTypeForm#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'AuditTypeForm','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fAuditTypeForm#Check'
		)
		begin
		
			select @errorText = ext.fAuditTypeForm#Check
				(
				 @AuditTypeFormSID
				,@AuditTypeSID
				,@FormSID
