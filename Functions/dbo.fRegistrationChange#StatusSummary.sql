...
GO
CREATE FUNCTION dbo.fRegistrationChange#StatusSummary
(
	@RegistrationYear smallint	-- the registration year to include in the analysis
)
returns table
/*********************************************************************************************************************************
Function: Registration Change - Status Summary
Notice  : Copyright © 2018 Softworks Group Inc.
Summary	: Returns reporting/statistical data on Registration Changes (excluding WITHDRAWN)
----------------------------------------------------------------------------------------------------------------------------------
...
Comments	
--------
This function supports reporting on the status of Registration Changes in a given registration year. @RegistrationYear is a 
...
Registration Changes 3 next-to-act parties  are possible:  Member, Administrator and None (form is closed status).

Note that any Registration Changes in the "WITHDRAWN" status are explicitly excluded from the statistics returned.
...
cross apply dbo.fRegistrationChange#CurrentStatus(frm.RegistrationChangeSID, -1) cs
where
...
begin
	...
	from
		dbo.fRegistrationChange#StatusSummary(@registrationYear) x
	order by
		 x.NextToActGroup
		,x.FormStatusSequence;
...
exec sf.pUnitTest#Execute
	 @ObjectName = 'dbo.fRegistrationChange#StatusSummary'
	,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
as
return
(
	...
	(
		select
		 ...
		 ,count(1) FormCount
		from
			dbo.fRegistrationChange#CurrentStatus(-1, @RegistrationYear) cs
		where
			cs.FormStatusSCD <> 'WITHDRAWN'
		group by
			cs.FormStatusSCD
