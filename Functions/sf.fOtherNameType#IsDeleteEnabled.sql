...
CREATE FUNCTION [sf].[fOtherNameType#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : sf.fOtherNameType#IsDeleteEnabled
...
select top (10)
   x.OtherNameTypeSID
  ,sf.fOtherNameType#IsDeleteEnabled(x.OtherNameTypeSID) IsDeleteEnabled
from
  sf.OtherNameType x
order by
  newid()
...
begin
	...
	if (select x.IsDefault from sf.OtherNameType x where x.OtherNameTypeSID = @OtherNameTypeSID) = @ON set @isDeleteEnabled = @OFF-- default records cannot be deleted
	...
	begin
		...
		(
			select
				  count(x01.PersonOtherNameSID)
			from
				sf.OtherNameType   x00
			left outer join
				  (select top (1) x.PersonOtherNameSID, x.OtherNameTypeSID from sf.PersonOtherName x where x.OtherNameTypeSID = @OtherNameTypeSID) x01 on x00.OtherNameTypeSID =  x01.OtherNameTypeSID
			where
				x00.OtherNameTypeSID = @OtherNameTypeSID
	...
	begin
		...
		(
			...
			from
				sf.OtherNameType x
			where
				x.OtherNameTypeSID = @OtherNameTypeSID
