...
GO
CREATE FUNCTION [sf].[fJob#Check]
	(
	 @JobSID                     int
	,@JobSCD                     varchar(132)
	,@JobLabel                   nvarchar(35)
	,@IsCancelEnabled            bit
	,@IsParallelEnabled          bit
	,@IsFullTraceEnabled         bit
	,@IsAlertOnSuccessEnabled    bit
	,@JobScheduleSID             int
	,@JobScheduleSequence        int
	,@IsRunAfterPredecessorsOnly bit
	,@MaxErrorRate               int
	,@MaxRetriesOnFailure        tinyint
	,@IsActive                   bit
	,@JobXID                     varchar(150)
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'Job','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
