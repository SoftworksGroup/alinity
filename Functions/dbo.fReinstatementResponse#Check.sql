...
GO
CREATE FUNCTION [dbo].[fReinstatementResponse#Check]
	(
	 @ReinstatementResponseSID int
	,@ReinstatementSID         int
	,@FormOwnerSID             int
	,@ReinstatementResponseXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fReinstatementResponse#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ReinstatementResponse','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fReinstatementResponse#Check'
		)
		begin
		
			select @errorText = ext.fReinstatementResponse#Check
				(
				 @ReinstatementResponseSID
				,@ReinstatementSID
				,@FormOwnerSID
