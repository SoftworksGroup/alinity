...
GO
CREATE FUNCTION dbo.fExamOffering#Check
(
   @ExamOfferingSID      int
  ,@ExamSID              int
  ,@OrgSID               int
  ...
  ,@VendorExamOfferingID varchar(25)
  ,@ExamOfferingXID      varchar(150)
...
/*********************************************************************************************************************************
ScalarF : dbo.fExamOffering#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.BookingCutOffDate A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.CatalogItemSID A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.ExamTime A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.SeatingCapacity A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.VendorExamOfferingID A value for "%1" is required.
...
-------------------------------------------------------------------------------------------------------------------------------- */
  begin
    ...
    -- a constant to reduce repetitive cast syntax in bit comparisons
    ...
    begin
      if datediff(second,@CreateTime,sysdatetimeoffset()) <= 2 -- record is only 2 seconds old - assume INSERT
      begin
        set @isInsert = @ON;
    ...
    begin
      ...
      (
        select 
           x.IsActive from 
          dbo.Org x where x.OrgSID = @OrgSID
      ) = @OFF -- and parent row is inactive
      begin
    ...
    if @errorMessageSCD is null
       and sf.fBusinessRuleIsEnforced(N'dbo',N'ExamOffering','ValueIsRequired.BookingCutOffDate') = @ON
    and @BookingCutOffDate is null
    begin
      set @errorMessageSCD = 'ValueIsRequired.BookingCutOffDate';
      set @columnNames = N'BookingCutOffDate';
      set @defaultMessageText = N'A value for "%1" is required.';
      set @arg1 = N'Booking Cut Off Date';
    end;

    --!</Rule>
    --!<Rule Author="$AutoRules | Tim Edlund">
    if @errorMessageSCD is null
       and sf.fBusinessRuleIsEnforced(N'dbo',N'ExamOffering','ValueIsRequired.CatalogItemSID') = @ON
    and @CatalogItemSID is null
    begin
      set @errorMessageSCD = 'ValueIsRequired.CatalogItemSID';
      set @columnNames = N'CatalogItemSID';
      set @defaultMessageText = N'A value for "%1" is required.';
      set @arg1 = N'Catalog Item';
    end;

    --!</Rule>
    --!<Rule Author="$AutoRules | Tim Edlund">
    if @errorMessageSCD is null
       and sf.fBusinessRuleIsEnforced(N'dbo',N'ExamOffering','ValueIsRequired.ExamTime') = @ON
    and @ExamTime is null
    begin
    ...
    --!<Rule Author="$AutoRules | Tim Edlund">
    if @errorMessageSCD is null
       and sf.fBusinessRuleIsEnforced(N'dbo',N'ExamOffering','ValueIsRequired.SeatingCapacity') = @ON
    and @SeatingCapacity is null
    begin
      set @errorMessageSCD = 'ValueIsRequired.SeatingCapacity';
      set @columnNames = N'SeatingCapacity';
      set @defaultMessageText = N'A value for "%1" is required.';
      set @arg1 = N'Seating Capacity';
    end;

    --!</Rule>
    --!<Rule Author="$AutoRules | Tim Edlund">
    if @errorMessageSCD is null
       and sf.fBusinessRuleIsEnforced(N'dbo',N'ExamOffering','ValueIsRequired.VendorExamOfferingID') = @ON
    and @VendorExamOfferingID is null
    begin
      set @errorMessageSCD = 'ValueIsRequired.VendorExamOfferingID';
      set @columnNames = N'VendorExamOfferingID';
      set @defaultMessageText = N'A value for "%1" is required.';
      set @arg1 = N'Vendor Exam Offering ID';
    end;

    --!</Rule>
    --!<Rule Author="Tim Edlund | Apr 2019" Updates="None">
    if @errorMessageSCD is null
       and @BookingCutOffDate > cast(@ExamTime as date)
    begin
      set @errorMessageSCD = 'MBR.DateWrongOrder.BookingCutoffDate';
      set @columnNames = N'BookingCutoffDate,ExamTime';
      set @defaultMessageText = N'The %1 cannot be after the %2.';
      set @arg1 = N'booking cut-off date';
      set @arg2 = N'exam time';
    end;

    --!</Rule>
    --!<Rule Author="?Template | Apr 2019" Updates="None">
    if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ExamOffering','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
       and 1 = 0
    begin
    ...
    begin
      ...
      (
        ...
        from     
          sf.vRoutine r
        where r.SchemaName = 'ext'
              and r.RoutineName = 'fExamOffering#Check'
      )
      begin
        select 
           @errorText = ext.fExamOffering#Check(@ExamOfferingSID,@ExamSID,@OrgSID,@ExamTime,@SeatingCapacity,@CatalogItemSID,@BookingCutOffDate,@VendorExamOfferingID,@ExamOfferingXID,@LegacyKey,@IsDeleted,@CreateUser,@CreateTime,@UpdateUser,@UpdateTime,@RowGUID);
      end;
    end;
    ...
    if @errorText not like N'<err>%'
       and @errorMessageSCD is not null
    begin
      set @errorText = sf.fCheckConstraintErrorString(@errorMessageSCD,@defaultMessageText,@columnNames,@ExamOfferingSID,@arg1,@arg2,@arg3,@arg4,@arg5);
    end;
