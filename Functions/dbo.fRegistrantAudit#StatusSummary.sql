...
GO
CREATE FUNCTION dbo.fRegistrantAudit#StatusSummary
(
	@RegistrationYear smallint	-- the registration year to include in the analysis
)
returns table
/*********************************************************************************************************************************
Function: Audit - Status Summary
Notice  : Copyright © 2018 Softworks Group Inc.
Summary	: Returns reporting/statistical data on Audits (excluding WITHDRAWN)
----------------------------------------------------------------------------------------------------------------------------------
...
Comments	
--------
This function supports reporting on the status of Audits in a given registration year. @RegistrationYear is a 
...
Audits 3 next-to-act parties  are possible:  Member, Administrator and None (form is closed status).

Note that any Audits in the "WITHDRAWN" status are explicitly excluded from the statistics returned.
...
cross apply dbo.fRegistrantAudit#CurrentStatus(frm.RegistrantAuditSID, -1) cs
where
...
begin
	...
	from
		dbo.fRegistrantAudit#StatusSummary(@registrationYear) x
	order by
		 x.NextToActGroup
		,x.FormStatusSequence;
...
exec sf.pUnitTest#Execute
	 @ObjectName = 'dbo.fRegistrantAudit#StatusSummary'
	,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
as
return
(
	...
	(
		select
		 ...
		 ,count(1) FormCount
		from
			dbo.fRegistrantAudit#CurrentStatus(-1, @RegistrationYear) cs
		where
			cs.FormStatusSCD <> 'WITHDRAWN'
		group by
			cs.FormStatusSCD
