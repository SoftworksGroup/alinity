...
GO
CREATE FUNCTION dbo.fRegistrantRenewal#Ext (@RegistrantRenewalSID int) -- key of record to check
returns @registrantRenewal#Ext table
(
	IsViewEnabled									 bit							 not null -- indicates whether either the (logged in) user or administrator can view the renewal
 ...
 ,IsWithdrawalEnabled						 bit							 not null -- indicates the renewal form can be withdrawn by administrators or SA's
 ...
 ,FormStatusSID									 int							 not null -- key of current/latest renewal status 
 ,FormStatusSCD									 varchar(25)			 not null -- current/latest renewal status		
 ,FormStatusLabel								 nvarchar(35)			 not null -- user-friendly name for the renewal status		
 ...
 ,TotalDue											 decimal(11, 2)		 null			-- amount owing on invoice associated with the renewal (blank if no invoice created)
 ,IsUnPaid											 bit							 not null -- indicates if the invoice associated with the renewal is unpaid
 ...
 ,CitySID												 int							 null
 ,PersonSID											 int							 not null -- key of person associated with the renewal
 ...
 ,PracticeRegisterLabel					 nvarchar(35)			 not null -- label (short name) of the register this renewal is being made to
 ,PracticeRegisterName					 nvarchar(65)			 not null -- name of the register this renewal is being made to
 ,RegistrantRenewalLabel				 nvarchar(80)			 not null -- a summary label for the renewal based on the register label and renewal status
-- Components unique to renewal process:
 ,IsRegisterChange							 bit							 not null -- indicates if this renewal involves a change in register from the originating registration
 ,IsAutoApprovalBlockedOnProfile bit							 not null -- indicates if renewal auto approval is blocked on the profile for this register
 ,HasOpenAudit									 bit							 not null -- indicates if the registrant has an open audit - normally blocks renewal
 ,IsRenewalOpen									 bit							 not null -- indicates if the renewal period is still open for saving (+30 min grace period included)
)
as
/*********************************************************************************************************************************
TableF	: Registrant Application Extended Columns
Notice	: Copyright © 2017 Softworks Group Inc.
Summary	: Returns a table of calculated columns for the RegistrantRenewal extended view (vRegistrantRenewal#Ext)
...
This function is called by the dbo.vRegistrantRenewal#Ext view to return a series of calculated values. By using a table function,
...
cross apply dbo.fRegistrantRenewal#Ext(x.RegistrantRenewalSID) rrx;
...
exec sf.pUnitTest#Execute
	@ObjectName = 'dbo.fRegistrantRenewal#Ext'
	,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
...
begin
	...
	declare
	 ...
	 ,@isViewEnabled									bit							 = cast(0 as bit)												-- indicates whether either the (logged in) user or administrator can view the renewal
	 ...
	 ,@isWithdrawalEnabled						bit							 = cast(0 as bit)												-- indicates the renewal form can be withdrawn by administrators or SA's
	 ...
	 ,@formStatusSID									int																											-- key of current/latest renewal status 
	 ,@formStatusSCD									varchar(25)																							-- current/latest renewal status		
	 ,@formStatusLabel								nvarchar(35)																						-- user-friendly name for the renewal status		
	 ...
	 ,@applicationUserSID							int							 = sf.fApplicationUserSessionUserSID()	-- key of currently logged in user - to enable read/write on renewal
	 ,@isAutoApprovalBlockedOnProfile bit																											-- indicates if renewal auto approval is blocked on the profile for this register
	 ,@hasOpenAudit										bit																											-- indicates if the registrant has an open audit - normally blocks renewal
	 ,@totalDue												decimal(11, 2)																					-- amount owing on invoice associated with the renewal (blank if no invoice created)
	 ,@totalPaid											decimal(11, 2)																					-- amount paid on the renewal invoice (blank if no invoice created)
	 ,@isUnPaid												bit							 = cast(0 as bit)												-- indicates if the invoice associated with the renewal is unpaid
	 ,@personMailingAddressSID				int																											-- key of the registrant's current mailing address (required for updates through the form)
	 ,@applicationEntitySID						int																											-- key of the renewal entity; required to check for generated PDF
	 ...
	 ,@practiceRegisterLabel					nvarchar(35)																						-- label (short name) of the register this renewal is being made to
	 ,@practiceRegisterName						nvarchar(65)																						-- name of the register this renewal is being made to
	 ,@registrantRenewalLabel					nvarchar(80)																						-- a summary label for the renewal based on the register label and renewal status
	 ,@isRegisterChange								bit							 = cast(0 as bit)												-- indicates if this renewal involves a change in register from the originating registration
	 ,@renewalEndTime									datetime																								-- time when renewal period has ended	(or when the registrants renewal is extended till)
	 ,@effectiveTime									datetime																								-- date and time the renewed registration becomes/was active
	 ,@isRenewalOpen									bit							 = cast(0 as bit);											-- indicates if the renewal period is still open for saving (+30 min grace period included)

	set @isAdminLoggedIn = sf.fIsGrantedToUserSID('ADMIN.RENEWAL', @applicationUserSID);

	-- retrieve the current status of the renewal and set
	...
	select
	 ...
	 ,@isAutoApprovalBlockedOnProfile = r.IsRenewalAutoApprovalBlocked
	 ...
	 ,@renewalEndTime									= (case
																				 when r.RenewalExtensionExpiryTime is not null and r.RenewalExtensionExpiryTime > rsy.RenewalEndTime then r.RenewalExtensionExpiryTime
	...
	outer apply dbo.fRegistrantRenewal#CurrentStatus(frm.RegistrantRenewalSID, -1) cs -- function determines the current status of the form
	where
		frm.RegistrantRenewalSID = @RegistrantRenewalSID;

	set @registrantRenewalLabel += N' (' + @registrationYearLabel + N')';
	...
	if sf.fNow() < dateadd(minute, 30, @renewalEndTime)
	begin
	...
	begin
		...
		where
			pdc.EntitySID = @RegistrantRenewalSID and pdc.IsPrimary = @ON and pdc.ApplicationEntitySID = @applicationEntitySID;
	...
	(
	 ...
	 ,RegistrantRenewalLabel
	 ,IsRegisterChange
	 ,IsAutoApprovalBlockedOnProfile
	 ,HasOpenAudit
	 ,IsRenewalOpen
	)
	select
