...
GO
CREATE FUNCTION [dbo].[fRegion#Check]
	(
	 @RegionSID          int
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'Region','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegion#Check'
		)
		begin
		
			select @errorText = ext.fRegion#Check
				(
				 @RegionSID
				,@RegionLabel
				,@RegionName
