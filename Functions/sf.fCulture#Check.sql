...
GO
CREATE FUNCTION [sf].[fCulture#Check]
	(
	 @CultureSID         int
	,@CultureSCD         varchar(10)
	,@CultureLabel       nvarchar(35)
	,@IsDefault          bit
	,@IsActive           bit
	,@CultureXID         varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.CultureSCD The "%1" must be uppercase and not contain spaces or any punctuation
...
begin
	...
	if @errorMessageSCD is null and @CultureSCD is not null and (sf.fIsStringContentValid(@CultureSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@CultureSCD) <> @CultureSCD collate Latin1_General_CS_AI)			-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.CultureSCD'
		set @columnNames        = N'CultureSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Culture System Code'
	end
	--!</Rule>
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'Culture','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
