...
GO
CREATE FUNCTION [dbo].[fRegistrantPractice#Check]
	(
	...
	,@OtherJurisdiction      nvarchar(100)
	,@OtherJurisdictionHours int
	,@TotalPracticeHours     int
	,@OrgSID                 int
	,@InsurancePolicyNo      varchar(25)
	,@InsuranceAmount        decimal(11,2)
	,@InsuranceCertificateNo varchar(25)
	,@RegistrantPracticeXID  varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantPractice#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.EmploymentStatusSID The %1 is marked inactive. Assign an active %1.
...
$AutoRules | Tim Edlund        ValueIsRequired.InsuranceCertificateNo A value for "%1" is required.
...
$AutoRules | Tim Edlund        ValueIsRequired.OtherJurisdiction A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.PlannedRetirementDate A value for "%1" is required.
Russell Poirier | May 2017     OutOfRange.RegistrationYear (see source for message text)
Tim Edlund | Mar 2019          MBR.RequiredWhen.OrgSID'; (see source for message text)
Tim Edlund | Mar 2019          MBR.RequiredWhen.InsurancePolicyNo'; (see source for message text)
Tim Edlund | Mar 2019          MBR.InsuranceCertificateRequired.InsuranceCertificateNo'; (see source for message text)
...
begin
	...
	begin
		if (select x.IsActive from dbo.EmploymentStatus x where x.EmploymentStatusSID = @EmploymentStatusSID) = @OFF							-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.EmploymentStatusSID'
			set @columnNames        = N'EmploymentStatusSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'employment status'
		end
	end
	--!</Rule>
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantPractice','ValueIsRequired.InsuranceAmount') = @ON and @InsuranceAmount is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantPractice','ValueIsRequired.InsuranceCertificateNo') = @ON and @InsuranceCertificateNo is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.InsuranceCertificateNo'
		set @columnNames        = N'InsuranceCertificateNo'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Insurance Certificate No'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantPractice','ValueIsRequired.InsurancePolicyNo') = @ON and @InsurancePolicyNo is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantPractice','ValueIsRequired.OrgSID') = @ON and @OrgSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantPractice','ValueIsRequired.OtherJurisdiction') = @ON and @OtherJurisdiction is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.OtherJurisdiction'
		set @columnNames        = N'OtherJurisdiction'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Other Jurisdiction'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantPractice','ValueIsRequired.PlannedRetirementDate') = @ON and @PlannedRetirementDate is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.PlannedRetirementDate'
		set @columnNames        = N'PlannedRetirementDate'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Planned Retirement Date'
	end
	--!</Rule>
	
	--!<Rule Author="Russell Poirier | May 2017" Updates="None">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantPractice','OutOfRange.RegistrationYear') = @ON
	begin
		
		declare
			@todayYear						smallint = sf.fTodayYear()

		if @RegistrationYear < (@todayYear - 10) or @RegistrationYear > (@todayYear + 1)
		begin
			set @errorMessageSCD    = 'OutOfRange.RegistrationYear'
			set @columnNames        = N'RegistrationYear'
			set @defaultMessageText = N'%1 is out of range. Accepted values are from: %3 to %4.'
			set @arg1               = cast(@RegistrationYear as nvarchar(4))
			set @arg2               = N'Registration Year'
			set @arg3               = cast(@todayYear - 10 as nvarchar(4))
			set @arg4               = cast(@todayYear + 1 as nvarchar(4))
		end
	end
	--!</Rule>
	
	--!<Rule Author="Tim Edlund | Mar 2019" Updates="None">
	if @errorMessageSCD is null and @InsurancePolicyNo is not null and @OrgSID is null
	begin
	...
	--!<Rule Author="Tim Edlund | Mar 2019" Updates="None">
	if @errorMessageSCD is null and @InsurancePolicyNo is null and @OrgSID is not null
	begin
	...
	--!<Rule Author="Tim Edlund | Mar 2019" Updates="None">
	if @errorMessageSCD is null and @isInsert = @ON and @InsurancePolicyNo is not null and @InsuranceCertificateNo is null
	begin
		...
		(
			...
			from
				dbo.Org o
			where
				o.InsurancePolicyNo = @InsurancePolicyNo and o.OrgSID = @OrgSID and o.IsInsuranceCertificateRequired = @ON
		)
		begin
			set @errorMessageSCD = 'MBR.InsuranceCertificateRequired.InsuranceCertificateNo';
			set @columnNames = N'InsuranceCertificateNo';
			set @defaultMessageText = N'An insurance certificate number is required for this policy.';
		end;
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantPractice','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantPractice#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantPractice#Check
				(
				 @RegistrantPracticeSID
				,@RegistrantSID
				,@RegistrationYear
