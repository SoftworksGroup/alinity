...
GO
CREATE FUNCTION [dbo].[fExam#Check]
	(
	 @ExamSID                   int
	,@ExamName                  nvarchar(50)
	,@ExamCategory              nvarchar(65)
	,@PassingScore              int
	,@EffectiveTime             datetime
	,@ExpiryTime                datetime
	,@IsOnlineExam              bit
	,@IsEnabledOnPortal         bit
	,@Sequence                  int
	,@CultureSID                int
	,@LastVerifiedTime          datetimeoffset(7)
	,@MinLagDaysBetweenAttempts smallint
	,@MaxAttemptsPerYear        tinyint
	,@VendorExamID              varchar(25)
	,@ExamXID                   varchar(150)
...
$AutoRules | Tim Edlund        ValueIsRequired.ExamCategory A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.LastVerifiedTime A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.MaxAttemptsPerYear A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.MinLagDaysBetweenAttempts A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.PassingScore A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.VendorExamID A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Exam','ValueIsRequired.ExamCategory') = @ON and @ExamCategory is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ExamCategory'
		set @columnNames        = N'ExamCategory'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Exam Category'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Exam','ValueIsRequired.LastVerifiedTime') = @ON and @LastVerifiedTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Exam','ValueIsRequired.MaxAttemptsPerYear') = @ON and @MaxAttemptsPerYear is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.MaxAttemptsPerYear'
		set @columnNames        = N'MaxAttemptsPerYear'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Max Attempts Per Year'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Exam','ValueIsRequired.MinLagDaysBetweenAttempts') = @ON and @MinLagDaysBetweenAttempts is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.MinLagDaysBetweenAttempts'
		set @columnNames        = N'MinLagDaysBetweenAttempts'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Min Lag Days Between Attempts'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Exam','ValueIsRequired.PassingScore') = @ON and @PassingScore is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Exam','ValueIsRequired.VendorExamID') = @ON and @VendorExamID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.VendorExamID'
		set @columnNames        = N'VendorExamID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Vendor Exam ID'
	end
	--!</Rule>
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Exam','BackDatingLimit.EffectiveTime') = @ON
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Exam','FutureDatingLimit.ExpiryTime') = @ON
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'Exam','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fExam#Check'
		)
		begin
		
			select @errorText = ext.fExam#Check
				(
				 @ExamSID
				,@ExamName
				,@ExamCategory
