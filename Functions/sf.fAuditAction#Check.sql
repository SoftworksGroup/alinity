...
GO
CREATE FUNCTION [sf].[fAuditAction#Check]
	(
	 @AuditActionSID     int
	,@AuditActionSCD     varchar(25)
	,@AuditActionName    nvarchar(50)
	,@IsDefault          bit
	,@AuditActionXID     varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.AuditActionSCD The "%1" must be uppercase and not contain spaces or any punctua
...
begin
	...
	if @errorMessageSCD is null and @AuditActionSCD is not null and (sf.fIsStringContentValid(@AuditActionSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@AuditActionSCD) <> @AuditActionSCD collate Latin1_General_CS_AI)												-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.AuditActionSCD'
		set @columnNames        = N'AuditActionSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Audit Action System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'AuditAction','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
