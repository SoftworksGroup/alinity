...
GO
CREATE FUNCTION [dbo].[fRegistrantLearningPlanStatus#Check]
	(
	 @RegistrantLearningPlanStatusSID int
	,@RegistrantLearningPlanSID       int
	,@FormStatusSID                   int
	,@RegistrantLearningPlanStatusXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantLearningPlanStatus#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantLearningPlanStatus','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantLearningPlanStatus#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantLearningPlanStatus#Check
				(
				 @RegistrantLearningPlanStatusSID
				,@RegistrantLearningPlanSID
				,@FormStatusSID
