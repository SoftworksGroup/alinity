...
GO
CREATE FUNCTION [sf].[fFileFormat#Check]
	(
	 @FileFormatSID      int
	,@FileFormatSCD      varchar(10)
	,@FileFormatLabel    nvarchar(35)
	,@IsDefault          bit
	,@FileFormatXID      varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.FileFormatSCD The "%1" must be uppercase and not contain spaces or any punctuat
...
begin
	...
	if @errorMessageSCD is null and @FileFormatSCD is not null and (sf.fIsStringContentValid(@FileFormatSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@FileFormatSCD) <> @FileFormatSCD collate Latin1_General_CS_AI)-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.FileFormatSCD'
		set @columnNames        = N'FileFormatSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'File Format System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'FileFormat','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
