...
GO
CREATE FUNCTION [sf].[fTask#Check]
	(
	 @TaskSID            int
	,@TaskTitle          nvarchar(65)
	,@TaskQueueSID       int
	,@TargetRowGUID      uniqueidentifier
	,@IsAlert            bit
	,@PriorityLevel      tinyint
	,@ApplicationUserSID int
	,@TaskStatusSID      int
	,@AssignedTime       datetimeoffset(7)
	,@DueDate            date
	,@NextFollowUpDate   date
	,@ClosedTime         datetimeoffset(7)
	,@ApplicationPageSID int
	,@TaskTriggerSID     int
...
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.TaskStatusSID The %1 is marked inactive. Assign an active %1.
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.TaskTriggerSID The %1 is marked inactive. Assign an active %1.
$AutoRules | Tim Edlund        ValueIsRequired.ApplicationPageSID A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.ApplicationUserSID A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.AssignedTime A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.ClosedTime A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.NextFollowUpDate A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.TargetRowGUID A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.TaskTriggerSID A value for "%1" is required.
Cory Ng | Jul 2014             MBR.FollowupDateBeforeDueDate.NextFollowUpDate (see source for message text)
...
begin
	...
	if @isInsert = @ON																											-- applies when inserting
	begin
		if (select x.IsActive from sf.TaskStatus x where x.TaskStatusSID = @TaskStatusSID) = @OFF				-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.TaskStatusSID'
			set @columnNames        = N'TaskStatusSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'task status'
		end
	end
	--!</Rule>
	...
	if @isInsert = @ON																											-- applies when inserting
	begin
		if (select x.IsActive from sf.TaskTrigger x where x.TaskTriggerSID = @TaskTriggerSID) = @OFF		-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.TaskTriggerSID'
			set @columnNames        = N'TaskTriggerSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'task trigger'
		end
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'Task','ValueIsRequired.ApplicationPageSID') = @ON and @ApplicationPageSID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ApplicationPageSID'
		set @columnNames        = N'ApplicationPageSID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Application Page'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'Task','ValueIsRequired.ApplicationUserSID') = @ON and @ApplicationUserSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'Task','ValueIsRequired.AssignedTime') = @ON and @AssignedTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.AssignedTime'
		set @columnNames        = N'AssignedTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Assigned Time'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'Task','ValueIsRequired.ClosedTime') = @ON and @ClosedTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ClosedTime'
		set @columnNames        = N'ClosedTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Closed Time'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'Task','ValueIsRequired.NextFollowUpDate') = @ON and @NextFollowUpDate is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.NextFollowUpDate'
		set @columnNames        = N'NextFollowUpDate'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Next Follow Up Date'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'Task','ValueIsRequired.TargetRowGUID') = @ON and @TargetRowGUID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.TargetRowGUID'
		set @columnNames        = N'TargetRowGUID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Target Row GUID'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'Task','ValueIsRequired.TaskTriggerSID') = @ON and @TaskTriggerSID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.TaskTriggerSID'
		set @columnNames        = N'TaskTriggerSID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Task Trigger'
	end
	...
	--!<Rule Author="Cory Ng | Jul 2014" Updates="None">
	if isnull(@NextFollowUpDate, dateadd(dd, -1, @DueDate)) >= @DueDate
	begin
		set @errorMessageSCD    = 'MBR.FollowupDateBeforeDueDate.NextFollowUpDate'
		set @columnNames        = N'NextFollowUpDate,DueDate'
		set @defaultMessageText = N'The next follow up date must be before the due date.'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'Task','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
