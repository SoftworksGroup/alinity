...
GO
CREATE FUNCTION [dbo].[fRegistrantRenewal#Check]
	(
	...
	,@ReasonSID                  int
	,@InvoiceSID                 int
	,@RegistrantRenewalXID       varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantRenewal#Check
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantRenewal','ValueIsRequired.InvoiceSID') = @ON and @InvoiceSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantRenewal','ValueIsRequired.LastValidateTime') = @ON and @LastValidateTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantRenewal','ValueIsRequired.NextFollowUp') = @ON and @NextFollowUp is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantRenewal','ValueIsRequired.ReasonSID') = @ON and @ReasonSID is null
	begin
	...
	begin
		...
		(
			select
				1
			from
				dbo.fRegistrantRenewal#CurrentStatus(@RegistrantRenewalSID, -1) cs
			where
...
-- SQL Prompt formatting on
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantRenewal','?SomeMessageCode.ColumnName') = 1					-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantRenewal#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantRenewal#Check
				(
				 @RegistrantRenewalSID
				,@RegistrationSID
				,@PracticeRegisterSectionSID
