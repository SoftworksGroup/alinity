...
GO
CREATE FUNCTION [dbo].[fPaymentType#Check]
	(
	 @PaymentTypeSID             int
	,@PaymentTypeSCD             varchar(15)
	,@PaymentTypeLabel           nvarchar(35)
	,@PaymentTypeCategory        nvarchar(65)
	,@GLAccountSID               int
	,@PaymentStatusSID           int
	,@IsReferenceRequired        bit
	,@DepositDateLagDays         smallint
	,@IsRefundExcludedFromGL     bit
	,@ExcludeDepositFromGLBefore date
	,@IsDefault                  bit
	,@IsActive                   bit
	,@PaymentTypeXID             varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPaymentType#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.PaymentTypeSCD The "%1" must be uppercase and not contain spaces or any punctua
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.GLAccountSID The %1 is marked inactive. Assign an active %1.
...
begin
	...
	if @errorMessageSCD is null and @PaymentTypeSCD is not null and (sf.fIsStringContentValid(@PaymentTypeSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@PaymentTypeSCD) <> @PaymentTypeSCD collate Latin1_General_CS_AI)												-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.PaymentTypeSCD'
		set @columnNames        = N'PaymentTypeSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Payment Type System Code'
	end
	--!</Rule>
	...
	begin
		if (select x.IsActive from dbo.GLAccount x where x.GLAccountSID = @GLAccountSID) = @OFF					-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.GLAccountSID'
			set @columnNames        = N'GLAccountSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'glaccount'
		end
	end
	--!</Rule>
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PaymentType','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPaymentType#Check'
		)
		begin
		
			select @errorText = ext.fPaymentType#Check
				(
				 @PaymentTypeSID
				,@PaymentTypeSCD
				,@PaymentTypeLabel
