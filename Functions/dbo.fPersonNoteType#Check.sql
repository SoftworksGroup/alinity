...
GO
CREATE FUNCTION [dbo].[fPersonNoteType#Check]
	(
	 @PersonNoteTypeSID      int
	,@PersonNoteTypeLabel    nvarchar(35)
	,@PersonNoteTypeCategory nvarchar(65)
	,@IsDefault              bit
	,@IsActive               bit
	,@PersonNoteTypeXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPersonNoteType#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.PersonNoteTypeCategory A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonNoteType','ValueIsRequired.PersonNoteTypeCategory') = @ON and @PersonNoteTypeCategory is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.PersonNoteTypeCategory'
		set @columnNames        = N'PersonNoteTypeCategory'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Person Note Type Category'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonNoteType','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPersonNoteType#Check'
		)
		begin
		
			select @errorText = ext.fPersonNoteType#Check
				(
				 @PersonNoteTypeSID
				,@PersonNoteTypeLabel
				,@PersonNoteTypeCategory
