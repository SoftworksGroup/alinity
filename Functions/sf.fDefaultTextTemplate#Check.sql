...
GO
CREATE FUNCTION [sf].[fDefaultTextTemplate#Check]
	(
	 @DefaultTextTemplateSID   int
	,@DefaultTextTemplateSCD   varchar(10)
	,@DefaultTextTemplateLabel nvarchar(35)
	,@TextTemplateSID          int
	,@DefaultTextTemplateXID   varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : sf.fDefaultTextTemplate#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.DefaultTextTemplateSCD The "%1" must be uppercase and not contain spaces or any
...
begin
	...
	if @errorMessageSCD is null and @DefaultTextTemplateSCD is not null and (sf.fIsStringContentValid(@DefaultTextTemplateSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@DefaultTextTemplateSCD) <> @DefaultTextTemplateSCD collate Latin1_General_CS_AI)				-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.DefaultTextTemplateSCD'
		set @columnNames        = N'DefaultTextTemplateSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Default Text Template System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'DefaultTextTemplate','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
