...
GO
CREATE FUNCTION [dbo].[fPaymentProcessorResponse#Check]
	(
	 @PaymentProcessorResponseSID int
	,@PaymentSID                  int
	,@ResponseTime                datetime
	,@ResponseSource              varchar(15)
	,@TransactionID               varchar(50)
	,@IsPaid                      bit
	,@PaymentProcessorResponseXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPaymentProcessorResponse#Check
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PaymentProcessorResponse','ValueIsRequired.TransactionID') = @ON and @TransactionID is null
	begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PaymentProcessorResponse','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPaymentProcessorResponse#Check'
		)
		begin
		
			select @errorText = ext.fPaymentProcessorResponse#Check
				(
				 @PaymentProcessorResponseSID
				,@PaymentSID
				,@ResponseTime
