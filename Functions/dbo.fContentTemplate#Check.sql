...
GO
CREATE FUNCTION [dbo].[fContentTemplate#Check]
	(
	 @ContentTemplateSID   int
	,@ContentTemplateSCD   varchar(15)
	,@ContentTemplateLabel nvarchar(35)
	,@ContentTemplateXID   varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fContentTemplate#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.ContentTemplateSCD The "%1" must be uppercase and not contain spaces or any pun
...
begin
	...
	if @errorMessageSCD is null and @ContentTemplateSCD is not null and (sf.fIsStringContentValid(@ContentTemplateSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@ContentTemplateSCD) <> @ContentTemplateSCD collate Latin1_General_CS_AI)								-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.ContentTemplateSCD'
		set @columnNames        = N'ContentTemplateSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Content Template System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ContentTemplate','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fContentTemplate#Check'
		)
		begin
		
			select @errorText = ext.fContentTemplate#Check
				(
				 @ContentTemplateSID
				,@ContentTemplateSCD
				,@ContentTemplateLabel
