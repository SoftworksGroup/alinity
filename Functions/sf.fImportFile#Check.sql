...
GO
CREATE FUNCTION [sf].[fImportFile#Check]
	(
	 @ImportFileSID        int
	,@FileFormatSID        int
	,@ApplicationEntitySID int
	,@FileName             nvarchar(100)
	,@LoadStartTime        datetimeoffset(7)
	,@LoadEndTime          datetimeoffset(7)
	,@IsFailed             bit
	,@MessageText          nvarchar(4000)
	,@ImportFileXID        varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        ValueIsRequired.LoadEndTime A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.LoadStartTime A value for "%1" is required.
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'ImportFile','ValueIsRequired.LoadEndTime') = @ON and @LoadEndTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.LoadEndTime'
		set @columnNames        = N'LoadEndTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Load End Time'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'ImportFile','ValueIsRequired.LoadStartTime') = @ON and @LoadStartTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.LoadStartTime'
		set @columnNames        = N'LoadStartTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Load Start Time'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'ImportFile','ValueIsRequired.MessageText') = @ON and @MessageText is null
	begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'ImportFile','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
