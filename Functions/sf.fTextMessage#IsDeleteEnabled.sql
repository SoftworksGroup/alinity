...
CREATE FUNCTION [sf].[fTextMessage#IsDeleteEnabled]
	(
	@TextMessageSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : sf.fTextMessage#IsDeleteEnabled
...
select top (10)
   x.TextMessageSID
  ,sf.fTextMessage#IsDeleteEnabled(x.TextMessageSID) IsDeleteEnabled
from
  sf.TextMessage x
order by
  newid()
	...
	begin
		...
		(
			...
			from
				sf.TextMessage x
			where
				x.TextMessageSID = @TextMessageSID
