...
GO
CREATE FUNCTION [dbo].[fRegistrantExam#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantExam#IsDeleteEnabled
...
select top (10)
   x.RegistrantExamSID
  ,dbo.fRegistrantExam#IsDeleteEnabled(x.RegistrantExamSID) IsDeleteEnabled
from
...
begin
	...
	begin
		...
		(
			...
			from
				dbo.RegistrantExam                x00
			left outer join
				  (select top (1) x.RegistrationChangeRequirementSID, x.RegistrantExamSID from dbo.RegistrationChangeRequirement x where isnull(x.RegistrantExamSID, -1) = @RegistrantExamSID) x01 on x00.RegistrantExamSID =  x01.RegistrantExamSID
			where
				x00.RegistrantExamSID = @RegistrantExamSID
	...
	begin
		...
		(
			...
			from
				dbo.RegistrantExam x
			where
				x.RegistrantExamSID = @RegistrantExamSID
