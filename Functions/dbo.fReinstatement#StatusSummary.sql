...
GO
CREATE FUNCTION dbo.fReinstatement#StatusSummary
(
	@RegistrationYear smallint	-- the registration year to include in the analysis
)
returns table
/*********************************************************************************************************************************
Function: Reinstatement - Status Summary
Notice  : Copyright © 2018 Softworks Group Inc.
Summary	: Returns reporting/statistical data on Reinstatements (excluding WITHDRAWN)
----------------------------------------------------------------------------------------------------------------------------------
...
Comments	
--------
This function supports reporting on the status of Reinstatements in a given registration year. @RegistrationYear is a 
...
Reinstatements 3 next-to-act parties  are possible:  Member, Administrator and None (form is closed status).

Note that any Reinstatements in the "WITHDRAWN" status are explicitly excluded from the statistics returned.
...
cross apply dbo.fReinstatement#CurrentStatus(frm.ReinstatementSID, -1) cs
where
...
begin
	...
	from
		dbo.fReinstatement#StatusSummary(@registrationYear) x
	order by
		 x.NextToActGroup
		,x.FormStatusSequence;
...
exec sf.pUnitTest#Execute
	 @ObjectName = 'dbo.fReinstatement#StatusSummary'
	,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
as
return
(
	...
	(
		select
		 ...
		 ,count(1) FormCount
		from
			dbo.fReinstatement#CurrentStatus(-1, @RegistrationYear) cs
		where
			cs.FormStatusSCD <> 'WITHDRAWN'
		group by
			cs.FormStatusSCD
