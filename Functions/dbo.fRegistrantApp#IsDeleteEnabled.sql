...
GO
CREATE FUNCTION [dbo].[fRegistrantApp#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantApp#IsDeleteEnabled
...
select top (10)
   x.RegistrantAppSID
  ,dbo.fRegistrantApp#IsDeleteEnabled(x.RegistrantAppSID) IsDeleteEnabled
from
  dbo.RegistrantApp x
order by
  newid()
...
begin
	...
	if exists																																-- if application has been submitted, or hit any final status except WITHDRAWN, block deletion
	(
		select
			1
		from
		...
		and
			(fs.FormStatusSCD = 'SUBMITTED' or (fs.IsFinal = @ON and fs.FormStatusSCD <> 'WITHDRAWN'))
	)
	begin
