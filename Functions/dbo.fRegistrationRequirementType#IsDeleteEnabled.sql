...
GO
CREATE FUNCTION [dbo].[fRegistrationRequirementType#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrationRequirementType#IsDeleteEnabled
...
select top (10)
  ...
  ,dbo.fRegistrationRequirementType#IsDeleteEnabled(x.RegistrationRequirementTypeSID) IsDeleteEnabled
from
...
begin
	...
	if (select x.IsDefault from dbo.RegistrationRequirementType x where x.RegistrationRequirementTypeSID = @RegistrationRequirementTypeSID) = @ON set @isDeleteEnabled = @OFF	-- default records cannot be deleted
	...
	begin
		...
		(
			select
				  count(x01.RegistrationRequirementSID)
			from
				dbo.RegistrationRequirementType x00
			left outer join
				  (select top (1) x.RegistrationRequirementSID, x.RegistrationRequirementTypeSID from dbo.RegistrationRequirement x where x.RegistrationRequirementTypeSID = @RegistrationRequirementTypeSID) x01 on x00.RegistrationRequirementTypeSID =  x01.RegistrationRequirementTypeSID
			where
				x00.RegistrationRequirementTypeSID = @RegistrationRequirementTypeSID
	...
	begin
		...
		(
			...
			from
				dbo.RegistrationRequirementType x
			where
				x.RegistrationRequirementTypeSID = @RegistrationRequirementTypeSID
