...
GO
CREATE FUNCTION [dbo].[fGLTransaction#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fGLTransaction#IsDeleteEnabled
...
select top (10)
   x.GLTransactionSID
  ,dbo.fGLTransaction#IsDeleteEnabled(x.GLTransactionSID) IsDeleteEnabled
from
  dbo.GLTransaction x
order by
  newid()
...
begin
	...
	begin
		...
		(
			...
			from
				dbo.GLTransaction x00
			left outer join
				  (select top (1) x.GLTransactionSID, x.ReversedGLTransactionSID from dbo.GLTransaction x where isnull(x.ReversedGLTransactionSID, -1) = @GLTransactionSID) x01 on x00.GLTransactionSID =  x01.ReversedGLTransactionSID
			where
				x00.GLTransactionSID = @GLTransactionSID
	...
	begin
		...
		(
			...
			from
				dbo.GLTransaction x
			where
				x.GLTransactionSID = @GLTransactionSID
