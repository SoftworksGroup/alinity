...
GO
CREATE FUNCTION [dbo].[fPersonDoc#ReinstatementFolderXml]
(
	 @PersonSID												int																	-- the id of the person to retrieve documents for
) returns xml
as
/*********************************************************************************************************************************
Function	: Person Doc - Reinstatement Folder XML
		...
		<SQLScript>
			<![CDATA[
				...
				select 
				 ...
				 ,p.LastName
				 ,p.FirstName
				 ,dbo.fPersonDoc#ReinstatementFolderXml(p.PersonSID) [xml]
				from
					sf.Person p;
...
exec sf.pUnitTest#Execute
			@ObjectName = N'dbo.fPersonDoc#ReinstatementFolderXml'
		,	@DefaultTestOnly = 1
...
begin
	...
	(
		select
			 rc.ReinstatementSID			[@EntitySID]
			,'dbo.Reinstatement'			[@EntitySCD]
			,rc.ReinstatementLabel		[@Name]
			,dbo.fPersonDoc#ItemXml(@PersonSID, 'dbo.Reinstatement', rc.ReinstatementSID)
		from
			sf.Person p
		join
			dbo.Registrant r on p.PersonSID = r.PersonSID
		join
			dbo.vReinstatement rc on r.RegistrantSID = rc.RegistrantSID
		where
