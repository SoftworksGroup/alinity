...
GO
CREATE FUNCTION [sf].[fJobRun#Check]
	(
	 @JobRunSID               int
	,@JobSID                  int
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.JobSID The %1 is marked inactive. Assign an active %1.
$AutoRules | Tim Edlund        ValueIsRequired.CancellationRequestTime A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.CurrentProcessLabel A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.EndTime A value for "%1" is required.
...
begin
	...
	begin
		if (select x.IsActive from sf.Job x where x.JobSID = @JobSID) = @OFF	-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.JobSID'
			set @columnNames        = N'JobSID'
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'JobRun','ValueIsRequired.CancellationRequestTime') = @ON and @CancellationRequestTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.CancellationRequestTime'
		set @columnNames        = N'CancellationRequestTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Cancellation Request Time'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'JobRun','ValueIsRequired.CurrentProcessLabel') = @ON and @CurrentProcessLabel is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.CurrentProcessLabel'
		set @columnNames        = N'CurrentProcessLabel'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Current Process Label'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'JobRun','ValueIsRequired.EndTime') = @ON and @EndTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.EndTime'
		set @columnNames        = N'EndTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'End Time'
	end
	--!</Rule>
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'JobRun','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
