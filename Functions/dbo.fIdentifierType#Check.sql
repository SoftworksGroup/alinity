...
GO
CREATE FUNCTION [dbo].[fIdentifierType#Check]
	(
	 @IdentifierTypeSID      int
	,@OrgSID                 int
	,@IdentifierTypeLabel    nvarchar(35)
	,@IdentifierTypeCategory nvarchar(65)
	,@IsOtherRegistration    bit
	,@DisplayRank            tinyint
	,@EditMask               varchar(50)
	,@IdentifierCode         varchar(15)
	,@IsDefault              bit
	,@IdentifierTypeXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fIdentifierType#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.IdentifierTypeCategory A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'IdentifierType','ValueIsRequired.IdentifierTypeCategory') = @ON and @IdentifierTypeCategory is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.IdentifierTypeCategory'
		set @columnNames        = N'IdentifierTypeCategory'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Identifier Type Category'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'IdentifierType','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fIdentifierType#Check'
		)
		begin
		
			select @errorText = ext.fIdentifierType#Check
				(
				 @IdentifierTypeSID
				,@OrgSID
				,@IdentifierTypeLabel
