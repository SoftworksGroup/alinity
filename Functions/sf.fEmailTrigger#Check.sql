...
GO
CREATE FUNCTION [sf].[fEmailTrigger#Check]
	(
	 @EmailTriggerSID       int
	,@EmailTriggerLabel     nvarchar(35)
	...
	,@EarliestSelectionDate date
	,@IsActive              bit
	,@EmailTriggerXID       varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : sf.fEmailTrigger#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.EarliestSelectionDate A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.JobScheduleSID A value for "%1" is required.
Russell Poirier | Feb 2019     SelectionTimeMissing.QuerySID'; (see source for message text)
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'EmailTrigger','ValueIsRequired.ApplicationUserSID') = @ON and @ApplicationUserSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'EmailTrigger','ValueIsRequired.EarliestSelectionDate') = @ON and @EarliestSelectionDate is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.EarliestSelectionDate'
		set @columnNames        = N'EarliestSelectionDate'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Earliest Selection Date'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'EmailTrigger','ValueIsRequired.JobScheduleSID') = @ON and @JobScheduleSID is null
	begin
	...
	--!<Rule Author="Russell Poirier | Feb 2019" Updates="None">
	...
	begin
		...
		(
			...
			from
				sf.Query qu
			where
				qu.QuerySID = @QuerySID
			and
				qu.QueryParameters.exist('/Parameters/Parameter[@ID eq "SelectionTime"]') = 1
			and
				charindex('[@SelectionTime]', QuerySQL, 1) > 0
		)
		begin
			set @errorMessageSCD = 'SelectionTimeMissing.QuerySID';
			set @columnNames = N'QuerySID'
			set @defaultMessageText = N'Query selected does not contain "SelectionTime" as a parameter or use the parameter in the query SQL.';
		end;
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'EmailTrigger','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
