...
CREATE FUNCTION [sf].[fAnnouncement#IsDeleteEnabled]
	(
	@AnnouncementSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : sf.fAnnouncement#IsDeleteEnabled
...
select top (10)
   x.AnnouncementSID
  ,sf.fAnnouncement#IsDeleteEnabled(x.AnnouncementSID) IsDeleteEnabled
from
  sf.Announcement x
order by
  newid()
...
begin
	...
	begin
		...
		(
			select
				  count(x01.AnnouncementApplicationGrantSID)
			from
				sf.Announcement                 x00
			left outer join
				  (select top (1) x.AnnouncementApplicationGrantSID, x.AnnouncementSID from sf.AnnouncementApplicationGrant x where x.AnnouncementSID = @AnnouncementSID) x01 on x00.AnnouncementSID =  x01.AnnouncementSID
			where
				x00.AnnouncementSID = @AnnouncementSID
