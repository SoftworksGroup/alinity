...
GO
CREATE FUNCTION [dbo].[fPersonDocContext#Check]
	(
	 @PersonDocContextSID  int
	,@PersonDocSID         int
	,@ApplicationEntitySID int
	,@EntitySID            int
	,@IsPrimary            bit
	,@PersonDocContextXID  varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPersonDocContext#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonDocContext','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPersonDocContext#Check'
		)
		begin
		
			select @errorText = ext.fPersonDocContext#Check
				(
				 @PersonDocContextSID
				,@PersonDocSID
				,@ApplicationEntitySID
