...
GO
CREATE FUNCTION [sf].[fPersonGroup#Check]
	(
	 @PersonGroupSID           int
	,@PersonGroupName          nvarchar(65)
	,@PersonGroupLabel         nvarchar(35)
	...
	,@SmartGroupCount          int
	,@SmartGroupCountTime      datetimeoffset(7)
	,@IsActive                 bit
	,@PersonGroupXID           varchar(150)
...
$AutoRules | Tim Edlund        ValueIsRequired.Description A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.PersonGroupCategory A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.QuerySID A value for "%1" is required.
Tim Edlund | Mar 2019          DescriptionRequired.Descriptio; (see source for message text)
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonGroup','ValueIsRequired.Description') = @ON and @Description is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.Description'
		set @columnNames        = N'Description'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Description'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonGroup','ValueIsRequired.PersonGroupCategory') = @ON and @PersonGroupCategory is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.PersonGroupCategory'
		set @columnNames        = N'PersonGroupCategory'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Person Group Category'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonGroup','ValueIsRequired.QuerySID') = @ON and @QuerySID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.QuerySID'
		set @columnNames        = N'QuerySID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Query'
	end
	...
	if @errorMessageSCD is null and @IsPreference = @ON and @Description is null
	begin
		set @errorMessageSCD = 'DescriptionRequired.Description';
		set @columnNames = N'IsPreference, Description';
		set @defaultMessageText = N'A description is required for preference groups (appears on Member Profile Update)';
	end;
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'PersonGroup','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
