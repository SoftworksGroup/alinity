...
GO
CREATE FUNCTION [dbo].[fEmploymentSupervisor#Check]
	(
	 @EmploymentSupervisorSID int
	,@RegistrantEmploymentSID int
	,@PersonSID               int
	,@ExpiryTime              datetime
	,@EmploymentSupervisorXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fEmploymentSupervisor#Check
...
Tim Edlund | Oct 2018          SupervisorInvalid.PersonSID'; (see source for message text)
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'EmploymentSupervisor','FutureDatingLimit.ExpiryTime') = @ON
	...
	if @errorMessageSCD is null and @isInsert = @ON -- do not enforce on updates
		 and @LegacyKey is null -- do not enforce this rule on conversions
		 and isnull(convert(bit, sf.fConfigParam#Value('EnforceMemberSupervisors')), @ON) = @ON
	begin
		...
		(
			...
			join
				dbo.Registrant																											r on es.PersonSID = r.PersonSID
			cross apply dbo.fRegistrant#LatestRegistration(r.RegistrantSID, null) rlrSup
			join
				dbo.PracticeRegister pr on rlrSup.PracticeRegisterSID = pr.PracticeRegisterSID and pr.IsEligibleSupervisor = @ON
			where
				es.EmploymentSupervisorSID = @EmploymentSupervisorSID and rlrSup.RegistrantIsCurrentlyActive = @ON
		)
		begin

			set @errorMessageSCD = 'SupervisorInvalid.PersonSID';
			set @columnNames = N'PersonSID';
			set @defaultMessageText = N'The supervisor identified is invalid. Business rules require supervisors to be active members on supervisor-eligible registers.';
		end;
	end;
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'EmploymentSupervisor','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fEmploymentSupervisor#Check'
		)
		begin
		
			select @errorText = ext.fEmploymentSupervisor#Check
				(
				 @EmploymentSupervisorSID
				,@RegistrantEmploymentSID
				,@PersonSID
