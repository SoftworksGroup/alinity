...
GO
CREATE FUNCTION [sf].[fAltLanguage#Check]
	(
	 @AltLanguageSID     int
	,@SourceGUID         uniqueidentifier
	,@FieldID            nvarchar(128)
	,@CultureSID         int
	,@AltLanguageXID     varchar(150)
...
$AutoRules | Tim Edlund        ValueIsRequired.FieldID A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'AltLanguage','ValueIsRequired.FieldID') = @ON and @FieldID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.FieldID'
		set @columnNames        = N'FieldID'
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'AltLanguage','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
