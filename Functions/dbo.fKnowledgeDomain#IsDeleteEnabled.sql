...
GO
CREATE FUNCTION [dbo].[fKnowledgeDomain#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fKnowledgeDomain#IsDeleteEnabled
...
select top (10)
  ...
  ,dbo.fKnowledgeDomain#IsDeleteEnabled(x.KnowledgeDomainSID) IsDeleteEnabled
from
...
begin
	...
	if (select x.IsDefault from dbo.KnowledgeDomain x where x.KnowledgeDomainSID = @KnowledgeDomainSID) = @ON set @isDeleteEnabled = @OFF	-- default records cannot be deleted
	...
	begin
		...
		(
			select
				  count(x01.ExamQuestionSID)
			from
				dbo.KnowledgeDomain x00
			left outer join
				  (select top (1) x.ExamQuestionSID, x.KnowledgeDomainSID from dbo.ExamQuestion x where x.KnowledgeDomainSID = @KnowledgeDomainSID) x01 on x00.KnowledgeDomainSID =  x01.KnowledgeDomainSID
			where
				x00.KnowledgeDomainSID = @KnowledgeDomainSID
	...
	begin
		...
		(
			...
			from
				dbo.KnowledgeDomain x
			where
				x.KnowledgeDomainSID = @KnowledgeDomainSID
