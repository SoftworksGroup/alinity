...
GO
CREATE FUNCTION [sf].[fConfigParam#Check]
	(
	 @ConfigParamSID     int
	,@ConfigParamSCD     varchar(25)
	,@ConfigParamName    nvarchar(50)
	,@DataType           varchar(15)
	,@MaxLength          int
	,@IsReadOnly         bit
	,@ConfigParamXID     varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
Tim Edlund | Mar 2012          MBR.NotInList.DataType (see source for message text)
Tim Edlund | Mar 2012          MBR.MaxLengthMissing.MaxLength (see source for message text)
...
begin
	...
	if @DataType not in ('nvarchar', 'varchar', 'nchar', 'char', 'tinyint', 'smallint', 'int', 'date', 'datetime', 'datetimeoffset', 'time', 'bit', 'xml')
	begin
		set @errorMessageSCD    = 'MBR.NotInList.DataType'
		set @defaultMessageText = N'The value "%1" is not a recognized option for %2.  Expected values are: %3.'
		set @columnNames        = N'DataType'
		set @Arg1               = convert(nvarchar(25), @DataType)
		set @Arg2               = N'Data Type'
		set @Arg3               = N'nvarchar, varchar, nchar, char, tinyint, smallint, int, date, datetime, datetimeoffset, time, bit, xml'
	end
	--!</Rule>
	
	--!<Rule Author="Tim Edlund | Mar 2012" Updates="None">
	if @DataType like '%char' and isnull(@MaxLength ,0) = 0
	begin
		set @errorMessageSCD      = 'MBR.MaxLengthMissing.MaxLength'
		set @columnNames          = N'MaxLength'
		set @defaultMessageText   = N'A maximum length is required when the data type is "%1".'
		set @Arg1                 = convert(nvarchar(25), @DataType)
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'ConfigParam','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
