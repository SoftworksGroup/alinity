...
GO
CREATE FUNCTION [dbo].[fPracticeRegisterCatalogItem#Check]
	(
	 @PracticeRegisterCatalogItemSID int
	,@PracticeRegisterSID            int
	,@CatalogItemSID                 int
	,@IsAppliedOnApplication         bit
	,@IsAppliedOnApplicationApproval bit
	,@IsAppliedOnRenewal             bit
	,@IsAppliedOnReinstatement       bit
	,@IsAppliedOnRegChange           bit
	,@IsAppliedToPAPSubscribers      bit
	,@PracticeRegisterSectionSID     int
	,@PracticeRegisterChangeSID      int
	,@FeeSequence                    smallint
	,@EffectiveTime                  datetime
	,@ExpiryTime                     datetime
	,@PracticeRegisterCatalogItemXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPracticeRegisterCatalogItem#Check
...
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.PracticeRegisterChangeSID The %1 is marked inactive. Assign an active
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.PracticeRegisterSectionSID The %1 is marked inactive. Assign an activ
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.PracticeRegisterSID The %1 is marked inactive. Assign an active %1.
$AutoRules | Tim Edlund        ValueIsRequired.PracticeRegisterChangeSID A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.PracticeRegisterSectionSID A value for "%1" is required.
...
Tim Edlund | Nov 2018          MBR.RegisterSectionMisMatch.PracticeRegisterSectionSID'; (see source for message text)
...
select
	...
	,dbo.fPracticeRegisterCatalogItem#Check
		(
		 PracticeRegisterCatalogItemSID
		,PracticeRegisterSID
		,CatalogItemSID
...
begin
	...
	begin
		if (select x.IsActive from dbo.PracticeRegisterChange x where x.PracticeRegisterChangeSID = @PracticeRegisterChangeSID) = @OFF			-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.PracticeRegisterChangeSID'
			set @columnNames        = N'PracticeRegisterChangeSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'practice register change'
		end
	end
	--!</Rule>
	...
	if @errorMessageSCD is null and sf.fIsActive(@EffectiveTime, @ExpiryTime) = @ON and @isInsert = @ON																		-- applies to active assignments when inserting
	begin
		if (select x.IsActive from dbo.PracticeRegister x where x.PracticeRegisterSID = @PracticeRegisterSID) = @OFF												-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.PracticeRegisterSID'
			set @columnNames        = N'PracticeRegisterSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PracticeRegisterCatalogItem','ValueIsRequired.PracticeRegisterChangeSID') = @ON and @PracticeRegisterChangeSID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.PracticeRegisterChangeSID'
		set @columnNames        = N'PracticeRegisterChangeSID'
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PracticeRegisterCatalogItem','ValueIsRequired.PracticeRegisterSectionSID') = @ON and @PracticeRegisterSectionSID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.PracticeRegisterSectionSID'
		set @columnNames        = N'PracticeRegisterSectionSID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Practice Register Section'
	end
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PracticeRegisterCatalogItem','BackDatingLimit.EffectiveTime') = @ON
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PracticeRegisterCatalogItem','FutureDatingLimit.ExpiryTime') = @ON
	...
	if @errorMessageSCD is null and @PracticeRegisterSectionSID is not null and @PracticeRegisterSID is not null
	begin
		...
		(
			...
			where
				prs.PracticeRegisterSID = @PracticeRegisterSID and prs.PracticeRegisterSectionSID = @PracticeRegisterSectionSID
		)
		begin

			select
				@arg1 = pr.PracticeRegisterLabel
			from
				dbo.PracticeRegister pr
			where
				pr.PracticeRegisterSID = @PracticeRegisterSID;

			set @errorMessageSCD = 'MBR.RegisterSectionMisMatch.PracticeRegisterSectionSID';
			set @columnNames = N'PracticeRegisterSID,PracticeRegisterSectionSID';
			set @defaultMessageText = N'The section is invalid. You must select a section within the "%1" register.';
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PracticeRegisterCatalogItem','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPracticeRegisterCatalogItem#Check'
		)
		begin
		
			select @errorText = ext.fPracticeRegisterCatalogItem#Check
				(
				 @PracticeRegisterCatalogItemSID
				,@PracticeRegisterSID
				,@CatalogItemSID
