...
GO
CREATE FUNCTION [dbo].[fCity#Check]
	(
	 @CitySID               int
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'City','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fCity#Check'
		)
		begin
		
			select @errorText = ext.fCity#Check
				(
				 @CitySID
				,@CityName
				,@StateProvinceSID
