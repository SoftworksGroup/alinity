...
GO
CREATE FUNCTION [dbo].[fRegion#IsDeleteEnabled]
	(
	@RegionSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegion#IsDeleteEnabled
...
select top (10)
   x.RegionSID
  ,dbo.fRegion#IsDeleteEnabled(x.RegionSID) IsDeleteEnabled
from
  dbo.Region x
order by
  newid()
...
begin
	...
	if (select x.IsDefault from dbo.Region x where x.RegionSID = @RegionSID) = @ON set @isDeleteEnabled = @OFF-- default records cannot be deleted
	...
	begin
		...
		(
			select
				...
				+ count(x03.RegionMappingSID)
				+ count(x04.CredentialProfileSID)
				+ count(x05.PersonProfileSID)
			from
				dbo.Region               x00
			left outer join
				  (select top (1) x.OrgSID, x.RegionSID from dbo.Org x where x.RegionSID = @RegionSID) x01 on x00.RegionSID =  x01.RegionSID
			left outer join
				  (select top (1) x.PersonMailingAddressSID, x.RegionSID from dbo.PersonMailingAddress x where x.RegionSID = @RegionSID) x02 on x00.RegionSID =  x02.RegionSID
			left outer join
				  (select top (1) x.RegionMappingSID, x.RegionSID from dbo.RegionMapping x where x.RegionSID = @RegionSID) x03 on x00.RegionSID =  x03.RegionSID
			left outer join
				  (select top (1) x.CredentialProfileSID, x.RegionSID from stg.CredentialProfile x where isnull(x.RegionSID, -1) = @RegionSID) x04 on x00.RegionSID =  x04.RegionSID
			left outer join
				  (select top (1) x.PersonProfileSID, x.RegionSID from stg.PersonProfile x where isnull(x.RegionSID, -1) = @RegionSID) x05 on x00.RegionSID =  x05.RegionSID
			where
				x00.RegionSID = @RegionSID
