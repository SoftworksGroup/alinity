...
GO
CREATE FUNCTION [dbo].[fCatalogItem#Check]
	(
	 @CatalogItemSID            int
	,@CatalogItemLabel          nvarchar(35)
	,@InvoiceItemDescription    nvarchar(500)
	,@IsLateFee                 bit
	,@ImageAlternateText        nvarchar(50)
	,@IsAvailableOnClientPortal bit
	,@IsComplaintPenalty        bit
	,@GLAccountSID              int
	,@IsTaxRate1Applied         bit
	,@IsTaxRate2Applied         bit
	,@IsTaxRate3Applied         bit
	,@IsTaxDeductible           bit
	,@EffectiveTime             datetime
	,@ExpiryTime                datetime
	,@FileTypeSCD               varchar(8)
	,@FileTypeSID               int
	,@CatalogItemXID            varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fCatalogItem#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.ImageAlternateText A value for "%1" is required.
...
Tim Edlund | Apr 2018          NoLateFeeProration.IsLateFee'; (see source for message text)
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'CatalogItem','ValueIsRequired.ImageAlternateText') = @ON and @ImageAlternateText is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ImageAlternateText'
		set @columnNames        = N'ImageAlternateText'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Image Alternate Text'
	end
	--!</Rule>
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'CatalogItem','BackDatingLimit.EffectiveTime') = @ON
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'CatalogItem','FutureDatingLimit.ExpiryTime') = @ON
	...
	if @errorMessageSCD is null and @IsLateFee = @ON
	begin
		...
		begin
			set @errorMessageSCD = 'NoLateFeeProration.IsLateFee';
			set @columnNames = N'IsLateFee'
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'CatalogItem','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fCatalogItem#Check'
		)
		begin
		
			select @errorText = ext.fCatalogItem#Check
				(
				 @CatalogItemSID
				,@CatalogItemLabel
				,@InvoiceItemDescription
