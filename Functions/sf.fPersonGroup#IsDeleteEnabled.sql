...
CREATE FUNCTION [sf].[fPersonGroup#IsDeleteEnabled]
	(
	@PersonGroupSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : sf.fPersonGroup#IsDeleteEnabled
...
select top (10)
   x.PersonGroupSID
  ,sf.fPersonGroup#IsDeleteEnabled(x.PersonGroupSID) IsDeleteEnabled
from
  sf.PersonGroup x
order by
  newid()
...
begin
	...
	begin
		...
		(
			select
				  count(x01.PersonGroupFolderSID)
			from
				sf.PersonGroup        x00
			left outer join
				  (select top (1) x.PersonGroupFolderSID, x.PersonGroupSID from dbo.PersonGroupFolder x where x.PersonGroupSID = @PersonGroupSID) x01 on x00.PersonGroupSID =  x01.PersonGroupSID
			where
				x00.PersonGroupSID = @PersonGroupSID
