...
GO
CREATE FUNCTION [dbo].[fSyncDataMap#Check]
	(
	 @SyncDataMapSID       int
	,@ApplicationEntitySID int
	,@SyncMode             varchar(4)
	,@IsDeleteProcessed    bit
	,@IsEnabled            bit
	,@SyncDataMapXID       varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fSyncDataMap#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'SyncDataMap','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fSyncDataMap#Check'
		)
		begin
		
			select @errorText = ext.fSyncDataMap#Check
				(
				 @SyncDataMapSID
				,@ApplicationEntitySID
				,@SyncMode
