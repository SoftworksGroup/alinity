...
GO
CREATE FUNCTION [sf].[fFormStatus#Check]
	(
	 @FormStatusSID      int
	,@FormStatusSCD      varchar(25)
	,@FormStatusLabel    nvarchar(35)
	,@IsFinal            bit
	,@IsDefault          bit
	,@FormStatusSequence int
	,@FormOwnerSID       int
	,@FormStatusXID      varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.FormStatusSCD The "%1" must be uppercase and not contain spaces or any punctuat
...
begin
	...
	if @errorMessageSCD is null and @FormStatusSCD is not null and (sf.fIsStringContentValid(@FormStatusSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@FormStatusSCD) <> @FormStatusSCD collate Latin1_General_CS_AI)-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.FormStatusSCD'
		set @columnNames        = N'FormStatusSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Form Status System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'FormStatus','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
