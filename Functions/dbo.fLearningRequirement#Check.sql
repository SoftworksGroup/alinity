...
GO
CREATE FUNCTION [dbo].[fLearningRequirement#Check]
	(
	 @LearningRequirementSID   int
	,@PracticeRegisterSID      int
	,@LearningRequirementLabel nvarchar(35)
	,@StartingRegistrationYear smallint
	,@Minimum                  decimal(5,2)
	,@Maximum                  decimal(5,2)
	,@MaximumCarryOver         decimal(5,2)
	,@LearningRequirementXID   varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fLearningRequirement#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'LearningRequirement','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fLearningRequirement#Check'
		)
		begin
		
			select @errorText = ext.fLearningRequirement#Check
				(
				 @LearningRequirementSID
				,@PracticeRegisterSID
				,@LearningRequirementLabel
