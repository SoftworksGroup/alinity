...
GO
CREATE FUNCTION [dbo].[fEmploymentRole#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fEmploymentRole#IsDeleteEnabled
...
select top (10)
   x.EmploymentRoleSID
  ,dbo.fEmploymentRole#IsDeleteEnabled(x.EmploymentRoleSID) IsDeleteEnabled
from
...
begin
	...
	if (select x.IsDefault from dbo.EmploymentRole x where x.EmploymentRoleSID = @EmploymentRoleSID) = @ON set @isDeleteEnabled = @OFF-- default records cannot be deleted
	...
	begin
		...
		(
			select
				  count(x01.RegistrantEmploymentSID)
			from
				dbo.EmploymentRole       x00
			left outer join
				  (select top (1) x.RegistrantEmploymentSID, x.EmploymentRoleSID from dbo.RegistrantEmployment x where x.EmploymentRoleSID = @EmploymentRoleSID) x01 on x00.EmploymentRoleSID =  x01.EmploymentRoleSID
			where
				x00.EmploymentRoleSID = @EmploymentRoleSID
	...
	begin
		...
		(
			...
			from
				dbo.EmploymentRole x
			where
				x.EmploymentRoleSID = @EmploymentRoleSID
