...
GO
CREATE FUNCTION [dbo].[fRegistrantAppResponse#Check]
	(
	 @RegistrantAppResponseSID int
	,@RegistrantAppSID         int
	,@FormOwnerSID             int
	,@RegistrantAppResponseXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantAppResponse#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAppResponse','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantAppResponse#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantAppResponse#Check
				(
				 @RegistrantAppResponseSID
				,@RegistrantAppSID
				,@FormOwnerSID
