...
GO
CREATE FUNCTION [dbo].[fPracticeRegisterChange#Check]
	(
	...
	,@IsEnabledForRegistrant     bit
	,@PracticeRegisterChangeXID  varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPracticeRegisterChange#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.ToolTip A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PracticeRegisterChange','ValueIsRequired.ToolTip') = @ON and @ToolTip is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ToolTip'
		set @columnNames        = N'ToolTip'
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PracticeRegisterChange','?SomeMessageCode.ColumnName') = 1		-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPracticeRegisterChange#Check'
		)
		begin
		
			select @errorText = ext.fPracticeRegisterChange#Check
				(
				 @PracticeRegisterChangeSID
				,@PracticeRegisterSID
				,@PracticeRegisterSectionSID
