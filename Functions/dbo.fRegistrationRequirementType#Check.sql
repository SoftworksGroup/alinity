...
GO
CREATE FUNCTION [dbo].[fRegistrationRequirementType#Check]
	(
	 @RegistrationRequirementTypeSID      int
	,@RegistrationRequirementTypeLabel    nvarchar(35)
	,@RegistrationRequirementTypeCode     varchar(20)
	,@RegistrationRequirementTypeCategory nvarchar(65)
	,@IsAppliedToPeople                   bit
	,@IsAppliedToOrganizations            bit
	,@IsDefault                           bit
	,@IsActive                            bit
	,@RegistrationRequirementTypeXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrationRequirementType#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.RegistrationRequirementTypeCategory A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationRequirementType','ValueIsRequired.RegistrationRequirementTypeCategory') = @ON and @RegistrationRequirementTypeCategory is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.RegistrationRequirementTypeCategory'
		set @columnNames        = N'RegistrationRequirementTypeCategory'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Registration Requirement Type Category'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationRequirementType','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrationRequirementType#Check'
		)
		begin
		
			select @errorText = ext.fRegistrationRequirementType#Check
				(
				 @RegistrationRequirementTypeSID
				,@RegistrationRequirementTypeLabel
				,@RegistrationRequirementTypeCode
