...
GO
CREATE FUNCTION [dbo].[fPAPSubscription#Check]
	(
	 @PAPSubscriptionSID int
	,@PersonSID          int
	,@InstitutionNo      varchar(3)
	,@TransitNo          varchar(5)
	,@AccountNo          varchar(15)
	,@WithdrawalAmount   decimal(11,2)
	,@EffectiveTime      datetime
	,@CancelledTime      datetime
	,@PAPSubscriptionXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPAPSubscription#Check
...
Tim Edlund | Dec 2017          MBR.MustBeGreaterThanZero.WithdrawalAmount (see source for message text)
Tim Edlund | Dec 2017          MBR.MustBeGreaterThanZero.WithdrawalAmount (see source for message text)
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PAPSubscription','ValueIsRequired.CancelledTime') = @ON and @CancelledTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PAPSubscription','BackDatingLimit.EffectiveTime') = @ON
	...
	--!<Rule Author="Tim Edlund | Dec 2017" Updates="None">
	if @errorMessageSCD is null and @WithdrawalAmount <= 0.00
	begin
		set @errorMessageSCD      = 'MBR.MustBeGreaterThanZero.WithdrawalAmount'
		set @columnNames          = N'WithdrawalAmount'
		set @defaultMessageText   = N'The %1 must be greater than zero'
		set @arg1                 = N'withdrawal amount'
	end
	--!</Rule>
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PAPSubscription','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPAPSubscription#Check'
		)
		begin
		
			select @errorText = ext.fPAPSubscription#Check
				(
				 @PAPSubscriptionSID
				,@PersonSID
				,@InstitutionNo
