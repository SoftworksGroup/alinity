...
GO
CREATE FUNCTION [dbo].[fReason#Check]
	(
	 @ReasonSID          int
	,@ReasonGroupSID     int
	,@ReasonName         nvarchar(50)
	,@ReasonCode         varchar(25)
	,@ReasonSequence     smallint
	,@ToolTip            nvarchar(500)
	,@IsActive           bit
	,@ReasonXID          varchar(150)
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Reason','ValueIsRequired.ToolTip') = @ON and @ToolTip is null
	begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'Reason','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fReason#Check'
		)
		begin
		
			select @errorText = ext.fReason#Check
				(
				 @ReasonSID
				,@ReasonGroupSID
				,@ReasonName
