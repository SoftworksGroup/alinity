...
GO
CREATE FUNCTION [dbo].[fRegistrantAppStatus#Check]
	(
	 @RegistrantAppStatusSID int
	,@RegistrantAppSID       int
	,@FormStatusSID          int
	,@RegistrantAppStatusXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantAppStatus#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAppStatus','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantAppStatus#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantAppStatus#Check
				(
				 @RegistrantAppStatusSID
				,@RegistrantAppSID
				,@FormStatusSID
