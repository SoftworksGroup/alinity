...
GO
CREATE FUNCTION [dbo].[fRegistrantAuditResponse#Check]
	(
	 @RegistrantAuditResponseSID int
	,@RegistrantAuditSID         int
	,@FormOwnerSID               int
	,@RegistrantAuditResponseXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantAuditResponse#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAuditResponse','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantAuditResponse#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantAuditResponse#Check
				(
				 @RegistrantAuditResponseSID
				,@RegistrantAuditSID
				,@FormOwnerSID
