...
GO
CREATE FUNCTION [dbo].[fRegistrantIdentifier#Check]
	(
	 @RegistrantIdentifierSID int
	,@RegistrantSID           int
	,@IdentifierValue         varchar(50)
	,@IdentifierTypeSID       int
	,@EffectiveDate           date
	,@ExpiryDate              date
	,@RegistrantIdentifierXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantIdentifier#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.DateSequenceReversed.EffectiveDate The "%1" must be dated before the "%2" or left blank if unkno
$AutoRules | Tim Edlund        ValueIsRequired.EffectiveDate A value for "%1" is required.
$AutoRules | Tim Edlund        BackDatingLimit.EffectiveDate The "%1" cannot be backdated. The backdating limit set is %2 day(s).
...
select
	...
	,IdentifierValue
	,dbo.fRegistrantIdentifier#Check
		(
		 RegistrantIdentifierSID
		,RegistrantSID
		,IdentifierValue
...
begin
	...
	if @errorMessageSCD is null and @ExpiryDate < @EffectiveDate						-- effective and expiry look reversed
	begin
		set @errorMessageSCD    = 'MBR.DateSequenceReversed.EffectiveDate'
		set @columnNames        = N'EffectiveDate,ExpiryDate'
		set @defaultMessageText = N'The "%1" must be dated before the "%2" or left blank if unknown.'
		set @arg1               = N'Effective Date'
		set @arg2               = N'Expiry Date'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantIdentifier','ValueIsRequired.EffectiveDate') = @ON and @EffectiveDate is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.EffectiveDate'
		set @columnNames        = N'EffectiveDate'
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantIdentifier','BackDatingLimit.EffectiveDate') = @ON
	and datediff(day, @EffectiveDate, sf.fDTOffsetToClientDate(@CreateTime)) > @backDateLimit
	begin
		set @errorMessageSCD    = 'BackDatingLimit.EffectiveDate'
		set @columnNames        = N'EffectiveDate'
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantIdentifier','FutureDatingLimit.ExpiryDate') = @ON
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantIdentifier','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantIdentifier#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantIdentifier#Check
				(
				 @RegistrantIdentifierSID
				,@RegistrantSID
				,@IdentifierValue
