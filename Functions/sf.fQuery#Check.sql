...
GO
CREATE FUNCTION [sf].[fQuery#Check]
	(
	 @QuerySID                 int
	,@QueryCategorySID         int
	,@ApplicationPageSID       int
	,@QueryLabel               nvarchar(35)
	,@ToolTip                  nvarchar(250)
	,@LastExecuteTime          datetimeoffset(7)
	,@LastExecuteUser          nvarchar(75)
	,@ExecuteCount             int
	,@QueryCode                varchar(30)
	,@IsActive                 bit
	,@IsApplicationPageDefault bit
	,@QueryXID                 varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.QueryCategorySID The %1 is marked inactive. Assign an active %1.
...
begin
	...
	begin
		if (select x.IsActive from sf.QueryCategory x where x.QueryCategorySID = @QueryCategorySID) = @OFF					-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.QueryCategorySID'
			set @columnNames        = N'QueryCategorySID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'query category'
		end
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'Query','ValueIsRequired.ToolTip') = @ON and @ToolTip is null
	begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'Query','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
