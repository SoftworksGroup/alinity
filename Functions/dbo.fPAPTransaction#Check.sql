...
GO
CREATE FUNCTION [dbo].[fPAPTransaction#Check]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPAPTransaction#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        ValueIsRequired.PaymentSID A value for "%1" is required.
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PAPTransaction','ValueIsRequired.PaymentSID') = @ON and @PaymentSID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.PaymentSID'
		set @columnNames        = N'PaymentSID'
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PAPTransaction','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPAPTransaction#Check'
		)
		begin
		
			select @errorText = ext.fPAPTransaction#Check
				(
				 @PAPTransactionSID
				,@PAPBatchSID
				,@PAPSubscriptionSID
