...
GO
CREATE FUNCTION [sf].[fApplicationUserSessionProperty#Check]
	(
	 @ApplicationUserSessionPropertySID int
	,@ApplicationUserSessionSID         int
	,@PropertyName                      varchar(100)
	,@ApplicationUserSessionPropertyXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : sf.fApplicationUserSessionProperty#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.ApplicationUserSessionSID The %1 is marked inactive. Assign an active
...
begin
	...
	begin
		if (select x.IsActive from sf.ApplicationUserSession x where x.ApplicationUserSessionSID = @ApplicationUserSessionSID) = @OFF					-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.ApplicationUserSessionSID'
			set @columnNames        = N'ApplicationUserSessionSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'application user session'
		end
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'ApplicationUserSessionProperty','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
