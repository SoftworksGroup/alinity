...
GO
CREATE FUNCTION [sf].[fTaskStatus#Check]
	(
	 @TaskStatusSID      int
	,@TaskStatusSCD      varchar(10)
	,@TaskStatusLabel    nvarchar(35)
	,@TaskStatusSequence int
	,@IsDerived          bit
	,@IsClosedStatus     bit
	,@IsActive           bit
	,@IsDefault          bit
	,@TaskStatusXID      varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.TaskStatusSCD The "%1" must be uppercase and not contain spaces or any punctuat
...
begin
	...
	if @errorMessageSCD is null and @TaskStatusSCD is not null and (sf.fIsStringContentValid(@TaskStatusSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@TaskStatusSCD) <> @TaskStatusSCD collate Latin1_General_CS_AI)-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.TaskStatusSCD'
		set @columnNames        = N'TaskStatusSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Task Status System Code'
	end
	--!</Rule>
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'TaskStatus','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
