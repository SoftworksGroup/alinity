...
GO
CREATE FUNCTION [dbo].[fRegistrant#Check]
	(
	 @RegistrantSID                  int
	,@PersonSID                      int
	,@RegistrantNo                   varchar(50)
	,@YearOfInitialEmployment        smallint
	,@IsOnPublicRegistry             bit
	,@CityNameOfBirth                nvarchar(30)
	,@CountrySID                     int
	,@DirectedAuditYearCompetence    smallint
	,@DirectedAuditYearPracticeHours smallint
	,@LateFeeExclusionYear           smallint
	,@IsRenewalAutoApprovalBlocked   bit
	,@RenewalExtensionExpiryTime     datetime
	,@ArchivedTime                   datetimeoffset(7)
	,@RegistrantXID                  varchar(150)
...
$AutoRules | Tim Edlund        ValueIsRequired.CityNameOfBirth A value for "%1" is required.
...
$AutoRules | Tim Edlund        ValueIsRequired.LateFeeExclusionYear A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.RenewalExtensionExpiryTime A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.YearOfInitialEmployment A value for "%1" is required.
$AutoRules | Tim Edlund        FutureDatingLimit.RenewalExtensionExpiryTime The "%1" cannot be future dated more than %2 day(s).
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Registrant','ValueIsRequired.ArchivedTime') = @ON and @ArchivedTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Registrant','ValueIsRequired.CityNameOfBirth') = @ON and @CityNameOfBirth is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.CityNameOfBirth'
		set @columnNames        = N'CityNameOfBirth'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'City Name Of Birth'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Registrant','ValueIsRequired.CountrySID') = @ON and @CountrySID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Registrant','ValueIsRequired.DirectedAuditYearCompetence') = @ON and @DirectedAuditYearCompetence is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Registrant','ValueIsRequired.DirectedAuditYearPracticeHours') = @ON and @DirectedAuditYearPracticeHours is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Registrant','ValueIsRequired.LateFeeExclusionYear') = @ON and @LateFeeExclusionYear is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.LateFeeExclusionYear'
		set @columnNames        = N'LateFeeExclusionYear'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Late Fee Exclusion Year'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Registrant','ValueIsRequired.RenewalExtensionExpiryTime') = @ON and @RenewalExtensionExpiryTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.RenewalExtensionExpiryTime'
		set @columnNames        = N'RenewalExtensionExpiryTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Renewal Extension Expiry Time'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Registrant','ValueIsRequired.YearOfInitialEmployment') = @ON and @YearOfInitialEmployment is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.YearOfInitialEmployment'
		set @columnNames        = N'YearOfInitialEmployment'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Year Of Initial Employment'
	end
	--!</Rule>
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Registrant','FutureDatingLimit.RenewalExtensionExpiryTime') = @ON
	and datediff(day, sf.fNow(), @RenewalExtensionExpiryTime) > @futureDateLimit
	begin
		set @errorMessageSCD    = 'FutureDatingLimit.RenewalExtensionExpiryTime'
		set @columnNames        = N'RenewalExtensionExpiryTime'
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'Registrant','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrant#Check'
		)
		begin
		
			select @errorText = ext.fRegistrant#Check
				(
				 @RegistrantSID
				,@PersonSID
				,@RegistrantNo
