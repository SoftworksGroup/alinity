...
GO
CREATE FUNCTION [dbo].[fPersonDoc#IsDeleteEnabled]
	(
	@PersonDocSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : dbo.fPersonDoc#IsDeleteEnabled
...
select top (10)
   x.PersonDocSID
  ,dbo.fPersonDoc#IsDeleteEnabled(x.PersonDocSID) IsDeleteEnabled
from
  dbo.PersonDoc x
order by
  newid()
...
begin
	...
	begin
		...
		(
			select
				  count(x01.PersonDocContextSID)
				+ count(x02.RegistrationChangeRequirementSID)
			from
				dbo.PersonDoc                     x00
			left outer join
				  (select top (1) x.PersonDocContextSID, x.PersonDocSID from dbo.PersonDocContext x where x.PersonDocSID = @PersonDocSID) x01 on x00.PersonDocSID =  x01.PersonDocSID
			left outer join
				  (select top (1) x.RegistrationChangeRequirementSID, x.PersonDocSID from dbo.RegistrationChangeRequirement x where isnull(x.PersonDocSID, -1) = @PersonDocSID) x02 on x00.PersonDocSID =  x02.PersonDocSID
			where
				x00.PersonDocSID = @PersonDocSID
