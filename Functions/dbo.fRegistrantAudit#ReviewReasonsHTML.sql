...
GO
CREATE function dbo.fRegistrantAudit#ReviewReasonsHTML
(
	@RegistrantAuditSID int			-- key of record to return review reason for information for or -1 for all
 ,@RegistrationYear smallint	-- year of form records to return, or -1 for all or when first param is provided
)
returns nvarchar(max)
/*********************************************************************************************************************************
Function	: RegistrantAudit - Review Reasons HTML
...
Comments	
--------
This function formats the review reasons on the audit and returns is as an HTML unordered list. The reasons are stored in an XML 
column on the profile update and the parsing for the the HTML is done in the function dbo.fRegistrantAudit#ReviewReasons. This
...
begin
	...
	select
		dbo.fRegistrantAudit#ReviewReasonsHTML(-1, @registrationYear)
...
exec sf.pUnitTest#Execute
	@ObjectName = 'dbo.fRegistrantAudit#ReviewReasonsHTML'
 ,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
as
begin
	...
	select
		@reasonsHtml = N'<ul>' + stuff((
																		 ...
																		 from
																				dbo.fRegistrantAudit#ReviewReasons(@RegistrantAuditSID, @RegistrationYear) r
																		 for xml path(''), type
