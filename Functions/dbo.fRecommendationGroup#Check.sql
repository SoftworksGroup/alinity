...
GO
CREATE FUNCTION [dbo].[fRecommendationGroup#Check]
	(
	 @RecommendationGroupSID   int
	,@RecommendationGroupSCD   varchar(15)
	,@RecommendationGroupLabel nvarchar(35)
	,@RecommendationGroupXID   varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRecommendationGroup#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.RecommendationGroupSCD The "%1" must be uppercase and not contain spaces or any
...
begin
	...
	if @errorMessageSCD is null and @RecommendationGroupSCD is not null and (sf.fIsStringContentValid(@RecommendationGroupSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@RecommendationGroupSCD) <> @RecommendationGroupSCD collate Latin1_General_CS_AI)				-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.RecommendationGroupSCD'
		set @columnNames        = N'RecommendationGroupSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Recommendation Group System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RecommendationGroup','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRecommendationGroup#Check'
		)
		begin
		
			select @errorText = ext.fRecommendationGroup#Check
				(
				 @RecommendationGroupSID
				,@RecommendationGroupSCD
				,@RecommendationGroupLabel
