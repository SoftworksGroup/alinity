...
CREATE FUNCTION [sf].[fEmailSender#IsDeleteEnabled]
	(
	@EmailSenderSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : sf.fEmailSender#IsDeleteEnabled
...
select top (10)
   x.EmailSenderSID
  ,sf.fEmailSender#IsDeleteEnabled(x.EmailSenderSID) IsDeleteEnabled
from
  sf.EmailSender x
order by
  newid()
...
begin
	...
	if (select x.IsDefault from sf.EmailSender x where x.EmailSenderSID = @EmailSenderSID) = @ON set @isDeleteEnabled = @OFF-- default records cannot be deleted
	...
	begin
		...
		(
			...
			from
				sf.EmailSender x
			where
				x.EmailSenderSID = @EmailSenderSID
