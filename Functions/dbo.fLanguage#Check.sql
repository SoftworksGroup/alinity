...
GO
CREATE FUNCTION [dbo].[fLanguage#Check]
	(
	 @LanguageSID        int
	,@LanguageLabel      nvarchar(35)
	,@IsDefault          bit
	,@IsActive           bit
	,@LanguageXID        varchar(150)
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'Language','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fLanguage#Check'
		)
		begin
		
			select @errorText = ext.fLanguage#Check
				(
				 @LanguageSID
				,@LanguageLabel
				,@IsDefault
