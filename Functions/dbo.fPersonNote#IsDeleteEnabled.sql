...
GO
CREATE FUNCTION [dbo].[fPersonNote#IsDeleteEnabled]
	(
	@PersonNoteSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : dbo.fPersonNote#IsDeleteEnabled
...
select top (10)
   x.PersonNoteSID
  ,dbo.fPersonNote#IsDeleteEnabled(x.PersonNoteSID) IsDeleteEnabled
from
  dbo.PersonNote x
order by
  newid()
...
begin
	...
	begin
		...
		(
			select
				  count(x01.PersonNoteContextSID)
			from
				dbo.PersonNote        x00
			left outer join
				  (select top (1) x.PersonNoteContextSID, x.PersonNoteSID from dbo.PersonNoteContext x where x.PersonNoteSID = @PersonNoteSID) x01 on x00.PersonNoteSID =  x01.PersonNoteSID
			where
				x00.PersonNoteSID = @PersonNoteSID
