...
GO
CREATE FUNCTION [dbo].[fPersonNote#RegistrationChangeFolderXml]
(
	 @PersonSID												int																	-- the id of the person to retrieve notes for
) returns xml
as
/*********************************************************************************************************************************
Function	: Person Note - RegistrationChange Folder XML
		...
		<SQLScript>
			<![CDATA[
				...
				select 
				 ...
				 ,p.LastName
				 ,p.FirstName
				 ,dbo.fPersonNote#RegistrationChangeFolderXml(p.PersonSID)XML
				from
					sf.Person p;
...
exec sf.pUnitTest#Execute
	@ObjectName = 'dbo.fPersonNote#RegistrationChangeFolderXml'
	,@DefaultTestOnly = 1
...
begin
	...
	(
		select
			 ra.RegistrationChangeSID					[@EntitySID]
			,'dbo.RegistrationChange'					[@EntitySCD]
			,ra.RegistrationChangeLabel  			[@Name]
			,dbo.fPersonNote#ItemXml(@PersonSID, 'dbo.RegistrationChange', ra.RegistrationChangeSID)
		from
			sf.Person p
		join
			dbo.Registrant r on p.PersonSID = r.PersonSID
		join
			dbo.vRegistrationChange ra on r.RegistrantSID = ra.RegistrationRegistrantSID
		where
