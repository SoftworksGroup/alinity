...
GO
CREATE FUNCTION [sf].[fFormOwner#Check]
	(
	 @FormOwnerSID       int
	,@FormOwnerSCD       varchar(25)
	,@FormOwnerLabel     nvarchar(35)
	,@IsAssignee         bit
	,@FormOwnerXID       varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.FormOwnerSCD The "%1" must be uppercase and not contain spaces or any punctuati
...
begin
	...
	if @errorMessageSCD is null and @FormOwnerSCD is not null and (sf.fIsStringContentValid(@FormOwnerSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@FormOwnerSCD) <> @FormOwnerSCD collate Latin1_General_CS_AI)	-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.FormOwnerSCD'
		set @columnNames        = N'FormOwnerSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Form Owner System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'FormOwner','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
