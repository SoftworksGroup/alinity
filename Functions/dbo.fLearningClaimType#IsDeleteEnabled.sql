...
GO
CREATE FUNCTION [dbo].[fLearningClaimType#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fLearningClaimType#IsDeleteEnabled
...
select top (10)
  ...
  ,dbo.fLearningClaimType#IsDeleteEnabled(x.LearningClaimTypeSID) IsDeleteEnabled
from
...
begin
	...
	if (select x.IsDefault from dbo.LearningClaimType x where x.LearningClaimTypeSID = @LearningClaimTypeSID) = @ON set @isDeleteEnabled = @OFF	-- default records cannot be deleted
	...
	begin
		...
		(
			select
				  count(x01.LearningPlanActivitySID)
			from
				dbo.LearningClaimType    x00
			left outer join
				  (select top (1) x.LearningPlanActivitySID, x.LearningClaimTypeSID from dbo.LearningPlanActivity x where x.LearningClaimTypeSID = @LearningClaimTypeSID) x01 on x00.LearningClaimTypeSID =  x01.LearningClaimTypeSID
			where
				x00.LearningClaimTypeSID = @LearningClaimTypeSID
	...
	begin
		...
		(
			...
			from
				dbo.LearningClaimType x
			where
				x.LearningClaimTypeSID = @LearningClaimTypeSID
