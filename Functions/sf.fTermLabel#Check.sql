...
GO
CREATE FUNCTION [sf].[fTermLabel#Check]
	(
	 @TermLabelSID           int
	,@TermLabelSCD           varchar(35)
	,@TermLabel              nvarchar(35)
	,@TermLabelUpdateTime    datetimeoffset(7)
	,@DefaultLabel           nvarchar(35)
	,@DefaultLabelUpdateTime datetimeoffset(7)
	,@TermLabelXID           varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.TermLabelSCD The "%1" must be uppercase and not contain spaces or any punctuati
...
begin
	...
	if @errorMessageSCD is null and @TermLabelSCD is not null and (sf.fIsStringContentValid(@TermLabelSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@TermLabelSCD) <> @TermLabelSCD collate Latin1_General_CS_AI)	-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.TermLabelSCD'
		set @columnNames        = N'TermLabelSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Term Label System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'TermLabel','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
