...
GO
CREATE FUNCTION [dbo].[fExamQuestionReference#Check]
	(
	 @ExamQuestionReferenceSID int
	,@ExamQuestionSID          int
	,@ReferenceTitle           nvarchar(125)
	,@ReferenceURL             varchar(250)
	,@ExamQuestionReferenceXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fExamQuestionReference#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.ExamQuestionSID The %1 is marked inactive. Assign an active %1.
...
begin
	...
	begin
		if (select x.IsActive from dbo.ExamQuestion x where x.ExamQuestionSID = @ExamQuestionSID) = @OFF-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.ExamQuestionSID'
			set @columnNames        = N'ExamQuestionSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'exam question'
		end
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ExamQuestionReference','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fExamQuestionReference#Check'
		)
		begin
		
			select @errorText = ext.fExamQuestionReference#Check
				(
				 @ExamQuestionReferenceSID
				,@ExamQuestionSID
				,@ReferenceTitle
