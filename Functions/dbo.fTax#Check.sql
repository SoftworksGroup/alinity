...
GO
CREATE FUNCTION [dbo].[fTax#Check]
	(
	 @TaxSID             int
	,@TaxLabel           nvarchar(35)
	,@TaxSequence        tinyint
	,@TaxXID             varchar(150)
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'Tax','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fTax#Check'
		)
		begin
		
			select @errorText = ext.fTax#Check
				(
				 @TaxSID
				,@TaxLabel
				,@TaxSequence
