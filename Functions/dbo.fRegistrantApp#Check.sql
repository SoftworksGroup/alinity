...
GO
CREATE FUNCTION [dbo].[fRegistrantApp#Check]
	(
	...
	,@FormVersionSID             int
	,@OrgSID                     int
	,@LastValidateTime           datetimeoffset(7)
	,@NextFollowUp               date
	,@RegistrationEffective      date
	,@IsAutoApprovalEnabled      bit
	,@ReasonSID                  int
	,@InvoiceSID                 int
	,@RegistrantAppXID           varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantApp#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.LastValidateTime A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantApp','ValueIsRequired.InvoiceSID') = @ON and @InvoiceSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantApp','ValueIsRequired.LastValidateTime') = @ON and @LastValidateTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.LastValidateTime'
		set @columnNames        = N'LastValidateTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Last Validate Time'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantApp','ValueIsRequired.NextFollowUp') = @ON and @NextFollowUp is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantApp','ValueIsRequired.OrgSID') = @ON and @OrgSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantApp','ValueIsRequired.ReasonSID') = @ON and @ReasonSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantApp','ValueIsRequired.RegistrationEffective') = @ON and @RegistrationEffective is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantApp','BackDatingLimit.RegistrationEffective') = @ON
	...
	begin
		if exists
		(
			select 1 from		dbo.RegistrationChange rc where rc.InvoiceSID = @InvoiceSID
		)
		begin
			set @errorMessageSCD = 'InvoiceSIDOnOtherForm.InvoiceSID';
			set @columnNames = N'InvoiceSID';
			set @defaultMessageText = N'Invoice# %1 has already been applied to %2 form. An invoice can only include one registration-type form.';			set @arg1 = @InvoiceSID;
			set @arg2 = N'a registration change';
		end;
	end;
	--!</Rule>		
...
-- SQL Prompt formatting on
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantApp','?SomeMessageCode.ColumnName') = 1							-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantApp#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantApp#Check
				(
				 @RegistrantAppSID
				,@RegistrationSID
				,@PracticeRegisterSectionSID
