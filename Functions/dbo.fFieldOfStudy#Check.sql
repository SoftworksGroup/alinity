...
GO
CREATE FUNCTION [dbo].[fFieldOfStudy#Check]
	(
	 @FieldOfStudySID      int
	,@FieldOfStudyName     nvarchar(50)
	,@FieldOfStudyCode     varchar(20)
	,@FieldOfStudyCategory nvarchar(65)
	,@IsDefault            bit
	,@IsActive             bit
	,@FieldOfStudyXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fFieldOfStudy#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.FieldOfStudyCategory A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'FieldOfStudy','ValueIsRequired.FieldOfStudyCategory') = @ON and @FieldOfStudyCategory is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.FieldOfStudyCategory'
		set @columnNames        = N'FieldOfStudyCategory'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Field Of Study Category'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'FieldOfStudy','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fFieldOfStudy#Check'
		)
		begin
		
			select @errorText = ext.fFieldOfStudy#Check
				(
				 @FieldOfStudySID
				,@FieldOfStudyName
				,@FieldOfStudyCode
