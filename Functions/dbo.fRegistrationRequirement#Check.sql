...
GO
CREATE FUNCTION [dbo].[fRegistrationRequirement#Check]
	(
	 @RegistrationRequirementSID     int
	,@RegistrationRequirementTypeSID int
	,@RegistrationRequirementLabel   nvarchar(35)
	,@PersonDocTypeSID               int
	,@ExamSID                        int
	,@ExpiryMonths                   smallint
	,@IsActive                       bit
	,@RegistrationRequirementXID     varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrationRequirement#Check
...
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.RegistrationRequirementTypeSID The %1 is marked inactive. Assign an a
$AutoRules | Tim Edlund        ValueIsRequired.ExamSID A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.PersonDocTypeSID A value for "%1" is required.
...
select
	...
	,dbo.fRegistrationRequirement#Check
		(
		 RegistrationRequirementSID
...
begin
	...
	if @isInsert = @ON																											-- applies when inserting
	begin
		if (select x.IsActive from dbo.RegistrationRequirementType x where x.RegistrationRequirementTypeSID = @RegistrationRequirementTypeSID) = @OFF	-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.RegistrationRequirementTypeSID'
			set @columnNames        = N'RegistrationRequirementTypeSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'registration requirement type'
		end
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationRequirement','ValueIsRequired.ExamSID') = @ON and @ExamSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationRequirement','ValueIsRequired.PersonDocTypeSID') = @ON and @PersonDocTypeSID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.PersonDocTypeSID'
		set @columnNames        = N'PersonDocTypeSID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Person Doc Type'
	end
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationRequirement','?SomeMessageCode.ColumnName') = 1								-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrationRequirement#Check'
		)
		begin
		
			select @errorText = ext.fRegistrationRequirement#Check
				(
				 @RegistrationRequirementSID
				,@RegistrationRequirementTypeSID
				,@RegistrationRequirementLabel
