...
GO
CREATE FUNCTION [sf].[fPersonEmailMessage#Check]
	(
	 @PersonEmailMessageSID int
	,@PersonSID             int
	,@EmailMessageSID       int
	,@EmailAddress          varchar(150)
	,@SelectedTime          datetimeoffset(7)
	...
	,@OpenedTime            datetimeoffset(7)
	,@MergeKey              int
	,@EmailTriggerSID       int
	,@ServiceMessageID      varchar(100)
	,@PersonEmailMessageXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : sf.fPersonEmailMessage#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
Tim Edlund | Apr 2015          MBR.InvalidDateOrder.NotReceivedNoticeTime (see source for message text)
Tim Edlund | Apr 2015          MBR.InvalidDateOrder.CancelledTime (see source for message text)
Tim Edlund | Apr 2015          MBR.ConditionalNull.CancelledTime (see source for message text)
...
begin
	...
	if @errorMessageSCD is null and @NotReceivedNoticeTime < @SentTime
	begin
		set @errorMessageSCD      = 'MBR.InvalidDateOrder.NotReceivedNoticeTime'
		set @columnNames          = N'NotReceivedNoticeTime, SentTime'
		set @defaultMessageText   = N'The %1 cannot be set before the %2 on the %3'
		set @arg1                 = N'not received notice time'
		set @arg2                 = N'sent time'
		set @arg3                 = N'message'
	end
	--!</Rule>
	
	--!<Rule Author="Tim Edlund | Apr 2015" Updates="None">
	if @errorMessageSCD is null and @CancelledTime < @SentTime
	begin
		set @errorMessageSCD      = 'MBR.InvalidDateOrder.CancelledTime'
		set @columnNames          = N'CancelledTime, SentTime'
		set @defaultMessageText   = N'The %1 cannot be set before the %2 on the %3'
	...
	--!<Rule Author="Tim Edlund | Apr 2015" Updates="None">
	if @errorMessageSCD is null and @CancelledTime is not null and @ConfirmedTime is not null
	begin
		set @errorMessageSCD = 'MBR.ConditionalNull.CancelledTime'
		set @columnNames          = N'CancelledTime, ConfirmedTime'
		set @defaultMessageText   = N'The %1 and %2 cannot both be filled in.'
		set @arg1                 = N'cancelled time'
		set @arg2                 = N'confirmed time'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'PersonEmailMessage','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
