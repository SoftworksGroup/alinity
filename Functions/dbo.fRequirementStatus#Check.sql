...
GO
CREATE FUNCTION [dbo].[fRequirementStatus#Check]
	(
	 @RequirementStatusSID      int
	,@RequirementStatusSCD      varchar(10)
	,@RequirementStatusLabel    nvarchar(35)
	,@IsFinal                   bit
	,@RequirementStatusSequence int
	,@IsDefault                 bit
	,@RequirementStatusXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRequirementStatus#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.RequirementStatusSCD The "%1" must be uppercase and not contain spaces or any p
...
begin
	...
	if @errorMessageSCD is null and @RequirementStatusSCD is not null and (sf.fIsStringContentValid(@RequirementStatusSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@RequirementStatusSCD) <> @RequirementStatusSCD collate Latin1_General_CS_AI)						-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.RequirementStatusSCD'
		set @columnNames        = N'RequirementStatusSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Requirement Status System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RequirementStatus','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRequirementStatus#Check'
		)
		begin
		
			select @errorText = ext.fRequirementStatus#Check
				(
				 @RequirementStatusSID
				,@RequirementStatusSCD
				,@RequirementStatusLabel
