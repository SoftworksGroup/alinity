...
GO
CREATE FUNCTION dbo.fRegistrantApp#StatusSummary
(
	@RegistrationYear smallint	-- the registration year to include in the analysis
)
returns table
/*********************************************************************************************************************************
Function: Application - Status Summary
Notice  : Copyright © 2018 Softworks Group Inc.
Summary	: Returns reporting/statistical data on Applications (excluding WITHDRAWN)
----------------------------------------------------------------------------------------------------------------------------------
...
Comments	
--------
This function supports reporting on the status of Applications in a given registration year. @RegistrationYear is a 
...
Applications 3 next-to-act parties  are possible:  Member, Administrator and None (form is closed status).

Note that any Applications in the "WITHDRAWN" status are explicitly excluded from the statistics returned.
...
cross apply dbo.fRegistrantApp#CurrentStatus(frm.RegistrantAppSID, -1) cs
where
...
begin
	...
	from
		dbo.fRegistrantApp#StatusSummary(@registrationYear) x
	order by
		 x.NextToActGroup
		,x.FormStatusSequence;
...
exec sf.pUnitTest#Execute
	 @ObjectName = 'dbo.fRegistrantApp#StatusSummary'
	,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
as
return
