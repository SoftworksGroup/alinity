...
GO
CREATE FUNCTION [sf].[fPersonOtherName#Check]
	(
	 @PersonOtherNameSID int
	,@PersonSID          int
	,@OtherNameTypeSID   int
	,@FirstName          nvarchar(30)
	,@CommonName         nvarchar(30)
	,@MiddleNames        nvarchar(30)
	,@LastName           nvarchar(35)
	,@PersonOtherNameXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : sf.fPersonOtherName#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.OtherNameTypeSID The %1 is marked inactive. Assign an active %1.
...
begin
	...
	begin
		if (select x.IsActive from sf.OtherNameType x where x.OtherNameTypeSID = @OtherNameTypeSID) = @OFF										-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.OtherNameTypeSID'
			set @columnNames        = N'OtherNameTypeSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'other name type'
		end
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonOtherName','ValueIsRequired.CommonName') = @ON and @CommonName is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonOtherName','ValueIsRequired.MiddleNames') = @ON and @MiddleNames is null
	begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'PersonOtherName','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
