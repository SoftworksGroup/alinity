...
CREATE FUNCTION [sf].[fTextSender#IsDeleteEnabled]
	(
	@TextSenderSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : sf.fTextSender#IsDeleteEnabled
...
select top (10)
   x.TextSenderSID
  ,sf.fTextSender#IsDeleteEnabled(x.TextSenderSID) IsDeleteEnabled
from
  sf.TextSender x
order by
  newid()
...
begin
	...
	if (select x.IsDefault from sf.TextSender x where x.TextSenderSID = @TextSenderSID) = @ON set @isDeleteEnabled = @OFF	-- default records cannot be deleted
