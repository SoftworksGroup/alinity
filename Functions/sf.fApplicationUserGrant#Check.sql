...
GO
CREATE FUNCTION [sf].[fApplicationUserGrant#Check]
	(
	...
	,@ExpiryTime              datetime
	,@ApplicationUserGrantXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : sf.fApplicationUserGrant#Check
...
Tim Edlund | Jul 2013          LicenseLimitExceeded.ApplicationGrantSID (see source for message text)
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'ApplicationUserGrant','BackDatingLimit.EffectiveTime') = @ON
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'ApplicationUserGrant','FutureDatingLimit.ExpiryTime') = @ON
	...
	--!<Rule Author="Tim Edlund | Jul 2013" Updates="Ensure this is positioned as LAST rule check!">
	if @errorMessageSCD is null and db_name() <> 'TenantServices'						-- an unlimited number of tenant users is allowed
	begin
		...
		begin
			...
			select
				@arg1 = cast(sf.fStringSegment(ag.ApplicationGrantSCD, '.', 1) as varchar(30))
			from
				sf.ApplicationGrant ag
			where
				ag.ApplicationGrantSID = @ApplicationGrantSID

			set @errorMessageSCD    = 'LicenseLimitExceeded.ApplicationGrantSID'
			set @columnNames        = N'ApplicationGrantSID'
			set @defaultMessageText = N'Unable to save this change because the license limit for the "%1" module would be exceeded. De-activate user licenses no longer required or contact help desk to acquire additional licenses.'		
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'ApplicationUserGrant','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
