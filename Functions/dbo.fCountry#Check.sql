...
GO
CREATE FUNCTION [dbo].[fCountry#Check]
	(
	 @CountrySID              int
	...
	,@ISOA3                   char(3)
	,@ISONumber               smallint
	,@IsStateProvinceRequired bit
	,@IsDefault               bit
	,@IsActive                bit
	,@CountryXID              varchar(150)
...
$AutoRules | Tim Edlund        ValueIsRequired.ISOA2 A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.ISOA3 A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.ISONumber A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Country','ValueIsRequired.ISOA2') = @ON and @ISOA2 is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ISOA2'
		set @columnNames        = N'ISOA2'
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Country','ValueIsRequired.ISOA3') = @ON and @ISOA3 is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ISOA3'
		set @columnNames        = N'ISOA3'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'ISOA3'
	end
	--!</Rule>
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Country','ValueIsRequired.ISONumber') = @ON and @ISONumber is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ISONumber'
		set @columnNames        = N'ISONumber'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'ISONumber'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'Country','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fCountry#Check'
		)
		begin
		
			select @errorText = ext.fCountry#Check
				(
				 @CountrySID
				,@CountryName
				,@ISOA2
