...
GO
CREATE FUNCTION dbo.fRegistrantLearningPlan#StatusSummary
(
	@RegistrationYear smallint	-- the registration year to include in the analysis
)
returns table
/*********************************************************************************************************************************
Function: Learning Plan - Status Summary
Notice  : Copyright © 2018 Softworks Group Inc.
Summary	: Returns reporting/statistical data on Learning Plans (excluding WITHDRAWN)
----------------------------------------------------------------------------------------------------------------------------------
...
Comments	
--------
This function supports reporting on the status of Learning Plans in a given registration year. @RegistrationYear is a 
...
Learning Plans 3 next-to-act parties  are possible:  Member, Administrator and None (form is closed status).

Note that any Learning Plans in the "WITHDRAWN" status are explicitly excluded from the statistics returned.
...
cross apply dbo.fRegistrantLearningPlan#CurrentStatus(frm.RegistrantLearningPlanSID, -1) cs
where
...
begin
	...
	from
		dbo.fRegistrantLearningPlan#StatusSummary(@registrationYear) x
	order by
		 x.NextToActGroup
		,x.FormStatusSequence;
...
exec sf.pUnitTest#Execute
	 @ObjectName = 'dbo.fRegistrantLearningPlan#StatusSummary'
	,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
as
return
(
	...
	(
		select
		 ...
		 ,count(1) FormCount
		from
			--			dbo.fRegistrantLearningPlan#CurrentStatus(-1, @RegistrationYear) cs
			dbo.RegistrantLearningPlan																												 rlp  --TODO: Tim Oct 2018 | replace when #currentStatus() is updated to current standard
		cross apply dbo.fRegistrantLearningPlan#CurrentStatus(rlp.RegistrantLearningPlanSID) cs
		where
			cs.RegistrationYear = @RegistrationYear and cs.FormStatusSCD <> 'WITHDRAWN'
		--			cs.FormStatusSCD <> 'WITHDRAWN'
		group by
			cs.FormStatusSCD
		 ,cs.FormStatusLabel
