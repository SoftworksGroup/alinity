...
GO
CREATE FUNCTION [dbo].[fInvoiceItem#Check]
	(
	 @InvoiceItemSID         int
	,@InvoiceSID             int
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fInvoiceItem#Check
...
Tim Edlund | Mar 2019          MBR.MultipleLateFees'; (see source for message text)
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'InvoiceItem','ValueIsRequired.CatalogItemSID') = @ON and @CatalogItemSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'InvoiceItem','ValueIsRequired.ReasonSID') = @ON and @ReasonSID is null
	begin
	...
	begin
		...
		(
			...
			where
				ii.InvoiceSID = @InvoiceSID and ci.IsLateFee = @ON and ii.AmountAfterTax > 0
		) > 1
		begin
			set @errorMessageSCD = 'MBR.MultipleLateFees';
			set @columnNames = N'CatalogItemSID';
			set @defaultMessageText = N'No more than one late-fee may be charged per invoice.';
		end;
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'InvoiceItem','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fInvoiceItem#Check'
		)
		begin
		
			select @errorText = ext.fInvoiceItem#Check
				(
				 @InvoiceItemSID
				,@InvoiceSID
				,@InvoiceItemDescription
