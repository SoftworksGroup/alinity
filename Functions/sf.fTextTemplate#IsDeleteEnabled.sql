...
CREATE FUNCTION [sf].[fTextTemplate#IsDeleteEnabled]
	(
	@TextTemplateSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : sf.fTextTemplate#IsDeleteEnabled
...
select top (10)
   x.TextTemplateSID
  ,sf.fTextTemplate#IsDeleteEnabled(x.TextTemplateSID) IsDeleteEnabled
from
  sf.TextTemplate x
order by
  newid()
...
begin
	...
	begin
		...
		(
			select
				  count(x01.DefaultTextTemplateSID)
				+ count(x02.TextTriggerSID)
			from
				sf.TextTemplate        x00
			left outer join
				  (select top (1) x.DefaultTextTemplateSID, x.TextTemplateSID from sf.DefaultTextTemplate x where x.TextTemplateSID = @TextTemplateSID) x01 on x00.TextTemplateSID =  x01.TextTemplateSID
			left outer join
				  (select top (1) x.TextTriggerSID, x.TextTemplateSID from sf.TextTrigger x where x.TextTemplateSID = @TextTemplateSID) x02 on x00.TextTemplateSID =  x02.TextTemplateSID
			where
				x00.TextTemplateSID = @TextTemplateSID
	...
	begin
		...
		(
			...
			from
				sf.TextTemplate x
			where
				x.TextTemplateSID = @TextTemplateSID
