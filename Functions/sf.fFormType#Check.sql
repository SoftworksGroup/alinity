...
GO
CREATE FUNCTION [sf].[fFormType#Check]
	(
	 @FormTypeSID        int
	,@FormTypeSCD        varchar(25)
	,@FormTypeLabel      nvarchar(35)
	,@FormOwnerSID       int
	,@IsDefault          bit
	,@FormTypeXID        varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.FormTypeSCD The "%1" must be uppercase and not contain spaces or any punctuatio
...
begin
	...
	if @errorMessageSCD is null and @FormTypeSCD is not null and (sf.fIsStringContentValid(@FormTypeSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@FormTypeSCD) <> @FormTypeSCD collate Latin1_General_CS_AI)		-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.FormTypeSCD'
		set @columnNames        = N'FormTypeSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Form Type System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'FormType','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
