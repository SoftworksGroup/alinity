...
GO
CREATE FUNCTION [dbo].[fTaxConfiguration#Check]
	(
	 @TaxConfigurationSID int
	,@TaxSID              int
	,@TaxRate             decimal(4,4)
	,@GLAccountSID        int
	,@EffectiveTime       datetime
	,@TaxConfigurationXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fTaxConfiguration#Check
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'TaxConfiguration','BackDatingLimit.EffectiveTime') = @ON
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'TaxConfiguration','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fTaxConfiguration#Check'
		)
		begin
		
			select @errorText = ext.fTaxConfiguration#Check
				(
				 @TaxConfigurationSID
				,@TaxSID
				,@TaxRate
