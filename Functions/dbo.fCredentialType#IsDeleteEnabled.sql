...
GO
CREATE FUNCTION [dbo].[fCredentialType#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fCredentialType#IsDeleteEnabled
...
select top (10)
   x.CredentialTypeSID
  ,dbo.fCredentialType#IsDeleteEnabled(x.CredentialTypeSID) IsDeleteEnabled
from
...
begin
	...
	if (select x.IsDefault from dbo.CredentialType x where x.CredentialTypeSID = @CredentialTypeSID) = @ON set @isDeleteEnabled = @OFF-- default records cannot be deleted
	...
	begin
		...
		(
			select
				  count(x01.CredentialSID)
			from
				dbo.CredentialType x00
			left outer join
				  (select top (1) x.CredentialSID, x.CredentialTypeSID from dbo.Credential x where x.CredentialTypeSID = @CredentialTypeSID) x01 on x00.CredentialTypeSID =  x01.CredentialTypeSID
			where
				x00.CredentialTypeSID = @CredentialTypeSID
	...
	begin
		...
		(
			...
			from
				dbo.CredentialType x
			where
				x.CredentialTypeSID = @CredentialTypeSID
