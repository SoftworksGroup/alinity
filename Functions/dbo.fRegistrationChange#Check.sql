...
GO
CREATE FUNCTION [dbo].[fRegistrationChange#Check]
	(
	...
	,@NextFollowUp               date
	,@RegistrationEffective      date
	,@ReservedRegistrantNo       varchar(50)
	,@ReasonSID                  int
	,@InvoiceSID                 int
	,@ComplaintSID               int
	,@RegistrationChangeXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrationChange#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.NextFollowUp A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.ReasonSID A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.RegistrationEffective A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.ReservedRegistrantNo A value for "%1" is required.
$AutoRules | Tim Edlund        BackDatingLimit.RegistrationEffective The "%1" cannot be backdated. The backdating limit set is %2 d
Tim Edlund | Apr 2018          MBR.RequirementsOutstanding'; (see source for message text)
Tim Edlund | Apr 2018          InvoiceSIDOnOtherForm.InvoiceSID'; (see source for message text)
Tim Edlund | Apr 2018          InvoiceSIDOnOtherForm.InvoiceSID'; (see source for message text)
Tim Edlund | Apr 2018          InvoiceSIDOnOtherForm.InvoiceSID'; (see source for message text)
Tim Edlund | Apr 2018          MBR.RegistrationCreated'; (see source for message text)
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationChange','ValueIsRequired.ComplaintSID') = @ON and @ComplaintSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationChange','ValueIsRequired.InvoiceSID') = @ON and @InvoiceSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationChange','ValueIsRequired.NextFollowUp') = @ON and @NextFollowUp is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.NextFollowUp'
		set @columnNames        = N'NextFollowUp'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Next Follow Up'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationChange','ValueIsRequired.ReasonSID') = @ON and @ReasonSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationChange','ValueIsRequired.RegistrationEffective') = @ON and @RegistrationEffective is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.RegistrationEffective'
		set @columnNames        = N'RegistrationEffective'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Registration Effective'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationChange','ValueIsRequired.ReservedRegistrantNo') = @ON and @ReservedRegistrantNo is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ReservedRegistrantNo'
		set @columnNames        = N'ReservedRegistrantNo'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Reserved Registrant No'
	end
	--!</Rule>
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationChange','BackDatingLimit.RegistrationEffective') = @ON
	and datediff(day, @RegistrationEffective, sf.fDTOffsetToClientDate(@CreateTime)) > @backDateLimit
	begin
		set @errorMessageSCD    = 'BackDatingLimit.RegistrationEffective'
		set @columnNames        = N'RegistrationEffective'
	...
	if @errorMessageSCD is null -- no pending requirements exist
		 and exists
	(
		select
			1
		from
			dbo.fRegistrationChange#CurrentStatus(@RegistrationChangeSID, -1) rccs
		where
			rccs.FormStatusSCD = 'APPROVED'
	) -- current status is  approved
	begin

		declare @outstandingRequirements int;
		...
		begin
			...
			set @errorMessageSCD = 'MBR.RequirementsOutstanding';
			set @defaultMessageText = N'The registration change cannot be approved because not all requirements have been met.';
	...
	--!<Rule Author="Tim Edlund | Apr 2018" Updates="None">
	if @errorMessageSCD is null and @InvoiceSID is not null
	begin
		if exists (select 1 from dbo.RegistrantApp ra where ra.InvoiceSID = @InvoiceSID)
		begin
			set @errorMessageSCD = 'InvoiceSIDOnOtherForm.InvoiceSID';
			set @columnNames = N'InvoiceSID';
			set @defaultMessageText = N'Invoice# %1 has already been applied to %2 form. An invoice can only include one registration-type form.';
			set @arg1 = @InvoiceSID;
			set @arg2 = N'an application';
		end;
	end;
	--!</Rule>
	...
	if @errorMessageSCD is null and @InvoiceSID is not null
	begin
		if exists (select 1 from dbo.RegistrantRenewal ra where ra.InvoiceSID = @InvoiceSID)
		begin
			set @errorMessageSCD = 'InvoiceSIDOnOtherForm.InvoiceSID';
			set @columnNames = N'InvoiceSID';
			set @defaultMessageText = N'Invoice# %1 has already been applied to %2 form. An invoice can only include one registration-type form.';
			set @arg1 = @InvoiceSID;
			set @arg2 = N'a renewal';
		end;
	end;
	--!</Rule>
	...
	if @errorMessageSCD is null and @InvoiceSID is not null
	begin
		if exists (select 1 from dbo.Reinstatement ra where ra.InvoiceSID = @InvoiceSID)
		begin
			set @errorMessageSCD = 'InvoiceSIDOnOtherForm.InvoiceSID';
			set @columnNames = N'InvoiceSID';
			set @defaultMessageText = N'Invoice# %1 has already been applied to %2 form. An invoice can only include one registration-type form.';
			set @arg1 = @InvoiceSID;
			set @arg2 = N'a reinstatement';
		end;
	end;
	--!</Rule>
	
	--!<Rule Author="Tim Edlund | Apr 2018" Updates="None">
	if @errorMessageSCD is null and exists
	(
		select 1 from		dbo.Registration rl where rl.FormGUID = @RowGUID
	) -- registration for this change was created
	begin
...
-- SQL Prompt formatting off
			set @errorMessageSCD = 'MBR.RegistrationCreated'; -- DO NOT PUT MESSAGE TEXT ON MULTIPLE LINES - rule parser called by sf.pSetup won't find it
			set @defaultMessageText = N'A registration (permit/license) has already been created for this approved form. Its status cannot be reversed. To make corrections, expire/cancel the registration and create a new registration change.';
-- SQL Prompt formatting on
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationChange','?SomeMessageCode.ColumnName') = 1				-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrationChange#Check'
		)
		begin
		
			select @errorText = ext.fRegistrationChange#Check
				(
				 @RegistrationChangeSID
				,@RegistrationSID
				,@PracticeRegisterSectionSID
