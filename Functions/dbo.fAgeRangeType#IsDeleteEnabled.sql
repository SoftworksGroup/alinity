...
GO
CREATE FUNCTION [dbo].[fAgeRangeType#IsDeleteEnabled]
	(
	@AgeRangeTypeSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : dbo.fAgeRangeType#IsDeleteEnabled
...
select top (10)
   x.AgeRangeTypeSID
  ,dbo.fAgeRangeType#IsDeleteEnabled(x.AgeRangeTypeSID) IsDeleteEnabled
from
  dbo.AgeRangeType x
order by
  newid()
...
begin
	...
	if (select x.IsDefault from dbo.AgeRangeType x where x.AgeRangeTypeSID = @AgeRangeTypeSID) = @ON set @isDeleteEnabled = @OFF-- default records cannot be deleted
	...
	begin
		...
		(
			select
				  count(x01.AgeRangeSID)
			from
				dbo.AgeRangeType x00
			left outer join
				  (select top (1) x.AgeRangeSID, x.AgeRangeTypeSID from dbo.AgeRange x where x.AgeRangeTypeSID = @AgeRangeTypeSID) x01 on x00.AgeRangeTypeSID =  x01.AgeRangeTypeSID
			where
				x00.AgeRangeTypeSID = @AgeRangeTypeSID
	...
	begin
		...
		(
			...
			from
				dbo.AgeRangeType x
			where
				x.AgeRangeTypeSID = @AgeRangeTypeSID
