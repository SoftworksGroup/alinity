...
GO
CREATE FUNCTION [dbo].[fComplainantType#Check]
	(
	 @ComplainantTypeSID      int
	,@ComplainantTypeLabel    nvarchar(35)
	,@ComplainantTypeCategory nvarchar(65)
	,@IsDefault               bit
	,@IsActive                bit
	,@ComplainantTypeXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fComplainantType#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.ComplainantTypeCategory A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'ComplainantType','ValueIsRequired.ComplainantTypeCategory') = @ON and @ComplainantTypeCategory is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ComplainantTypeCategory'
		set @columnNames        = N'ComplainantTypeCategory'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Complainant Type Category'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ComplainantType','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fComplainantType#Check'
		)
		begin
		
			select @errorText = ext.fComplainantType#Check
				(
				 @ComplainantTypeSID
				,@ComplainantTypeLabel
				,@ComplainantTypeCategory
