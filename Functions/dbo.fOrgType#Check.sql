...
GO
CREATE FUNCTION [dbo].[fOrgType#Check]
	(
	 @OrgTypeSID         int
	,@OrgTypeName        nvarchar(50)
	,@OrgTypeCode        varchar(20)
	,@SectorCode         varchar(5)
	,@OrgTypeCategory    nvarchar(65)
	,@IsDefault          bit
	,@IsActive           bit
	,@OrgTypeXID         varchar(150)
...
$AutoRules | Tim Edlund        ValueIsRequired.OrgTypeCategory A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.SectorCode A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'OrgType','ValueIsRequired.OrgTypeCategory') = @ON and @OrgTypeCategory is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.OrgTypeCategory'
		set @columnNames        = N'OrgTypeCategory'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Org Type Category'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'OrgType','ValueIsRequired.SectorCode') = @ON and @SectorCode is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.SectorCode'
		set @columnNames        = N'SectorCode'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Sector Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'OrgType','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fOrgType#Check'
		)
		begin
		
			select @errorText = ext.fOrgType#Check
				(
				 @OrgTypeSID
				,@OrgTypeName
				,@OrgTypeCode
