...
GO
CREATE FUNCTION [dbo].[fRegionMapping#Check]
	(
	 @RegionMappingSID   int
	,@PostalCodeMask     varchar(10)
	,@RegionSID          int
	,@RegionMappingXID   varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegionMapping#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegionMapping','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegionMapping#Check'
		)
		begin
		
			select @errorText = ext.fRegionMapping#Check
				(
				 @RegionMappingSID
				,@PostalCodeMask
				,@RegionSID
