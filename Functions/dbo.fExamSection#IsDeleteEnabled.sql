...
GO
CREATE FUNCTION [dbo].[fExamSection#IsDeleteEnabled]
	(
	@ExamSectionSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : dbo.fExamSection#IsDeleteEnabled
...
select top (10)
   x.ExamSectionSID
  ,dbo.fExamSection#IsDeleteEnabled(x.ExamSectionSID) IsDeleteEnabled
from
  dbo.ExamSection x
order by
  newid()
	...
	begin
		...
		(
			...
			from
				dbo.ExamSection  x00
			left outer join
				  (select top (1) x.ExamQuestionSID, x.ExamSectionSID from dbo.ExamQuestion x where x.ExamSectionSID = @ExamSectionSID) x01 on x00.ExamSectionSID =  x01.ExamSectionSID
			where
				x00.ExamSectionSID = @ExamSectionSID
	...
	begin
		...
		(
			...
			from
				dbo.ExamSection x
			where
				x.ExamSectionSID = @ExamSectionSID
