...
GO
CREATE FUNCTION dbo.fProfileUpdate#ReviewReasons
(
...
returns table
/*********************************************************************************************************************************
Function	: Profile Update - Review Reasons
...
Comments	
--------
This table function parses the "ReasonList" column on the profile update record and returns the reason information for each key stored 
...
function requires either a Profile Update year (typically current year) and/or a specific form record to return review reasons 
for.  If only 1 parameter is provided, then the other parameter must be passed as -1.  DO NOT PASS AS NULL.

Normalization 
-------------
This table function will return from 0 to many records.  If a profile update record has no review reasons then no record is returned.
For that reason be sure to OUTER APPLY to this function if the profile update record is required in the final data set whether or not
...
begin
	...
	select
		x.*
	from
		dbo.fProfileUpdate#ReviewReasons(-1, @registrationYear) x
	order by
		x.ProfileUpdateSID
	 ,x.ReasonSequence;
...
exec sf.pUnitTest#Execute
	@ObjectName = 'dbo.fProfileUpdate#ReviewReasons'
 ,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
as
return
(
	...
	(
		...
		join
			dbo.ProfileUpdate																			 frm on f.ProfileUpdateSID = frm.ProfileUpdateSID
