...
GO
CREATE FUNCTION [dbo].[fRegistrantEmploymentPracticeArea#Check]
	(
	 @RegistrantEmploymentPracticeAreaSID int
	,@RegistrantEmploymentSID             int
	,@PracticeAreaSID                     int
	,@IsPrimary                           bit
	,@RegistrantEmploymentPracticeAreaXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantEmploymentPracticeArea#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.PracticeAreaSID The %1 is marked inactive. Assign an active %1.
...
begin
	...
	begin
		if (select x.IsActive from dbo.PracticeArea x where x.PracticeAreaSID = @PracticeAreaSID) = @OFF-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.PracticeAreaSID'
			set @columnNames        = N'PracticeAreaSID'
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantEmploymentPracticeArea','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantEmploymentPracticeArea#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantEmploymentPracticeArea#Check
				(
				 @RegistrantEmploymentPracticeAreaSID
				,@RegistrantEmploymentSID
				,@PracticeAreaSID
