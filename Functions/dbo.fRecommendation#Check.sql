...
GO
CREATE FUNCTION [dbo].[fRecommendation#Check]
	(
	 @RecommendationSID      int
	,@RecommendationGroupSID int
	,@ButtonLabel            nvarchar(20)
	,@RecommendationSequence smallint
	,@ToolTip                nvarchar(500)
	,@IsActive               bit
	,@RecommendationXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRecommendation#Check
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Recommendation','ValueIsRequired.ToolTip') = @ON and @ToolTip is null
	begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'Recommendation','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRecommendation#Check'
		)
		begin
		
			select @errorText = ext.fRecommendation#Check
				(
				 @RecommendationSID
				,@RecommendationGroupSID
				,@ButtonLabel
