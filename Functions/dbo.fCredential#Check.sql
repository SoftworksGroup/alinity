...
GO
CREATE FUNCTION [dbo].[fCredential#Check]
	(
	 @CredentialSID         int
	,@CredentialTypeSID     int
	,@CredentialLabel       nvarchar(35)
	,@ToolTip               nvarchar(500)
	,@IsRelatedToProfession bit
	,@IsProgramRequired     bit
	,@IsSpecialization      bit
	,@IsActive              bit
	,@CredentialCode        varchar(15)
	,@CredentialXID         varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.CredentialTypeSID The %1 is marked inactive. Assign an active %1.
...
begin
	...
	begin
		if (select x.IsActive from dbo.CredentialType x where x.CredentialTypeSID = @CredentialTypeSID) = @OFF						-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.CredentialTypeSID'
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Credential','ValueIsRequired.ToolTip') = @ON and @ToolTip is null
	begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'Credential','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fCredential#Check'
		)
		begin
		
			select @errorText = ext.fCredential#Check
				(
				 @CredentialSID
				,@CredentialTypeSID
				,@CredentialLabel
