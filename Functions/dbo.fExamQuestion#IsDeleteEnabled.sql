...
GO
CREATE FUNCTION [dbo].[fExamQuestion#IsDeleteEnabled]
	(
	@ExamQuestionSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : dbo.fExamQuestion#IsDeleteEnabled
...
select top (10)
   x.ExamQuestionSID
  ,dbo.fExamQuestion#IsDeleteEnabled(x.ExamQuestionSID) IsDeleteEnabled
from
  dbo.ExamQuestion x
order by
  newid()
...
begin
	...
	begin
		...
		(
			select
				  count(x01.ExamQuestionChoiceSID)
				+ count(x02.ExamQuestionReferenceSID)
			from
				dbo.ExamQuestion          x00
			left outer join
				  (select top (1) x.ExamQuestionChoiceSID, x.ExamQuestionSID from dbo.ExamQuestionChoice x where x.ExamQuestionSID = @ExamQuestionSID) x01 on x00.ExamQuestionSID =  x01.ExamQuestionSID
			left outer join
				  (select top (1) x.ExamQuestionReferenceSID, x.ExamQuestionSID from dbo.ExamQuestionReference x where x.ExamQuestionSID = @ExamQuestionSID) x02 on x00.ExamQuestionSID =  x02.ExamQuestionSID
			where
				x00.ExamQuestionSID = @ExamQuestionSID
	...
	begin
		...
		(
			...
			from
				dbo.ExamQuestion x
			where
				x.ExamQuestionSID = @ExamQuestionSID
