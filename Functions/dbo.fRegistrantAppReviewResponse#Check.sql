...
GO
CREATE FUNCTION [dbo].[fRegistrantAppReviewResponse#Check]
	(
	 @RegistrantAppReviewResponseSID int
	,@RegistrantAppReviewSID         int
	,@FormOwnerSID                   int
	,@RegistrantAppReviewResponseXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantAppReviewResponse#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAppReviewResponse','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantAppReviewResponse#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantAppReviewResponse#Check
				(
				 @RegistrantAppReviewResponseSID
				,@RegistrantAppReviewSID
				,@FormOwnerSID
