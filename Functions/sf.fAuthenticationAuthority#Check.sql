...
GO
CREATE FUNCTION [sf].[fAuthenticationAuthority#Check]
	(
	 @AuthenticationAuthoritySID   int
	,@AuthenticationAuthoritySCD   varchar(10)
	,@AuthenticationAuthorityLabel nvarchar(35)
	,@IsActive                     bit
	,@IsDefault                    bit
	,@AuthenticationAuthorityXID   varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : sf.fAuthenticationAuthority#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.AuthenticationAuthoritySCD The "%1" must be uppercase and not contain spaces or
...
begin
	...
	if @errorMessageSCD is null and @AuthenticationAuthoritySCD is not null and (sf.fIsStringContentValid(@AuthenticationAuthoritySCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@AuthenticationAuthoritySCD) <> @AuthenticationAuthoritySCD collate Latin1_General_CS_AI)-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.AuthenticationAuthoritySCD'
		set @columnNames        = N'AuthenticationAuthoritySCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Authentication Authority System Code'
	end
	--!</Rule>
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'AuthenticationAuthority','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
