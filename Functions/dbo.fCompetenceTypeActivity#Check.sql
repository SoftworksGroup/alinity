...
GO
CREATE FUNCTION [dbo].[fCompetenceTypeActivity#Check]
	(
	...
	,@ExpiryTime                datetime
	,@CompetenceTypeActivityXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fCompetenceTypeActivity#Check
...
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.CompetenceActivitySID The %1 is marked inactive. Assign an active %1.
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.CompetenceTypeSID The %1 is marked inactive. Assign an active %1.
...
begin
	...
	begin
		if (select x.IsActive from dbo.CompetenceActivity x where x.CompetenceActivitySID = @CompetenceActivitySID) = @OFF						-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.CompetenceActivitySID'
			set @columnNames        = N'CompetenceActivitySID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'competence activity'
		end
	end
	--!</Rule>
	...
	if @errorMessageSCD is null and sf.fIsActive(@EffectiveTime, @ExpiryTime) = @ON and @isInsert = @ON															-- applies to active assignments when inserting
	begin
		if (select x.IsActive from dbo.CompetenceType x where x.CompetenceTypeSID = @CompetenceTypeSID) = @OFF												-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.CompetenceTypeSID'
			set @columnNames        = N'CompetenceTypeSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'competence type'
		end
	end
	--!</Rule>
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'CompetenceTypeActivity','BackDatingLimit.EffectiveTime') = @ON
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'CompetenceTypeActivity','FutureDatingLimit.ExpiryTime') = @ON
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'CompetenceTypeActivity','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fCompetenceTypeActivity#Check'
		)
		begin
		
			select @errorText = ext.fCompetenceTypeActivity#Check
				(
				 @CompetenceTypeActivitySID
				,@CompetenceTypeSID
				,@CompetenceActivitySID
