...
GO
CREATE FUNCTION [dbo].[fComplaintType#Check]
	(
	 @ComplaintTypeSID      int
	,@ComplaintTypeLabel    nvarchar(35)
	,@ComplaintTypeCategory nvarchar(65)
	,@IsDefault             bit
	,@IsActive              bit
	,@ComplaintTypeXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fComplaintType#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.ComplaintTypeCategory A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'ComplaintType','ValueIsRequired.ComplaintTypeCategory') = @ON and @ComplaintTypeCategory is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ComplaintTypeCategory'
		set @columnNames        = N'ComplaintTypeCategory'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Complaint Type Category'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ComplaintType','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fComplaintType#Check'
		)
		begin
		
			select @errorText = ext.fComplaintType#Check
				(
				 @ComplaintTypeSID
				,@ComplaintTypeLabel
				,@ComplaintTypeCategory
