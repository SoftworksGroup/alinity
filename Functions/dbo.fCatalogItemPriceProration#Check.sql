...
GO
CREATE FUNCTION [dbo].[fCatalogItemPriceProration#Check]
	(
	 @CatalogItemPriceProrationSID int
	,@CatalogItemPriceSID          int
	,@StartMonthDay                char(4)
	,@Price                        decimal(11,2)
	,@PercentageOfCurrentPrice     decimal(6,3)
	,@CatalogItemPriceProrationXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fCatalogItemPriceProration#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
Tim Edlund | Mar 2018          MBR.EitherOneOrOtherRequired.Price (see source for message text)
Tim Edlund | Mar 2018          MBR.OutOfRange.PercentageOfCurrentPrice'; (see source for message text)
Tim Edlund | Mar 2018          MBR.OutOfRange.PercentageOfCurrentPrice (see source for message text)
Tim Edlund | Mar 2018          MBR.EitherOneOrOtherRequired.Price (see source for message text)
Tim Edlund | Apr 2018          MBR.NoLateFeeProration.Price'; (see source for message text)
...
select
	...
	,StartMonthDay
	,dbo.fCatalogItemPriceProration#Check
		(
		 CatalogItemPriceProrationSID
		,CatalogItemPriceSID
		,StartMonthDay
...
begin
	...
	if @errorMessageSCD is null and isnull(@Price,0.0) = 0.0 and isnull(@PercentageOfCurrentPrice,0) = 0.0
	begin
		set @errorMessageSCD      = 'MBR.EitherOneOrOtherRequired.Price'
		set @columnNames          = N'Price, PercentageOfCurrentPrice'
		set @defaultMessageText   = N'Either the "%1" or the "%2" must be set (but not both)'
		set @arg1                 = N'Prorated Price'
		set @arg2									= N'Percentage of Full Price'
	end
	--!</Rule>
	
	--!<Rule Author="Tim Edlund | Mar 2018" Updates="None">
	if @errorMessageSCD is null and isnull(@Price, 0.0) <> 0.0
	begin
		...
		begin
			set @errorMessageSCD = 'MBR.OutOfRange.PercentageOfCurrentPrice';
			set @columnNames = N'PercentageOfCurrentPrice';
			set @defaultMessageText = N'The %1 must be between %2 and %3';
			set @arg1 = N'Prorated Price';
			set @arg2 = N'0.00';

			select
				@arg3 = ltrim(cip.Price) + ' (full price)'
			from
				dbo.CatalogItemPriceProration cipp
			join
				dbo.CatalogItemPrice					cip on cipp.CatalogItemPriceSID = cip.CatalogItemPriceSID
	...
	--!<Rule Author="Tim Edlund | Mar 2018" Updates="None">
	if @errorMessageSCD is null and @PercentageOfCurrentPrice < 0.0 or @PercentageOfCurrentPrice > 100.0
	begin
		set @errorMessageSCD      = 'MBR.OutOfRange.PercentageOfCurrentPrice'
		set @columnNames          = N'PercentageOfCurrentPrice'
		set @defaultMessageText   = N'The %1 must be between %2 and %3'
		set @arg1                 = N'Percentage of Full Price'
		set @arg2									= N'0.0'
		set @arg3									= N'100.0'
	end
	--!</Rule>
	...
	begin
		...
		(
			...
			join
				dbo.CatalogItemPrice																			 cip on cipp.CatalogItemPriceSID = cip.CatalogItemPriceSID
			join
				dbo.CatalogItem																						 ci on cip.CatalogItemSID = ci.CatalogItemSID and ci.IsLateFee = @ON
			where
				cipp.CatalogItemPriceProrationSID = @CatalogItemPriceProrationSID
		)
		begin
			set @errorMessageSCD = 'MBR.NoLateFeeProration.Price';
			set @columnNames = N'Price';
			set @defaultMessageText = N'Late fees cannot be prorated. Remove prorating records before setting as a late fee.';
		end;
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'CatalogItemPriceProration','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fCatalogItemPriceProration#Check'
		)
		begin
		
			select @errorText = ext.fCatalogItemPriceProration#Check
				(
				 @CatalogItemPriceProrationSID
				,@CatalogItemPriceSID
				,@StartMonthDay
