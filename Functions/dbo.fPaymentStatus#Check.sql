...
GO
CREATE FUNCTION [dbo].[fPaymentStatus#Check]
	(
	 @PaymentStatusSID      int
	,@PaymentStatusSCD      varchar(25)
	,@PaymentStatusLabel    nvarchar(35)
	,@IsPaid                bit
	,@PaymentStatusSequence int
	,@PaymentStatusXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPaymentStatus#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.PaymentStatusSCD The "%1" must be uppercase and not contain spaces or any punct
...
begin
	...
	if @errorMessageSCD is null and @PaymentStatusSCD is not null and (sf.fIsStringContentValid(@PaymentStatusSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@PaymentStatusSCD) <> @PaymentStatusSCD collate Latin1_General_CS_AI)										-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.PaymentStatusSCD'
		set @columnNames        = N'PaymentStatusSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Payment Status System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PaymentStatus','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPaymentStatus#Check'
		)
		begin
		
			select @errorText = ext.fPaymentStatus#Check
				(
				 @PaymentStatusSID
				,@PaymentStatusSCD
				,@PaymentStatusLabel
