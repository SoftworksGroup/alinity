...
GO
CREATE FUNCTION [dbo].[fPersonGroupFolder#Check]
	(
	 @PersonGroupFolderSID       int
	,@ParentPersonGroupFolderSID int
	,@PersonGroupSID             int
	,@FolderName                 nvarchar(35)
	,@IsRoot                     bit
	,@PersonGroupFolderXID       varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPersonGroupFolder#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.ParentPersonGroupFolderSID A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonGroupFolder','ValueIsRequired.ParentPersonGroupFolderSID') = @ON and @ParentPersonGroupFolderSID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ParentPersonGroupFolderSID'
		set @columnNames        = N'ParentPersonGroupFolderSID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Parent Person Group Folder'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonGroupFolder','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPersonGroupFolder#Check'
		)
		begin
		
			select @errorText = ext.fPersonGroupFolder#Check
				(
				 @PersonGroupFolderSID
				,@ParentPersonGroupFolderSID
				,@PersonGroupSID
