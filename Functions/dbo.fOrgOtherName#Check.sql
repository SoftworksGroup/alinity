...
GO
CREATE FUNCTION [dbo].[fOrgOtherName#Check]
	(
	 @OrgOtherNameSID    int
	,@OrgSID             int
	,@OrgName            nvarchar(150)
	,@ExpiryDate         date
	,@OrgOtherNameXID    varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fOrgOtherName#Check
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'OrgOtherName','FutureDatingLimit.ExpiryDate') = @ON
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'OrgOtherName','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fOrgOtherName#Check'
		)
		begin
		
			select @errorText = ext.fOrgOtherName#Check
				(
				 @OrgOtherNameSID
				,@OrgSID
				,@OrgName
