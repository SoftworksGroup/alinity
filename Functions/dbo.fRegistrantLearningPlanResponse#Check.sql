...
GO
CREATE FUNCTION [dbo].[fRegistrantLearningPlanResponse#Check]
	(
	 @RegistrantLearningPlanResponseSID int
	,@RegistrantLearningPlanSID         int
	,@FormOwnerSID                      int
	,@RegistrantLearningPlanResponseXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantLearningPlanResponse#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantLearningPlanResponse','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantLearningPlanResponse#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantLearningPlanResponse#Check
				(
				 @RegistrantLearningPlanResponseSID
				,@RegistrantLearningPlanSID
				,@FormOwnerSID
