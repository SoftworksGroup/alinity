...
GO
CREATE FUNCTION [dbo].[fComplaintSeverity#Check]
	(
	 @ComplaintSeveritySID      int
	,@ComplaintSeverityLabel    nvarchar(35)
	,@ComplaintSeverityCategory nvarchar(65)
	,@IsDefault                 bit
	,@IsActive                  bit
	,@ComplaintSeverityXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fComplaintSeverity#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.ComplaintSeverityCategory A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'ComplaintSeverity','ValueIsRequired.ComplaintSeverityCategory') = @ON and @ComplaintSeverityCategory is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ComplaintSeverityCategory'
		set @columnNames        = N'ComplaintSeverityCategory'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Complaint Severity Category'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ComplaintSeverity','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fComplaintSeverity#Check'
		)
		begin
		
			select @errorText = ext.fComplaintSeverity#Check
				(
				 @ComplaintSeveritySID
				,@ComplaintSeverityLabel
				,@ComplaintSeverityCategory
