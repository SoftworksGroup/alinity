...
GO
CREATE FUNCTION [dbo].[fPersonDoc#ComplaintFolderXml]
(
...
as
/*********************************************************************************************************************************
Function	: Person Doc - Complaint Folder XML
	...
	<Test Name="All" Description="Return all Items">
		<SQLScript>
			<![CDATA[
				...
				select 
				 ...
				 ,p.LastName
				 ,p.FirstName
				 ,dbo.fPersonDoc#ComplaintFolderXml(p.PersonSID) ProfileUpdateFolderXML
				from
					sf.Person p;
		...
		<SQLScript>
			<![CDATA[
				...
				select 
				 @responseXML = dbo.fPersonDoc#ComplaintFolderXml(@personSID) 
...
exec sf.pUnitTest#Execute																								
			@ObjectName = N'dbo.fPersonDoc#ComplaintFolderXml'
		,	@DefaultTestOnly = 1
...
begin
	...
	return ( select
			...
			,dbo.fPersonDoc#ComplaintEventFolderXml(c.ComplaintSID)
			,dbo.fPersonDoc#ItemXml(@PersonSID, 'dbo.Complaint', c.ComplaintSID)
		from
			sf.Person p
		join
