...
CREATE FUNCTION [sf].[fEmailTemplate#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : sf.fEmailTemplate#IsDeleteEnabled
...
select top (10)
   x.EmailTemplateSID
  ,sf.fEmailTemplate#IsDeleteEnabled(x.EmailTemplateSID) IsDeleteEnabled
from
  sf.EmailTemplate x
order by
  newid()
...
begin
	...
	begin
		...
		(
			select
				  count(x01.DefaultEmailTemplateSID)
				+ count(x02.EmailTriggerSID)
			from
				sf.EmailTemplate        x00
			left outer join
				  (select top (1) x.DefaultEmailTemplateSID, x.EmailTemplateSID from sf.DefaultEmailTemplate x where x.EmailTemplateSID = @EmailTemplateSID) x01 on x00.EmailTemplateSID =  x01.EmailTemplateSID
			left outer join
				  (select top (1) x.EmailTriggerSID, x.EmailTemplateSID from sf.EmailTrigger x where x.EmailTemplateSID = @EmailTemplateSID) x02 on x00.EmailTemplateSID =  x02.EmailTemplateSID
			where
				x00.EmailTemplateSID = @EmailTemplateSID
	...
	begin
		...
		(
			...
			from
				sf.EmailTemplate x
			where
				x.EmailTemplateSID = @EmailTemplateSID
