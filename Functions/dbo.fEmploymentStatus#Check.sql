...
GO
CREATE FUNCTION [dbo].[fEmploymentStatus#Check]
	(
	 @EmploymentStatusSID  int
	,@EmploymentStatusName nvarchar(50)
	,@EmploymentStatusCode varchar(20)
	,@IsDefault            bit
	,@IsEmploymentExpected bit
	,@IsActive             bit
	,@EmploymentStatusXID  varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fEmploymentStatus#Check
...
Tim Edlund | Jul 2018          MBR.MissingDefault.IsDefault'; (see source for message text)
...
begin
	...
	if not exists (select 1 from dbo.EmploymentStatus x where x.IsDefault = @ON) and exists(select 1 from dbo.EmploymentStatus x where datediff(second, x.UpdateTime, sysdatetimeoffset()) > 60)
	begin
		set @errorMessageSCD = 'MBR.MissingDefault.IsDefault';
		set @columnNames = N'IsDefault';
		set @defaultMessageText =
			N'A default %1 record is required by the application. (Setting another record as the new default automatically un-sets the previous one.)';
		set @arg1 = N'employment status';
	end;
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'EmploymentStatus','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fEmploymentStatus#Check'
		)
		begin
		
			select @errorText = ext.fEmploymentStatus#Check
				(
				 @EmploymentStatusSID
				,@EmploymentStatusName
				,@EmploymentStatusCode
