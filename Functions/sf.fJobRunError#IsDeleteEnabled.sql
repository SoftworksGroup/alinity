...
CREATE FUNCTION [sf].[fJobRunError#IsDeleteEnabled]
	(
	@JobRunErrorSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : sf.fJobRunError#IsDeleteEnabled
...
select top (10)
   x.JobRunErrorSID
  ,sf.fJobRunError#IsDeleteEnabled(x.JobRunErrorSID) IsDeleteEnabled
from
  sf.JobRunError x
order by
  newid()
	...
	begin
		...
		(
			...
			from
				sf.JobRunError x
			where
				x.JobRunErrorSID = @JobRunErrorSID
