...
GO
CREATE FUNCTION [dbo].[fLearningClaimType#Check]
	(
	 @LearningClaimTypeSID      int
	,@LearningClaimTypeLabel    nvarchar(35)
	,@LearningClaimTypeCategory nvarchar(65)
	,@IsValidForRenewal         bit
	,@IsComplete                bit
	,@IsWithdrawn               bit
	,@IsDefault                 bit
	,@IsActive                  bit
	,@LearningClaimTypeXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fLearningClaimType#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.LearningClaimTypeCategory A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'LearningClaimType','ValueIsRequired.LearningClaimTypeCategory') = @ON and @LearningClaimTypeCategory is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.LearningClaimTypeCategory'
		set @columnNames        = N'LearningClaimTypeCategory'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Learning Claim Type Category'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'LearningClaimType','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fLearningClaimType#Check'
		)
		begin
		
			select @errorText = ext.fLearningClaimType#Check
				(
				 @LearningClaimTypeSID
				,@LearningClaimTypeLabel
				,@LearningClaimTypeCategory
