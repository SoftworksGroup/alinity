...
GO
CREATE FUNCTION [sf].[fPersonTextMessage#Check]
	(
	...
	,@Body                  nvarchar(1600)
	,@NotReceivedNoticeTime datetime
	,@ConfirmedTime         datetimeoffset(7)
	,@CancelledTime         datetimeoffset(7)
	,@DeliveredTime         datetimeoffset(7)
	,@MergeKey              int
	,@TextTriggerSID        int
	,@ServiceMessageID      varchar(100)
	,@PersonTextMessageXID  varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : sf.fPersonTextMessage#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.TextTriggerSID The %1 is marked inactive. Assign an active %1.
$AutoRules | Tim Edlund        ValueIsRequired.Body A value for "%1" is required.
...
$AutoRules | Tim Edlund        ValueIsRequired.DeliveredTime A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.MergeKey A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.MobilePhone A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.NotReceivedNoticeTime A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.SentTime A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.ServiceMessageID A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.TextTriggerSID A value for "%1" is required.
...
begin
	...
	begin
		if (select x.IsActive from sf.TextTrigger x where x.TextTriggerSID = @TextTriggerSID) = @OFF		-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.TextTriggerSID'
			set @columnNames        = N'TextTriggerSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'text trigger'
		end
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonTextMessage','ValueIsRequired.Body') = @ON and @Body is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.Body'
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonTextMessage','ValueIsRequired.CancelledTime') = @ON and @CancelledTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonTextMessage','ValueIsRequired.ConfirmedTime') = @ON and @ConfirmedTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonTextMessage','ValueIsRequired.DeliveredTime') = @ON and @DeliveredTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.DeliveredTime'
		set @columnNames        = N'DeliveredTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Delivered Time'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonTextMessage','ValueIsRequired.MergeKey') = @ON and @MergeKey is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.MergeKey'
		set @columnNames        = N'MergeKey'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Merge Key'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonTextMessage','ValueIsRequired.MobilePhone') = @ON and @MobilePhone is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonTextMessage','ValueIsRequired.NotReceivedNoticeTime') = @ON and @NotReceivedNoticeTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.NotReceivedNoticeTime'
		set @columnNames        = N'NotReceivedNoticeTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Not Received Notice Time'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonTextMessage','ValueIsRequired.SentTime') = @ON and @SentTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.SentTime'
		set @columnNames        = N'SentTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Sent Time'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonTextMessage','ValueIsRequired.ServiceMessageID') = @ON and @ServiceMessageID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ServiceMessageID'
		set @columnNames        = N'ServiceMessageID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Service Message ID'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonTextMessage','ValueIsRequired.TextTriggerSID') = @ON and @TextTriggerSID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.TextTriggerSID'
		set @columnNames        = N'TextTriggerSID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Text Trigger'
	end
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'PersonTextMessage','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
