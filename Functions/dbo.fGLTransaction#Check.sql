...
GO
CREATE FUNCTION [dbo].[fGLTransaction#Check]
	(
	 @GLTransactionSID         int
	,@PaymentSID               int
	,@InvoicePaymentSID        int
	,@DebitGLAccountCode       varchar(50)
	,@CreditGLAccountCode      varchar(50)
	,@Amount                   decimal(11,2)
	,@GLPostingDate            date
	,@PaymentCheckSum          int
	,@InvoicePaymentCheckSum   int
	,@ReversedGLTransactionSID int
	,@IsExcluded               bit
	,@GLTransactionXID         varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fGLTransaction#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        ValueIsRequired.InvoicePaymentCheckSum A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.InvoicePaymentSID A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.ReversedGLTransactionSID A value for "%1" is required.
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'GLTransaction','ValueIsRequired.InvoicePaymentCheckSum') = @ON and @InvoicePaymentCheckSum is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.InvoicePaymentCheckSum'
		set @columnNames        = N'InvoicePaymentCheckSum'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Invoice Payment Check Sum'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'GLTransaction','ValueIsRequired.InvoicePaymentSID') = @ON and @InvoicePaymentSID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.InvoicePaymentSID'
		set @columnNames        = N'InvoicePaymentSID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Invoice Payment'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'GLTransaction','ValueIsRequired.ReversedGLTransactionSID') = @ON and @ReversedGLTransactionSID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ReversedGLTransactionSID'
		set @columnNames        = N'ReversedGLTransactionSID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Reversed GLTransaction'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'GLTransaction','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fGLTransaction#Check'
		)
		begin
		
			select @errorText = ext.fGLTransaction#Check
				(
				 @GLTransactionSID
				,@PaymentSID
				,@InvoicePaymentSID
