...
GO
CREATE FUNCTION [dbo].[fPersonGroupFolder#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPersonGroupFolder#IsDeleteEnabled
...
select top (10)
  ...
  ,dbo.fPersonGroupFolder#IsDeleteEnabled(x.PersonGroupFolderSID) IsDeleteEnabled
from
...
begin
	...
	begin
		...
		(
			select
				  count(x01.PersonGroupDocSID)
			from
				dbo.PersonGroupFolder x00
			left outer join
				  (select top (1) x.PersonGroupDocSID, x.PersonGroupFolderSID from dbo.PersonGroupDoc x where x.PersonGroupFolderSID = @PersonGroupFolderSID) x01 on x00.PersonGroupFolderSID =  x01.PersonGroupFolderSID
			where
				x00.PersonGroupFolderSID = @PersonGroupFolderSID
	...
	begin
		...
		(
			...
			from
				dbo.PersonGroupFolder x
			where
				x.PersonGroupFolderSID = @PersonGroupFolderSID
