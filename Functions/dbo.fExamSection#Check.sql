...
GO
CREATE FUNCTION [dbo].[fExamSection#Check]
	(
	 @ExamSectionSID      int
	,@ExamSID             int
	,@Sequence            smallint
	,@SectionTitle        nvarchar(85)
	,@RandomQuestionCount smallint
	,@WeightPerQuestion   smallint
	,@MinimumCorrect      smallint
	,@ExamSectionXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fExamSection#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ExamSection','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fExamSection#Check'
		)
		begin
		
			select @errorText = ext.fExamSection#Check
				(
				 @ExamSectionSID
				,@ExamSID
				,@Sequence
