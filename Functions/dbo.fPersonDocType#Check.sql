...
GO
CREATE FUNCTION [dbo].[fPersonDocType#Check]
	(
	 @PersonDocTypeSID      int
	,@PersonDocTypeSCD      varchar(15)
	,@PersonDocTypeLabel    nvarchar(35)
	,@PersonDocTypeCategory nvarchar(65)
	,@IsDefault             bit
	,@IsActive              bit
	,@PersonDocTypeXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPersonDocType#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.PersonDocTypeSCD The "%1" must be uppercase and not contain spaces or any punct
...
begin
	...
	if @errorMessageSCD is null and @PersonDocTypeSCD is not null and (sf.fIsStringContentValid(@PersonDocTypeSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@PersonDocTypeSCD) <> @PersonDocTypeSCD collate Latin1_General_CS_AI)										-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.PersonDocTypeSCD'
		set @columnNames        = N'PersonDocTypeSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Person Doc Type System Code'
	end
	--!</Rule>
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonDocType','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPersonDocType#Check'
		)
		begin
		
			select @errorText = ext.fPersonDocType#Check
				(
				 @PersonDocTypeSID
				,@PersonDocTypeSCD
				,@PersonDocTypeLabel
