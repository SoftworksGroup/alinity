...
GO
CREATE FUNCTION [dbo].[fComplaintContact#Check]
	(
	 @ComplaintContactSID     int
	,@ComplaintSID            int
	,@PersonSID               int
	,@ComplaintContactRoleSID int
	,@EffectiveTime           datetime
	,@ExpiryTime              datetime
	,@ComplaintContactXID     varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fComplaintContact#Check
...
Tim Edlund | Jan 2019          MBR.MemberMismatch.PersonSID'; (see source for message text)
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'ComplaintContact','BackDatingLimit.EffectiveTime') = @ON
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'ComplaintContact','FutureDatingLimit.ExpiryTime') = @ON
	...
	begin
		...
		(
			...
			where
				cc.ComplaintContactSID = @ComplaintContactSID -- apply rule when this is the member contact record
		)
		begin
			...
			(
				...
				from
					dbo.ComplaintContact cc
				join
					dbo.Complaint				 c on cc.ComplaintSID = c.ComplaintSID
				join
					dbo.Registrant			 r on c.RegistrantSID = r.RegistrantSID
				where
					cc.ComplaintContactSID = @ComplaintContactSID and r.PersonSID = cc.PersonSID
			)
			begin
				set @errorMessageSCD = 'MBR.MemberMismatch.PersonSID';
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ComplaintContact','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fComplaintContact#Check'
		)
		begin
		
			select @errorText = ext.fComplaintContact#Check
				(
				 @ComplaintContactSID
				,@ComplaintSID
				,@PersonSID
