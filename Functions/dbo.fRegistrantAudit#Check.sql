...
GO
CREATE FUNCTION [dbo].[fRegistrantAudit#Check]
	(
	...
	,@RegistrantAuditXID    varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantAudit#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.AuditTypeSID The %1 is marked inactive. Assign an active %1.
...
begin
	...
	begin
		if (select x.IsActive from dbo.AuditType x where x.AuditTypeSID = @AuditTypeSID) = @OFF					-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.AuditTypeSID'
			set @columnNames        = N'AuditTypeSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'audit type'
		end
	end
	--!</Rule>
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAudit','ValueIsRequired.LastValidateTime') = @ON and @LastValidateTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAudit','ValueIsRequired.NextFollowUp') = @ON and @NextFollowUp is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAudit','ValueIsRequired.ReasonSID') = @ON and @ReasonSID is null
	begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAudit','OutOfRange.RegistrationYear') = @ON
	begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAudit','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantAudit#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantAudit#Check
				(
				 @RegistrantAuditSID
				,@RegistrantSID
				,@AuditTypeSID
