...
GO
CREATE FUNCTION [sf].[fDefaultEmailTemplate#Check]
	(
	 @DefaultEmailTemplateSID   int
	,@DefaultEmailTemplateSCD   varchar(10)
	,@DefaultEmailTemplateLabel nvarchar(35)
	,@EmailTemplateSID          int
	,@DefaultEmailTemplateXID   varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : sf.fDefaultEmailTemplate#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.DefaultEmailTemplateSCD The "%1" must be uppercase and not contain spaces or an
...
begin
	...
	if @errorMessageSCD is null and @DefaultEmailTemplateSCD is not null and (sf.fIsStringContentValid(@DefaultEmailTemplateSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@DefaultEmailTemplateSCD) <> @DefaultEmailTemplateSCD collate Latin1_General_CS_AI)			-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.DefaultEmailTemplateSCD'
		set @columnNames        = N'DefaultEmailTemplateSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Default Email Template System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'DefaultEmailTemplate','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
