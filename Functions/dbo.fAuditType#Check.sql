...
GO
CREATE FUNCTION [dbo].[fAuditType#Check]
	(
	 @AuditTypeSID       int
	,@AuditTypeLabel     nvarchar(35)
	,@AuditTypeCategory  nvarchar(65)
	,@IsDefault          bit
	,@IsActive           bit
	,@AuditTypeXID       varchar(150)
...
$AutoRules | Tim Edlund        ValueIsRequired.AuditTypeCategory A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'AuditType','ValueIsRequired.AuditTypeCategory') = @ON and @AuditTypeCategory is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.AuditTypeCategory'
		set @columnNames        = N'AuditTypeCategory'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Audit Type Category'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'AuditType','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fAuditType#Check'
		)
		begin
		
			select @errorText = ext.fAuditType#Check
				(
				 @AuditTypeSID
				,@AuditTypeLabel
				,@AuditTypeCategory
