...
GO
CREATE FUNCTION [dbo].[fCredentialType#Check]
	(
	 @CredentialTypeSID      int
	,@CredentialTypeLabel    nvarchar(35)
	,@CredentialTypeCategory nvarchar(65)
	,@IsDefault              bit
	,@IsActive               bit
	,@CredentialTypeXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fCredentialType#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.CredentialTypeCategory A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'CredentialType','ValueIsRequired.CredentialTypeCategory') = @ON and @CredentialTypeCategory is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.CredentialTypeCategory'
		set @columnNames        = N'CredentialTypeCategory'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Credential Type Category'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'CredentialType','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fCredentialType#Check'
		)
		begin
		
			select @errorText = ext.fCredentialType#Check
				(
				 @CredentialTypeSID
				,@CredentialTypeLabel
				,@CredentialTypeCategory
