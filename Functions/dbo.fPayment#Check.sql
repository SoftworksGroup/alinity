...
GO
CREATE FUNCTION [dbo].[fPayment#Check]
	(
	 @PaymentSID          int
	,@PersonSID           int
	,@PaymentTypeSID      int
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.PaymentTypeSID The %1 is marked inactive. Assign an active %1.
...
$AutoRules | Tim Edlund        ValueIsRequired.DepositDate A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.GLPostingDate A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.LastResponseCode A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.NameOnCard A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.PaymentCard A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.ReasonSID A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.Reference A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.TransactionID A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.VerifiedTime A value for "%1" is required.
Tim Edlund | Nov 2018          MBR.PeriodIsLocked.GLPostingDate'; (see source for message text)
Tim Edlund | Sep 2017          MBR.DepositGLAccount (see source for message text)
Tim Edlund | Nov 2018          MBR.DepositBeforePosting.DepositDate (see source for message text)
Tim Edlund | Oct 2017          PaymentCancel.CancelledTime'; (see source for message text)
...
begin
	...
	begin
		if (select x.IsActive from dbo.PaymentType x where x.PaymentTypeSID = @PaymentTypeSID) = @OFF		-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.PaymentTypeSID'
			set @columnNames        = N'PaymentTypeSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'payment type'
		end
	end
	--!</Rule>
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Payment','ValueIsRequired.CancelledTime') = @ON and @CancelledTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Payment','ValueIsRequired.DepositDate') = @ON and @DepositDate is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.DepositDate'
		set @columnNames        = N'DepositDate'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Deposit Date'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Payment','ValueIsRequired.GLPostingDate') = @ON and @GLPostingDate is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Payment','ValueIsRequired.LastResponseCode') = @ON and @LastResponseCode is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.LastResponseCode'
		set @columnNames        = N'LastResponseCode'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Last Response Code'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Payment','ValueIsRequired.NameOnCard') = @ON and @NameOnCard is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.NameOnCard'
		set @columnNames        = N'NameOnCard'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Name On Card'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Payment','ValueIsRequired.PaymentCard') = @ON and @PaymentCard is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.PaymentCard'
		set @columnNames        = N'PaymentCard'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Payment Card'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Payment','ValueIsRequired.ReasonSID') = @ON and @ReasonSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Payment','ValueIsRequired.Reference') = @ON and @Reference is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.Reference'
		set @columnNames        = N'Reference'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Reference'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Payment','ValueIsRequired.TransactionID') = @ON and @TransactionID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.TransactionID'
		set @columnNames        = N'TransactionID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Transaction ID'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'Payment','ValueIsRequired.VerifiedTime') = @ON and @VerifiedTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.VerifiedTime'
		set @columnNames        = N'VerifiedTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Verified Time'
	end
	--!</Rule>
	...
	if @errorMessageSCD is null and @GLAccountCode is not null and not exists(select 1 from dbo.GLAccount ga where ga.GLAccountCode = @GLAccountCode and ga.IsBankAccount = @ON)
	begin
		set @errorMessageSCD      = 'MBR.DepositGLAccount'
		set @columnNames          = N'GLAccountCode'
		set @defaultMessageText   = N'The GL Account set for depositing is invalid. You must assign a bank account. Check GL Accounts in Settings.'
	end
	--!</Rule>
	
	--!<Rule Author="Tim Edlund | Nov 2018" Updates="None">
	if @errorMessageSCD is null and @DepositDate < @GLPostingDate
	begin
		set @errorMessageSCD      = 'MBR.DepositBeforePosting.DepositDate'
		set @columnNames          = N'DepositDate,GLPostingDate'
		set @defaultMessageText   = N'The deposit date cannot be before to the GL posting date. (Deposit: "%1" GL Posting: "%2")'
		set @arg1                 = @DepositDate
		set @arg2									= @GLPostingDate
	end
	--!</Rule>
	...
	begin
		...
		(
			...
			where
				ip.PaymentSID = @PaymentSID and isnull(ip.AmountApplied,0.00) > 0.00
			and
		...
		begin
			set @errorMessageSCD = 'PaymentCancel.CancelledTime';
			set @columnNames = N'CancelledTime';
			set @defaultMessageText = N'A payment cannot be set to cancelled until all applications of the payment to invoices are unapplied. Use Cancel option.';
		end;
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'Payment','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPayment#Check'
		)
		begin
		
			select @errorText = ext.fPayment#Check
				(
				 @PaymentSID
				,@PersonSID
				,@PaymentTypeSID
