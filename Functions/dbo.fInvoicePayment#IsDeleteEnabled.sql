...
GO
CREATE FUNCTION [dbo].[fInvoicePayment#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fInvoicePayment#IsDeleteEnabled
...
select top (10)
   x.InvoicePaymentSID
  ,dbo.fInvoicePayment#IsDeleteEnabled(x.InvoicePaymentSID) IsDeleteEnabled
from
...
begin
	...
	begin
		...
		(
			...
			from
				dbo.InvoicePayment x00
			left outer join
				  (select top (1) x.GLTransactionSID, x.InvoicePaymentSID from dbo.GLTransaction x where isnull(x.InvoicePaymentSID, -1) = @InvoicePaymentSID) x01 on x00.InvoicePaymentSID =  x01.InvoicePaymentSID
			where
				x00.InvoicePaymentSID = @InvoicePaymentSID
	...
	begin
		...
		(
			...
			from
				dbo.InvoicePayment x
			where
				x.InvoicePaymentSID = @InvoicePaymentSID
