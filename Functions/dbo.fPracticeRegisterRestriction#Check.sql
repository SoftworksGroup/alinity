...
GO
CREATE FUNCTION [dbo].[fPracticeRegisterRestriction#Check]
	(
	 @PracticeRegisterRestrictionSID int
	,@PracticeRegisterSID            int
	,@PracticeRestrictionSID         int
	,@PracticeRegisterSectionSID     int
	,@EffectiveTime                  datetime
	,@ExpiryTime                     datetime
	,@PracticeRegisterRestrictionXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPracticeRegisterRestriction#Check
...
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.PracticeRestrictionSID The %1 is marked inactive. Assign an active %1
...
select
	...
	,dbo.fPracticeRegisterRestriction#Check
		(
		 PracticeRegisterRestrictionSID
		,PracticeRegisterSID
...
begin
	...
	if @errorMessageSCD is null and sf.fIsActive(@EffectiveTime, @ExpiryTime) = @ON and @isInsert = @ON																		-- applies to active assignments when inserting
	begin
		if (select x.IsActive from dbo.PracticeRestriction x where x.PracticeRestrictionSID = @PracticeRestrictionSID) = @OFF								-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.PracticeRestrictionSID'
			set @columnNames        = N'PracticeRestrictionSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'practice restriction'
		end
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PracticeRegisterRestriction','ValueIsRequired.PracticeRegisterSectionSID') = @ON and @PracticeRegisterSectionSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PracticeRegisterRestriction','BackDatingLimit.EffectiveTime') = @ON
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PracticeRegisterRestriction','FutureDatingLimit.ExpiryTime') = @ON
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PracticeRegisterRestriction','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPracticeRegisterRestriction#Check'
		)
		begin
		
			select @errorText = ext.fPracticeRegisterRestriction#Check
				(
				 @PracticeRegisterRestrictionSID
				,@PracticeRegisterSID
				,@PracticeRestrictionSID
