...
GO
CREATE FUNCTION [sf].[fTextTrigger#Check]
	(
	 @TextTriggerSID     int
	,@TextTriggerLabel   nvarchar(35)
	,@TextTemplateSID    int
	,@QuerySID           int
	,@MinDaysToRepeat    int
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'TextTrigger','ValueIsRequired.ApplicationUserSID') = @ON and @ApplicationUserSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'TextTrigger','ValueIsRequired.JobScheduleSID') = @ON and @JobScheduleSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'TextTrigger','ValueIsRequired.LastEndTime') = @ON and @LastEndTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'TextTrigger','ValueIsRequired.LastStartTime') = @ON and @LastStartTime is null
	begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'TextTrigger','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
