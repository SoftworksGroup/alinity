...
GO
CREATE FUNCTION [dbo].[fRegistrantAppReview#Check]
	(
	 @RegistrantAppReviewSID int
	...
	,@RegistrantAppReviewXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantAppReview#Check
...
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.RecommendationSID The %1 is marked inactive. Assign an active %1.
...
$AutoRules | Tim Edlund        ValueIsRequired.RecommendationSID A value for "%1" is required.
...
select
	...
	,dbo.fRegistrantAppReview#Check
		(
		 RegistrantAppReviewSID
		,RegistrantAppSID
		,FormVersionSID
...
begin
	...
	if @isInsert = @ON																											-- applies when inserting
	begin
		if (select x.IsActive from dbo.Recommendation x where x.RecommendationSID = @RecommendationSID) = @OFF											-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.RecommendationSID'
			set @columnNames        = N'RecommendationSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'recommendation'
		end
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAppReview','ValueIsRequired.LastValidateTime') = @ON and @LastValidateTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAppReview','ValueIsRequired.ReasonSID') = @ON and @ReasonSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAppReview','ValueIsRequired.RecommendationSID') = @ON and @RecommendationSID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.RecommendationSID'
		set @columnNames        = N'RecommendationSID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Recommendation'
	end
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAppReview','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantAppReview#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantAppReview#Check
				(
				 @RegistrantAppReviewSID
				,@RegistrantAppSID
				,@FormVersionSID
