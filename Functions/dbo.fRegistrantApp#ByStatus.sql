...
GO
CREATE FUNCTION dbo.fRegistrantApp#ByStatus
(
	@RegistrationScheduleYearSID int	-- the registration year key to include in the analysis (Required)
 ,@FormStatusSID							 int	-- status of applications to include (or NULL for all)
...
returns table
/*********************************************************************************************************************************
Function: Applications - By Status
Notice  : Copyright © 2018 Softworks Group Inc.
Summary	: Returns reporting/statistical values on applications for status and/or date range
----------------------------------------------------------------------------------------------------------------------------------
...
Comments	
--------
This function supports quick searches and reporting routines on Applications.  It returns one row for each application selected.
All criteria except the Registration Year are optional.  The main use cases are:

Applications by Status: Admin enters a date range and a status.  For example "Submitted" - returns registrants who are in a 
submitted status where the Update-Time on the Application record is between the criteria dates or, the criteria dates
are NULL.  

Alternatively, the function can be called without any status or date range using the @IsOpenOnly criteria to return applications
...
join
	dbo.RegistrationScheduleYear rsy on ra.RegistrationYear = rsy.RegistrationYear
order by
	newid();
...
begin
	...
	select
		x.*
	from
		dbo.fRegistrantApp#ByStatus(@RegistrationScheduleYearSID, @FormStatusSID, @UpdateFrom, @UpdateTo, @IsOpenOnly, @IsPaidOnly) x;
...
select top (1)
	...
	,@UpdateFrom = dateadd(day, -1, ra.UpdateTime)
	,@UpdateTo = ra.UpdateTime
from
	dbo.RegistrantApp ra
join
	dbo.RegistrationScheduleYear rsy on ra.RegistrationYear = rsy.RegistrationYear
...
exec sf.pUnitTest#Execute
	 @ObjectName = 'dbo.fRegistrantApp#ByStatus'
	,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
as
return
(
	...
	cross apply dbo.fRegistrantApp#CurrentStatus(-1, rsy.RegistrationYear) cs -- get status of each record in the year
	join
