...
GO
CREATE FUNCTION [dbo].[fAuditType#IsDeleteEnabled]
	(
	@AuditTypeSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : dbo.fAuditType#IsDeleteEnabled
...
select top (10)
   x.AuditTypeSID
  ,dbo.fAuditType#IsDeleteEnabled(x.AuditTypeSID) IsDeleteEnabled
from
  dbo.AuditType x
order by
  newid()
...
begin
	...
	if (select x.IsDefault from dbo.AuditType x where x.AuditTypeSID = @AuditTypeSID) = @ON set @isDeleteEnabled = @OFF	-- default records cannot be deleted
	...
	begin
		...
		(
			...
			from
				dbo.AuditType       x00
			left outer join
				  (select top (1) x.AuditTypeFormSID, x.AuditTypeSID from dbo.AuditTypeForm x where x.AuditTypeSID = @AuditTypeSID) x01 on x00.AuditTypeSID =  x01.AuditTypeSID
			left outer join
				  (select top (1) x.RegistrantAuditSID, x.AuditTypeSID from dbo.RegistrantAudit x where x.AuditTypeSID = @AuditTypeSID) x02 on x00.AuditTypeSID =  x02.AuditTypeSID
			where
				x00.AuditTypeSID = @AuditTypeSID
