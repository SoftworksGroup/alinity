...
CREATE FUNCTION [sf].[fUnitTest#IsDeleteEnabled]
	(
	@UnitTestSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : sf.fUnitTest#IsDeleteEnabled
...
select top (10)
   x.UnitTestSID
  ,sf.fUnitTest#IsDeleteEnabled(x.UnitTestSID) IsDeleteEnabled
from
  sf.UnitTest x
order by
  newid()
...
begin
	...
	if (select x.IsDefault from sf.UnitTest x where x.UnitTestSID = @UnitTestSID) = @ON set @isDeleteEnabled = @OFF	-- default records cannot be deleted
