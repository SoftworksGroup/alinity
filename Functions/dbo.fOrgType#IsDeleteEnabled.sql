...
GO
CREATE FUNCTION [dbo].[fOrgType#IsDeleteEnabled]
	(
	@OrgTypeSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : dbo.fOrgType#IsDeleteEnabled
...
select top (10)
   x.OrgTypeSID
  ,dbo.fOrgType#IsDeleteEnabled(x.OrgTypeSID) IsDeleteEnabled
from
  dbo.OrgType x
order by
  newid()
...
begin
	...
	if (select x.IsDefault from dbo.OrgType x where x.OrgTypeSID = @OrgTypeSID) = @ON set @isDeleteEnabled = @OFF	-- default records cannot be deleted
	...
	begin
		...
		(
			...
			from
				dbo.OrgType x00
			left outer join
				  (select top (1) x.OrgSID, x.OrgTypeSID from dbo.Org x where x.OrgTypeSID = @OrgTypeSID) x01 on x00.OrgTypeSID =  x01.OrgTypeSID
			where
				x00.OrgTypeSID = @OrgTypeSID
