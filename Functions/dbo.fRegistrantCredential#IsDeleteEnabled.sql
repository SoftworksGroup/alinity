...
GO
CREATE FUNCTION [dbo].[fRegistrantCredential#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantCredential#IsDeleteEnabled
...
select top (10)
  ...
  ,dbo.fRegistrantCredential#IsDeleteEnabled(x.RegistrantCredentialSID) IsDeleteEnabled
from
	...
	begin
		...
		(
			select
				...
				+ count(x02.RegistrationProfileSID)
				+ count(x03.RegistrationProfileSID)
			from
				dbo.RegistrantCredential x00
			left outer join
				  (select top (1) x.RegistrationProfileSID, x.Education1RegistrantCredentialSID from dbo.RegistrationProfile x where isnull(x.Education1RegistrantCredentialSID, -1) = @RegistrantCredentialSID) x01 on x00.RegistrantCredentialSID =  x01.Education1RegistrantCredentialSID
			left outer join
				  (select top (1) x.RegistrationProfileSID, x.Education2RegistrantCredentialSID from dbo.RegistrationProfile x where isnull(x.Education2RegistrantCredentialSID, -1) = @RegistrantCredentialSID) x02 on x00.RegistrantCredentialSID =  x02.Education2RegistrantCredentialSID
			left outer join
				  (select top (1) x.RegistrationProfileSID, x.Education3RegistrantCredentialSID from dbo.RegistrationProfile x where isnull(x.Education3RegistrantCredentialSID, -1) = @RegistrantCredentialSID) x03 on x00.RegistrantCredentialSID =  x03.Education3RegistrantCredentialSID
			where
				x00.RegistrantCredentialSID = @RegistrantCredentialSID
	...
	begin
		...
		(
			...
			from
				dbo.RegistrantCredential x
			where
				x.RegistrantCredentialSID = @RegistrantCredentialSID
