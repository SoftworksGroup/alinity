...
GO
CREATE FUNCTION [sf].[fFormVersion#Check]
	(
	 @FormVersionSID     int
	,@FormSID            int
	,@VersionNo          smallint
	,@RevisionNo         smallint
	,@IsSaveDisplayed    bit
	,@ApprovedTime       datetimeoffset(7)
	,@FormVersionXID     varchar(150)
...
$AutoRules | Tim Edlund        ValueIsRequired.ApprovedTime A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'FormVersion','ValueIsRequired.ApprovedTime') = @ON and @ApprovedTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ApprovedTime'
		set @columnNames        = N'ApprovedTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Approved Time'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'FormVersion','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
