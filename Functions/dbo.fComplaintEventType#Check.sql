...
GO
CREATE FUNCTION [dbo].[fComplaintEventType#Check]
	(
	 @ComplaintEventTypeSID   int
	,@ComplaintEventTypeSCD   varchar(20)
	,@ComplaintEventTypeLabel nvarchar(35)
	,@IsDefault               bit
	,@ComplaintEventTypeXID   varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fComplaintEventType#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.ComplaintEventTypeSCD The "%1" must be uppercase and not contain spaces or any
...
begin
	...
	if @errorMessageSCD is null and @ComplaintEventTypeSCD is not null and (sf.fIsStringContentValid(@ComplaintEventTypeSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@ComplaintEventTypeSCD) <> @ComplaintEventTypeSCD collate Latin1_General_CS_AI)					-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.ComplaintEventTypeSCD'
		set @columnNames        = N'ComplaintEventTypeSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Complaint Event Type System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ComplaintEventType','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fComplaintEventType#Check'
		)
		begin
		
			select @errorText = ext.fComplaintEventType#Check
				(
				 @ComplaintEventTypeSID
				,@ComplaintEventTypeSCD
				,@ComplaintEventTypeLabel
