...
GO
CREATE FUNCTION [dbo].[fRegistrantLanguage#Check]
	(
	 @RegistrantLanguageSID int
	,@RegistrantSID         int
	,@LanguageSID           int
	,@IsSpoken              bit
	,@IsWritten             bit
	,@RegistrantLanguageXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantLanguage#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.LanguageSID The %1 is marked inactive. Assign an active %1.
...
begin
	...
	begin
		if (select x.IsActive from dbo.Language x where x.LanguageSID = @LanguageSID) = @OFF						-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.LanguageSID'
			set @columnNames        = N'LanguageSID'
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantLanguage','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantLanguage#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantLanguage#Check
				(
				 @RegistrantLanguageSID
				,@RegistrantSID
				,@LanguageSID
