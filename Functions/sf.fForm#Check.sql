...
GO
CREATE FUNCTION [sf].[fForm#Check]
	(
	 @FormSID            int
	,@FormTypeSID        int
	,@FormName           nvarchar(65)
	,@FormLabel          nvarchar(35)
	,@FormContext        varchar(25)
	,@AuthorCredit       nvarchar(500)
	,@IsActive           bit
...
$AutoRules | Tim Edlund        ValueIsRequired.FormContext A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'Form','ValueIsRequired.FormContext') = @ON and @FormContext is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.FormContext'
		set @columnNames        = N'FormContext'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Form Context'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'Form','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
