...
GO
CREATE FUNCTION [dbo].[fLearningRequirementProration#Check]
	(
	 @LearningRequirementProrationSID int
	,@LearningRequirementSID          int
	,@StartMonthDay                   char(4)
	,@LearningRequirementMinimum      decimal(5,2)
	,@PercentageOfFullMinimum         decimal(6,3)
	,@LearningRequirementProrationXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fLearningRequirementProration#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
Tim Edlund | Jul 2018          MBR.EitherOneOrOtherRequired.LearningRequirementMinimum (see source for message text)
Tim Edlund | Jul 2018          MBR.EitherOneOrOtherRequired.LearningRequirementMinimum (see source for message text)
Tim Edlund | Jul 2018          MBR.OutOfRange.PercentageOfFullMinimum'; (see source for message text)
Tim Edlund | Jul 2018          MBR.OutOfRange.PercentageOfFullMinimum (see source for message text)
Tim Edlund | Jul 2018          MBR.InvalidStartMonthDay.StartMonthDay (see source for message text)
...
select
	...
	,StartMonthDay
	,dbo.fLearningRequirementProration#Check
		(
		 LearningRequirementProrationSID
...
begin
	...
	if @errorMessageSCD is null and isnull(@LearningRequirementMinimum,0.0) = 0.0 and isnull(@PercentageOfFullMinimum,0) = 0.0
	begin
		set @errorMessageSCD      = 'MBR.EitherOneOrOtherRequired.LearningRequirementMinimum'
		set @columnNames          = N'LearningRequirementMinimum, PercentageOfFullMinimum'
		set @defaultMessageText   = N'Either the "%1" or the "%2" must be set (but not both)'
		set @arg1                 = N'Prorated Requirement (units)'
		set @arg2									= N'Percentage of Full-Term Requirement'
	end
	--!</Rule>
	...
	begin
		...
		set @columnNames          = N'LearningRequirementMinimum, PercentageOfFullMinimum'
		set @defaultMessageText   = N'Either the "%1" or the "%2" must be set (but not both)'
		set @arg1                 = N'Prorated Requirement'
		set @arg2									= N'Percentage of Full-Term Requirement'
	end
	--!</Rule>
	
	--!<Rule Author="Tim Edlund | Jul 2018" Updates="None">
	if @errorMessageSCD is null and isnull(@LearningRequirementMinimum, 0.0) <> 0.0
	begin

		if @LearningRequirementMinimum < 0.00 or @LearningRequirementMinimum >
										 (
											 select
													lr.Minimum
											 from
		...
		begin
			set @errorMessageSCD = 'MBR.OutOfRange.PercentageOfFullMinimum';
			set @columnNames = N'LearningRequirementMinimum';
			set @defaultMessageText = N'The %1 must be between %2 and %3';
			set @arg1 = N'Prorated Requirement';
			set @arg2 = N'0.00';

			select
				@arg3 = ltrim(lr.Minimum) + ' (full-term requirement)'
			from
				dbo.LearningRequirementProration lrp
			join
				dbo.LearningRequirement					lr on lrp.LearningRequirementSID = lr.LearningRequirementSID
	...
	--!<Rule Author="Tim Edlund | Jul 2018" Updates="None">
	if @errorMessageSCD is null and @PercentageOfFullMinimum < 0.0 or @PercentageOfFullMinimum > 100.0
	begin
		set @errorMessageSCD      = 'MBR.OutOfRange.PercentageOfFullMinimum'
		set @columnNames          = N'PercentageOfFullMinimum'
		set @defaultMessageText   = N'The %1 must be between %2 and %3'
		set @arg1                 = N'Percentage of Full-Term Requirement'
		set @arg2									= N'0.0'
		set @arg3									= N'100.0'
	end
	--!</Rule>
	...
	--!<Rule Author="Tim Edlund | Jul 2018" Updates="None">
	if @errorMessageSCD is null and try_cast(ltrim(year(sysdatetime())) + @StartMonthDay as date) is null
	begin
		set @errorMessageSCD      = 'MBR.InvalidStartMonthDay.StartMonthDay'
		set @columnNames          = N'StartMonthDay'
		set @defaultMessageText   = N'The start month and day "%1" is invalid (use "MMDD" format).'
		set @arg1                 = @StartMonthDay
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'LearningRequirementProration','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fLearningRequirementProration#Check'
		)
		begin
		
			select @errorText = ext.fLearningRequirementProration#Check
				(
				 @LearningRequirementProrationSID
				,@LearningRequirementSID
				,@StartMonthDay
