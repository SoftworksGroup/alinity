...
GO
CREATE FUNCTION [sf].[fMessageLink#Check]
	(
	 @MessageLinkSID     int
	,@MessageLinkSCD     varchar(30)
	,@MessageLinkLabel   nvarchar(35)
	,@ApplicationPageSID int
	,@MessageLinkXID     varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.MessageLinkSCD The "%1" must be uppercase and not contain spaces or any punctua
...
begin
	...
	if @errorMessageSCD is null and @MessageLinkSCD is not null and (sf.fIsStringContentValid(@MessageLinkSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@MessageLinkSCD) <> @MessageLinkSCD collate Latin1_General_CS_AI)												-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.MessageLinkSCD'
		set @columnNames        = N'MessageLinkSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Message Link System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'MessageLink','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
