...
GO
CREATE FUNCTION dbo.fRegistrantApp#ReviewReasons
(
	@RegistrantAppSID int				-- key of record to return review reasons for or -1 for all
 ,@RegistrationYear smallint	-- year of form records to return, or -1 for all or when first param is provided
)
returns table
/*********************************************************************************************************************************
Function	: Application - Review Reasons
Notice		: Copyright © 2018 Softworks Group Inc.
Summary		: Returns the set of administrative Review Reasons (if any) associated with application forms
----------------------------------------------------------------------------------------------------------------------------------
...
Comments	
--------
This table function parses the "ReasonList" column on the application record and returns the reason information for each key stored 
...
function requires either a Application year (typically current year) and/or a specific form record to return review reasons 
for.  If only 1 parameter is provided, then the other parameter must be passed as -1.  DO NOT PASS AS NULL.

Normalization 
-------------
This table function will return from 0 to many records.  If a application record has no review reasons then no record is returned.
For that reason be sure to OUTER APPLY to this function if the application record is required in the final data set whether or not
...
begin
	...
	select
		x.*
	from
		dbo.fRegistrantApp#ReviewReasons(-1, @registrationYear) x
	order by
		x.RegistrantAppSID
	 ,x.ReasonSequence;
...
exec sf.pUnitTest#Execute
	@ObjectName = 'dbo.fRegistrantApp#ReviewReasons'
 ,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
as
return
(
	...
	(
		...
		join
			dbo.RegistrantApp																			 frm on f.RegistrantAppSID = frm.RegistrantAppSID
