...
CREATE FUNCTION [sf].[fCrossWalk#IsDeleteEnabled]
	(
	@CrossWalkSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : sf.fCrossWalk#IsDeleteEnabled
...
select top (10)
   x.CrossWalkSID
  ,sf.fCrossWalk#IsDeleteEnabled(x.CrossWalkSID) IsDeleteEnabled
from
  sf.CrossWalk x
order by
  newid()
...
begin
	...
	if (select x.IsDefault from sf.CrossWalk x where x.CrossWalkSID = @CrossWalkSID) = @ON set @isDeleteEnabled = @OFF-- default records cannot be deleted
	...
	begin
		...
		(
			select
				  count(x01.CrossWalkPairSID)
			from
				sf.CrossWalk     x00
			left outer join
				  (select top (1) x.CrossWalkPairSID, x.CrossWalkSID from sf.CrossWalkPair x where x.CrossWalkSID = @CrossWalkSID) x01 on x00.CrossWalkSID =  x01.CrossWalkSID
			where
				x00.CrossWalkSID = @CrossWalkSID
