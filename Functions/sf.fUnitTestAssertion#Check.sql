...
GO
CREATE FUNCTION [sf].[fUnitTestAssertion#Check]
	(
	 @UnitTestAssertionSID int
	,@UnitTestSID          int
	,@AssertionType        varchar(25)
	,@Value                nvarchar(1000)
	,@ResultSet            tinyint
	,@RowNo                int
	,@ColumnNo             int
	,@UnitTestAssertionXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : sf.fUnitTestAssertion#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        ValueIsRequired.Value A value for "%1" is required.
Tim Edlund | Jan 2018          MBR.NotInList (see source for message text)
Tim Edlund | Jan 2018          MBR.AssertionValueRequired.Value (see source for message text)
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'UnitTestAssertion','ValueIsRequired.Value') = @ON and @Value is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.Value'
		set @columnNames        = N'Value'
	...
	if @errorMessageSCD is null and @AssertionType not in ('ScalarValue','RowCount','ExecutionTime','Inconclusive','NotEmptyResultSet','EmptyResultSet')
	begin
		set @errorMessageSCD    = 'MBR.NotInList'
		set @columnNames        = N'AssertionType'
		set @defaultMessageText = N'The %1 provided "%2" is not valid. It must be one of: %3'
		set @arg1               = 'assertion type'
		set @arg2								= @AssertionType
		set @arg3								= '"ScalarValue","RowCount","ExecutionTime","Inconclusive","NotEmptyResultSet", or, "EmptyResultSet"'
	end
	--!</Rule>
	
	--!<Rule Author="Tim Edlund | Jan 2018" Updates="None">
	if @errorMessageSCD is null and isnull(ltrim(@Value), '') = '' and @AssertionType in ('DataCheckSum', 'ExecutionTime', 'ExpectedSchema', 'RowCount', 'ScalarValue')
	begin
		set @errorMessageSCD      = 'MBR.AssertionValueRequired.Value'
		set @columnNames        = N'Value'
		set @defaultMessageText = N'A value must be specified for the assertion type "%1".'
		set @arg1               = @AssertionType
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'UnitTestAssertion','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
