...
GO
CREATE FUNCTION [dbo].[fPersonNote#LearningPlanActivityFolderXml]
(
...
as
/*********************************************************************************************************************************
Function	: Person Note - LearningPlanActivity Folder XML
...
<TestHarness>
		...
		<SQLScript>
			<![CDATA[
				...
				select 
				 ...
				 ,p.LastName
				 ,p.FirstName
				 ,dbo.fPersonNote#LearningPlanActivityFolderXml(p.PersonSID) ProfileUpdateFolderXML
				from
					sf.Person p;
	...
	declare
				...
				select 
				 @responseXML = dbo.fPersonNote#LearningPlanActivityFolderXml(@PersonSID) 
...
exec sf.pUnitTest#Execute																								
			@ObjectName = N'dbo.fPersonNote#LearningPlanActivityFolderXml'
		,	@DefaultTestOnly = 1
...
begin
	...
	return ( select
			...
			,dbo.fPersonNote#ItemXml(r.PersonSID, 'dbo.LearningPlanActivity', lpa.LearningPlanActivitySID)
		from
