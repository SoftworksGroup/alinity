...
GO
CREATE FUNCTION [dbo].[fGLAccount#IsDeleteEnabled]
	(
	@GLAccountSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : dbo.fGLAccount#IsDeleteEnabled
...
select top (10)
   x.GLAccountSID
  ,dbo.fGLAccount#IsDeleteEnabled(x.GLAccountSID) IsDeleteEnabled
from
  dbo.GLAccount x
order by
  newid()
...
begin
	...
	begin
		...
		(
			select
				  count(x01.CatalogItemSID)
				+ count(x02.PaymentTypeSID)
				+ count(x03.TaxConfigurationSID)
			from
				dbo.GLAccount        x00
			left outer join
				  (select top (1) x.CatalogItemSID, x.GLAccountSID from dbo.CatalogItem x where x.GLAccountSID = @GLAccountSID) x01 on x00.GLAccountSID =  x01.GLAccountSID
			left outer join
				  (select top (1) x.PaymentTypeSID, x.GLAccountSID from dbo.PaymentType x where x.GLAccountSID = @GLAccountSID) x02 on x00.GLAccountSID =  x02.GLAccountSID
			left outer join
				  (select top (1) x.TaxConfigurationSID, x.GLAccountSID from dbo.TaxConfiguration x where x.GLAccountSID = @GLAccountSID) x03 on x00.GLAccountSID =  x03.GLAccountSID
			where
				x00.GLAccountSID = @GLAccountSID
