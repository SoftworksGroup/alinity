...
GO
CREATE FUNCTION [dbo].[fRegistrationChange#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrationChange#IsDeleteEnabled
...
select top (10)
  ...
  ,dbo.fRegistrationChange#IsDeleteEnabled(x.RegistrationChangeSID) IsDeleteEnabled
from
...
begin
	...
	begin
		...
		(
			select
				  count(x01.RegistrationChangeRequirementSID)
			from
				dbo.RegistrationChange            x00
			left outer join
				  (select top (1) x.RegistrationChangeRequirementSID, x.RegistrationChangeSID from dbo.RegistrationChangeRequirement x where x.RegistrationChangeSID = @RegistrationChangeSID) x01 on x00.RegistrationChangeSID =  x01.RegistrationChangeSID
			where
				x00.RegistrationChangeSID = @RegistrationChangeSID
	...
	begin
		...
		(
			...
			from
				dbo.RegistrationChange x
			where
				x.RegistrationChangeSID = @RegistrationChangeSID
