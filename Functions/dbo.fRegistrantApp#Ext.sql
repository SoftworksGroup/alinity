...
GO
CREATE FUNCTION dbo.fRegistrantApp#Ext (@RegistrantAppSID int) -- key of record to check
returns @registrantApp#Ext table
(
	IsViewEnabled						bit								not null	-- indicates whether either the (logged in) user or administrator can view the application
 ...
 ,IsWithdrawalEnabled			bit								not null	-- indicates the application form can be withdrawn by administrators or SA's
 ...
 ,FormStatusLabel					nvarchar(35)			not null	-- user-friendly name for the application status		
 ...
 ,TotalDue								decimal(11, 2)		null			-- amount owing on invoice associated with the application (blank if no invoice created)
 ,IsUnPaid								bit								not null	-- indicates if the invoice associated with the application is unpaid
 ...
 ,CitySID									int								null
 ,PersonSID								int								not null	-- sf.Person key of the applicant
 ...
 ,PracticeRegisterLabel		nvarchar(35)			not null	-- label (short name) of the register this application is being made to
 ,PracticeRegisterName		nvarchar(65)			not null	-- name of the register this application is being made to
 ,RegistrantAppLabel			nvarchar(80)			not null	-- a summary label for the application based on the register label and application status
 ...
 ,RecommendationLabel			nvarchar(20)			null			-- recommendation term to display summarizing reviews on the application
)
as
/*********************************************************************************************************************************
TableF	: Registrant Application Extended Columns
Notice	: Copyright © 2017 Softworks Group Inc.
Summary	: Returns a table of calculated columns for the RegistrantApp extended view (vRegistrantApp#Ext)
----------------------------------------------------------------------------------------------------------------------------------
History	: Author	  					| Month Year	| Change Summary
				...
				: Tim Edlund					| Jun 2018		| FK in RegistrantApp changed from RegistrantSID to RegistrationSID + standards updates
...
Comments	
--------
This function is called by the dbo.vRegistrantApp#Ext view to return a series of calculated values. By using a table function,
...
<TestHarness>
	...
	<![CDATA[
		...
		cross apply
			dbo.fRegistrantApp#Ext(frm.RegistrantAppSID) rax
		order by
			newid()
...
exec sf.pUnitTest#Execute
	 @ObjectName = 'dbo.fRegistrantApp#Ext'
	,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
...
begin
	...
	declare
	 ...
	 ,@isViewEnabled					 bit							= cast(0 as bit)											-- indicates whether either the (logged in) user or administrator can view the application
	 ...
	 ,@isWithdrawalEnabled		 bit							= cast(0 as bit)											-- indicates the application form can be withdrawn by administrators or SA's
	 ...
	 ,@formStatusLabel				 nvarchar(35)																						-- user-friendly name for the application status		
	 ...
	 ,@totalDue								 decimal(11, 2)																					-- amount owing on invoice associated with the application (blank if no invoice created)
	 ,@totalPaid							 decimal(11, 2)																					-- amount paid on the application invoice (blank if no invoice created)
	 ,@isUnPaid								 bit							= cast(0 as bit)											-- indicates if the invoice associated with the application is unpaid
	 ...
	 ,@practiceRegisterLabel	 nvarchar(35)																						-- label (short name) of the register this application is being made to
	 ,@practiceRegisterName		 nvarchar(65)																						-- name of the register this application is being made to
	 ,@registrantAppLabel			 nvarchar(80)																						-- a summary label for the application based on the register label and application status
	...
	-- retrieve the current status of the application and set
	...
	select
	 ...
	 ,@registrantAppLabel			= pr.PracticeRegisterLabel + N' - ' + (case when cs.IsInProgress = cast(1 as bit) then 'In Progress' else cs.FormStatusLabel end)
	...
	outer apply dbo.fRegistrantApp#CurrentStatus(frm.RegistrantAppSID, -1) cs -- the function determines the current status of the form
	where
		frm.RegistrantAppSID = @RegistrantAppSID;

	set @registrantAppLabel += N' (' + @registrationYearLabel + N')';
	...
	begin
		...
		begin
			...
			begin
				set @isWithdrawalEnabled = @isEditEnabled;
			end;
			else
			begin
	...
	begin
		...
		where
			ae.ApplicationEntitySCD = 'dbo.RegistrantApp';
		...
		where
			pdc.EntitySID = @RegistrantAppSID and pdc.IsPrimary = @ON and pdc.ApplicationEntitySID = @applicationEntitySID;
