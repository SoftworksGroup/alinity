...
GO
CREATE FUNCTION [dbo].[fLearningRequirementCompetenceType#Check]
	(
	 @LearningRequirementCompetenceTypeSID int
	,@LearningRequirementSID               int
	,@CompetenceTypeSID                    int
	,@LearningRequirementCompetenceTypeXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fLearningRequirementCompetenceType#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'LearningRequirementCompetenceType','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fLearningRequirementCompetenceType#Check'
		)
		begin
		
			select @errorText = ext.fLearningRequirementCompetenceType#Check
				(
				 @LearningRequirementCompetenceTypeSID
				,@LearningRequirementSID
				,@CompetenceTypeSID
