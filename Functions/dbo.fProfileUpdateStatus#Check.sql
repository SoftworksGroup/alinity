...
GO
CREATE FUNCTION [dbo].[fProfileUpdateStatus#Check]
	(
	 @ProfileUpdateStatusSID int
	,@ProfileUpdateSID       int
	,@FormStatusSID          int
	,@ProfileUpdateStatusXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fProfileUpdateStatus#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ProfileUpdateStatus','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fProfileUpdateStatus#Check'
		)
		begin
		
			select @errorText = ext.fProfileUpdateStatus#Check
				(
				 @ProfileUpdateStatusSID
				,@ProfileUpdateSID
				,@FormStatusSID
