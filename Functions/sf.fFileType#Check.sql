...
GO
CREATE FUNCTION [sf].[fFileType#Check]
	(
	 @FileTypeSID        int
	,@FileTypeSCD        varchar(8)
	,@FileTypeLabel      nvarchar(35)
	,@MimeType           varchar(255)
	,@IsInline           bit
	,@IsActive           bit
	,@FileTypeXID        varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.FileTypeSCD The "%1" must be uppercase and not contain spaces or any punctuatio
...
begin
	...
	if @errorMessageSCD is null and @FileTypeSCD is not null and (sf.fIsStringContentValid(@FileTypeSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@FileTypeSCD) <> @FileTypeSCD collate Latin1_General_CS_AI)		-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.FileTypeSCD'
		set @columnNames        = N'FileTypeSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'File Type System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'FileType','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
