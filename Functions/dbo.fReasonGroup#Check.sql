...
GO
CREATE FUNCTION [dbo].[fReasonGroup#Check]
	(
	 @ReasonGroupSID     int
	,@ReasonGroupSCD     varchar(20)
	,@ReasonGroupLabel   nvarchar(35)
	,@IsLockedGroup      bit
	,@ReasonGroupXID     varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fReasonGroup#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.ReasonGroupSCD The "%1" must be uppercase and not contain spaces or any punctua
...
begin
	...
	if @errorMessageSCD is null and @ReasonGroupSCD is not null and (sf.fIsStringContentValid(@ReasonGroupSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@ReasonGroupSCD) <> @ReasonGroupSCD collate Latin1_General_CS_AI)												-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.ReasonGroupSCD'
		set @columnNames        = N'ReasonGroupSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Reason Group System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ReasonGroup','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fReasonGroup#Check'
		)
		begin
		
			select @errorText = ext.fReasonGroup#Check
				(
				 @ReasonGroupSID
				,@ReasonGroupSCD
				,@ReasonGroupLabel
