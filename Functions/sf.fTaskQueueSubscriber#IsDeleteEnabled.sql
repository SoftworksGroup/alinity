...
CREATE FUNCTION [sf].[fTaskQueueSubscriber#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : sf.fTaskQueueSubscriber#IsDeleteEnabled
...
select top (10)
  ...
  ,sf.fTaskQueueSubscriber#IsDeleteEnabled(x.TaskQueueSubscriberSID) IsDeleteEnabled
from
...
begin
	...
	begin
		...
		(
			...
			from
				sf.TaskQueueSubscriber x
			where
				x.TaskQueueSubscriberSID = @TaskQueueSubscriberSID
			) <> sf.fApplicationUserSession#UserName() set @isDeleteEnabled = sf.fIsGranted('ADMIN.BASE')
	
	end

	-- Monica Lane | Nov 2014
	-- After a a person is assigned a task they can no longer be
	-- deleted from the task queue even where their tasks are
	-- cancelled or closed - required for audit trail.
	...
	(
		...
		join
			sf.Task									t			on tqs.ApplicationUserSID = t.ApplicationUserSID and tqs.TaskQueueSID = t.TaskQueueSID
		join
