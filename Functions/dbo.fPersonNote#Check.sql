...
GO
CREATE FUNCTION [dbo].[fPersonNote#Check]
	(
	 @PersonNoteSID       int
	,@PersonSID           int
	,@PersonNoteTypeSID   int
	,@NoteTitle           nvarchar(65)
	,@ShowToRegistrant    bit
	,@ApplicationGrantSID int
	,@PersonNoteXID       varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.PersonNoteTypeSID The %1 is marked inactive. Assign an active %1.
$AutoRules | Tim Edlund        ValueIsRequired.ApplicationGrantSID A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.NoteTitle A value for "%1" is required.
...
begin
	...
	begin
		if (select x.IsActive from dbo.PersonNoteType x where x.PersonNoteTypeSID = @PersonNoteTypeSID) = @OFF						-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.PersonNoteTypeSID'
			set @columnNames        = N'PersonNoteTypeSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'person note type'
		end
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonNote','ValueIsRequired.ApplicationGrantSID') = @ON and @ApplicationGrantSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonNote','ValueIsRequired.NoteTitle') = @ON and @NoteTitle is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.NoteTitle'
		set @columnNames        = N'NoteTitle'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Note Title'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonNote','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPersonNote#Check'
		)
		begin
		
			select @errorText = ext.fPersonNote#Check
				(
				 @PersonNoteSID
				,@PersonSID
				,@PersonNoteTypeSID
