...
GO
CREATE FUNCTION [sf].[fPersonMailingPreference#Check]
	(
	 @PersonMailingPreferenceSID int
	,@PersonSID                  int
	,@MailingPreferenceSID       int
	,@EffectiveTime              datetime
	,@ExpiryTime                 datetime
	,@PersonMailingPreferenceXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : sf.fPersonMailingPreference#Check
...
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.MailingPreferenceSID The %1 is marked inactive. Assign an active %1.
...
begin
	...
	begin
		if (select x.IsActive from sf.MailingPreference x where x.MailingPreferenceSID = @MailingPreferenceSID) = @OFF								-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.MailingPreferenceSID'
			set @columnNames        = N'MailingPreferenceSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'mailing preference'
		end
	end
	--!</Rule>
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonMailingPreference','BackDatingLimit.EffectiveTime') = @ON
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'PersonMailingPreference','FutureDatingLimit.ExpiryTime') = @ON
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'PersonMailingPreference','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
