...
GO
CREATE FUNCTION [dbo].[fCompetenceType#Check]
	(
	 @CompetenceTypeSID      int
	,@CompetenceTypeLabel    nvarchar(35)
	,@CompetenceTypeCategory nvarchar(65)
	,@IsDefault              bit
	,@IsActive               bit
	,@CompetenceTypeXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fCompetenceType#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.CompetenceTypeCategory A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'CompetenceType','ValueIsRequired.CompetenceTypeCategory') = @ON and @CompetenceTypeCategory is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.CompetenceTypeCategory'
		set @columnNames        = N'CompetenceTypeCategory'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Competence Type Category'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'CompetenceType','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fCompetenceType#Check'
		)
		begin
		
			select @errorText = ext.fCompetenceType#Check
				(
				 @CompetenceTypeSID
				,@CompetenceTypeLabel
				,@CompetenceTypeCategory
