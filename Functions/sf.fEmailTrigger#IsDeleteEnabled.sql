...
CREATE FUNCTION [sf].[fEmailTrigger#IsDeleteEnabled]
	(
	@EmailTriggerSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : sf.fEmailTrigger#IsDeleteEnabled
...
select top (10)
   x.EmailTriggerSID
  ,sf.fEmailTrigger#IsDeleteEnabled(x.EmailTriggerSID) IsDeleteEnabled
from
  sf.EmailTrigger x
order by
  newid()
...
begin
	...
	begin
		...
		(
			select
				  count(x01.PersonEmailMessageSID)
			from
				sf.EmailTrigger       x00
			left outer join
				  (select top (1) x.PersonEmailMessageSID, x.EmailTriggerSID from sf.PersonEmailMessage x where isnull(x.EmailTriggerSID, -1) = @EmailTriggerSID) x01 on x00.EmailTriggerSID =  x01.EmailTriggerSID
			where
				x00.EmailTriggerSID = @EmailTriggerSID
	...
	begin
		...
		(
			...
			from
				sf.EmailTrigger x
			where
				x.EmailTriggerSID = @EmailTriggerSID
