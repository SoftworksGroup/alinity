...
GO
CREATE FUNCTION [dbo].[fIdentifierType#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fIdentifierType#IsDeleteEnabled
...
select top (10)
   x.IdentifierTypeSID
  ,dbo.fIdentifierType#IsDeleteEnabled(x.IdentifierTypeSID) IsDeleteEnabled
from
...
begin
	...
	if (select x.IsDefault from dbo.IdentifierType x where x.IdentifierTypeSID = @IdentifierTypeSID) = @ON set @isDeleteEnabled = @OFF-- default records cannot be deleted
	...
	begin
		...
		(
			select
				  count(x01.RegistrantIdentifierSID)
			from
				dbo.IdentifierType       x00
			left outer join
				  (select top (1) x.RegistrantIdentifierSID, x.IdentifierTypeSID from dbo.RegistrantIdentifier x where x.IdentifierTypeSID = @IdentifierTypeSID) x01 on x00.IdentifierTypeSID =  x01.IdentifierTypeSID
			where
				x00.IdentifierTypeSID = @IdentifierTypeSID
	...
	begin
		...
		(
			...
			from
				dbo.IdentifierType x
			where
				x.IdentifierTypeSID = @IdentifierTypeSID
