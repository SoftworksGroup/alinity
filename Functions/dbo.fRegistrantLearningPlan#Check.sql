...
GO
CREATE FUNCTION [dbo].[fRegistrantLearningPlan#Check]
	(
	...
	,@RegistrantLearningPlanXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantLearningPlan#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.ParentRowGUID A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantLearningPlan','ValueIsRequired.LastValidateTime') = @ON and @LastValidateTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantLearningPlan','ValueIsRequired.NextFollowUp') = @ON and @NextFollowUp is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantLearningPlan','ValueIsRequired.ParentRowGUID') = @ON and @ParentRowGUID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ParentRowGUID'
		set @columnNames        = N'ParentRowGUID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Parent Row GUID'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantLearningPlan','ValueIsRequired.ReasonSID') = @ON and @ReasonSID is null
	begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantLearningPlan','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantLearningPlan#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantLearningPlan#Check
				(
				 @RegistrantLearningPlanSID
				,@RegistrantSID
				,@RegistrationYear
