...
GO
CREATE FUNCTION dbo.fRegistrantLearningPlan#ReviewReasons
(
	@RegistrantLearningPlanSID int			-- key of record to return review reasons for or -1 for all
 ,@RegistrationYear					 smallint -- year of form records to return, or -1 for all or when first param is provided
)
returns table
/*********************************************************************************************************************************
Function	: Learning Plan - Review Reasons
Notice		: Copyright © 2018 Softworks Group Inc.
Summary		: Returns the set of administrative Review Reasons (if any) associated with learning plan forms
----------------------------------------------------------------------------------------------------------------------------------
...
Comments	
--------
This table function parses the "ReasonList" column on the learning plan record and returns the reason information for each key stored 
...
function requires either a Learning Plan year (typically current year) and/or a specific form record to return review reasons 
for.  If only 1 parameter is provided, then the other parameter must be passed as -1.  DO NOT PASS AS NULL.

Normalization 
-------------
This table function will return from 0 to many records.  If a learning plan record has no review reasons then no record is returned.
For that reason be sure to OUTER APPLY to this function if the learning plan record is required in the final data set whether or not
...
begin
	...
	select
		x.*
	from
		dbo.fRegistrantLearningPlan#ReviewReasons(-1, @registrationYear) x
	order by
...
exec sf.pUnitTest#Execute
	@ObjectName = 'dbo.fRegistrantLearningPlan#ReviewReasons'
 ,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
as
return
(
	...
	(
		...
		(
			...
			where
				(
					@RegistrantLearningPlanSID = -1 or frm.RegistrantLearningPlanSID = @RegistrantLearningPlanSID
				)
				and (@RegistrationYear			 = -1 or frm.RegistrationYear = @RegistrationYear)
		)																												 f
		join
			dbo.RegistrantLearningPlan														 frm on f.RegistrantLearningPlanSID = frm.RegistrantLearningPlanSID
