...
GO
CREATE FUNCTION [dbo].[fRegistrationSnapshotType#Check]
	(
	 @RegistrationSnapshotTypeSID       int
	,@RegistrationSnapshotTypeLabel     nvarchar(35)
	,@RegistrationSnapshotTypeSCD       varchar(15)
	,@RegistrationSnapshotLabelTemplate nvarchar(50)
	,@IsDefault                         bit
	,@IsActive                          bit
	,@RegistrationSnapshotTypeXID       varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrationSnapshotType#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.RegistrationSnapshotTypeSCD The "%1" must be uppercase and not contain spaces o
...
begin
	...
	if @errorMessageSCD is null and @RegistrationSnapshotTypeSCD is not null and (sf.fIsStringContentValid(@RegistrationSnapshotTypeSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@RegistrationSnapshotTypeSCD) <> @RegistrationSnapshotTypeSCD collate Latin1_General_CS_AI)															-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.RegistrationSnapshotTypeSCD'
		set @columnNames        = N'RegistrationSnapshotTypeSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Registration Snapshot Type System Code'
	end
	--!</Rule>
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationSnapshotType','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrationSnapshotType#Check'
		)
		begin
		
			select @errorText = ext.fRegistrationSnapshotType#Check
				(
				 @RegistrationSnapshotTypeSID
				,@RegistrationSnapshotTypeLabel
				,@RegistrationSnapshotTypeSCD
