...
GO
CREATE FUNCTION [sf].[fTaskTrigger#Check]
	(
	 @TaskTriggerSID          int
	,@TaskTriggerLabel        nvarchar(35)
	,@TaskTitleTemplate       nvarchar(65)
	,@QuerySID                int
	,@TaskQueueSID            int
	,@ApplicationUserSID      int
	,@IsAlert                 bit
	,@PriorityLevel           tinyint
	,@TargetCompletionDays    smallint
	,@OpenTaskLimit           int
	,@IsRegeneratedIfClosed   bit
	,@ApplicationAction       varchar(75)
	,@JobScheduleSID          int
	,@LastStartTime           datetimeoffset(7)
	,@LastEndTime             datetimeoffset(7)
	,@IsActive                bit
	,@TaskTriggerXID          varchar(150)
...
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.QuerySID The %1 is marked inactive. Assign an active %1.
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.TaskQueueSID The %1 is marked inactive. Assign an active %1.
$AutoRules | Tim Edlund        ValueIsRequired.ApplicationAction A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.ApplicationUserSID A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.JobScheduleSID A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.LastEndTime A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.LastStartTime A value for "%1" is required.
...
begin
	...
	if @isInsert = @ON																											-- applies when inserting
	begin
		if (select x.IsActive from sf.Query x where x.QuerySID = @QuerySID) = @OFF											-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.QuerySID'
			set @columnNames        = N'QuerySID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'query'
		end
	end
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'TaskTrigger','ValueIsRequired.ApplicationAction') = @ON and @ApplicationAction is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ApplicationAction'
		set @columnNames        = N'ApplicationAction'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Application Action'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'TaskTrigger','ValueIsRequired.ApplicationUserSID') = @ON and @ApplicationUserSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'TaskTrigger','ValueIsRequired.JobScheduleSID') = @ON and @JobScheduleSID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.JobScheduleSID'
		set @columnNames        = N'JobScheduleSID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Job Schedule'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'TaskTrigger','ValueIsRequired.LastEndTime') = @ON and @LastEndTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.LastEndTime'
		set @columnNames        = N'LastEndTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Last End Time'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'TaskTrigger','ValueIsRequired.LastStartTime') = @ON and @LastStartTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.LastStartTime'
		set @columnNames        = N'LastStartTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Last Start Time'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'TaskTrigger','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
