...
GO
CREATE FUNCTION [sf].[fUnitTest#Check]
	(
	 @UnitTestSID        int
...
------------------------------ ----------------------------------------------------------------------------------------------------
Tim Edlund | Jan 2018          CannotStartWithNumber.TestName (see source for message text)
...
begin
	...
	if @errorMessageSCD is null and charindex(left(@TestName, 1), N'0123456789') > 0
	begin
		set @errorMessageSCD      = 'CannotStartWithNumber.TestName'
		set @columnNames          = N'TestName'
		set @defaultMessageText   = N'The %1 cannot begin with a number'
		set @arg1                 = 'test name'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'UnitTest','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
