...
GO
CREATE FUNCTION dbo.fRegistrantRenewal#StatusSummary
(
	@RegistrationYear smallint	-- the registration year to include in the analysis
)
returns table
/*********************************************************************************************************************************
Function: Renewal - Status Summary
Notice  : Copyright © 2018 Softworks Group Inc.
Summary	: Returns reporting/statistical data on Renewals (excluding WITHDRAWN)
----------------------------------------------------------------------------------------------------------------------------------
...
Comments	
--------
This function supports reporting on the status of Renewals in a given registration year. @RegistrationYear is a required parameter
and cannot be replaced with -1 to return results for all years in the same query. The function returns one row for each status 
along with the count of forms in that status.  The content returned also includes the form owner. This makes it possible to produce 
reports that first group by the "next to act" party and then the statuses within those sections. For Renewals 3 next-to-act parties 
are possible:  Member, Administrator and None (form is closed status).

Note that any Renewals in the "WITHDRAWN" status are explicitly excluded from the statistics returned.
...
cross apply dbo.fRegistrantRenewal#CurrentStatus(frm.RegistrantRenewalSID, -1) cs
where
...
begin
	...
	from
		dbo.fRegistrantRenewal#StatusSummary(@registrationYear) x
	order by
		 x.NextToActGroup
		,x.FormStatusSequence;
...
exec sf.pUnitTest#Execute
	 @ObjectName = 'dbo.fRegistrantRenewal#StatusSummary'
	,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
as
return
