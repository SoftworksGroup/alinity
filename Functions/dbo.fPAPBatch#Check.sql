...
GO
CREATE FUNCTION [dbo].[fPAPBatch#Check]
	(
	 @PAPBatchSID        int
	,@BatchID            varchar(12)
	,@BatchSequence      int
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        ValueIsRequired.BatchSequence A value for "%1" is required.
...
Tim Edlund | Apr 2018          MBR.ConditionallyRequired.BatchSequence'; (see source for message text)
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PAPBatch','ValueIsRequired.BatchSequence') = @ON and @BatchSequence is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.BatchSequence'
		set @columnNames        = N'BatchSequence'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Batch Sequence'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PAPBatch','ValueIsRequired.LockedTime') = @ON and @LockedTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PAPBatch','ValueIsRequired.ProcessedTime') = @ON and @ProcessedTime is null
	begin
	...
	if @errorMessageSCD is null and @LockedTime is not null and @BatchSequence is null
	begin
		set @errorMessageSCD = 'MBR.ConditionallyRequired.BatchSequence';
		set @columnNames = N'LockedTime, BatchSequence';
		set @defaultMessageText = N'The %1 value must be provided when %2 is filled in.';
		set @arg1 = N'batch sequence #';
		set @arg2 = N'locked time';
	end;
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PAPBatch','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPAPBatch#Check'
		)
		begin
		
			select @errorText = ext.fPAPBatch#Check
				(
				 @PAPBatchSID
				,@BatchID
				,@BatchSequence
