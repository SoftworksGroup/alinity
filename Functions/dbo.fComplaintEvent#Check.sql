...
GO
CREATE FUNCTION [dbo].[fComplaintEvent#Check]
	(
	 @ComplaintEventSID     int
	,@ComplaintSID          int
	,@ComplaintEventTypeSID int
	,@Description           nvarchar(500)
	,@DueDate               date
	,@CompleteTime          datetime
	,@ComplaintEventXID     varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fComplaintEvent#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        ValueIsRequired.CompleteTime A value for "%1" is required.
Tim Edlund | Jan 2019          MBR.EventDateTooEarly.DueDate'; (see source for message text)
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'ComplaintEvent','ValueIsRequired.CompleteTime') = @ON and @CompleteTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.CompleteTime'
		set @columnNames        = N'CompleteTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Complete Time'
	end
	--!</Rule>
	...
	begin
		...
		(
			...
			where
				ce.ComplaintEventSID = @ComplaintEventSID and (@DueDate < c.OpenedDate or @CompleteTime < c.OpenedDate)
		)
		begin

			select
				@arg1 = format(c.OpenedDate, 'dd-MMM-yyyy')
			from
				dbo.Complaint c
			where
				c.ComplaintSID = @ComplaintSID;

			set @errorMessageSCD = 'MBR.EventDateTooEarly.DueDate';
			set @columnNames = N'DueDate,CompleteTime';
			set @defaultMessageText = N'The due date and commplete time cannot be before the complaint was opened (%1)';
		end;
	end;
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ComplaintEvent','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fComplaintEvent#Check'
		)
		begin
		
			select @errorText = ext.fComplaintEvent#Check
				(
				 @ComplaintEventSID
				,@ComplaintSID
				,@ComplaintEventTypeSID
