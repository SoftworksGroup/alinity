...
GO
CREATE function dbo.fProfileUpdate#ReviewReasonsHTML
(
	@ProfileUpdateSID int				-- key of record to return review reason for information for or -1 for all
 ,@RegistrationYear smallint	-- year of form records to return, or -1 for all or when first param is provided
)
returns nvarchar(max)
/*********************************************************************************************************************************
Function	: ProfileUpdate - Review Reasons HTML
...
Comments	
--------
This function formats the review reasons on the profile update and returns is as an HTML unordered list. The reasons are stored in
an XML column on the profile update and the parsing for the the HTML is done in the function dbo.fProfileUpdate#ReviewReasons. This
...
begin
	...
	select
		dbo.fProfileUpdate#ReviewReasonsHTML(-1, @registrationYear)
...
exec sf.pUnitTest#Execute
	@ObjectName = 'dbo.fProfileUpdate#ReviewReasonsHTML'
 ,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
as
begin
	...
	select
		@reasonsHtml = N'<ul>' + stuff((
																		 ...
																		 from
																				dbo.fProfileUpdate#ReviewReasons(@ProfileUpdateSID, @RegistrationYear) r
																		 for xml path(''), type
