...
GO
CREATE FUNCTION [dbo].[fEmploymentType#Check]
	(
	 @EmploymentTypeSID      int
	,@EmploymentTypeName     nvarchar(50)
	,@EmploymentTypeCode     varchar(20)
	,@EmploymentTypeCategory nvarchar(65)
	,@IsDefault              bit
	,@IsActive               bit
	,@EmploymentTypeXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fEmploymentType#Check
...
$AutoRules | Tim Edlund        ValueIsRequired.EmploymentTypeCategory A value for "%1" is required.
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'EmploymentType','ValueIsRequired.EmploymentTypeCategory') = @ON and @EmploymentTypeCategory is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.EmploymentTypeCategory'
		set @columnNames        = N'EmploymentTypeCategory'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Employment Type Category'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'EmploymentType','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fEmploymentType#Check'
		)
		begin
		
			select @errorText = ext.fEmploymentType#Check
				(
				 @EmploymentTypeSID
				,@EmploymentTypeName
				,@EmploymentTypeCode
