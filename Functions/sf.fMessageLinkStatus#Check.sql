...
GO
CREATE FUNCTION [sf].[fMessageLinkStatus#Check]
	(
	 @MessageLinkStatusSID   int
	,@MessageLinkStatusSCD   varchar(10)
	,@MessageLinkStatusLabel nvarchar(35)
	,@IsResendEnabled        bit
	,@IsActive               bit
	,@IsDefault              bit
	,@MessageLinkStatusXID   varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : sf.fMessageLinkStatus#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.MessageLinkStatusSCD The "%1" must be uppercase and not contain spaces or any p
...
begin
	...
	if @errorMessageSCD is null and @MessageLinkStatusSCD is not null and (sf.fIsStringContentValid(@MessageLinkStatusSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@MessageLinkStatusSCD) <> @MessageLinkStatusSCD collate Latin1_General_CS_AI)						-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.MessageLinkStatusSCD'
		set @columnNames        = N'MessageLinkStatusSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Message Link Status System Code'
	end
	--!</Rule>
	...
	begin
		...
		set @arg1               = N'message link status'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'MessageLinkStatus','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
