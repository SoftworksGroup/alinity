...
GO
CREATE FUNCTION [dbo].[fPracticeScope#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPracticeScope#IsDeleteEnabled
...
select top (10)
   x.PracticeScopeSID
  ,dbo.fPracticeScope#IsDeleteEnabled(x.PracticeScopeSID) IsDeleteEnabled
from
  dbo.PracticeScope x
order by
  newid()
...
begin
	...
	if (select x.IsDefault from dbo.PracticeScope x where x.PracticeScopeSID = @PracticeScopeSID) = @ON set @isDeleteEnabled = @OFF	-- default records cannot be deleted
	...
	begin
		...
		(
			...
			from
				dbo.PracticeScope        x00
			left outer join
				  (select top (1) x.RegistrantEmploymentSID, x.PracticeScopeSID from dbo.RegistrantEmployment x where x.PracticeScopeSID = @PracticeScopeSID) x01 on x00.PracticeScopeSID =  x01.PracticeScopeSID
			where
				x00.PracticeScopeSID = @PracticeScopeSID
	...
	begin
		...
		(
			...
			from
				dbo.PracticeScope x
			where
				x.PracticeScopeSID = @PracticeScopeSID
