...
GO
CREATE FUNCTION [dbo].[fPracticeRestriction#Check]
	(
	 @PracticeRestrictionSID   int
	,@PracticeRestrictionLabel nvarchar(35)
	,@IsDisplayedOnLicense     bit
	,@IsActive                 bit
	,@IsSupervisionRequired    bit
	,@PracticeRestrictionXID   varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPracticeRestriction#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PracticeRestriction','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPracticeRestriction#Check'
		)
		begin
		
			select @errorText = ext.fPracticeRestriction#Check
				(
				 @PracticeRestrictionSID
				,@PracticeRestrictionLabel
				,@IsDisplayedOnLicense
