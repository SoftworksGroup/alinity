...
GO
CREATE FUNCTION [sf].[fTaskQueue#IsDeleteEnabled]
	(
	@TaskQueueSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : sf.fTaskQueue#IsDeleteEnabled
...
select top (10)
   x.TaskQueueSID
  ,sf.fTaskQueue#IsDeleteEnabled(x.TaskQueueSID) IsDeleteEnabled
from
  sf.TaskQueue x
order by
  newid()
...
begin
	...
	if (select x.IsDefault from sf.TaskQueue x where x.TaskQueueSID = @TaskQueueSID) = @ON set @isDeleteEnabled = @OFF-- default records cannot be deleted
	...
	begin
		...
		(
			select
				  count(x01.TaskSID)
				+ count(x02.TaskTriggerSID)
			from
				sf.TaskQueue       x00
			left outer join
				  (select top (1) x.TaskSID, x.TaskQueueSID from sf.Task x where x.TaskQueueSID = @TaskQueueSID) x01 on x00.TaskQueueSID =  x01.TaskQueueSID
			left outer join
				  (select top (1) x.TaskTriggerSID, x.TaskQueueSID from sf.TaskTrigger x where x.TaskQueueSID = @TaskQueueSID) x02 on x00.TaskQueueSID =  x02.TaskQueueSID
			where
				x00.TaskQueueSID = @TaskQueueSID
