...
GO
CREATE FUNCTION [dbo].[fProfileUpdate#Check]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fProfileUpdate#Check
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'ProfileUpdate','ValueIsRequired.LastValidateTime') = @ON and @LastValidateTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'ProfileUpdate','ValueIsRequired.NextFollowUp') = @ON and @NextFollowUp is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'ProfileUpdate','ValueIsRequired.ParentRowGUID') = @ON and @ParentRowGUID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'ProfileUpdate','ValueIsRequired.ReasonSID') = @ON and @ReasonSID is null
	begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ProfileUpdate','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fProfileUpdate#Check'
		)
		begin
		
			select @errorText = ext.fProfileUpdate#Check
				(
				 @ProfileUpdateSID
				,@PersonSID
				,@RegistrationYear
