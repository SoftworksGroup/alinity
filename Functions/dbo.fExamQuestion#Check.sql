...
GO
CREATE FUNCTION [dbo].[fExamQuestion#Check]
	(
	 @ExamQuestionSID    int
	,@ExamSectionSID     int
	,@Sequence           smallint
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fExamQuestion#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.KnowledgeDomainSID The %1 is marked inactive. Assign an active %1.
...
begin
	...
	begin
		if (select x.IsActive from dbo.KnowledgeDomain x where x.KnowledgeDomainSID = @KnowledgeDomainSID) = @OFF						-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.KnowledgeDomainSID'
			set @columnNames        = N'KnowledgeDomainSID'
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ExamQuestion','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fExamQuestion#Check'
		)
		begin
		
			select @errorText = ext.fExamQuestion#Check
				(
				 @ExamQuestionSID
				,@ExamSectionSID
				,@Sequence
