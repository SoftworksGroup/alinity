...
GO
CREATE FUNCTION [dbo].[fPracticeRegisterType#Check]
	(
	 @PracticeRegisterTypeSID      int
	,@PracticeRegisterTypeSCD      varchar(15)
	,@PracticeRegisterTypeLabel    nvarchar(35)
	,@PracticeRegisterTypeCategory nvarchar(65)
	,@IsDefault                    bit
	,@IsActive                     bit
	,@PracticeRegisterTypeXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPracticeRegisterType#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.PracticeRegisterTypeSCD The "%1" must be uppercase and not contain spaces or an
...
begin
	...
	if @errorMessageSCD is null and @PracticeRegisterTypeSCD is not null and (sf.fIsStringContentValid(@PracticeRegisterTypeSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@PracticeRegisterTypeSCD) <> @PracticeRegisterTypeSCD collate Latin1_General_CS_AI)			-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.PracticeRegisterTypeSCD'
		set @columnNames        = N'PracticeRegisterTypeSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Practice Register Type System Code'
	end
	--!</Rule>
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PracticeRegisterType','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPracticeRegisterType#Check'
		)
		begin
		
			select @errorText = ext.fPracticeRegisterType#Check
				(
				 @PracticeRegisterTypeSID
				,@PracticeRegisterTypeSCD
				,@PracticeRegisterTypeLabel
