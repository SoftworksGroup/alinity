...
GO
CREATE FUNCTION [dbo].[fAgeRange#Check]
	(
	 @AgeRangeSID        int
	,@AgeRangeTypeSID    int
	,@AgeRangeLabel      nvarchar(35)
	,@StartAge           smallint
	,@EndAge             smallint
	,@IsDefault          bit
	,@AgeRangeXID        varchar(150)
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'AgeRange','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fAgeRange#Check'
		)
		begin
		
			select @errorText = ext.fAgeRange#Check
				(
				 @AgeRangeSID
				,@AgeRangeTypeSID
				,@AgeRangeLabel
