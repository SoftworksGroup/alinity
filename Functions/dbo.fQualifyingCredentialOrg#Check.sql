...
GO
CREATE FUNCTION [dbo].[fQualifyingCredentialOrg#Check]
	(
	 @QualifyingCredentialOrgSID int
	,@CredentialSID              int
	,@OrgSID                     int
	,@IsActive                   bit
	,@QualifyingCredentialOrgXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fQualifyingCredentialOrg#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'QualifyingCredentialOrg','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fQualifyingCredentialOrg#Check'
		)
		begin
		
			select @errorText = ext.fQualifyingCredentialOrg#Check
				(
				 @QualifyingCredentialOrgSID
				,@CredentialSID
				,@OrgSID
