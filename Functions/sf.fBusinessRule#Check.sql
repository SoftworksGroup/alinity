...
GO
CREATE FUNCTION [sf].[fBusinessRule#Check]
	(
	 @BusinessRuleSID      int
	,@ApplicationEntitySID int
	,@MessageSID           int
	,@ColumnName           nvarchar(128)
	,@BusinessRuleStatus   char(1)
	,@BusinessRuleXID      varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : sf.fBusinessRule#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
Tim Edlund | Oct 2011          MBR.CannotBeTurnedOff.BusinessRuleStatus (see source for message text)
Tim Edlund | Nov 2011          MBR.NotInList.BusinessRuleStatus (see source for message text)
...
begin
	...
	--!<Rule Author="Tim Edlund | Oct 2011" Updates="None">
  if @BusinessRuleStatus = 'x'
  begin

    if exists (select 1 from sf.[Message] m where m.MessageSID = @MessageSID and m.MessageSCD like 'MBR.%')
    begin
      set @errorMessageSCD      = 'MBR.CannotBeTurnedOff.BusinessRuleStatus'
      set @columnNames          = N'IsEnforced'
      set @defaultMessageText   = N'Mandatory business rules cannot be turned off.'
    end

  end
  --!</Rule>
	
	--!<Rule Author="Tim Edlund | Nov 2011" Updates="None">
  if @BusinessRuleStatus not in ('o', 'x', 'p', '!')
  begin
    set @errorMessageSCD    = 'MBR.NotInList.BusinessRuleStatus'
    set @defaultMessageText = N'The value "%1" is not a recognized option for %2.  Expected values are: %3.'
		set @columnNames				= N'BusinessRuleStatus'
		set @Arg1               = convert(nchar(1), @BusinessRuleStatus)
    set @Arg2               = N'Business Rule Status'
    set @Arg3               = N'"o"(on), "x"(off), "p"(pending), "!"(in-process)'
  end
  --!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'BusinessRule','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
