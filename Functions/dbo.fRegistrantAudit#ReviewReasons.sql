...
GO
CREATE FUNCTION dbo.fRegistrantAudit#ReviewReasons
(
	@RegistrantAuditSID int				-- key of record to return review reasons for or -1 for all
 ,@RegistrationYear		smallint	-- year of form records to return, or -1 for all or when first param is provided
)
returns table
/*********************************************************************************************************************************
Function	: Audit - Review Reasons
Notice		: Copyright © 2018 Softworks Group Inc.
Summary		: Returns the set of administrative Review Reasons (if any) associated with audit forms
----------------------------------------------------------------------------------------------------------------------------------
...
Comments	
--------
This table function parses the "ReasonList" column on the audit record and returns the reason information for each key stored 
...
function requires either a Audit year (typically current year) and/or a specific form record to return review reasons 
for.  If only 1 parameter is provided, then the other parameter must be passed as -1.  DO NOT PASS AS NULL.

Normalization 
-------------
This table function will return from 0 to many records.  If a audit record has no review reasons then no record is returned.
For that reason be sure to OUTER APPLY to this function if the audit record is required in the final data set whether or not
...
begin
	...
	select
		x.*
	from
		dbo.fRegistrantAudit#ReviewReasons(-1, @registrationYear) x
	order by
...
exec sf.pUnitTest#Execute
	@ObjectName = 'dbo.fRegistrantAudit#ReviewReasons'
 ,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
as
return
(
	...
	(
		...
		join
			dbo.RegistrantAudit																		 frm on f.RegistrantAuditSID = frm.RegistrantAuditSID
