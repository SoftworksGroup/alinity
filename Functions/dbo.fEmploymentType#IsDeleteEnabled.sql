...
GO
CREATE FUNCTION [dbo].[fEmploymentType#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fEmploymentType#IsDeleteEnabled
...
select top (10)
   x.EmploymentTypeSID
  ,dbo.fEmploymentType#IsDeleteEnabled(x.EmploymentTypeSID) IsDeleteEnabled
from
...
begin
	...
	if (select x.IsDefault from dbo.EmploymentType x where x.EmploymentTypeSID = @EmploymentTypeSID) = @ON set @isDeleteEnabled = @OFF-- default records cannot be deleted
	...
	begin
		...
		(
			...
			from
				dbo.EmploymentType       x00
			left outer join
				  (select top (1) x.RegistrantEmploymentSID, x.EmploymentTypeSID from dbo.RegistrantEmployment x where x.EmploymentTypeSID = @EmploymentTypeSID) x01 on x00.EmploymentTypeSID =  x01.EmploymentTypeSID
			where
				x00.EmploymentTypeSID = @EmploymentTypeSID
	...
	begin
		...
		(
			...
			from
				dbo.EmploymentType x
			where
				x.EmploymentTypeSID = @EmploymentTypeSID
