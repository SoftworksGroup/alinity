...
GO
CREATE FUNCTION [dbo].[fPersonMailingAddress#Check]
	(
	...
	,@PersonMailingAddressXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPersonMailingAddress#Check
...
Robin Payne | Feb 2018         ValueIsAlsoRequired.PostalCode (see source for message text)
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonMailingAddress','ValueIsRequired.LastVerifiedTime') = @ON and @LastVerifiedTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonMailingAddress','ValueIsRequired.PostalCode') = @ON and @PostalCode is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonMailingAddress','ValueIsRequired.StreetAddress2') = @ON and @StreetAddress2 is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonMailingAddress','ValueIsRequired.StreetAddress3') = @ON and @StreetAddress3 is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonMailingAddress','BackDatingLimit.EffectiveTime') = @ON
	...
	-- This check must be before, the postal code validation step
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonMailingAddress','ValueIsAlsoRequired.PostalCode') = @ON
	and (@StreetAddress1 is not null and @PostalCode is null)
	begin
		set @errorMessageSCD    = 'ValueIsAlsoRequired.PostalCode'
		set @columnNames        = N'PostalCode, StreetAddress1'
		set @defaultMessageText = N'%1 is required if %2 is also filled in'
		set @arg1               = N'Postal Code'
		set @arg2               = N'Street Address'
	end
	--!</Rule>
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonMailingAddress','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPersonMailingAddress#Check'
		)
		begin
		
			select @errorText = ext.fPersonMailingAddress#Check
				(
				 @PersonMailingAddressSID
				,@PersonSID
				,@StreetAddress1
