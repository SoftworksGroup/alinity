...
GO
CREATE FUNCTION [sf].[fTaskTrigger#Ext]
(
	@TaskTriggerSID									int
)
returns @taskTrigger#Ext	table
(
...
Summary	: Returns a table of calculated columns for the Task Trigger extended view (vTaskTrigger#Ext)
----------------------------------------------------------------------------------------------------------------------------------
...
Comments	
--------
This function is called by the dbo.vTaskTrigger#Ext view to return a series of calculated columns.  This function is not intended 
...
Because task triggers can only be executed by the task trigger job, the time calculated to next run the trigger is compared with 
the time when the task trigger job will next run.  If the trigger is scheduled to run BEFORE the next time the task-trigger-job
will run, the task-trigger-job's scheduled time is used instead.
...
from 
	sf.TaskTrigger j
outer apply
	sf.fJob#Ext(tt.TaskTriggerSID) jx
...
begin
	...
	begin
		...
		where
			j.JobSCD = 'sf.pTaskTrigger#Execute'																-- Indicates this job is used to execute scheduled task triggers		 
		and
