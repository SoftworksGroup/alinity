...
GO
CREATE FUNCTION [sf].[fTaskQueue#Check]
	(
	 @TaskQueueSID       int
	,@TaskQueueLabel     nvarchar(35)
	,@TaskQueueCode      varchar(30)
	,@IsAutoAssigned     bit
	,@IsOpenSubscription bit
	,@ApplicationUserSID int
	,@IsActive           bit
	,@IsDefault          bit
	,@TaskQueueXID       varchar(150)
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'TaskQueue','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
