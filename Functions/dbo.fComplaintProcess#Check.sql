...
GO
CREATE FUNCTION [dbo].[fComplaintProcess#Check]
	(
	 @ComplaintProcessSID       int
	,@ComplaintProcessLabel     nvarchar(35)
	,@IsDefault                 bit
	,@ParentComplaintProcessSID int
	,@ComplaintProcessXID       varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fComplaintProcess#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        ValueIsRequired.ParentComplaintProcessSID A value for "%1" is required.
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'ComplaintProcess','ValueIsRequired.ParentComplaintProcessSID') = @ON and @ParentComplaintProcessSID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.ParentComplaintProcessSID'
		set @columnNames        = N'ParentComplaintProcessSID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Parent Complaint Process'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ComplaintProcess','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fComplaintProcess#Check'
		)
		begin
		
			select @errorText = ext.fComplaintProcess#Check
				(
				 @ComplaintProcessSID
				,@ComplaintProcessLabel
				,@IsDefault
