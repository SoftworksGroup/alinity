...
GO
CREATE FUNCTION [dbo].[fRegistrantPracticeRestriction#Check]
	(
	 @RegistrantPracticeRestrictionSID int
	,@RegistrantSID                    int
	...
	,@ComplaintSID                     int
	,@RegistrantPracticeRestrictionXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantPracticeRestriction#Check
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantPracticeRestriction','ValueIsRequired.ComplaintSID') = @ON and @ComplaintSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantPracticeRestriction','BackDatingLimit.EffectiveTime') = @ON
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantPracticeRestriction','FutureDatingLimit.ExpiryTime') = @ON
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantPracticeRestriction','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantPracticeRestriction#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantPracticeRestriction#Check
				(
				 @RegistrantPracticeRestrictionSID
				,@RegistrantSID
				,@PracticeRestrictionSID
