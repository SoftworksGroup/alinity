...
GO
CREATE FUNCTION [dbo].[fRegistrationChangeRequirement#Check]
	(
	 @RegistrationChangeRequirementSID int
	,@RegistrationChangeSID            int
	,@RegistrationRequirementSID       int
	,@PersonDocSID                     int
	,@RegistrantExamSID                int
	,@ExpiryMonths                     smallint
	,@RequirementStatusSID             int
	,@RequirementSequence              int
	,@RegistrationChangeRequirementXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrationChangeRequirement#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.RegistrationRequirementSID The %1 is marked inactive. Assign an activ
$AutoRules | Tim Edlund        ValueIsRequired.PersonDocSID A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.RegistrantExamSID A value for "%1" is required.
...
begin
	...
	begin
		if (select x.IsActive from dbo.RegistrationRequirement x where x.RegistrationRequirementSID = @RegistrationRequirementSID) = @OFF			-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.RegistrationRequirementSID'
			set @columnNames        = N'RegistrationRequirementSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'registration requirement'
		end
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationChangeRequirement','ValueIsRequired.PersonDocSID') = @ON and @PersonDocSID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.PersonDocSID'
		set @columnNames        = N'PersonDocSID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Person Doc'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationChangeRequirement','ValueIsRequired.RegistrantExamSID') = @ON and @RegistrantExamSID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.RegistrantExamSID'
		set @columnNames        = N'RegistrantExamSID'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Registrant Exam'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationChangeRequirement','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrationChangeRequirement#Check'
		)
		begin
		
			select @errorText = ext.fRegistrationChangeRequirement#Check
				(
				 @RegistrationChangeRequirementSID
				,@RegistrationChangeSID
				,@RegistrationRequirementSID
