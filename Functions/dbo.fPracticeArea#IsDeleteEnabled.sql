...
GO
CREATE FUNCTION [dbo].[fPracticeArea#IsDeleteEnabled]
	(
	@PracticeAreaSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : dbo.fPracticeArea#IsDeleteEnabled
...
select top (10)
   x.PracticeAreaSID
  ,dbo.fPracticeArea#IsDeleteEnabled(x.PracticeAreaSID) IsDeleteEnabled
from
  dbo.PracticeArea x
order by
  newid()
...
begin
	...
	if (select x.IsDefault from dbo.PracticeArea x where x.PracticeAreaSID = @PracticeAreaSID) = @ON set @isDeleteEnabled = @OFF-- default records cannot be deleted
	...
	begin
		...
		(
			select
				  count(x01.RegistrantEmploymentPracticeAreaSID)
			from
				dbo.PracticeArea                     x00
			left outer join
				  (select top (1) x.RegistrantEmploymentPracticeAreaSID, x.PracticeAreaSID from dbo.RegistrantEmploymentPracticeArea x where x.PracticeAreaSID = @PracticeAreaSID) x01 on x00.PracticeAreaSID =  x01.PracticeAreaSID
			where
				x00.PracticeAreaSID = @PracticeAreaSID
	...
	begin
		...
		(
			...
			from
				dbo.PracticeArea x
			where
				x.PracticeAreaSID = @PracticeAreaSID
