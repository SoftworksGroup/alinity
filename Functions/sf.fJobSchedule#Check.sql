...
GO
CREATE FUNCTION [sf].[fJobSchedule#Check]
	(
	 @JobScheduleSID        int
	...
	,@IsRunSaturday         bit
	,@IsRunSunday           bit
	,@RepeatIntervalMinutes smallint
	,@StartTime             time(0)
	,@EndTime               time(0)
	,@StartDate             date
	,@EndDate               date
	,@JobScheduleXID        varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        ValueIsRequired.EndDate A value for "%1" is required.
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'JobSchedule','ValueIsRequired.EndDate') = @ON and @EndDate is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.EndDate'
		set @columnNames        = N'EndDate'
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'JobSchedule','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
