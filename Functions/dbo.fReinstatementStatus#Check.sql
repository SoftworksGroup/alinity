...
GO
CREATE FUNCTION [dbo].[fReinstatementStatus#Check]
	(
	 @ReinstatementStatusSID int
	,@ReinstatementSID       int
	,@FormStatusSID          int
	,@ReinstatementStatusXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fReinstatementStatus#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ReinstatementStatus','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fReinstatementStatus#Check'
		)
		begin
		
			select @errorText = ext.fReinstatementStatus#Check
				(
				 @ReinstatementStatusSID
				,@ReinstatementSID
				,@FormStatusSID
