...
GO
CREATE FUNCTION [dbo].[fExamOffering#IsDeleteEnabled]
	(
	@ExamOfferingSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : dbo.fExamOffering#IsDeleteEnabled
...
select top (10)
   x.ExamOfferingSID
  ,dbo.fExamOffering#IsDeleteEnabled(x.ExamOfferingSID) IsDeleteEnabled
from
  dbo.ExamOffering x
order by
  newid()
...
begin
	...
	begin
		...
		(
			select
				  count(x01.RegistrantExamSID)
				+ count(x02.RegistrantExamProfileSID)
			from
				dbo.ExamOffering          x00
			left outer join
				  (select top (1) x.RegistrantExamSID, x.ExamOfferingSID from dbo.RegistrantExam x where isnull(x.ExamOfferingSID, -1) = @ExamOfferingSID) x01 on x00.ExamOfferingSID =  x01.ExamOfferingSID
			left outer join
				  (select top (1) x.RegistrantExamProfileSID, x.ExamOfferingSID from stg.RegistrantExamProfile x where isnull(x.ExamOfferingSID, -1) = @ExamOfferingSID) x02 on x00.ExamOfferingSID =  x02.ExamOfferingSID
			where
				x00.ExamOfferingSID = @ExamOfferingSID
	...
	begin
		...
		(
			...
			from
				dbo.ExamOffering x
			where
				x.ExamOfferingSID = @ExamOfferingSID
