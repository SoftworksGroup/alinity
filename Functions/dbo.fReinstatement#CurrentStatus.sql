...
GO
CREATE FUNCTION dbo.fReinstatement#CurrentStatus
(
	@ReinstatementSID int				-- key of record to return status information for or -1 for all
 ,@RegistrationYear			smallint	-- year of form records to return, or -1 for all or when first param is provided
)
returns table
/*********************************************************************************************************************************
Function	: Reinstatement Current (form) Status
Notice		: Copyright © 2018 Softworks Group Inc.
Summary		: Returns the latest form status for 1 or all Reinstatement forms 
----------------------------------------------------------------------------------------------------------------------------------
...
Comments	
--------
This table function modularizes the logic required to determine the latest status for one Reinstatement form, all forms, 
...
Maintenance Notes
----------------- 
*** Any logic change made to this function must also be made in dbo.fRegistration#FormStatus$Reinstatement ***

The data set returned is intentionally limited.  The application makes extensive use of this function for searching and 
extending attribution can lead to performance issues.  For additional attribution use the dbo.fReinstatement#Ext and
dbo.fReinstatement#ByStatus functions.
...
begin
	select x.* from		dbo.fReinstatement#CurrentStatus(-1, @registrationYear) x;
end;
			]]>
		</SQLScript>
		<Assertions>
...
select top (1)
	@ReinstatementSID = frm.ReinstatementSID
from
	dbo.Reinstatement frm
order by
	newid();

if @@rowcount = 0 or @ReinstatementSID is null
begin
	raiserror('** ERROR ** No Suitable Data Found For Test!', 17, 1);
end;
else
begin
	select x.* from	dbo.fReinstatement#CurrentStatus(@ReinstatementSID, -1) x;
end;
			]]>
		</SQLScript>
		<Assertions>
...
exec sf.pUnitTest#Execute
	 @ObjectName = 'dbo.fReinstatement#CurrentStatus'
	,@DefaultTestOnly = 1
-------------------------------------------------------------------------------------------------------------------------------- */
as
return
	...
	(
		...
		(
			...
			(
				...
				where
					(
						@ReinstatementSID = -1 or frm.ReinstatementSID = @ReinstatementSID
					)
