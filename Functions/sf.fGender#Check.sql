...
GO
CREATE FUNCTION [sf].[fGender#Check]
	(
	 @GenderSID          int
	,@GenderSCD          char(1)
	,@GenderLabel        nvarchar(35)
	,@IsActive           bit
	,@GenderXID          varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.GenderSCD The "%1" must be uppercase and not contain spaces or any punctuation
...
begin
	...
	if @errorMessageSCD is null and @GenderSCD is not null and (sf.fIsStringContentValid(@GenderSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@GenderSCD) <> @GenderSCD collate Latin1_General_CS_AI)				-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.GenderSCD'
		set @columnNames        = N'GenderSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Gender System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'Gender','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
