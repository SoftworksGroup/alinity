...
GO
CREATE FUNCTION [dbo].[fPersonGroupDoc#Check]
	(
	 @PersonGroupDocSID    int
	,@PersonGroupFolderSID int
	,@DocumentTitle        nvarchar(75)
	,@FileTypeSID          int
	,@FileTypeSCD          varchar(8)
	,@PersonGroupDocXID    varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPersonGroupDoc#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonGroupDoc','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPersonGroupDoc#Check'
		)
		begin
		
			select @errorText = ext.fPersonGroupDoc#Check
				(
				 @PersonGroupDocSID
				,@PersonGroupFolderSID
				,@DocumentTitle
