...
GO
CREATE FUNCTION [sf].[fDataSource#Check]
	(
	 @DataSourceSID      int
	,@DataSourceLabel    nvarchar(35)
	,@DBObjectName       nvarchar(257)
	,@ToolTip            nvarchar(500)
	,@FileFormatSID      int
	,@LastExecuteTime    datetimeoffset(7)
	,@LastExecuteUser    nvarchar(75)
	,@ExecuteCount       int
	,@DataSourceXID      varchar(150)
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'DataSource','ValueIsRequired.ToolTip') = @ON and @ToolTip is null
	begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'DataSource','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
