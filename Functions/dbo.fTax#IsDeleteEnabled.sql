...
GO
CREATE FUNCTION [dbo].[fTax#IsDeleteEnabled]
	(
	@TaxSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : dbo.fTax#IsDeleteEnabled
...
select top (10)
   x.TaxSID
  ,dbo.fTax#IsDeleteEnabled(x.TaxSID) IsDeleteEnabled
from
  dbo.Tax x
order by
  newid()
...
begin
	...
	begin
		...
		(
			select
				  count(x01.TaxConfigurationSID)
			from
				dbo.Tax              x00
			left outer join
				  (select top (1) x.TaxConfigurationSID, x.TaxSID from dbo.TaxConfiguration x where x.TaxSID = @TaxSID) x01 on x00.TaxSID =  x01.TaxSID
			where
				x00.TaxSID = @TaxSID
