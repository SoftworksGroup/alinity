...
GO
CREATE FUNCTION [dbo].[fPayment#IsDeleteEnabled]
	(
	@PaymentSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : dbo.fPayment#IsDeleteEnabled
...
select top (10)
   x.PaymentSID
  ,dbo.fPayment#IsDeleteEnabled(x.PaymentSID) IsDeleteEnabled
from
  dbo.Payment x
order by
  newid()
...
begin
	...
	begin
		...
		(
			select
				  count(x01.GLTransactionSID)
				+ count(x02.InvoicePaymentSID)
				+ count(x03.PAPTransactionSID)
				+ count(x04.PaymentProcessorResponseSID)
			from
				dbo.Payment                  x00
			left outer join
				  (select top (1) x.GLTransactionSID, x.PaymentSID from dbo.GLTransaction x where x.PaymentSID = @PaymentSID) x01 on x00.PaymentSID =  x01.PaymentSID
			left outer join
				  (select top (1) x.InvoicePaymentSID, x.PaymentSID from dbo.InvoicePayment x where x.PaymentSID = @PaymentSID) x02 on x00.PaymentSID =  x02.PaymentSID
			left outer join
				  (select top (1) x.PAPTransactionSID, x.PaymentSID from dbo.PAPTransaction x where isnull(x.PaymentSID, -1) = @PaymentSID) x03 on x00.PaymentSID =  x03.PaymentSID
			left outer join
				  (select top (1) x.PaymentProcessorResponseSID, x.PaymentSID from dbo.PaymentProcessorResponse x where x.PaymentSID = @PaymentSID) x04 on x00.PaymentSID =  x04.PaymentSID
			where
				x00.PaymentSID = @PaymentSID
