...
GO
CREATE FUNCTION [dbo].[fPAPSubscription#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPAPSubscription#IsDeleteEnabled
...
select top (10)
  ...
  ,dbo.fPAPSubscription#IsDeleteEnabled(x.PAPSubscriptionSID) IsDeleteEnabled
from
...
begin
	...
	begin
		...
		(
			select
				  count(x01.PAPTransactionSID)
			from
				dbo.PAPSubscription x00
			left outer join
				  (select top (1) x.PAPTransactionSID, x.PAPSubscriptionSID from dbo.PAPTransaction x where x.PAPSubscriptionSID = @PAPSubscriptionSID) x01 on x00.PAPSubscriptionSID =  x01.PAPSubscriptionSID
			where
				x00.PAPSubscriptionSID = @PAPSubscriptionSID
	...
	begin
		...
		(
			...
			from
				dbo.PAPSubscription x
			where
				x.PAPSubscriptionSID = @PAPSubscriptionSID
