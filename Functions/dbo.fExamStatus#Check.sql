...
GO
CREATE FUNCTION [dbo].[fExamStatus#Check]
	(
	 @ExamStatusSID      int
	,@ExamStatusSCD      varchar(15)
	,@ExamStatusLabel    nvarchar(35)
	,@Sequence           int
	,@IsDefault          bit
	,@ExamStatusXID      varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.ExamStatusSCD The "%1" must be uppercase and not contain spaces or any punctuat
...
begin
	...
	if @errorMessageSCD is null and @ExamStatusSCD is not null and (sf.fIsStringContentValid(@ExamStatusSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@ExamStatusSCD) <> @ExamStatusSCD collate Latin1_General_CS_AI)-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.ExamStatusSCD'
		set @columnNames        = N'ExamStatusSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Exam Status System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Apr 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'ExamStatus','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fExamStatus#Check'
		)
		begin
		
			select @errorText = ext.fExamStatus#Check
				(
				 @ExamStatusSID
				,@ExamStatusSCD
				,@ExamStatusLabel
