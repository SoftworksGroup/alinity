...
CREATE FUNCTION [sf].[fUnitType#IsDeleteEnabled]
	(
	@UnitTypeSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : sf.fUnitType#IsDeleteEnabled
...
select top (10)
   x.UnitTypeSID
  ,sf.fUnitType#IsDeleteEnabled(x.UnitTypeSID) IsDeleteEnabled
from
  sf.UnitType x
order by
  newid()
...
begin
	...
	if (select x.IsDefault from sf.UnitType x where x.UnitTypeSID = @UnitTypeSID) = @ON set @isDeleteEnabled = @OFF	-- default records cannot be deleted
	...
	begin
		...
		(
			select
				  count(x01.LearningModelSID)
			from
				sf.UnitType       x00
			left outer join
				  (select top (1) x.LearningModelSID, x.UnitTypeSID from dbo.LearningModel x where isnull(x.UnitTypeSID, -1) = @UnitTypeSID) x01 on x00.UnitTypeSID =  x01.UnitTypeSID
			where
				x00.UnitTypeSID = @UnitTypeSID
