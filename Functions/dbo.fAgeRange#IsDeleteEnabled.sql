...
GO
CREATE FUNCTION [dbo].[fAgeRange#IsDeleteEnabled]
	(
	@AgeRangeSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : dbo.fAgeRange#IsDeleteEnabled
...
select top (10)
   x.AgeRangeSID
  ,dbo.fAgeRange#IsDeleteEnabled(x.AgeRangeSID) IsDeleteEnabled
from
  dbo.AgeRange x
order by
  newid()
...
begin
	...
	if (select x.IsDefault from dbo.AgeRange x where x.AgeRangeSID = @AgeRangeSID) = @ON set @isDeleteEnabled = @OFF-- default records cannot be deleted
	...
	begin
		...
		(
			...
			from
				dbo.AgeRange             x00
			left outer join
				  (select top (1) x.RegistrantEmploymentSID, x.AgeRangeSID from dbo.RegistrantEmployment x where x.AgeRangeSID = @AgeRangeSID) x01 on x00.AgeRangeSID =  x01.AgeRangeSID
			where
				x00.AgeRangeSID = @AgeRangeSID
