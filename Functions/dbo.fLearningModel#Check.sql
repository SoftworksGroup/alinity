...
GO
CREATE FUNCTION [dbo].[fLearningModel#Check]
	(
	 @LearningModelSID    int
	,@LearningModelSCD    varchar(15)
	,@LearningModelLabel  nvarchar(35)
	,@IsDefault           bit
	,@UnitTypeSID         int
	,@CycleLengthYears    smallint
	,@IsCycleStartedYear1 bit
	,@MaximumCarryOver    decimal(5,2)
	,@LearningModelXID    varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fLearningModel#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.LearningModelSCD The "%1" must be uppercase and not contain spaces or any punct
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.UnitTypeSID The %1 is marked inactive. Assign an active %1.
...
begin
	...
	if @errorMessageSCD is null and @LearningModelSCD is not null and (sf.fIsStringContentValid(@LearningModelSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@LearningModelSCD) <> @LearningModelSCD collate Latin1_General_CS_AI)										-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.LearningModelSCD'
		set @columnNames        = N'LearningModelSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Learning Model System Code'
	end
	--!</Rule>
	...
	begin
		if (select x.IsActive from sf.UnitType x where x.UnitTypeSID = @UnitTypeSID) = @OFF							-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.UnitTypeSID'
			set @columnNames        = N'UnitTypeSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'unit type'
		end
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'LearningModel','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fLearningModel#Check'
		)
		begin
		
			select @errorText = ext.fLearningModel#Check
				(
				 @LearningModelSID
				,@LearningModelSCD
				,@LearningModelLabel
