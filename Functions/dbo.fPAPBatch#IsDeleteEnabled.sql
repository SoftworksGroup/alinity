...
GO
CREATE FUNCTION [dbo].[fPAPBatch#IsDeleteEnabled]
	(
	@PAPBatchSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : dbo.fPAPBatch#IsDeleteEnabled
...
select top (10)
   x.PAPBatchSID
  ,dbo.fPAPBatch#IsDeleteEnabled(x.PAPBatchSID) IsDeleteEnabled
from
  dbo.PAPBatch x
order by
  newid()
	...
	begin
		...
		(
			...
			from
				dbo.PAPBatch       x00
			left outer join
				  (select top (1) x.PAPTransactionSID, x.PAPBatchSID from dbo.PAPTransaction x where x.PAPBatchSID = @PAPBatchSID) x01 on x00.PAPBatchSID =  x01.PAPBatchSID
			where
				x00.PAPBatchSID = @PAPBatchSID
