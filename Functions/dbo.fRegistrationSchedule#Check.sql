...
GO
CREATE FUNCTION [dbo].[fRegistrationSchedule#Check]
	(
	 @RegistrationScheduleSID   int
	,@RegistrationScheduleLabel nvarchar(35)
	,@IsDefault                 bit
	,@IsActive                  bit
	,@RegistrationScheduleXID   varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrationSchedule#Check
...
Tim Edlund | Jan 2018          OneRegistrationScheduleOnly.RegistrationScheduleLabel (see source for message text)
...
begin
	...
	if @errorMessageSCD is null and (select count(1) from dbo.RegistrationSchedule) > 1
	begin
		set @errorMessageSCD      = 'OneRegistrationScheduleOnly.RegistrationScheduleLabel'
		set @columnNames          = N'RegistrationScheduleLabel'
		set @defaultMessageText   = N'This version of Alinity supports one registration schedule only. A second schedule is not supported.'
	end
	--!</Rule>	
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationSchedule','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrationSchedule#Check'
		)
		begin
		
			select @errorText = ext.fRegistrationSchedule#Check
				(
				 @RegistrationScheduleSID
				,@RegistrationScheduleLabel
				,@IsDefault
