...
GO
CREATE FUNCTION [dbo].[fRegistrationSnapshot#Check]
	(
	 @RegistrationSnapshotSID     int
	,@RegistrationSnapshotTypeSID int
	,@RegistrationSnapshotLabel   nvarchar(35)
	,@RegistrationYear            smallint
	,@QueuedTime                  datetimeoffset(7)
	,@LockedTime                  datetimeoffset(7)
	,@LastCodeUpdateTime          datetimeoffset(7)
	,@LastVerifiedTime            datetimeoffset(7)
	,@JobRunSID                   int
	,@RegistrationSnapshotXID     varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrationSnapshot#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.AssignmentToInactiveParent.RegistrationSnapshotTypeSID The %1 is marked inactive. Assign an acti
$AutoRules | Tim Edlund        ValueIsRequired.JobRunSID A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.LastCodeUpdateTime A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.LastVerifiedTime A value for "%1" is required.
$AutoRules | Tim Edlund        ValueIsRequired.LockedTime A value for "%1" is required.
...
select
	...
	,Description
	,dbo.fRegistrationSnapshot#Check
		(
		 RegistrationSnapshotSID
...
begin
	...
	begin
		if (select x.IsActive from dbo.RegistrationSnapshotType x where x.RegistrationSnapshotTypeSID = @RegistrationSnapshotTypeSID) = @OFF-- and parent row is inactive
		begin
			set @errorMessageSCD    = 'MBR.AssignmentToInactiveParent.RegistrationSnapshotTypeSID'
			set @columnNames        = N'RegistrationSnapshotTypeSID'
			set @defaultMessageText = N'The %1 is marked inactive. Assign an active %1.'
			set @arg1               = N'registration snapshot type'
		end
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationSnapshot','ValueIsRequired.JobRunSID') = @ON and @JobRunSID is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.JobRunSID'
		set @columnNames        = N'JobRunSID'
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationSnapshot','ValueIsRequired.LastCodeUpdateTime') = @ON and @LastCodeUpdateTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.LastCodeUpdateTime'
		set @columnNames        = N'LastCodeUpdateTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Last Code Update Time'
	end
	--!</Rule>
	
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationSnapshot','ValueIsRequired.LastVerifiedTime') = @ON and @LastVerifiedTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationSnapshot','ValueIsRequired.LockedTime') = @ON and @LockedTime is null
	begin
		set @errorMessageSCD    = 'ValueIsRequired.LockedTime'
		set @columnNames        = N'LockedTime'
		set @defaultMessageText = N'A value for "%1" is required.'
		set @arg1               = N'Locked Time'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrationSnapshot','?SomeMessageCode.ColumnName') = 1				-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrationSnapshot#Check'
		)
		begin
		
			select @errorText = ext.fRegistrationSnapshot#Check
				(
				 @RegistrationSnapshotSID
				,@RegistrationSnapshotTypeSID
				,@RegistrationSnapshotLabel
