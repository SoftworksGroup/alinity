...
GO
CREATE FUNCTION [sf].[fExportFile#Check]
	(
	 @ExportFileSID       int
	,@ExportSourceGUID    uniqueidentifier
	,@FileFormatSID       int
	,@ProcessedTime       datetimeoffset(7)
	,@IsFailed            bit
	,@MessageText         nvarchar(4000)
	,@ExportFileXID       varchar(150)
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'ExportFile','ValueIsRequired.MessageText') = @ON and @MessageText is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'ExportFile','ValueIsRequired.ProcessedTime') = @ON and @ProcessedTime is null
	begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'ExportFile','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
