...
GO
CREATE FUNCTION [sf].[fControlType#Check]
	(
	 @ControlTypeSID     int
	,@ControlTypeSCD     varchar(15)
	,@ControlTypeLabel   nvarchar(35)
	,@IsDefault          bit
	,@ControlTypeXID     varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.ControlTypeSCD The "%1" must be uppercase and not contain spaces or any punctua
...
begin
	...
	if @errorMessageSCD is null and @ControlTypeSCD is not null and (sf.fIsStringContentValid(@ControlTypeSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@ControlTypeSCD) <> @ControlTypeSCD collate Latin1_General_CS_AI)												-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.ControlTypeSCD'
		set @columnNames        = N'ControlTypeSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Control Type System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'ControlType','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
