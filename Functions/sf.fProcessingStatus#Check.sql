...
GO
CREATE FUNCTION [sf].[fProcessingStatus#Check]
	(
	 @ProcessingStatusSID   int
	,@ProcessingStatusSCD   varchar(10)
	,@ProcessingStatusLabel nvarchar(35)
	,@IsClosedStatus        bit
	,@IsActive              bit
	,@IsDefault             bit
	,@ProcessingStatusXID   varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : sf.fProcessingStatus#Check
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.ProcessingStatusSCD The "%1" must be uppercase and not contain spaces or any pu
...
begin
	...
	if @errorMessageSCD is null and @ProcessingStatusSCD is not null and (sf.fIsStringContentValid(@ProcessingStatusSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@ProcessingStatusSCD) <> @ProcessingStatusSCD collate Latin1_General_CS_AI)							-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.ProcessingStatusSCD'
		set @columnNames        = N'ProcessingStatusSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Processing Status System Code'
	end
	--!</Rule>
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'ProcessingStatus','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
