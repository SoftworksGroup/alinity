...
GO
CREATE FUNCTION [sf].[fExportJob#Check]
	(
	 @ExportJobSID       int
	,@ExportJobName      nvarchar(65)
	,@ExportJobCode      varchar(15)
	,@FileFormatSID      int
	,@JobScheduleSID     int
	,@LastExecuteTime    datetimeoffset(7)
	,@LastExecuteUser    nvarchar(75)
	,@ExecuteCount       int
	,@ExportJobXID       varchar(150)
...
begin
	...
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'sf',N'ExportJob','ValueIsRequired.JobScheduleSID') = @ON and @JobScheduleSID is null
	begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'ExportJob','?SomeMessageCode.ColumnName') = 1-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
