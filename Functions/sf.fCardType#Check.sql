...
GO
CREATE FUNCTION [sf].[fCardType#Check]
	(
	 @CardTypeSID        int
	,@CardTypeSCD        varchar(25)
	,@CardTypeLabel      nvarchar(35)
	,@CardTypeXID        varchar(150)
...
------------------------------ ----------------------------------------------------------------------------------------------------
$AutoRules | Tim Edlund        MBR.SystemCodeFormat.CardTypeSCD The "%1" must be uppercase and not contain spaces or any punctuatio
...
begin
	...
	if @errorMessageSCD is null and @CardTypeSCD is not null and (sf.fIsStringContentValid(@CardTypeSCD, N'abcdefghijklmnopqrstuvwxyz0123456789:.+-') = @OFF
	or upper(@CardTypeSCD) <> @CardTypeSCD collate Latin1_General_CS_AI)		-- use case sensitive collation to check if uppercase
	begin
		set @errorMessageSCD    = 'MBR.SystemCodeFormat.CardTypeSCD'
		set @columnNames        = N'CardTypeSCD'
		set @defaultMessageText = N'The "%1" must be uppercase and not contain spaces or any punctuation other than periods.'
		set @arg1               = N'Card Type System Code'
	end
	--!</Rule>
	
	--!<Rule Author="?Template | Mar 2019" Updates="None">
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'sf',N'CardType','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
