...
GO
CREATE function dbo.fReinstatement#ReviewReasonsHTML
(
	@ReinstatementSID int				-- key of record to return review reason for information for or -1 for all
 ,@RegistrationYear smallint	-- year of form records to return, or -1 for all or when first param is provided
)
returns nvarchar(max)
/*********************************************************************************************************************************
Function	: Reinstatement - Review Reasons HTML
...
column on the profile update and the parsing for the the HTML is done in the function dbo.fReinstatement#ReviewReasons. This 
...
begin
	...
	select
		dbo.fReinstatement#ReviewReasonsHTML(-1, @registrationYear)
...
exec sf.pUnitTest#Execute
	@ObjectName = 'dbo.fReinstatement#ReviewReasonsHTML'
 ,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
as
begin
	...
	select
		@reasonsHtml = N'<ul>' + stuff((
																		 ...
																		 from
																				dbo.fReinstatement#ReviewReasons(@ReinstatementSID, @RegistrationYear) r
																		 for xml path(''), type
