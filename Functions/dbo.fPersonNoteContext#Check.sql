...
GO
CREATE FUNCTION [dbo].[fPersonNoteContext#Check]
	(
	 @PersonNoteContextSID int
	,@PersonNoteSID        int
	,@ApplicationEntitySID int
	,@EntitySID            int
	,@PersonNoteContextXID varchar(150)
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fPersonNoteContext#Check
...
begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'PersonNoteContext','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fPersonNoteContext#Check'
		)
		begin
		
			select @errorText = ext.fPersonNoteContext#Check
				(
				 @PersonNoteContextSID
				,@PersonNoteSID
				,@ApplicationEntitySID
