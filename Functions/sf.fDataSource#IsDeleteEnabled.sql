...
CREATE FUNCTION [sf].[fDataSource#IsDeleteEnabled]
	(
	@DataSourceSID int
	)
returns bit
as
/*********************************************************************************************************************************
ScalarF : sf.fDataSource#IsDeleteEnabled
...
select top (10)
   x.DataSourceSID
  ,sf.fDataSource#IsDeleteEnabled(x.DataSourceSID) IsDeleteEnabled
from
  sf.DataSource x
order by
  newid()
...
begin
	...
	begin
		...
		(
			select
				  count(x01.DataSourceApplicationPageSID)
			from
				sf.DataSource                x00
			left outer join
				  (select top (1) x.DataSourceApplicationPageSID, x.DataSourceSID from sf.DataSourceApplicationPage x where x.DataSourceSID = @DataSourceSID) x01 on x00.DataSourceSID =  x01.DataSourceSID
			where
				x00.DataSourceSID = @DataSourceSID
