...
CREATE FUNCTION [sf].[fPersonEmailAddress#IsDeleteEnabled]
	(
...
as
/*********************************************************************************************************************************
ScalarF : sf.fPersonEmailAddress#IsDeleteEnabled
...
select top (10)
  ...
  ,sf.fPersonEmailAddress#IsDeleteEnabled(x.PersonEmailAddressSID) IsDeleteEnabled
from
...
begin
	...
	begin
		...
		(
			select
				  count(x01.PersonProfileSID)
			from
				sf.PersonEmailAddress x00
			left outer join
				  (select top (1) x.PersonProfileSID, x.PersonEmailAddressSID from stg.PersonProfile x where isnull(x.PersonEmailAddressSID, -1) = @PersonEmailAddressSID) x01 on x00.PersonEmailAddressSID =  x01.PersonEmailAddressSID
			where
				x00.PersonEmailAddressSID = @PersonEmailAddressSID
	...
	begin
		...
		(
			...
			from
				sf.PersonEmailAddress x
			where
				x.PersonEmailAddressSID = @PersonEmailAddressSID
