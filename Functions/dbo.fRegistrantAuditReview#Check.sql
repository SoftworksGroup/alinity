...
GO
CREATE FUNCTION [dbo].[fRegistrantAuditReview#Check]
	(
...
as
/*********************************************************************************************************************************
ScalarF : dbo.fRegistrantAuditReview#Check
...
begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAuditReview','ValueIsRequired.LastValidateTime') = @ON and @LastValidateTime is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAuditReview','ValueIsRequired.ReasonSID') = @ON and @ReasonSID is null
	begin
	...
	--!<Rule Author="$AutoRules | Tim Edlund">
	if @errorMessageSCD is null and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAuditReview','ValueIsRequired.RecommendationSID') = @ON and @RecommendationSID is null
	begin
	...
	if @errorMessageSCD is null --and sf.fBusinessRuleIsEnforced(N'dbo',N'RegistrantAuditReview','?SomeMessageCode.ColumnName') = 1	-- check if rule is ON - REMOVE for mandatory rules
	and 1 = 0
	begin
	...
	begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'fRegistrantAuditReview#Check'
		)
		begin
		
			select @errorText = ext.fRegistrantAuditReview#Check
				(
				 @RegistrantAuditReviewSID
				,@RegistrantAuditSID
				,@FormVersionSID
