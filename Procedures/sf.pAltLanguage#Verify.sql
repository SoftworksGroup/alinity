...
GO
CREATE PROCEDURE [sf].[pAltLanguage#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.AltLanguage
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.AltLanguage for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.AltLanguage table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fAltLanguage#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pAltLanguage#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.AltLanguage'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.AltLanguage'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.AltLanguage'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.AltLanguage'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.AltLanguage'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_AltLanguage'
		)
		begin
			exec sp_executesql N'alter table sf.AltLanguage drop constraint ck_AltLanguage'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.AltLanguage with CHECK add constraint ck_AltLanguage
			check
				(
				sf.fAltLanguage#Check
					(
					 AltLanguageSID
					,SourceGUID
					,FieldID
			...
			select @RowCount = count(1) from sf.AltLanguage											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.AltLanguage with NOCHECK add constraint ck_AltLanguage
			check
				(
				sf.fAltLanguage#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.AltLanguage'
		and
			br.BusinessRuleStatus <> 'x'
