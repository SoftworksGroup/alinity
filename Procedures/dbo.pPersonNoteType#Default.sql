...
GO
CREATE PROCEDURE [dbo].[pPersonNoteType#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPersonNoteType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.PersonNoteType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PersonNoteType table. When a new record is to be added from the UI, this procedure
...
columns of the vPersonNoteType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pPersonNoteType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pPersonNoteType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@personNoteTypeSID      int               = -1												-- specific default required by EF - do not override
		,@personNoteTypeLabel    nvarchar(35)      = null											-- no default provided from DB constraint - OK to override
		,@personNoteTypeCategory nvarchar(65)      = null											-- no default provided from DB constraint - OK to override
		...
		,@personNoteTypeXID      varchar(150)      = null											-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @personNoteTypeLabel     = isnull(context.node.value('@PersonNoteTypeLabel'   ,'nvarchar(35)'),@personNoteTypeLabel)
			,@personNoteTypeCategory  = isnull(context.node.value('@PersonNoteTypeCategory','nvarchar(65)'),@personNoteTypeCategory)
			,@isDefault               = isnull(context.node.value('@IsDefault'             ,'bit'         ),@isDefault)
			,@isActive                = isnull(context.node.value('@IsActive'              ,'bit'         ),@isActive)
			,@personNoteTypeXID       = isnull(context.node.value('@PersonNoteTypeXID'     ,'varchar(150)'),@personNoteTypeXID)
