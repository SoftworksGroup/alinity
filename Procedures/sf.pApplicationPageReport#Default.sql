...
GO
CREATE PROCEDURE [sf].[pApplicationPageReport#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pApplicationPageReport#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.ApplicationPageReport records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.ApplicationPageReport table. When a new record is to be added from the UI, this procedure
...
columns of the vApplicationPageReport entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pApplicationPageReport
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pApplicationPageReport of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@applicationPageReportSID int               = -1											-- specific default required by EF - do not override
		...
		,@applicationPageReportXID varchar(150)      = null										-- no default provided from DB constraint - OK to override
		...
		,@applicationReportName    nvarchar(65)      = N'?'										-- not a base table column (default ignored)
		...
		,@isCustom                 bit               = 0											-- not a base table column (default ignored)
		,@applicationReportRowGUID uniqueidentifier  = newid()								-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@applicationPageReportXID  = isnull(context.node.value('@ApplicationPageReportXID','varchar(150)'),@applicationPageReportXID)
