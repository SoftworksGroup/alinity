...
GO
CREATE PROCEDURE [sf].[pCulture#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pCulture#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.Culture records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.Culture table. When a new record is to be added from the UI, this procedure
...
columns of the vCulture entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pCulture
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pCulture of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@cultureSID         int               = -1														-- specific default required by EF - do not override
		,@cultureSCD         varchar(10)       = null													-- no default provided from DB constraint - OK to override
		,@cultureLabel       nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		...
		,@cultureXID         varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @cultureSCD    = isnull(context.node.value('@CultureSCD'  ,'varchar(10)' ),@cultureSCD)
			,@cultureLabel  = isnull(context.node.value('@CultureLabel','nvarchar(35)'),@cultureLabel)
			,@isDefault     = isnull(context.node.value('@IsDefault'   ,'bit'         ),@isDefault)
			,@isActive      = isnull(context.node.value('@IsActive'    ,'bit'         ),@isActive)
			,@cultureXID    = isnull(context.node.value('@CultureXID'  ,'varchar(150)'),@cultureXID)
