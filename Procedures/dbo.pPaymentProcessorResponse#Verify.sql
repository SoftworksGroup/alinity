...
GO
CREATE PROCEDURE [dbo].[pPaymentProcessorResponse#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PaymentProcessorResponse
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PaymentProcessorResponse for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PaymentProcessorResponse table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPaymentProcessorResponse#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPaymentProcessorResponse#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PaymentProcessorResponse'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PaymentProcessorResponse'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PaymentProcessorResponse'					-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PaymentProcessorResponse'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PaymentProcessorResponse'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PaymentProcessorResponse'
		)
		begin
			exec sp_executesql N'alter table dbo.PaymentProcessorResponse drop constraint ck_PaymentProcessorResponse'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PaymentProcessorResponse with CHECK add constraint ck_PaymentProcessorResponse
			check
				(
				dbo.fPaymentProcessorResponse#Check
					(
			...
			select @RowCount = count(1) from dbo.PaymentProcessorResponse				-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PaymentProcessorResponse'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.PaymentProcessorResponseSID
				,x.RowGUID
			from
				dbo.PaymentProcessorResponse x
			order by
				x.PaymentProcessorResponseSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPaymentProcessorResponse#Check
							(
							 x.PaymentProcessorResponseSID
							,x.PaymentSID
							,x.ResponseTime
					...
					from
						dbo.PaymentProcessorResponse  x
					where
						x.PaymentProcessorResponseSID = @nextSID
			...
			alter table dbo.PaymentProcessorResponse with NOCHECK add constraint ck_PaymentProcessorResponse
			check
				(
				dbo.fPaymentProcessorResponse#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PaymentProcessorResponse'
		and
			br.BusinessRuleStatus <> 'x'
