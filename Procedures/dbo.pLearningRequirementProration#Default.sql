...
GO
CREATE PROCEDURE [dbo].[pLearningRequirementProration#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pLearningRequirementProration#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.LearningRequirementProration records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.LearningRequirementProration table. When a new record is to be added from the UI, this procedure
...
columns of the vLearningRequirementProration entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pLearningRequirementProration
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pLearningRequirementProration of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@learningRequirementProrationSID int               = -1							-- specific default required by EF - do not override
		,@learningRequirementSID          int               = null						-- no default provided from DB constraint - OK to override
		,@startMonthDay                   char(4)           = null						-- no default provided from DB constraint - OK to override
		,@learningRequirementMinimum      decimal(5,2)      = (1.0)						-- default provided from DB constraint - OK to override
		,@percentageOfFullMinimum         decimal(6,3)      = (0.0)						-- default provided from DB constraint - OK to override
		,@userDefinedColumns              xml               = null						-- no default provided from DB constraint - OK to override
		,@learningRequirementProrationXID varchar(150)      = null						-- no default provided from DB constraint - OK to override
		...
		,@learningRequirementLabel        nvarchar(35)      = N'?'						-- not a base table column (default ignored)
		,@startingRegistrationYear        smallint          = 0								-- not a base table column (default ignored)
		,@minimum                         decimal(5,2)      = convert(decimal(5,2), 0.0)								-- not a base table column (default ignored)
		,@maximum                         decimal(5,2)      = convert(decimal(5,2), 0.0)								-- not a base table column (default ignored)
		,@maximumCarryOver                decimal(5,2)      = convert(decimal(5,2), 0.0)								-- not a base table column (default ignored)
		,@learningRequirementRowGUID      uniqueidentifier  = newid()					-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @learningRequirementSID           = isnull(context.node.value('@LearningRequirementSID'         ,'int'         ),@learningRequirementSID)
			,@startMonthDay                    = isnull(context.node.value('@StartMonthDay'                  ,'char(4)'     ),@startMonthDay)
			,@learningRequirementMinimum       = isnull(context.node.value('@LearningRequirementMinimum'     ,'decimal(5,2)'),@learningRequirementMinimum)
			,@percentageOfFullMinimum          = isnull(context.node.value('@PercentageOfFullMinimum'        ,'decimal(6,3)'),@percentageOfFullMinimum)
			,@learningRequirementProrationXID  = isnull(context.node.value('@LearningRequirementProrationXID','varchar(150)'),@learningRequirementProrationXID)
