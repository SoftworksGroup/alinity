...
GO
CREATE PROCEDURE [dbo].[pRegistrationRequirement#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrationRequirement
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrationRequirement for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrationRequirement table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrationRequirement#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrationRequirement#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrationRequirement'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrationRequirement'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationRequirement'					-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationRequirement'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrationRequirement'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrationRequirement'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrationRequirement drop constraint ck_RegistrationRequirement'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrationRequirement with CHECK add constraint ck_RegistrationRequirement
			check
				(
			...
			select @RowCount = count(1) from dbo.RegistrationRequirement				-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrationRequirement'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrationRequirement#Check
							(
							 x.RegistrationRequirementSID
							,x.RegistrationRequirementTypeSID
							,x.RegistrationRequirementLabel
							,x.PersonDocTypeSID
							,x.ExamSID
							,x.ExpiryMonths
							,x.IsActive
							,x.RegistrationRequirementXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrationRequirement  x
					where
						x.RegistrationRequirementSID = @nextSID
			...
			alter table dbo.RegistrationRequirement with NOCHECK add constraint ck_RegistrationRequirement
			check
				(
				dbo.fRegistrationRequirement#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrationRequirement'
		and
			br.BusinessRuleStatus <> 'x'
