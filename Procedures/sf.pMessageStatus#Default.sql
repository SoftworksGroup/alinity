...
GO
CREATE PROCEDURE [sf].[pMessageStatus#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pMessageStatus#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.MessageStatus records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.MessageStatus table. When a new record is to be added from the UI, this procedure
...
columns of the vMessageStatus entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pMessageStatus
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pMessageStatus of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@messageStatusSID   int               = -1														-- specific default required by EF - do not override
		,@messageStatusSCD   varchar(10)       = null													-- no default provided from DB constraint - OK to override
		,@messageStatusLabel nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		...
		,@messageStatusXID   varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @messageStatusSCD    = isnull(context.node.value('@MessageStatusSCD'  ,'varchar(10)'  ),@messageStatusSCD)
			,@messageStatusLabel  = isnull(context.node.value('@MessageStatusLabel','nvarchar(35)' ),@messageStatusLabel)
			...
			,@messageStatusXID    = isnull(context.node.value('@MessageStatusXID'  ,'varchar(150)' ),@messageStatusXID)
