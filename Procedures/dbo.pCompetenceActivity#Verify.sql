...
GO
CREATE PROCEDURE [dbo].[pCompetenceActivity#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.CompetenceActivity
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.CompetenceActivity for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.CompetenceActivity table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fCompetenceActivity#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pCompetenceActivity#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.CompetenceActivity'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.CompetenceActivity'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.CompetenceActivity'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.CompetenceActivity'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.CompetenceActivity'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_CompetenceActivity'
		)
		begin
			exec sp_executesql N'alter table dbo.CompetenceActivity drop constraint ck_CompetenceActivity'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.CompetenceActivity with CHECK add constraint ck_CompetenceActivity
			check
				(
				dbo.fCompetenceActivity#Check
					(
			...
			select @RowCount = count(1) from dbo.CompetenceActivity							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'CompetenceActivity'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fCompetenceActivity#Check
							(
							 x.CompetenceActivitySID
							,x.CompetenceActivityLabel
							,x.CompetenceActivityName
							,x.UnitValue
							,x.IsActive
							,x.CompetenceActivityXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.CompetenceActivity  x
					where
						x.CompetenceActivitySID = @nextSID
			...
			alter table dbo.CompetenceActivity with NOCHECK add constraint ck_CompetenceActivity
			check
				(
				dbo.fCompetenceActivity#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.CompetenceActivity'
		and
			br.BusinessRuleStatus <> 'x'
