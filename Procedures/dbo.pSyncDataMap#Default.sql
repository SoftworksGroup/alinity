...
GO
CREATE PROCEDURE [dbo].[pSyncDataMap#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pSyncDataMap#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.SyncDataMap records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.SyncDataMap table. When a new record is to be added from the UI, this procedure
...
columns of the vSyncDataMap entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pSyncDataMap
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pSyncDataMap of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@syncDataMapSID           int               = -1											-- specific default required by EF - do not override
		,@applicationEntitySID     int               = null										-- no default provided from DB constraint - OK to override
		,@syncMode                 varchar(4)        = 'PUSH'									-- default provided from DB constraint - OK to override
		,@isDeleteProcessed        bit               = CONVERT(bit,(0))				-- default provided from DB constraint - OK to override
		,@isEnabled                bit               = CONVERT(bit,(1))				-- default provided from DB constraint - OK to override
		,@userDefinedColumns       xml               = null										-- no default provided from DB constraint - OK to override
		,@syncDataMapXID           varchar(150)      = null										-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			...
			,@syncMode              = isnull(context.node.value('@SyncMode'            ,'varchar(4)'  ),@syncMode)
			,@isDeleteProcessed     = isnull(context.node.value('@IsDeleteProcessed'   ,'bit'         ),@isDeleteProcessed)
			,@isEnabled             = isnull(context.node.value('@IsEnabled'           ,'bit'         ),@isEnabled)
			,@syncDataMapXID        = isnull(context.node.value('@SyncDataMapXID'      ,'varchar(150)'),@syncDataMapXID)
