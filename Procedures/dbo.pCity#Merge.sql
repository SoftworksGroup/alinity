...
GO
CREATE PROCEDURE dbo.pCity#Merge
	@CitySIDFrom	int			-- record key to unassign 
 ,@CitySIDTo		int			-- replacement record key
 ,@ReturnSelect bit = 0 -- when 1 result (count) of updates are returned in a message (1 row dataset)
as
/*********************************************************************************************************************************
Sproc    : City - Merge
Notice   : Copyright © 2019 Softworks Group Inc.
Summary  : Replaces assignments of one City key value with another to support master table data clean-up
----------------------------------------------------------------------------------------------------------------------------------
...
Comments	
--------
This is a utility procedure called to replace the assignment of one City with another. The procedure is used when 
...
logic that may be configured to apply when City assignments are changed. In most cases application of additional logic is 
...
from
	dbo.City x
join
	(select y.StateProvinceSID from dbo.City y where y.CitySID = @fromSID) y on 1=1
where
	x.StateProvinceSID = y.StateProvinceSID
and
	x.CitySID <> @fromSID
...
begin
	...
	begin try
		...
		select @fromLabel = x.CityName from dbo.City x where x.CitySID = @CitySIDFrom
		...
		select @toLabel = x.CityName from dbo.City x where x.CitySID = @CitySIDTo
