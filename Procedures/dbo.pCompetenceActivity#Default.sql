...
GO
CREATE PROCEDURE [dbo].[pCompetenceActivity#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pCompetenceActivity#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.CompetenceActivity records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.CompetenceActivity table. When a new record is to be added from the UI, this procedure
...
columns of the vCompetenceActivity entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pCompetenceActivity
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pCompetenceActivity of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@competenceActivitySID   int               = -1											-- specific default required by EF - do not override
		,@competenceActivityLabel nvarchar(35)      = null										-- no default provided from DB constraint - OK to override
		,@competenceActivityName  nvarchar(65)      = null										-- no default provided from DB constraint - OK to override
		,@unitValue               decimal(5,2)      = (1.0)										-- default provided from DB constraint - OK to override
		,@helpPrompt              nvarchar(max)     = null										-- no default provided from DB constraint - OK to override
		...
		,@competenceActivityXID   varchar(150)      = null										-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @competenceActivityLabel  = isnull(context.node.value('@CompetenceActivityLabel','nvarchar(35)' ),@competenceActivityLabel)
			,@competenceActivityName   = isnull(context.node.value('@CompetenceActivityName' ,'nvarchar(65)' ),@competenceActivityName)
			,@unitValue                = isnull(context.node.value('@UnitValue'              ,'decimal(5,2)' ),@unitValue)
			,@helpPrompt               = isnull(context.node.value('@HelpPrompt'             ,'nvarchar(max)'),@helpPrompt)
			,@isActive                 = isnull(context.node.value('@IsActive'               ,'bit'          ),@isActive)
			,@competenceActivityXID    = isnull(context.node.value('@CompetenceActivityXID'  ,'varchar(150)' ),@competenceActivityXID)
