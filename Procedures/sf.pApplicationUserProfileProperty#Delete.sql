...
GO
CREATE PROCEDURE [sf].[pApplicationUserProfileProperty#Delete]
	 @ApplicationUserProfilePropertySID int               = null -- required! id of row to delete - must be set in custom logic if not passed
...
as
/*********************************************************************************************************************************
Procedure : sf.pApplicationUserProfileProperty#Delete
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : deletes 1 row in the sf.ApplicationUserProfileProperty table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.ApplicationUserProfileProperty table. The procedure requires a primary key value to locate the record
to delete.
...
and in the ext.pApplicationUserProfileProperty procedure for client-specific deletion rules.
...
begin
	...
	begin try
		...
		if @RowStamp is null select @RowStamp = x.RowStamp from sf.ApplicationUserProfileProperty x where x.ApplicationUserProfilePropertySID = @ApplicationUserProfilePropertySID

		-- apply the table-specific pre-delete logic (if any)

    --! <PreDelete>

		-- Ji Zhou | Feb 2013  -- Ensure the row can be deleted. This error should not appear if the UI is enabling
    -- the deletion option based on the value returned from the IsDeleteEnabled function
		-- in the #Ext view.
    set @IsDeleteEnabled = sf.fApplicationUserProfileProperty#IsDeleteEnabled(@ApplicationUserProfilePropertySID)
    ...
    begin
      ...
      exec sf.pMessage#Get
        ...
        ,@MessageText   = @errorText output
        ,@DefaultText   = N'Deletion of this %1 record is not allowed because you do not have sufficient grant.'
        ,@Arg1          = N'application user profile property'
