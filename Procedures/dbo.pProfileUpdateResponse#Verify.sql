...
GO
CREATE PROCEDURE [dbo].[pProfileUpdateResponse#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ProfileUpdateResponse
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ProfileUpdateResponse for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ProfileUpdateResponse table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fProfileUpdateResponse#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pProfileUpdateResponse#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ProfileUpdateResponse'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ProfileUpdateResponse'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ProfileUpdateResponse'						-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ProfileUpdateResponse'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ProfileUpdateResponse'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ProfileUpdateResponse'
		)
		begin
			exec sp_executesql N'alter table dbo.ProfileUpdateResponse drop constraint ck_ProfileUpdateResponse'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ProfileUpdateResponse with CHECK add constraint ck_ProfileUpdateResponse
			check
				(
				dbo.fProfileUpdateResponse#Check
					(
			...
			select @RowCount = count(1) from dbo.ProfileUpdateResponse					-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ProfileUpdateResponse'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.ProfileUpdateResponseSID
				,x.RowGUID
			from
				dbo.ProfileUpdateResponse x
			order by
				x.ProfileUpdateResponseSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fProfileUpdateResponse#Check
							(
							 x.ProfileUpdateResponseSID
							,x.ProfileUpdateSID
							,x.FormOwnerSID
							,x.ProfileUpdateResponseXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.ProfileUpdateResponse  x
					where
						x.ProfileUpdateResponseSID = @nextSID
			...
			alter table dbo.ProfileUpdateResponse with NOCHECK add constraint ck_ProfileUpdateResponse
			check
				(
				dbo.fProfileUpdateResponse#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ProfileUpdateResponse'
		and
			br.BusinessRuleStatus <> 'x'
