...
GO
CREATE PROCEDURE [dbo].[pProfileUpdateStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ProfileUpdateStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ProfileUpdateStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ProfileUpdateStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fProfileUpdateStatus#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pProfileUpdateStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ProfileUpdateStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ProfileUpdateStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ProfileUpdateStatus'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ProfileUpdateStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ProfileUpdateStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ProfileUpdateStatus'
		)
		begin
			exec sp_executesql N'alter table dbo.ProfileUpdateStatus drop constraint ck_ProfileUpdateStatus'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ProfileUpdateStatus with CHECK add constraint ck_ProfileUpdateStatus
			check
				(
				dbo.fProfileUpdateStatus#Check
					(
			...
			select @RowCount = count(1) from dbo.ProfileUpdateStatus						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ProfileUpdateStatus'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fProfileUpdateStatus#Check
							(
							 x.ProfileUpdateStatusSID
							,x.ProfileUpdateSID
							,x.FormStatusSID
							,x.ProfileUpdateStatusXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.ProfileUpdateStatus  x
					where
						x.ProfileUpdateStatusSID = @nextSID
			...
			alter table dbo.ProfileUpdateStatus with NOCHECK add constraint ck_ProfileUpdateStatus
			check
				(
				dbo.fProfileUpdateStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ProfileUpdateStatus'
		and
			br.BusinessRuleStatus <> 'x'
