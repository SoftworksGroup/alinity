...
GO
CREATE PROCEDURE [sf].[pFormType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.FormType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.FormType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.FormType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fFormType#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pFormType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.FormType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.FormType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.FormType'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.FormType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.FormType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_FormType'
		)
		begin
			exec sp_executesql N'alter table sf.FormType drop constraint ck_FormType'											-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.FormType with CHECK add constraint ck_FormType
			check
				(
				sf.fFormType#Check
					(
			...
			select @RowCount = count(1) from sf.FormType												-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.FormType with NOCHECK add constraint ck_FormType
			check
				(
				sf.fFormType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.FormType'
		and
			br.BusinessRuleStatus <> 'x'
