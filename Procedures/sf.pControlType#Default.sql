...
GO
CREATE PROCEDURE [sf].[pControlType#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pControlType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.ControlType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.ControlType table. When a new record is to be added from the UI, this procedure
...
columns of the vControlType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pControlType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pControlType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@controlTypeSID     int               = -1														-- specific default required by EF - do not override
		,@controlTypeSCD     varchar(15)       = null													-- no default provided from DB constraint - OK to override
		,@controlTypeLabel   nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		...
		,@controlTypeXID     varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @controlTypeSCD    = isnull(context.node.value('@ControlTypeSCD'  ,'varchar(15)' ),@controlTypeSCD)
			,@controlTypeLabel  = isnull(context.node.value('@ControlTypeLabel','nvarchar(35)'),@controlTypeLabel)
			,@isDefault         = isnull(context.node.value('@IsDefault'       ,'bit'         ),@isDefault)
			,@controlTypeXID    = isnull(context.node.value('@ControlTypeXID'  ,'varchar(150)'),@controlTypeXID)
