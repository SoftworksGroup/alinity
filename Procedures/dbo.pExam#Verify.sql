...
GO
CREATE PROCEDURE [dbo].[pExam#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.Exam
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.Exam for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.Exam table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fExam#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pExam#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.Exam'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.Exam'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.Exam'															-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.Exam'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.Exam'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_Exam'
		)
		begin
			exec sp_executesql N'alter table dbo.Exam drop constraint ck_Exam'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.Exam with CHECK add constraint ck_Exam
			check
				(
				dbo.fExam#Check
					(
			...
			select @RowCount = count(1) from dbo.Exam														-- rules validated - calculate row count to return
		...
		begin
				...
				begin try
					...
					select
						@isValid = dbo.fExam#Check
							...
							,x.CultureSID
							,x.LastVerifiedTime
							,x.MinLagDaysBetweenAttempts
							,x.MaxAttemptsPerYear
							,x.VendorExamID
							,x.ExamXID
							,x.LegacyKey
							,x.IsDeleted
			...
			alter table dbo.Exam with NOCHECK add constraint ck_Exam
			check
				(
				dbo.fExam#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.Exam'
		and
			br.BusinessRuleStatus <> 'x'
