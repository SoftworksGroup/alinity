...
GO
CREATE PROCEDURE [dbo].[pComplaintContactRole#Insert]
	 @ComplaintContactRoleSID  int               = null output							-- identity value assigned to the new record
	,@ComplaintContactRoleSCD  varchar(20)       = null											-- required! if not passed value must be set in custom logic prior to insert
	,@ComplaintContactRoleName nvarchar(50)      = null											-- required! if not passed value must be set in custom logic prior to insert
...
as
/*********************************************************************************************************************************
Procedure : dbo.pComplaintContactRole#Insert
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : inserts 1 row into the dbo.ComplaintContactRole table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.ComplaintContactRole table with parameter values represent columns of the table. Additional
parameters are provided for all columns in the vComplaintContactRole entity view, however, the base logic of the procedure inserts the
...
begin
	...
	set @ComplaintContactRoleSID = null																			-- initialize output parameter
	...
	begin try
		...
		begin
			...
			select @recordSID = x.ComplaintContactRoleSID from dbo.ComplaintContactRole x where x.IsDefault = @ON
		...
		select
			...
			,@ComplaintContactRoleSID = scope_identity()												-- capture key value of new row
