...
GO
CREATE PROCEDURE [sf].[pFormList#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pFormList#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.FormList records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.FormList table. When a new record is to be added from the UI, this procedure
...
columns of the vFormList entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pFormList
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pFormList of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@formListSID        int               = -1														-- specific default required by EF - do not override
		,@formListCode       varchar(15)       = null													-- no default provided from DB constraint - OK to override
		,@formListLabel      nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		...
		,@formListXID        varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @formListCode   = isnull(context.node.value('@FormListCode' ,'varchar(15)'  ),@formListCode)
			,@formListLabel  = isnull(context.node.value('@FormListLabel','nvarchar(35)' ),@formListLabel)
			,@toolTip        = isnull(context.node.value('@ToolTip'      ,'nvarchar(500)'),@toolTip)
			,@formListXID    = isnull(context.node.value('@FormListXID'  ,'varchar(150)' ),@formListXID)
