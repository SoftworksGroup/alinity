...
GO
CREATE PROCEDURE [sf].[pUnexpectedError#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.UnexpectedError
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.UnexpectedError for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.UnexpectedError table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fUnexpectedError#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pUnexpectedError#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.UnexpectedError'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.UnexpectedError'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.UnexpectedError'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.UnexpectedError'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.UnexpectedError'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_UnexpectedError'
		)
		begin
			exec sp_executesql N'alter table sf.UnexpectedError drop constraint ck_UnexpectedError'				-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.UnexpectedError with CHECK add constraint ck_UnexpectedError
			check
				(
			...
			select @RowCount = count(1) from sf.UnexpectedError									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'UnexpectedError'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.UnexpectedErrorSID
				,x.RowGUID
			from
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fUnexpectedError#Check
							(
							 x.UnexpectedErrorSID
							,x.MessageSCD
							,x.ProcName
			...
			alter table sf.UnexpectedError with NOCHECK add constraint ck_UnexpectedError
			check
				(
				sf.fUnexpectedError#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.UnexpectedError'
		and
			br.BusinessRuleStatus <> 'x'
