...
GO
CREATE PROCEDURE [dbo].[pRegistrantAppResponse#Update]
	 @RegistrantAppResponseSID   int               = null -- required! id of row to update - must be set in custom logic if not passed
	,@RegistrantAppSID           int               = null -- table column values to update:
	,@FormOwnerSID               int               = null
	,@FormResponse               xml               = null
	,@UserDefinedColumns         xml               = null
	,@RegistrantAppResponseXID   varchar(150)      = null
...
as
/*********************************************************************************************************************************
Procedure : dbo.pRegistrantAppResponse#Update
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : updates 1 row in the dbo.RegistrantAppResponse table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to update the dbo.RegistrantAppResponse table. The procedure requires a primary key to locate the record to update.
Additional parameters are provided for all columns in the vRegistrantAppResponse entity view, however, the base logic of the procedure
updates the columns of the table only. Table-specific logic can be added through tagged sections (pre and post update) and a call
to an extended procedure supports client-specific logic. Logic implemented within code tags (table-specific logic) is part of the
base product and applies to all client configurations. Calls to the extended procedure occur immediately after the table-specific
logic in both "pre-update" and "post-update" contexts.  A transaction is used to commit/rollback all changes as a logical unit.

Client specific customizations must be implemented in the ext.pRegistrantAppResponse procedure. The extended procedure is only called
...
Business rule compliance is checked through a table constraint which calls fRegistrantAppResponseCheck to test all rules.
...
begin
	...
	begin try
		...
		if @RegistrantAppResponseSID is null
		begin
		...
		set @RegistrantAppResponseXID = ltrim(rtrim(@RegistrantAppResponseXID))
		...
		if len(@RegistrantAppResponseXID) = 0 set @RegistrantAppResponseXID = null
		...
		begin
			...
			select
				...
				,@RegistrantAppResponseXID   = isnull(@RegistrantAppResponseXID,rar.RegistrantAppResponseXID)
				...
				,@DisplayName                = isnull(@DisplayName,rar.DisplayName)
			from
				dbo.vRegistrantAppResponse rar
			where
				rar.RegistrantAppResponseSID = @RegistrantAppResponseSID
		...
		if @FormOwnerSCD is not null and @FormOwnerSID = (select x.FormOwnerSID from dbo.RegistrantAppResponse x where x.RegistrantAppResponseSID = @RegistrantAppResponseSID)
		begin
		...
		(
			...
			where
				r.SchemaName = 'ext'
			and
				r.RoutineName = 'pRegistrantAppResponse'
		)
		begin
		
			exec @errorNo = ext.pRegistrantAppResponse
				 @Mode                       = 'update.pre'
				,@RegistrantAppResponseSID   = @RegistrantAppResponseSID
				,@RegistrantAppSID           = @RegistrantAppSID output
				,@FormOwnerSID               = @FormOwnerSID output
				,@FormResponse               = @FormResponse output
				,@UserDefinedColumns         = @UserDefinedColumns output
				,@RegistrantAppResponseXID   = @RegistrantAppResponseXID output
		...
		set
			...
			,RegistrantAppResponseXID = @RegistrantAppResponseXID
			,LegacyKey = @LegacyKey
			,UpdateUser = @UpdateUser
		...
		begin
			...
			if exists (select 1 from dbo.RegistrantAppResponse where RegistrantAppResponseSID = @registrantAppResponseSID)
			begin
				...
				exec sf.pMessage#Get
					...
					,@Arg1        = 'dbo.RegistrantAppResponse'
		...
		begin
			...
			exec sf.pMessage#Get
				...
				,@Arg1        = 'update'
				,@Arg2        = 'dbo.RegistrantAppResponse'
				,@Arg3        = @rowsAffected
				,@Arg4        = @registrantAppResponseSID
		...
		begin
			...
			select
				 ent.RegistrantAppResponseSID
			from
				dbo.vRegistrantAppResponse ent
			where
				ent.RegistrantAppResponseSID = @RegistrantAppResponseSID
