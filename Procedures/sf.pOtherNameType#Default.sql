...
GO
CREATE PROCEDURE [sf].[pOtherNameType#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pOtherNameType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.OtherNameType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.OtherNameType table. When a new record is to be added from the UI, this procedure
...
columns of the vOtherNameType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pOtherNameType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pOtherNameType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@otherNameTypeSID   int               = -1														-- specific default required by EF - do not override
		,@otherNameTypeLabel nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		...
		,@otherNameTypeXID   varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @otherNameTypeLabel  = isnull(context.node.value('@OtherNameTypeLabel','nvarchar(35)'),@otherNameTypeLabel)
			,@isDefault           = isnull(context.node.value('@IsDefault'         ,'bit'         ),@isDefault)
			,@isActive            = isnull(context.node.value('@IsActive'          ,'bit'         ),@isActive)
			,@otherNameTypeXID    = isnull(context.node.value('@OtherNameTypeXID'  ,'varchar(150)'),@otherNameTypeXID)
