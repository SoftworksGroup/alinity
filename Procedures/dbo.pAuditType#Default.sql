...
GO
CREATE PROCEDURE [dbo].[pAuditType#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pAuditType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.AuditType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.AuditType table. When a new record is to be added from the UI, this procedure
...
columns of the vAuditType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pAuditType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pAuditType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@auditTypeSID       int               = -1														-- specific default required by EF - do not override
		,@auditTypeLabel     nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		,@auditTypeCategory  nvarchar(65)      = null													-- no default provided from DB constraint - OK to override
		...
		,@auditTypeXID       varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @auditTypeLabel     = isnull(context.node.value('@AuditTypeLabel'   ,'nvarchar(35)'),@auditTypeLabel)
			,@auditTypeCategory  = isnull(context.node.value('@AuditTypeCategory','nvarchar(65)'),@auditTypeCategory)
			,@isDefault          = isnull(context.node.value('@IsDefault'        ,'bit'         ),@isDefault)
			,@isActive           = isnull(context.node.value('@IsActive'         ,'bit'         ),@isActive)
			,@auditTypeXID       = isnull(context.node.value('@AuditTypeXID'     ,'varchar(150)'),@auditTypeXID)
