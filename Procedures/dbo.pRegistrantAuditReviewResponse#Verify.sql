...
GO
CREATE PROCEDURE [dbo].[pRegistrantAuditReviewResponse#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantAuditReviewResponse
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantAuditReviewResponse for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantAuditReviewResponse table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantAuditReviewResponse#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantAuditReviewResponse#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantAuditReviewResponse'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantAuditReviewResponse'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAuditReviewResponse'		-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAuditReviewResponse'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantAuditReviewResponse'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantAuditReviewResponse'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantAuditReviewResponse drop constraint ck_RegistrantAuditReviewResponse'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantAuditReviewResponse with CHECK add constraint ck_RegistrantAuditReviewResponse
			check
				(
				dbo.fRegistrantAuditReviewResponse#Check
					(
					 RegistrantAuditReviewResponseSID
					,RegistrantAuditReviewSID
					,FormOwnerSID
					,RegistrantAuditReviewResponseXID
					,LegacyKey
					,IsDeleted
					,CreateUser
					,CreateTime
			...
			select @RowCount = count(1) from dbo.RegistrantAuditReviewResponse	-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantAuditReviewResponse'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrantAuditReviewResponseSID
				,x.RowGUID
			from
				dbo.RegistrantAuditReviewResponse x
			order by
				x.RegistrantAuditReviewResponseSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantAuditReviewResponse#Check
							(
							 x.RegistrantAuditReviewResponseSID
							,x.RegistrantAuditReviewSID
							,x.FormOwnerSID
							,x.RegistrantAuditReviewResponseXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantAuditReviewResponse  x
					where
						x.RegistrantAuditReviewResponseSID = @nextSID
			...
			alter table dbo.RegistrantAuditReviewResponse with NOCHECK add constraint ck_RegistrantAuditReviewResponse
			check
				(
				dbo.fRegistrantAuditReviewResponse#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantAuditReviewResponse'
		and
			br.BusinessRuleStatus <> 'x'
