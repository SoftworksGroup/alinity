...
GO
CREATE PROCEDURE [dbo].[pRegistrantEmploymentPracticeArea#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantEmploymentPracticeArea
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantEmploymentPracticeArea for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantEmploymentPracticeArea table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantEmploymentPracticeArea#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantEmploymentPracticeArea#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantEmploymentPracticeArea'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantEmploymentPracticeArea'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantEmploymentPracticeArea'	-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantEmploymentPracticeArea'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantEmploymentPracticeArea'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantEmploymentPracticeArea'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantEmploymentPracticeArea drop constraint ck_RegistrantEmploymentPracticeArea'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantEmploymentPracticeArea with CHECK add constraint ck_RegistrantEmploymentPracticeArea
			check
				(
				dbo.fRegistrantEmploymentPracticeArea#Check
					(
			...
			select @RowCount = count(1) from dbo.RegistrantEmploymentPracticeArea													-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantEmploymentPracticeArea'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrantEmploymentPracticeAreaSID
				,x.RowGUID
			from
				dbo.RegistrantEmploymentPracticeArea x
			order by
				x.RegistrantEmploymentPracticeAreaSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantEmploymentPracticeArea#Check
							(
							 x.RegistrantEmploymentPracticeAreaSID
							,x.RegistrantEmploymentSID
							,x.PracticeAreaSID
							,x.IsPrimary
							,x.RegistrantEmploymentPracticeAreaXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantEmploymentPracticeArea  x
					where
						x.RegistrantEmploymentPracticeAreaSID = @nextSID
			...
			alter table dbo.RegistrantEmploymentPracticeArea with NOCHECK add constraint ck_RegistrantEmploymentPracticeArea
			check
				(
				dbo.fRegistrantEmploymentPracticeArea#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantEmploymentPracticeArea'
		and
			br.BusinessRuleStatus <> 'x'
