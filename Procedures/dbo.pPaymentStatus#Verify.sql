...
GO
CREATE PROCEDURE [dbo].[pPaymentStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PaymentStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PaymentStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PaymentStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPaymentStatus#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPaymentStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PaymentStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PaymentStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PaymentStatus'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PaymentStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PaymentStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PaymentStatus'
		)
		begin
			exec sp_executesql N'alter table dbo.PaymentStatus drop constraint ck_PaymentStatus'					-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PaymentStatus with CHECK add constraint ck_PaymentStatus
			check
				(
				dbo.fPaymentStatus#Check
					(
					 PaymentStatusSID
					,PaymentStatusSCD
					,PaymentStatusLabel
			...
			select @RowCount = count(1) from dbo.PaymentStatus									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PaymentStatus'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPaymentStatus#Check
							(
							 x.PaymentStatusSID
							,x.PaymentStatusSCD
			...
			alter table dbo.PaymentStatus with NOCHECK add constraint ck_PaymentStatus
			check
				(
				dbo.fPaymentStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PaymentStatus'
		and
			br.BusinessRuleStatus <> 'x'
