...
GO
CREATE PROCEDURE [dbo].[pRegistrationSchedule#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pRegistrationSchedule#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.RegistrationSchedule records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.RegistrationSchedule table. When a new record is to be added from the UI, this procedure
...
columns of the vRegistrationSchedule entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pRegistrationSchedule
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pRegistrationSchedule of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@registrationScheduleSID   int               = -1										-- specific default required by EF - do not override
		,@registrationScheduleLabel nvarchar(35)      = null									-- no default provided from DB constraint - OK to override
		...
		,@registrationScheduleXID   varchar(150)      = null									-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @registrationScheduleLabel  = isnull(context.node.value('@RegistrationScheduleLabel','nvarchar(35)'),@registrationScheduleLabel)
			,@isDefault                  = isnull(context.node.value('@IsDefault'                ,'bit'         ),@isDefault)
			,@isActive                   = isnull(context.node.value('@IsActive'                 ,'bit'         ),@isActive)
			,@registrationScheduleXID    = isnull(context.node.value('@RegistrationScheduleXID'  ,'varchar(150)'),@registrationScheduleXID)
