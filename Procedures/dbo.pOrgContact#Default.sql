...
GO
CREATE PROCEDURE [dbo].[pOrgContact#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pOrgContact#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.OrgContact records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.OrgContact table. When a new record is to be added from the UI, this procedure
...
columns of the vOrgContact entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pOrgContact
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pOrgContact of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@orgContactSID                  int               = -1								-- specific default required by EF - do not override
		...
		,@isReviewAdmin                  bit               = CONVERT(bit,(0))	-- default provided from DB constraint - OK to override
		,@title                          nvarchar(65)      = null							-- no default provided from DB constraint - OK to override
		,@directPhone                    varchar(25)       = null							-- no default provided from DB constraint - OK to override
		,@isAdminContact                 bit               = CONVERT(bit,(0))	-- default provided from DB constraint - OK to override
		...
		,@orgContactXID                  varchar(150)      = null							-- no default provided from DB constraint - OK to override
		...
		,@isOwner                        bit																	-- not a base table column (default ignored)
		,@registrantEmploymentSID        int																	-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@isReviewAdmin        = isnull(context.node.value('@IsReviewAdmin'      ,'bit'         ),@isReviewAdmin)
			,@title                = isnull(context.node.value('@Title'              ,'nvarchar(65)'),@title)
			,@directPhone          = isnull(context.node.value('@DirectPhone'        ,'varchar(25)' ),@directPhone)
			,@isAdminContact       = isnull(context.node.value('@IsAdminContact'     ,'bit'         ),@isAdminContact)
			,@ownershipPercentage  = isnull(context.node.value('@OwnershipPercentage','smallint'    ),@ownershipPercentage)
			,@orgContactXID        = isnull(context.node.value('@OrgContactXID'      ,'varchar(150)'),@orgContactXID)
