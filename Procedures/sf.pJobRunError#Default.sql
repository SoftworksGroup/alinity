...
GO
CREATE PROCEDURE [sf].[pJobRunError#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pJobRunError#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.JobRunError records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.JobRunError table. When a new record is to be added from the UI, this procedure
...
columns of the vJobRunError entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pJobRunError
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pJobRunError of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@jobRunErrorSID          int               = -1											-- specific default required by EF - do not override
		...
		,@dataSource              nvarchar(257)     = null										-- no default provided from DB constraint - OK to override
		,@recordKey               varchar(50)       = null										-- no default provided from DB constraint - OK to override
		,@userDefinedColumns      xml               = null										-- no default provided from DB constraint - OK to override
		,@jobRunErrorXID          varchar(150)      = null										-- no default provided from DB constraint - OK to override
		...
		,@jobSID                  int               = 0												-- not a base table column (default ignored)
		,@conversationHandle      uniqueidentifier  = newid()									-- not a base table column (default ignored)
		,@startTime               datetimeoffset(7) = sysdatetimeoffset()			-- not a base table column (default ignored)
		,@endTime                 datetimeoffset(7)														-- not a base table column (default ignored)
		,@totalRecords            int               = 0												-- not a base table column (default ignored)
		,@totalErrors             int               = 0												-- not a base table column (default ignored)
		,@recordsProcessed        int               = 0												-- not a base table column (default ignored)
		...
		,@isFailureCleared        bit               = 0												-- not a base table column (default ignored)
		...
		,@jobRunRowGUID           uniqueidentifier  = newid()									-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@dataSource      = isnull(context.node.value('@DataSource'    ,'nvarchar(257)' ),@dataSource)
			,@recordKey       = isnull(context.node.value('@RecordKey'     ,'varchar(50)'   ),@recordKey)
			,@jobRunErrorXID  = isnull(context.node.value('@JobRunErrorXID','varchar(150)'  ),@jobRunErrorXID)
