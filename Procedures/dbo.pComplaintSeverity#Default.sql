...
GO
CREATE PROCEDURE [dbo].[pComplaintSeverity#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pComplaintSeverity#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.ComplaintSeverity records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.ComplaintSeverity table. When a new record is to be added from the UI, this procedure
...
columns of the vComplaintSeverity entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pComplaintSeverity
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pComplaintSeverity of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@complaintSeveritySID      int               = -1										-- specific default required by EF - do not override
		,@complaintSeverityLabel    nvarchar(35)      = null									-- no default provided from DB constraint - OK to override
		,@complaintSeverityCategory nvarchar(65)      = null									-- no default provided from DB constraint - OK to override
		...
		,@complaintSeverityXID      varchar(150)      = null									-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @complaintSeverityLabel     = isnull(context.node.value('@ComplaintSeverityLabel'   ,'nvarchar(35)' ),@complaintSeverityLabel)
			,@complaintSeverityCategory  = isnull(context.node.value('@ComplaintSeverityCategory','nvarchar(65)' ),@complaintSeverityCategory)
			...
			,@complaintSeverityXID       = isnull(context.node.value('@ComplaintSeverityXID'     ,'varchar(150)' ),@complaintSeverityXID)
