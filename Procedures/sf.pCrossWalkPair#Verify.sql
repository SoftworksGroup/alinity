...
GO
CREATE PROCEDURE [sf].[pCrossWalkPair#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.CrossWalkPair
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.CrossWalkPair for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.CrossWalkPair table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fCrossWalkPair#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pCrossWalkPair#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.CrossWalkPair'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.CrossWalkPair'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.CrossWalkPair'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.CrossWalkPair'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.CrossWalkPair'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_CrossWalkPair'
		)
		begin
			exec sp_executesql N'alter table sf.CrossWalkPair drop constraint ck_CrossWalkPair'						-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.CrossWalkPair with CHECK add constraint ck_CrossWalkPair
			check
				(
				sf.fCrossWalkPair#Check
					(
					 CrossWalkPairSID
					,CrossWalkSID
					,SourceValue
			...
			select @RowCount = count(1) from sf.CrossWalkPair										-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'CrossWalkPair'
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fCrossWalkPair#Check
							(
							 x.CrossWalkPairSID
							,x.CrossWalkSID
							,x.SourceValue
			...
			alter table sf.CrossWalkPair with NOCHECK add constraint ck_CrossWalkPair
			check
				(
				sf.fCrossWalkPair#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.CrossWalkPair'
		and
			br.BusinessRuleStatus <> 'x'
