...
GO
CREATE PROCEDURE [sf].[pJobRunError#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.JobRunError
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.JobRunError for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.JobRunError table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fJobRunError#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pJobRunError#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.JobRunError'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.JobRunError'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.JobRunError'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.JobRunError'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.JobRunError'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_JobRunError'
		)
		begin
			exec sp_executesql N'alter table sf.JobRunError drop constraint ck_JobRunError'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.JobRunError with CHECK add constraint ck_JobRunError
			check
				(
				sf.fJobRunError#Check
					(
					 JobRunErrorSID
					,JobRunSID
					,MessageText
			...
			select @RowCount = count(1) from sf.JobRunError											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.JobRunError with NOCHECK add constraint ck_JobRunError
			check
				(
				sf.fJobRunError#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.JobRunError'
		and
			br.BusinessRuleStatus <> 'x'
