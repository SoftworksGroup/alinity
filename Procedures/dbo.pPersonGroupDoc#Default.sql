...
GO
CREATE PROCEDURE [dbo].[pPersonGroupDoc#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPersonGroupDoc#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.PersonGroupDoc records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PersonGroupDoc table. When a new record is to be added from the UI, this procedure
...
columns of the vPersonGroupDoc entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pPersonGroupDoc
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pPersonGroupDoc of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@personGroupDocSID          int               = -1										-- specific default required by EF - do not override
		,@personGroupFolderSID       int               = null									-- no default provided from DB constraint - OK to override
		...
		,@personGroupDocXID          varchar(150)      = null									-- no default provided from DB constraint - OK to override
		...
		,@parentPersonGroupFolderSID int																			-- not a base table column (default ignored)
		,@personGroupSID             int               = 0										-- not a base table column (default ignored)
		,@folderName                 nvarchar(35)      = N'?'									-- not a base table column (default ignored)
		,@isRoot                     bit               = 0										-- not a base table column (default ignored)
		,@personGroupFolderRowGUID   uniqueidentifier  = newid()							-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @personGroupFolderSID  = isnull(context.node.value('@PersonGroupFolderSID','int'           ),@personGroupFolderSID)
			...
			,@personGroupDocXID     = isnull(context.node.value('@PersonGroupDocXID'   ,'varchar(150)'  ),@personGroupDocXID)
