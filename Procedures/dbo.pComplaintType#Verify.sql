...
GO
CREATE PROCEDURE [dbo].[pComplaintType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ComplaintType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ComplaintType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ComplaintType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fComplaintType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pComplaintType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ComplaintType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ComplaintType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ComplaintType'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ComplaintType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ComplaintType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ComplaintType'
		)
		begin
			exec sp_executesql N'alter table dbo.ComplaintType drop constraint ck_ComplaintType'					-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ComplaintType with CHECK add constraint ck_ComplaintType
			check
				(
				dbo.fComplaintType#Check
					(
					 ComplaintTypeSID
					,ComplaintTypeLabel
			...
			select @RowCount = count(1) from dbo.ComplaintType									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ComplaintType'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fComplaintType#Check
							(
							 x.ComplaintTypeSID
							,x.ComplaintTypeLabel
							,x.ComplaintTypeCategory
							,x.IsDefault
							,x.IsActive
							,x.ComplaintTypeXID
							,x.LegacyKey
			...
			alter table dbo.ComplaintType with NOCHECK add constraint ck_ComplaintType
			check
				(
				dbo.fComplaintType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ComplaintType'
		and
			br.BusinessRuleStatus <> 'x'
