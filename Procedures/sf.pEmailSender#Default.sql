...
GO
CREATE PROCEDURE [sf].[pEmailSender#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pEmailSender#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.EmailSender records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.EmailSender table. When a new record is to be added from the UI, this procedure
...
columns of the vEmailSender entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pEmailSender
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pEmailSender of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@emailSenderSID     int               = -1														-- specific default required by EF - do not override
		...
		,@isPrivate          bit               = (0)													-- default provided from DB constraint - OK to override
		...
		,@emailSenderXID     varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			...
			,@isPrivate           = isnull(context.node.value('@IsPrivate'         ,'bit'         ),@isPrivate)
			,@isDefault           = isnull(context.node.value('@IsDefault'         ,'bit'         ),@isDefault)
			,@emailSenderXID      = isnull(context.node.value('@EmailSenderXID'    ,'varchar(150)'),@emailSenderXID)
