...
GO
CREATE PROCEDURE [dbo].[pPersonNote#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PersonNote
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PersonNote for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PersonNote table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPersonNote#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPersonNote#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PersonNote'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PersonNote'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PersonNote'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PersonNote'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PersonNote'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PersonNote'
		)
		begin
			exec sp_executesql N'alter table dbo.PersonNote drop constraint ck_PersonNote'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PersonNote with CHECK add constraint ck_PersonNote
			check
				(
				dbo.fPersonNote#Check
					(
					 PersonNoteSID
					,PersonSID
					,PersonNoteTypeSID
			...
			select @RowCount = count(1) from dbo.PersonNote											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.PersonNote with NOCHECK add constraint ck_PersonNote
			check
				(
				dbo.fPersonNote#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PersonNote'
		and
			br.BusinessRuleStatus <> 'x'
