...
GO
CREATE PROCEDURE [dbo].[pPracticeRegisterType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PracticeRegisterType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PracticeRegisterType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PracticeRegisterType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPracticeRegisterType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPracticeRegisterType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PracticeRegisterType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PracticeRegisterType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRegisterType'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRegisterType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PracticeRegisterType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PracticeRegisterType'
		)
		begin
			exec sp_executesql N'alter table dbo.PracticeRegisterType drop constraint ck_PracticeRegisterType'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PracticeRegisterType with CHECK add constraint ck_PracticeRegisterType
			check
				(
				dbo.fPracticeRegisterType#Check
					(
			...
			select @RowCount = count(1) from dbo.PracticeRegisterType						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PracticeRegisterType'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.PracticeRegisterTypeSID
				,x.RowGUID
			from
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPracticeRegisterType#Check
							(
							 x.PracticeRegisterTypeSID
							,x.PracticeRegisterTypeSCD
							,x.PracticeRegisterTypeLabel
							,x.PracticeRegisterTypeCategory
							,x.IsDefault
							,x.IsActive
							,x.PracticeRegisterTypeXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.PracticeRegisterType  x
					where
						x.PracticeRegisterTypeSID = @nextSID
			...
			alter table dbo.PracticeRegisterType with NOCHECK add constraint ck_PracticeRegisterType
			check
				(
				dbo.fPracticeRegisterType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PracticeRegisterType'
		and
			br.BusinessRuleStatus <> 'x'
