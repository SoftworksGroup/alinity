...
GO
CREATE PROCEDURE [sf].[pProcessingStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ProcessingStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ProcessingStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ProcessingStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fProcessingStatus#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pProcessingStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ProcessingStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ProcessingStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ProcessingStatus'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ProcessingStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ProcessingStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ProcessingStatus'
		)
		begin
			exec sp_executesql N'alter table sf.ProcessingStatus drop constraint ck_ProcessingStatus'			-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ProcessingStatus with CHECK add constraint ck_ProcessingStatus
			check
				(
				sf.fProcessingStatus#Check
					(
					 ProcessingStatusSID
					,ProcessingStatusSCD
			...
			select @RowCount = count(1) from sf.ProcessingStatus								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ProcessingStatus'
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fProcessingStatus#Check
							(
							 x.ProcessingStatusSID
			...
			alter table sf.ProcessingStatus with NOCHECK add constraint ck_ProcessingStatus
			check
				(
				sf.fProcessingStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ProcessingStatus'
		and
			br.BusinessRuleStatus <> 'x'
