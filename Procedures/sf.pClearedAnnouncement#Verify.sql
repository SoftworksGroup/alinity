...
GO
CREATE PROCEDURE [sf].[pClearedAnnouncement#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ClearedAnnouncement
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ClearedAnnouncement for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ClearedAnnouncement table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fClearedAnnouncement#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pClearedAnnouncement#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ClearedAnnouncement'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ClearedAnnouncement'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ClearedAnnouncement'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ClearedAnnouncement'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ClearedAnnouncement'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ClearedAnnouncement'
		)
		begin
			exec sp_executesql N'alter table sf.ClearedAnnouncement drop constraint ck_ClearedAnnouncement'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ClearedAnnouncement with CHECK add constraint ck_ClearedAnnouncement
			check
				(
				sf.fClearedAnnouncement#Check
					(
			...
			select @RowCount = count(1) from sf.ClearedAnnouncement							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ClearedAnnouncement'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.ClearedAnnouncementSID
				,x.RowGUID
			from
				sf.ClearedAnnouncement x
			order by
				x.ClearedAnnouncementSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fClearedAnnouncement#Check
							(
							 x.ClearedAnnouncementSID
							,x.AnnouncementSID
							,x.ApplicationUserSID
							,x.ClearedAnnouncementXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						sf.ClearedAnnouncement  x
					where
						x.ClearedAnnouncementSID = @nextSID
			...
			alter table sf.ClearedAnnouncement with NOCHECK add constraint ck_ClearedAnnouncement
			check
				(
				sf.fClearedAnnouncement#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ClearedAnnouncement'
		and
			br.BusinessRuleStatus <> 'x'
