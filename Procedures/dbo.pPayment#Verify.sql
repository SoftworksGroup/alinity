...
GO
CREATE PROCEDURE [dbo].[pPayment#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.Payment
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.Payment for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.Payment table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPayment#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPayment#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.Payment'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.Payment'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.Payment'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.Payment'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.Payment'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_Payment'
		)
		begin
			exec sp_executesql N'alter table dbo.Payment drop constraint ck_Payment'											-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.Payment with CHECK add constraint ck_Payment
			check
				(
				dbo.fPayment#Check
					(
			...
			select @RowCount = count(1) from dbo.Payment												-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.Payment with NOCHECK add constraint ck_Payment
			check
				(
				dbo.fPayment#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.Payment'
		and
			br.BusinessRuleStatus <> 'x'
