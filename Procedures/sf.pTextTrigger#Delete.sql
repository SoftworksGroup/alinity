...
GO
CREATE PROCEDURE [sf].[pTextTrigger#Delete]
	 @TextTriggerSID                   int               = null -- required! id of row to delete - must be set in custom logic if not passed
	...
	,@TextTemplateLabel                nvarchar(35)      = null
	...
	,@TextTemplateRowGUID              uniqueidentifier  = null
	,@PersonSID                        int               = null
	,@CultureSID                       int               = null
...
as
/*********************************************************************************************************************************
Procedure : sf.pTextTrigger#Delete
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : deletes 1 row in the sf.TextTrigger table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.TextTrigger table. The procedure requires a primary key value to locate the record
to delete.
...
and in the ext.pTextTrigger procedure for client-specific deletion rules.
...
begin
	...
	begin try
		...
		if @RowStamp is null select @RowStamp = x.RowStamp from sf.TextTrigger x where x.TextTriggerSID = @TextTriggerSID
