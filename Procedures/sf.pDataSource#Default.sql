...
GO
CREATE PROCEDURE [sf].[pDataSource#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pDataSource#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.DataSource records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.DataSource table. When a new record is to be added from the UI, this procedure
...
columns of the vDataSource entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pDataSource
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pDataSource of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@dataSourceSID       int               = -1													-- specific default required by EF - do not override
		,@dataSourceLabel     nvarchar(35)      = null												-- no default provided from DB constraint - OK to override
		,@dBObjectName        nvarchar(257)     = null												-- no default provided from DB constraint - OK to override
		...
		,@exportDefaults      xml               = CONVERT(xml,'<ExportDefaults />')											-- default provided from DB constraint - OK to override
		...
		,@dataSourceXID       varchar(150)      = null												-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @dataSourceLabel  = isnull(context.node.value('@DataSourceLabel','nvarchar(35)'     ),@dataSourceLabel)
			,@dBObjectName     = isnull(context.node.value('@DBObjectName'   ,'nvarchar(257)'    ),@dBObjectName)
			...
			,@dataSourceXID    = isnull(context.node.value('@DataSourceXID'  ,'varchar(150)'     ),@dataSourceXID)
