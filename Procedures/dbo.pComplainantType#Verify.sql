...
GO
CREATE PROCEDURE [dbo].[pComplainantType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ComplainantType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ComplainantType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ComplainantType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fComplainantType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pComplainantType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ComplainantType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ComplainantType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ComplainantType'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ComplainantType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ComplainantType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ComplainantType'
		)
		begin
			exec sp_executesql N'alter table dbo.ComplainantType drop constraint ck_ComplainantType'			-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ComplainantType with CHECK add constraint ck_ComplainantType
			check
				(
				dbo.fComplainantType#Check
					(
					 ComplainantTypeSID
			...
			select @RowCount = count(1) from dbo.ComplainantType								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ComplainantType'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fComplainantType#Check
							(
							 x.ComplainantTypeSID
							,x.ComplainantTypeLabel
							,x.ComplainantTypeCategory
							,x.IsDefault
							,x.IsActive
							,x.ComplainantTypeXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
			...
			alter table dbo.ComplainantType with NOCHECK add constraint ck_ComplainantType
			check
				(
				dbo.fComplainantType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ComplainantType'
		and
			br.BusinessRuleStatus <> 'x'
