...
GO
CREATE PROCEDURE [sf].[pEmailTemplateAttachment#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.EmailTemplateAttachment
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.EmailTemplateAttachment for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.EmailTemplateAttachment table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fEmailTemplateAttachment#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pEmailTemplateAttachment#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.EmailTemplateAttachment'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.EmailTemplateAttachment'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.EmailTemplateAttachment'						-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.EmailTemplateAttachment'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.EmailTemplateAttachment'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_EmailTemplateAttachment'
		)
		begin
			exec sp_executesql N'alter table sf.EmailTemplateAttachment drop constraint ck_EmailTemplateAttachment'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.EmailTemplateAttachment with CHECK add constraint ck_EmailTemplateAttachment
			check
				(
				sf.fEmailTemplateAttachment#Check
					(
			...
			select @RowCount = count(1) from sf.EmailTemplateAttachment					-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'EmailTemplateAttachment'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.EmailTemplateAttachmentSID
				,x.RowGUID
			from
				sf.EmailTemplateAttachment x
			order by
				x.EmailTemplateAttachmentSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fEmailTemplateAttachment#Check
							(
							 x.EmailTemplateAttachmentSID
							,x.EmailTemplateSID
							,x.DocumentTitle
					...
					from
						sf.EmailTemplateAttachment  x
					where
						x.EmailTemplateAttachmentSID = @nextSID
			...
			alter table sf.EmailTemplateAttachment with NOCHECK add constraint ck_EmailTemplateAttachment
			check
				(
				sf.fEmailTemplateAttachment#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.EmailTemplateAttachment'
		and
			br.BusinessRuleStatus <> 'x'
