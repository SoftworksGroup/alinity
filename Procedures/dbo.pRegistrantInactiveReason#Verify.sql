...
GO
CREATE PROCEDURE [dbo].[pRegistrantInactiveReason#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantInactiveReason
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantInactiveReason for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantInactiveReason table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantInactiveReason#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantInactiveReason#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantInactiveReason'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantInactiveReason'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantInactiveReason'					-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantInactiveReason'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantInactiveReason'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantInactiveReason'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantInactiveReason drop constraint ck_RegistrantInactiveReason'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantInactiveReason with CHECK add constraint ck_RegistrantInactiveReason
			check
				(
				dbo.fRegistrantInactiveReason#Check
					(
			...
			select @RowCount = count(1) from dbo.RegistrantInactiveReason				-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantInactiveReason'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantInactiveReason#Check
							(
							 x.RegistrantInactiveReasonSID
							,x.RegistrantInactiveReasonLabel
							,x.IsActive
							,x.RegistrantInactiveReasonXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantInactiveReason  x
					where
						x.RegistrantInactiveReasonSID = @nextSID
			...
			alter table dbo.RegistrantInactiveReason with NOCHECK add constraint ck_RegistrantInactiveReason
			check
				(
				dbo.fRegistrantInactiveReason#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantInactiveReason'
		and
			br.BusinessRuleStatus <> 'x'
