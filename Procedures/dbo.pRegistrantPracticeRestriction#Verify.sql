...
GO
CREATE PROCEDURE [dbo].[pRegistrantPracticeRestriction#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantPracticeRestriction
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantPracticeRestriction for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantPracticeRestriction table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantPracticeRestriction#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantPracticeRestriction#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantPracticeRestriction'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantPracticeRestriction'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantPracticeRestriction'		-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantPracticeRestriction'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantPracticeRestriction'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantPracticeRestriction'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantPracticeRestriction drop constraint ck_RegistrantPracticeRestriction'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantPracticeRestriction with CHECK add constraint ck_RegistrantPracticeRestriction
			check
				(
				dbo.fRegistrantPracticeRestriction#Check
					(
			...
			select @RowCount = count(1) from dbo.RegistrantPracticeRestriction	-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantPracticeRestriction'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrantPracticeRestrictionSID
				,x.RowGUID
			from
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantPracticeRestriction#Check
							(
							 x.RegistrantPracticeRestrictionSID
							,x.RegistrantSID
							,x.PracticeRestrictionSID
							,x.EffectiveTime
							,x.ExpiryTime
							,x.IsDisplayedOnLicense
							,x.ComplaintSID
							,x.RegistrantPracticeRestrictionXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantPracticeRestriction  x
					where
						x.RegistrantPracticeRestrictionSID = @nextSID
			...
			alter table dbo.RegistrantPracticeRestriction with NOCHECK add constraint ck_RegistrantPracticeRestriction
			check
				(
				dbo.fRegistrantPracticeRestriction#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantPracticeRestriction'
		and
			br.BusinessRuleStatus <> 'x'
