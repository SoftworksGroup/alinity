...
GO
CREATE PROCEDURE [dbo].[pLanguage#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.Language
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.Language for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.Language table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fLanguage#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pLanguage#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.Language'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.Language'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.Language'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.Language'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.Language'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_Language'
		)
		begin
			exec sp_executesql N'alter table dbo.Language drop constraint ck_Language'										-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.Language with CHECK add constraint ck_Language
			check
				(
				dbo.fLanguage#Check
					(
			...
			select @RowCount = count(1) from dbo.Language												-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.Language with NOCHECK add constraint ck_Language
			check
				(
				dbo.fLanguage#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.Language'
		and
			br.BusinessRuleStatus <> 'x'
