...
GO
CREATE PROCEDURE [sf].[pFormListItem#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.FormListItem
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.FormListItem for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.FormListItem table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fFormListItem#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pFormListItem#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.FormListItem'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.FormListItem'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.FormListItem'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.FormListItem'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.FormListItem'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_FormListItem'
		)
		begin
			exec sp_executesql N'alter table sf.FormListItem drop constraint ck_FormListItem'							-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.FormListItem with CHECK add constraint ck_FormListItem
			check
				(
				sf.fFormListItem#Check
					(
					 FormListItemSID
					,FormListSID
					,FormListItemCode
			...
			select @RowCount = count(1) from sf.FormListItem										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fFormListItem#Check
							(
							 x.FormListItemSID
							,x.FormListSID
							,x.FormListItemCode
							,x.FormListItemLabel
							,x.FormListItemSequence
							,x.ToolTip
							,x.IsActive
							,x.FormListItemXID
							,x.LegacyKey
			...
			alter table sf.FormListItem with NOCHECK add constraint ck_FormListItem
			check
				(
				sf.fFormListItem#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.FormListItem'
		and
			br.BusinessRuleStatus <> 'x'
