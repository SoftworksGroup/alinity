...
GO
CREATE PROCEDURE [sf].[pPerson#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.Person
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.Person for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.Person table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fPerson#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pPerson#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.Person'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.Person'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.Person'														-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.Person'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.Person'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_Person'
		)
		begin
			exec sp_executesql N'alter table sf.Person drop constraint ck_Person'													-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.Person with CHECK add constraint ck_Person
			check
				(
				sf.fPerson#Check
					(
			...
			select @RowCount = count(1) from sf.Person													-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.Person with NOCHECK add constraint ck_Person
			check
				(
				sf.fPerson#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.Person'
		and
			br.BusinessRuleStatus <> 'x'
