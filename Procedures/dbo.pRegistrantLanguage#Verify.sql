...
GO
CREATE PROCEDURE [dbo].[pRegistrantLanguage#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantLanguage
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantLanguage for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantLanguage table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantLanguage#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantLanguage#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantLanguage'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantLanguage'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantLanguage'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantLanguage'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantLanguage'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantLanguage'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantLanguage drop constraint ck_RegistrantLanguage'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantLanguage with CHECK add constraint ck_RegistrantLanguage
			check
				(
				dbo.fRegistrantLanguage#Check
					(
			...
			select @RowCount = count(1) from dbo.RegistrantLanguage							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantLanguage'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrantLanguageSID
				,x.RowGUID
			from
				dbo.RegistrantLanguage x
			order by
				x.RegistrantLanguageSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantLanguage#Check
							(
							 x.RegistrantLanguageSID
							,x.RegistrantSID
							,x.LanguageSID
					...
					from
						dbo.RegistrantLanguage  x
					where
						x.RegistrantLanguageSID = @nextSID
			...
			alter table dbo.RegistrantLanguage with NOCHECK add constraint ck_RegistrantLanguage
			check
				(
				dbo.fRegistrantLanguage#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantLanguage'
		and
			br.BusinessRuleStatus <> 'x'
