...
GO
CREATE PROCEDURE [dbo].[pPersonDocContext#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PersonDocContext
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PersonDocContext for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PersonDocContext table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPersonDocContext#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPersonDocContext#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PersonDocContext'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PersonDocContext'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PersonDocContext'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PersonDocContext'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PersonDocContext'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PersonDocContext'
		)
		begin
			exec sp_executesql N'alter table dbo.PersonDocContext drop constraint ck_PersonDocContext'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PersonDocContext with CHECK add constraint ck_PersonDocContext
			check
				(
				dbo.fPersonDocContext#Check
					(
					 PersonDocContextSID
					,PersonDocSID
			...
			select @RowCount = count(1) from dbo.PersonDocContext								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PersonDocContext'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.PersonDocContextSID
				,x.RowGUID
			from
				dbo.PersonDocContext x
			order by
				x.PersonDocContextSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPersonDocContext#Check
							(
							 x.PersonDocContextSID
							,x.PersonDocSID
			...
			alter table dbo.PersonDocContext with NOCHECK add constraint ck_PersonDocContext
			check
				(
				dbo.fPersonDocContext#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PersonDocContext'
		and
			br.BusinessRuleStatus <> 'x'
