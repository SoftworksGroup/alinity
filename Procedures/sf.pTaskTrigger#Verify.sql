...
GO
CREATE PROCEDURE [sf].[pTaskTrigger#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.TaskTrigger
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.TaskTrigger for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.TaskTrigger table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fTaskTrigger#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pTaskTrigger#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.TaskTrigger'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.TaskTrigger'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.TaskTrigger'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.TaskTrigger'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.TaskTrigger'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_TaskTrigger'
		)
		begin
			exec sp_executesql N'alter table sf.TaskTrigger drop constraint ck_TaskTrigger'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.TaskTrigger with CHECK add constraint ck_TaskTrigger
			check
				(
			...
			select @RowCount = count(1) from sf.TaskTrigger											-- rules validated - calculate row count to return
		...
		begin
				...
				begin try
					...
					select
						@isValid = sf.fTaskTrigger#Check
							...
							,x.IsAlert
							,x.PriorityLevel
							,x.TargetCompletionDays
							,x.OpenTaskLimit
							,x.IsRegeneratedIfClosed
							,x.ApplicationAction
							,x.JobScheduleSID
							,x.LastStartTime
							,x.LastEndTime
			...
			alter table sf.TaskTrigger with NOCHECK add constraint ck_TaskTrigger
			check
				(
				sf.fTaskTrigger#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.TaskTrigger'
		and
			br.BusinessRuleStatus <> 'x'
