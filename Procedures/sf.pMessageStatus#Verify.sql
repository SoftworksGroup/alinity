...
GO
CREATE PROCEDURE [sf].[pMessageStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.MessageStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.MessageStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.MessageStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fMessageStatus#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pMessageStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.MessageStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.MessageStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.MessageStatus'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.MessageStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.MessageStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_MessageStatus'
		)
		begin
			exec sp_executesql N'alter table sf.MessageStatus drop constraint ck_MessageStatus'						-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.MessageStatus with CHECK add constraint ck_MessageStatus
			check
				(
				sf.fMessageStatus#Check
					(
					 MessageStatusSID
					,MessageStatusSCD
					,MessageStatusLabel
			...
			select @RowCount = count(1) from sf.MessageStatus										-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'MessageStatus'
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fMessageStatus#Check
							(
							 x.MessageStatusSID
							,x.MessageStatusSCD
			...
			alter table sf.MessageStatus with NOCHECK add constraint ck_MessageStatus
			check
				(
				sf.fMessageStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.MessageStatus'
		and
			br.BusinessRuleStatus <> 'x'
