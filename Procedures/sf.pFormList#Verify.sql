...
GO
CREATE PROCEDURE [sf].[pFormList#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.FormList
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.FormList for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.FormList table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fFormList#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pFormList#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.FormList'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.FormList'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.FormList'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.FormList'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.FormList'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_FormList'
		)
		begin
			exec sp_executesql N'alter table sf.FormList drop constraint ck_FormList'											-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.FormList with CHECK add constraint ck_FormList
			check
				(
				sf.fFormList#Check
					(
			...
			select @RowCount = count(1) from sf.FormList												-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.FormList with NOCHECK add constraint ck_FormList
			check
				(
				sf.fFormList#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.FormList'
		and
			br.BusinessRuleStatus <> 'x'
