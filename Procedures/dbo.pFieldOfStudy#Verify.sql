...
GO
CREATE PROCEDURE [dbo].[pFieldOfStudy#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.FieldOfStudy
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.FieldOfStudy for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.FieldOfStudy table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fFieldOfStudy#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pFieldOfStudy#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.FieldOfStudy'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.FieldOfStudy'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.FieldOfStudy'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.FieldOfStudy'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.FieldOfStudy'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_FieldOfStudy'
		)
		begin
			exec sp_executesql N'alter table dbo.FieldOfStudy drop constraint ck_FieldOfStudy'						-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.FieldOfStudy with CHECK add constraint ck_FieldOfStudy
			check
				(
				dbo.fFieldOfStudy#Check
					(
					 FieldOfStudySID
					,FieldOfStudyName
					,FieldOfStudyCode
			...
			select @RowCount = count(1) from dbo.FieldOfStudy										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fFieldOfStudy#Check
							(
							 x.FieldOfStudySID
							,x.FieldOfStudyName
							,x.FieldOfStudyCode
							,x.FieldOfStudyCategory
							,x.IsDefault
							,x.IsActive
							,x.FieldOfStudyXID
							,x.LegacyKey
			...
			alter table dbo.FieldOfStudy with NOCHECK add constraint ck_FieldOfStudy
			check
				(
				dbo.fFieldOfStudy#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.FieldOfStudy'
		and
			br.BusinessRuleStatus <> 'x'
