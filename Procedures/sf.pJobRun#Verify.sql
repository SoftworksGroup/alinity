...
GO
CREATE PROCEDURE [sf].[pJobRun#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.JobRun
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.JobRun for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.JobRun table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fJobRun#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pJobRun#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.JobRun'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.JobRun'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.JobRun'														-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.JobRun'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.JobRun'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_JobRun'
		)
		begin
			exec sp_executesql N'alter table sf.JobRun drop constraint ck_JobRun'													-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.JobRun with CHECK add constraint ck_JobRun
			check
				(
				sf.fJobRun#Check
					(
			...
			select @RowCount = count(1) from sf.JobRun													-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fJobRun#Check
							(
							 x.JobRunSID
							,x.JobSID
							,x.ConversationHandle
							,x.StartTime
							,x.EndTime
							,x.TotalRecords
							,x.TotalErrors
							,x.RecordsProcessed
							,x.CurrentProcessLabel
							,x.IsFailed
							,x.IsFailureCleared
							,x.CancellationRequestTime
							,x.IsCancelled
							,x.JobRunXID
							,x.LegacyKey
							,x.IsDeleted
			...
			alter table sf.JobRun with NOCHECK add constraint ck_JobRun
			check
				(
				sf.fJobRun#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.JobRun'
		and
			br.BusinessRuleStatus <> 'x'
