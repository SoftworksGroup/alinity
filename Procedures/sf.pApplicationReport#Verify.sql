...
GO
CREATE PROCEDURE [sf].[pApplicationReport#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ApplicationReport
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ApplicationReport for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ApplicationReport table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fApplicationReport#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pApplicationReport#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ApplicationReport'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ApplicationReport'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationReport'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationReport'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ApplicationReport'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ApplicationReport'
		)
		begin
			exec sp_executesql N'alter table sf.ApplicationReport drop constraint ck_ApplicationReport'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ApplicationReport with CHECK add constraint ck_ApplicationReport
			check
				(
				sf.fApplicationReport#Check
					(
			...
			select @RowCount = count(1) from sf.ApplicationReport								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ApplicationReport'
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fApplicationReport#Check
							(
							 x.ApplicationReportSID
							,x.ApplicationReportName
							,x.IconFillColor
							,x.DisplayRank
							,x.IsCustom
							,x.ApplicationReportXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						sf.ApplicationReport  x
					where
						x.ApplicationReportSID = @nextSID
			...
			alter table sf.ApplicationReport with NOCHECK add constraint ck_ApplicationReport
			check
				(
				sf.fApplicationReport#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ApplicationReport'
		and
			br.BusinessRuleStatus <> 'x'
