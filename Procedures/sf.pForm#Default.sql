...
GO
CREATE PROCEDURE [sf].[pForm#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pForm#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.Form records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.Form table. When a new record is to be added from the UI, this procedure
...
columns of the vForm entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pForm
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pForm of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@formSID                          int               = -1							-- specific default required by EF - do not override
		,@formTypeSID                      int               = null						-- no default provided from DB constraint - OK to override
		,@formName                         nvarchar(65)      = null						-- no default provided from DB constraint - OK to override
		,@formLabel                        nvarchar(35)      = null						-- no default provided from DB constraint - OK to override
		,@formContext                      varchar(25)       = null						-- no default provided from DB constraint - OK to override
		,@authorCredit                     nvarchar(500)     = (('Anonymous Work'+char((13)))+char((10)))+'See https://commons.wikimedia.org/wiki/Anonymous_works'-- default provided from DB constraint - OK to override
		,@isActive                         bit               = (1)						-- default provided from DB constraint - OK to override
		,@usageTerms                       nvarchar(max)     = null						-- no default provided from DB constraint - OK to override
		...
		,@formInstructions                 nvarchar(max)     = null						-- no default provided from DB constraint - OK to override
		,@versionHistory                   xml               = null						-- no default provided from DB constraint - OK to override
		,@userDefinedColumns               xml               = null						-- no default provided from DB constraint - OK to override
		,@formXID                          varchar(150)      = null						-- no default provided from DB constraint - OK to override
		...
		,@formTypeSCD                      varchar(25)       = '?'						-- not a base table column (default ignored)
		,@formTypeLabel                    nvarchar(35)      = N'?'						-- not a base table column (default ignored)
		,@formOwnerSID                     int               = 0							-- not a base table column (default ignored)
		,@formTypeIsDefault                bit               = 0							-- not a base table column (default ignored)
		,@formTypeRowGUID                  uniqueidentifier  = newid()				-- not a base table column (default ignored)
		...
		,@latestVersionNo                  smallint														-- not a base table column (default ignored)
		,@latestRevisionNo                 smallint														-- not a base table column (default ignored)
		,@latestVersionFormVersionSID      int																-- not a base table column (default ignored)
		,@latestRevisionFormVersionSID     int																-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @formTypeSID         = isnull(context.node.value('@FormTypeSID'       ,'int'          ),@formTypeSID)
			,@formName            = isnull(context.node.value('@FormName'          ,'nvarchar(65)' ),@formName)
			,@formLabel           = isnull(context.node.value('@FormLabel'         ,'nvarchar(35)' ),@formLabel)
			,@formContext         = isnull(context.node.value('@FormContext'       ,'varchar(25)'  ),@formContext)
			,@authorCredit        = isnull(context.node.value('@AuthorCredit'      ,'nvarchar(500)'),@authorCredit)
			,@isActive            = isnull(context.node.value('@IsActive'          ,'bit'          ),@isActive)
			,@usageTerms          = isnull(context.node.value('@UsageTerms'        ,'nvarchar(max)'),@usageTerms)
			...
			,@formInstructions    = isnull(context.node.value('@FormInstructions'  ,'nvarchar(max)'),@formInstructions)
			,@formXID             = isnull(context.node.value('@FormXID'           ,'varchar(150)' ),@formXID)
		...
		if isnull(@formTypeSID,0) = 0 select @formTypeSID = x.FormTypeSID from sf.FormType  x where x.IsDefault = @ON
