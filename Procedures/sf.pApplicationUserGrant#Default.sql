...
GO
CREATE PROCEDURE [sf].[pApplicationUserGrant#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pApplicationUserGrant#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.ApplicationUserGrant records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.ApplicationUserGrant table. When a new record is to be added from the UI, this procedure
...
columns of the vApplicationUserGrant entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pApplicationUserGrant
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pApplicationUserGrant of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@applicationUserGrantSID          int               = -1							-- specific default required by EF - do not override
		...
		,@applicationUserGrantXID          varchar(150)      = null						-- no default provided from DB constraint - OK to override
		...
		,@moduleSCD                        varchar(30)												-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@applicationUserGrantXID  = isnull(context.node.value('@ApplicationUserGrantXID','varchar(150)' ),@applicationUserGrantXID)
