...
GO
CREATE PROCEDURE [sf].[pApplicationUserSession#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ApplicationUserSession
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ApplicationUserSession for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ApplicationUserSession table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fApplicationUserSession#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pApplicationUserSession#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ApplicationUserSession'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ApplicationUserSession'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationUserSession'						-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationUserSession'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ApplicationUserSession'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ApplicationUserSession'
		)
		begin
			exec sp_executesql N'alter table sf.ApplicationUserSession drop constraint ck_ApplicationUserSession'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ApplicationUserSession with CHECK add constraint ck_ApplicationUserSession
			check
				(
				sf.fApplicationUserSession#Check
					(
					 ApplicationUserSessionSID
					,ApplicationUserSID
					,IsActive
					,IPAddress
					,ApplicationUserSessionXID
					,LegacyKey
					,IsDeleted
					,CreateUser
					,CreateTime
			...
			select @RowCount = count(1) from sf.ApplicationUserSession					-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ApplicationUserSession'
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fApplicationUserSession#Check
							(
							 x.ApplicationUserSessionSID
							,x.ApplicationUserSID
							,x.IsActive
							,x.IPAddress
							,x.ApplicationUserSessionXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						sf.ApplicationUserSession  x
					where
						x.ApplicationUserSessionSID = @nextSID
			...
			alter table sf.ApplicationUserSession with NOCHECK add constraint ck_ApplicationUserSession
			check
				(
				sf.fApplicationUserSession#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ApplicationUserSession'
		and
			br.BusinessRuleStatus <> 'x'
