...
GO
CREATE PROCEDURE [dbo].[pSyncDataMap#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.SyncDataMap
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.SyncDataMap for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.SyncDataMap table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fSyncDataMap#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pSyncDataMap#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.SyncDataMap'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.SyncDataMap'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.SyncDataMap'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.SyncDataMap'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.SyncDataMap'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_SyncDataMap'
		)
		begin
			exec sp_executesql N'alter table dbo.SyncDataMap drop constraint ck_SyncDataMap'							-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.SyncDataMap with CHECK add constraint ck_SyncDataMap
			check
				(
				dbo.fSyncDataMap#Check
					(
					 SyncDataMapSID
			...
			select @RowCount = count(1) from dbo.SyncDataMap										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fSyncDataMap#Check
							(
							 x.SyncDataMapSID
			...
			alter table dbo.SyncDataMap with NOCHECK add constraint ck_SyncDataMap
			check
				(
				dbo.fSyncDataMap#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.SyncDataMap'
		and
			br.BusinessRuleStatus <> 'x'
