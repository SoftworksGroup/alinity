...
GO
CREATE PROCEDURE [sf].[pTaskQueue#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.TaskQueue
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.TaskQueue for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.TaskQueue table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fTaskQueue#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pTaskQueue#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.TaskQueue'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.TaskQueue'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.TaskQueue'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.TaskQueue'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.TaskQueue'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_TaskQueue'
		)
		begin
			exec sp_executesql N'alter table sf.TaskQueue drop constraint ck_TaskQueue'										-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.TaskQueue with CHECK add constraint ck_TaskQueue
			check
				(
				sf.fTaskQueue#Check
					(
			...
			select @RowCount = count(1) from sf.TaskQueue												-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fTaskQueue#Check
							(
							 x.TaskQueueSID
							,x.TaskQueueLabel
							,x.TaskQueueCode
							,x.IsAutoAssigned
							,x.IsOpenSubscription
			...
			alter table sf.TaskQueue with NOCHECK add constraint ck_TaskQueue
			check
				(
				sf.fTaskQueue#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.TaskQueue'
		and
			br.BusinessRuleStatus <> 'x'
