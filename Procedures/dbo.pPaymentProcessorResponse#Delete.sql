...
GO
CREATE PROCEDURE [dbo].[pPaymentProcessorResponse#Delete]
	 @PaymentProcessorResponseSID int               = null -- required! id of row to delete - must be set in custom logic if not passed
	...
	,@ReasonSID                   int               = null
	,@PaymentRowGUID              uniqueidentifier  = null
	,@IsDeleteEnabled             bit               = null
	,@zContext                    xml               = null -- other values defining context for the delete (if any)
	,@IsManual                    bit               = null
as
/*********************************************************************************************************************************
Procedure : dbo.pPaymentProcessorResponse#Delete
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : deletes 1 row in the dbo.PaymentProcessorResponse table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PaymentProcessorResponse table. The procedure requires a primary key value to locate the record
to delete.
...
and in the ext.pPaymentProcessorResponse procedure for client-specific deletion rules.
...
begin
	...
	begin try
		...
		if @RowStamp is null select @RowStamp = x.RowStamp from dbo.PaymentProcessorResponse x where x.PaymentProcessorResponseSID = @PaymentProcessorResponseSID
