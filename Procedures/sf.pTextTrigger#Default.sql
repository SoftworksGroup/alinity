...
GO
CREATE PROCEDURE [sf].[pTextTrigger#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pTextTrigger#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.TextTrigger records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.TextTrigger table. When a new record is to be added from the UI, this procedure
...
columns of the vTextTrigger entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pTextTrigger
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pTextTrigger of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@textTriggerSID                   int               = -1							-- specific default required by EF - do not override
		,@textTriggerLabel                 nvarchar(35)      = null						-- no default provided from DB constraint - OK to override
		,@textTemplateSID                  int               = null						-- no default provided from DB constraint - OK to override
		...
		,@textTriggerXID                   varchar(150)      = null						-- no default provided from DB constraint - OK to override
		...
		,@textTemplateLabel                nvarchar(35)      = N'?'						-- not a base table column (default ignored)
		,@priorityLevel                    tinyint           = 0							-- not a base table column (default ignored)
		,@body                             nvarchar(1600)    = N'?'						-- not a base table column (default ignored)
		...
		,@textTemplateRowGUID              uniqueidentifier  = newid()				-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @textTriggerLabel    = isnull(context.node.value('@TextTriggerLabel'  ,'nvarchar(35)'     ),@textTriggerLabel)
			,@textTemplateSID     = isnull(context.node.value('@TextTemplateSID'   ,'int'              ),@textTemplateSID)
			...
			,@textTriggerXID      = isnull(context.node.value('@TextTriggerXID'    ,'varchar(150)'     ),@textTriggerXID)
