...
GO
CREATE PROCEDURE [sf].[pPersonEmailAddress#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.PersonEmailAddress
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.PersonEmailAddress for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.PersonEmailAddress table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fPersonEmailAddress#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pPersonEmailAddress#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.PersonEmailAddress'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.PersonEmailAddress'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.PersonEmailAddress'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.PersonEmailAddress'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.PersonEmailAddress'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_PersonEmailAddress'
		)
		begin
			exec sp_executesql N'alter table sf.PersonEmailAddress drop constraint ck_PersonEmailAddress'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.PersonEmailAddress with CHECK add constraint ck_PersonEmailAddress
			check
				(
			...
			select @RowCount = count(1) from sf.PersonEmailAddress							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PersonEmailAddress'
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fPersonEmailAddress#Check
							(
							 x.PersonEmailAddressSID
							,x.PersonSID
							,x.EmailAddress
					...
					from
						sf.PersonEmailAddress  x
					where
						x.PersonEmailAddressSID = @nextSID
			...
			alter table sf.PersonEmailAddress with NOCHECK add constraint ck_PersonEmailAddress
			check
				(
				sf.fPersonEmailAddress#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.PersonEmailAddress'
		and
			br.BusinessRuleStatus <> 'x'
