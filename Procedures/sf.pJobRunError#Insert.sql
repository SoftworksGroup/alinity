...
GO
CREATE PROCEDURE [sf].[pJobRunError#Insert]
	 @JobRunErrorSID          int               = null output								-- identity value assigned to the new record
	,@JobRunSID               int               = null											-- required! if not passed value must be set in custom logic prior to insert
	,@MessageText             nvarchar(4000)    = null											-- required! if not passed value must be set in custom logic prior to insert
	,@DataSource              nvarchar(257)     = null											-- required! if not passed value must be set in custom logic prior to insert
	,@RecordKey               varchar(50)       = null											-- required! if not passed value must be set in custom logic prior to insert
...
as
/*********************************************************************************************************************************
Procedure : sf.pJobRunError#Insert
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : inserts 1 row into the sf.JobRunError table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.JobRunError table with parameter values represent columns of the table. Additional
parameters are provided for all columns in the vJobRunError entity view, however, the base logic of the procedure inserts the
...
begin
	...
	set @JobRunErrorSID = null																							-- initialize output parameter
	...
	begin try
		...
		select
			...
			,@JobRunErrorSID = scope_identity()																	-- capture key value of new row
