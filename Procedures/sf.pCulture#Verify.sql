...
GO
CREATE PROCEDURE [sf].[pCulture#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.Culture
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.Culture for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.Culture table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fCulture#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pCulture#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.Culture'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.Culture'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.Culture'														-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.Culture'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.Culture'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_Culture'
		)
		begin
			exec sp_executesql N'alter table sf.Culture drop constraint ck_Culture'												-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.Culture with CHECK add constraint ck_Culture
			check
				(
				sf.fCulture#Check
					(
			...
			select @RowCount = count(1) from sf.Culture													-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.Culture with NOCHECK add constraint ck_Culture
			check
				(
				sf.fCulture#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.Culture'
		and
			br.BusinessRuleStatus <> 'x'
