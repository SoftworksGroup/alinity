...
GO
CREATE PROCEDURE [sf].[pTaskQueue#Insert]
	 @TaskQueueSID                     int               = null output			-- identity value assigned to the new record
	,@TaskQueueLabel                   nvarchar(35)      = null							-- required! if not passed value must be set in custom logic prior to insert
	,@TaskQueueCode                    varchar(30)       = null							-- required! if not passed value must be set in custom logic prior to insert
	,@UsageNotes                       nvarchar(max)     = null							
	,@IsAutoAssigned                   bit               = null							-- default: (0)
	,@IsOpenSubscription               bit               = null							-- default: (1)
	...
	,@SubscriberCount                  int               = null							-- not a base table column (default ignored)
as
/*********************************************************************************************************************************
Procedure : sf.pTaskQueue#Insert
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : inserts 1 row into the sf.TaskQueue table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.TaskQueue table with parameter values represent columns of the table. Additional
parameters are provided for all columns in the vTaskQueue entity view, however, the base logic of the procedure inserts the
...
begin
	...
	set @TaskQueueSID = null																								-- initialize output parameter
	...
	begin try
		...
		set @IsAutoAssigned = isnull(@IsAutoAssigned,(0))
		set @IsOpenSubscription = isnull(@IsOpenSubscription,(1))
		...
		begin
			...
			select @recordSID = x.TaskQueueSID from sf.TaskQueue x where x.IsDefault = @ON
		...
		select
			...
			,@TaskQueueSID = scope_identity()																		-- capture key value of new row
