...
GO
CREATE PROCEDURE [dbo].[pExamQuestionChoice#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ExamQuestionChoice
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ExamQuestionChoice for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ExamQuestionChoice table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fExamQuestionChoice#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pExamQuestionChoice#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ExamQuestionChoice'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ExamQuestionChoice'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ExamQuestionChoice'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ExamQuestionChoice'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ExamQuestionChoice'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ExamQuestionChoice'
		)
		begin
			exec sp_executesql N'alter table dbo.ExamQuestionChoice drop constraint ck_ExamQuestionChoice'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ExamQuestionChoice with CHECK add constraint ck_ExamQuestionChoice
			check
				(
				dbo.fExamQuestionChoice#Check
					(
			...
			select @RowCount = count(1) from dbo.ExamQuestionChoice							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ExamQuestionChoice'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.ExamQuestionChoiceSID
				,x.RowGUID
			from
				dbo.ExamQuestionChoice x
			order by
				x.ExamQuestionChoiceSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fExamQuestionChoice#Check
							(
							 x.ExamQuestionChoiceSID
							,x.ExamQuestionSID
							,x.Sequence
					...
					from
						dbo.ExamQuestionChoice  x
					where
						x.ExamQuestionChoiceSID = @nextSID
			...
			alter table dbo.ExamQuestionChoice with NOCHECK add constraint ck_ExamQuestionChoice
			check
				(
				dbo.fExamQuestionChoice#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ExamQuestionChoice'
		and
			br.BusinessRuleStatus <> 'x'
