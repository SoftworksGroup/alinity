...
GO
CREATE PROCEDURE [sf].[pJobScheduleEvent#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pJobScheduleEvent#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.JobScheduleEvent records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.JobScheduleEvent table. When a new record is to be added from the UI, this procedure
...
columns of the vJobScheduleEvent entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pJobScheduleEvent
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pJobScheduleEvent of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@jobScheduleEventSID int               = -1													-- specific default required by EF - do not override
		,@eventName           nvarchar(35)      = null												-- no default provided from DB constraint - OK to override
		,@eventDescription    nvarchar(max)     = null												-- no default provided from DB constraint - OK to override
		,@userDefinedColumns  xml               = null												-- no default provided from DB constraint - OK to override
		,@jobScheduleEventXID varchar(150)      = null												-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @eventName            = isnull(context.node.value('@EventName'          ,'nvarchar(35)' ),@eventName)
			,@eventDescription     = isnull(context.node.value('@EventDescription'   ,'nvarchar(max)'),@eventDescription)
			,@jobScheduleEventXID  = isnull(context.node.value('@JobScheduleEventXID','varchar(150)' ),@jobScheduleEventXID)
