...
GO
CREATE PROCEDURE [sf].[pFormType#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pFormType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.FormType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.FormType table. When a new record is to be added from the UI, this procedure
...
columns of the vFormType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pFormType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pFormType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@formTypeSID        int               = -1														-- specific default required by EF - do not override
		,@formTypeSCD        varchar(25)       = null													-- no default provided from DB constraint - OK to override
		,@formTypeLabel      nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		...
		,@formTypeXID        varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @formTypeSCD    = isnull(context.node.value('@FormTypeSCD'  ,'varchar(25)'  ),@formTypeSCD)
			,@formTypeLabel  = isnull(context.node.value('@FormTypeLabel','nvarchar(35)' ),@formTypeLabel)
			...
			,@formTypeXID    = isnull(context.node.value('@FormTypeXID'  ,'varchar(150)' ),@formTypeXID)
