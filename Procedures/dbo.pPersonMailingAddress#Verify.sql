...
GO
CREATE PROCEDURE [dbo].[pPersonMailingAddress#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PersonMailingAddress
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PersonMailingAddress for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PersonMailingAddress table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPersonMailingAddress#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPersonMailingAddress#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PersonMailingAddress'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PersonMailingAddress'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PersonMailingAddress'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PersonMailingAddress'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PersonMailingAddress'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PersonMailingAddress'
		)
		begin
			exec sp_executesql N'alter table dbo.PersonMailingAddress drop constraint ck_PersonMailingAddress'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PersonMailingAddress with CHECK add constraint ck_PersonMailingAddress
			check
				(
				dbo.fPersonMailingAddress#Check
					(
			...
			select @RowCount = count(1) from dbo.PersonMailingAddress						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PersonMailingAddress'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPersonMailingAddress#Check
							(
							 x.PersonMailingAddressSID
							,x.PersonSID
							,x.StreetAddress1
					...
					from
						dbo.PersonMailingAddress  x
					where
						x.PersonMailingAddressSID = @nextSID
			...
			alter table dbo.PersonMailingAddress with NOCHECK add constraint ck_PersonMailingAddress
			check
				(
				dbo.fPersonMailingAddress#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PersonMailingAddress'
		and
			br.BusinessRuleStatus <> 'x'
