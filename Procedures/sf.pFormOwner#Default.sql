...
GO
CREATE PROCEDURE [sf].[pFormOwner#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pFormOwner#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.FormOwner records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.FormOwner table. When a new record is to be added from the UI, this procedure
...
columns of the vFormOwner entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pFormOwner
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pFormOwner of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@formOwnerSID       int               = -1														-- specific default required by EF - do not override
		,@formOwnerSCD       varchar(25)       = null													-- no default provided from DB constraint - OK to override
		,@formOwnerLabel     nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		,@isAssignee         bit               = (0)													-- default provided from DB constraint - OK to override
		...
		,@formOwnerXID       varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @formOwnerSCD    = isnull(context.node.value('@FormOwnerSCD'  ,'varchar(25)'  ),@formOwnerSCD)
			,@formOwnerLabel  = isnull(context.node.value('@FormOwnerLabel','nvarchar(35)' ),@formOwnerLabel)
			,@isAssignee      = isnull(context.node.value('@IsAssignee'    ,'bit'          ),@isAssignee)
			,@description     = isnull(context.node.value('@Description'   ,'nvarchar(max)'),@description)
			,@formOwnerXID    = isnull(context.node.value('@FormOwnerXID'  ,'varchar(150)' ),@formOwnerXID)
