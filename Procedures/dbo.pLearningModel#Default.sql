...
GO
CREATE PROCEDURE [dbo].[pLearningModel#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pLearningModel#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.LearningModel records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.LearningModel table. When a new record is to be added from the UI, this procedure
...
columns of the vLearningModel entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pLearningModel
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pLearningModel of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@learningModelSID    int               = -1													-- specific default required by EF - do not override
		,@learningModelSCD    varchar(15)       = null												-- no default provided from DB constraint - OK to override
		,@learningModelLabel  nvarchar(35)      = null												-- no default provided from DB constraint - OK to override
		,@isDefault           bit               = (0)													-- default provided from DB constraint - OK to override
		,@unitTypeSID         int               = null												-- no default provided from DB constraint - OK to override
		,@cycleLengthYears    smallint          = (1)													-- default provided from DB constraint - OK to override
		,@isCycleStartedYear1 bit               = CONVERT(bit,(0))						-- default provided from DB constraint - OK to override
		...
		,@learningModelXID    varchar(150)      = null												-- no default provided from DB constraint - OK to override
		...
		,@unitTypeIsDefault   bit																							-- not a base table column (default ignored)
		,@unitTypeIsActive    bit																							-- not a base table column (default ignored)
		,@unitTypeRowGUID     uniqueidentifier																-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @learningModelSCD     = isnull(context.node.value('@LearningModelSCD'   ,'varchar(15)' ),@learningModelSCD)
			,@learningModelLabel   = isnull(context.node.value('@LearningModelLabel' ,'nvarchar(35)'),@learningModelLabel)
			,@isDefault            = isnull(context.node.value('@IsDefault'          ,'bit'         ),@isDefault)
			,@unitTypeSID          = isnull(context.node.value('@UnitTypeSID'        ,'int'         ),@unitTypeSID)
			,@cycleLengthYears     = isnull(context.node.value('@CycleLengthYears'   ,'smallint'    ),@cycleLengthYears)
			,@isCycleStartedYear1  = isnull(context.node.value('@IsCycleStartedYear1','bit'         ),@isCycleStartedYear1)
			,@maximumCarryOver     = isnull(context.node.value('@MaximumCarryOver'   ,'decimal(5,2)'),@maximumCarryOver)
			,@learningModelXID     = isnull(context.node.value('@LearningModelXID'   ,'varchar(150)'),@learningModelXID)
		...
		if isnull(@unitTypeSID,0) = 0 select @unitTypeSID = x.UnitTypeSID from sf.UnitType  x where x.IsDefault = @ON
