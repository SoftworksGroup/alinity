...
GO
CREATE PROCEDURE [sf].[pMailingPreference#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.MailingPreference
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.MailingPreference for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.MailingPreference table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fMailingPreference#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pMailingPreference#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.MailingPreference'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.MailingPreference'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.MailingPreference'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.MailingPreference'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.MailingPreference'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_MailingPreference'
		)
		begin
			exec sp_executesql N'alter table sf.MailingPreference drop constraint ck_MailingPreference'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.MailingPreference with CHECK add constraint ck_MailingPreference
			check
				(
				sf.fMailingPreference#Check
					(
			...
			select @RowCount = count(1) from sf.MailingPreference								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'MailingPreference'
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fMailingPreference#Check
							(
							 x.MailingPreferenceSID
							,x.MailingPreferenceLabel
							,x.IsAutoOptIn
							,x.IsActive
							,x.MailingPreferenceXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						sf.MailingPreference  x
					where
						x.MailingPreferenceSID = @nextSID
			...
			alter table sf.MailingPreference with NOCHECK add constraint ck_MailingPreference
			check
				(
				sf.fMailingPreference#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.MailingPreference'
		and
			br.BusinessRuleStatus <> 'x'
