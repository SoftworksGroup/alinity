...
GO
CREATE PROCEDURE [dbo].[pKnowledgeDomain#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.KnowledgeDomain
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.KnowledgeDomain for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.KnowledgeDomain table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fKnowledgeDomain#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pKnowledgeDomain#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.KnowledgeDomain'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.KnowledgeDomain'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.KnowledgeDomain'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.KnowledgeDomain'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.KnowledgeDomain'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_KnowledgeDomain'
		)
		begin
			exec sp_executesql N'alter table dbo.KnowledgeDomain drop constraint ck_KnowledgeDomain'			-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.KnowledgeDomain with CHECK add constraint ck_KnowledgeDomain
			check
				(
				dbo.fKnowledgeDomain#Check
					(
					 KnowledgeDomainSID
			...
			select @RowCount = count(1) from dbo.KnowledgeDomain								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'KnowledgeDomain'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.KnowledgeDomainSID
				,x.RowGUID
			from
				dbo.KnowledgeDomain x
			order by
				x.KnowledgeDomainSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fKnowledgeDomain#Check
							(
							 x.KnowledgeDomainSID
							,x.KnowledgeDomainLabel
							,x.IsDefault
							,x.IsActive
							,x.KnowledgeDomainXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
			...
			alter table dbo.KnowledgeDomain with NOCHECK add constraint ck_KnowledgeDomain
			check
				(
				dbo.fKnowledgeDomain#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.KnowledgeDomain'
		and
			br.BusinessRuleStatus <> 'x'
