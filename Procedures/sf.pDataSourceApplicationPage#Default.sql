...
GO
CREATE PROCEDURE [sf].[pDataSourceApplicationPage#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pDataSourceApplicationPage#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.DataSourceApplicationPage records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.DataSourceApplicationPage table. When a new record is to be added from the UI, this procedure
...
columns of the vDataSourceApplicationPage entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pDataSourceApplicationPage
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pDataSourceApplicationPage of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@dataSourceApplicationPageSID int               = -1									-- specific default required by EF - do not override
		,@dataSourceSID                int               = null								-- no default provided from DB constraint - OK to override
		...
		,@dataSourceApplicationPageXID varchar(150)      = null								-- no default provided from DB constraint - OK to override
		...
		,@dataSourceLabel              nvarchar(35)      = N'?'								-- not a base table column (default ignored)
		,@dBObjectName                 nvarchar(257)     = N'?'								-- not a base table column (default ignored)
		...
		,@dataSourceRowGUID            uniqueidentifier  = newid()						-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @dataSourceSID                 = isnull(context.node.value('@DataSourceSID'               ,'int'         ),@dataSourceSID)
			,@applicationPageSID            = isnull(context.node.value('@ApplicationPageSID'          ,'int'         ),@applicationPageSID)
			,@dataSourceApplicationPageXID  = isnull(context.node.value('@DataSourceApplicationPageXID','varchar(150)'),@dataSourceApplicationPageXID)
