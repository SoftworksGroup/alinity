...
GO
CREATE PROCEDURE [dbo].[pRegistrationChangeRequirement#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrationChangeRequirement
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrationChangeRequirement for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrationChangeRequirement table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrationChangeRequirement#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrationChangeRequirement#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrationChangeRequirement'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrationChangeRequirement'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationChangeRequirement'		-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationChangeRequirement'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrationChangeRequirement'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrationChangeRequirement'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrationChangeRequirement drop constraint ck_RegistrationChangeRequirement'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrationChangeRequirement with CHECK add constraint ck_RegistrationChangeRequirement
			check
				(
				dbo.fRegistrationChangeRequirement#Check
					(
			...
			select @RowCount = count(1) from dbo.RegistrationChangeRequirement	-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrationChangeRequirement'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrationChangeRequirementSID
				,x.RowGUID
			from
				dbo.RegistrationChangeRequirement x
			order by
				x.RegistrationChangeRequirementSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrationChangeRequirement#Check
							(
							 x.RegistrationChangeRequirementSID
							...
							,x.RegistrantExamSID
							,x.ExpiryMonths
							,x.RequirementStatusSID
							,x.RequirementSequence
							,x.RegistrationChangeRequirementXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrationChangeRequirement  x
					where
						x.RegistrationChangeRequirementSID = @nextSID
			...
			alter table dbo.RegistrationChangeRequirement with NOCHECK add constraint ck_RegistrationChangeRequirement
			check
				(
				dbo.fRegistrationChangeRequirement#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrationChangeRequirement'
		and
			br.BusinessRuleStatus <> 'x'
