...
GO
CREATE PROCEDURE [dbo].[pLearningRequirement#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.LearningRequirement
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.LearningRequirement for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.LearningRequirement table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fLearningRequirement#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pLearningRequirement#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.LearningRequirement'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.LearningRequirement'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.LearningRequirement'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.LearningRequirement'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.LearningRequirement'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_LearningRequirement'
		)
		begin
			exec sp_executesql N'alter table dbo.LearningRequirement drop constraint ck_LearningRequirement'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.LearningRequirement with CHECK add constraint ck_LearningRequirement
			check
				(
				dbo.fLearningRequirement#Check
					(
			...
			select @RowCount = count(1) from dbo.LearningRequirement						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'LearningRequirement'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fLearningRequirement#Check
							(
							 x.LearningRequirementSID
							,x.PracticeRegisterSID
							,x.LearningRequirementLabel
							,x.StartingRegistrationYear
							,x.Minimum
							,x.Maximum
							,x.MaximumCarryOver
							,x.LearningRequirementXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.LearningRequirement  x
					where
						x.LearningRequirementSID = @nextSID
			...
			alter table dbo.LearningRequirement with NOCHECK add constraint ck_LearningRequirement
			check
				(
				dbo.fLearningRequirement#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.LearningRequirement'
		and
			br.BusinessRuleStatus <> 'x'
