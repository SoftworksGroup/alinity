...
GO
CREATE PROCEDURE [sf].[pTaskStatus#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pTaskStatus#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.TaskStatus records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.TaskStatus table. When a new record is to be added from the UI, this procedure
...
columns of the vTaskStatus entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pTaskStatus
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pTaskStatus of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@taskStatusSID      int               = -1														-- specific default required by EF - do not override
		,@taskStatusSCD      varchar(10)       = null													-- no default provided from DB constraint - OK to override
		,@taskStatusLabel    nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		,@usageNotes         nvarchar(max)     = null													-- no default provided from DB constraint - OK to override
		,@taskStatusSequence int               = (0)													-- default provided from DB constraint - OK to override
		,@isDerived          bit               = CONVERT(bit,(0))							-- default provided from DB constraint - OK to override
		,@isClosedStatus     bit               = (0)													-- default provided from DB constraint - OK to override
		...
		,@taskStatusXID      varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @taskStatusSCD       = isnull(context.node.value('@TaskStatusSCD'     ,'varchar(10)'  ),@taskStatusSCD)
			,@taskStatusLabel     = isnull(context.node.value('@TaskStatusLabel'   ,'nvarchar(35)' ),@taskStatusLabel)
			,@usageNotes          = isnull(context.node.value('@UsageNotes'        ,'nvarchar(max)'),@usageNotes)
			,@taskStatusSequence  = isnull(context.node.value('@TaskStatusSequence','int'          ),@taskStatusSequence)
			,@isDerived           = isnull(context.node.value('@IsDerived'         ,'bit'          ),@isDerived)
			,@isClosedStatus      = isnull(context.node.value('@IsClosedStatus'    ,'bit'          ),@isClosedStatus)
			,@isActive            = isnull(context.node.value('@IsActive'          ,'bit'          ),@isActive)
			,@isDefault           = isnull(context.node.value('@IsDefault'         ,'bit'          ),@isDefault)
			,@taskStatusXID       = isnull(context.node.value('@TaskStatusXID'     ,'varchar(150)' ),@taskStatusXID)
