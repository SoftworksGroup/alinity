...
GO
CREATE PROCEDURE [dbo].[pComplaint#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.Complaint
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.Complaint for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.Complaint table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fComplaint#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pComplaint#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.Complaint'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.Complaint'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.Complaint'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.Complaint'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.Complaint'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_Complaint'
		)
		begin
			exec sp_executesql N'alter table dbo.Complaint drop constraint ck_Complaint'									-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.Complaint with CHECK add constraint ck_Complaint
			check
				(
			...
			select @RowCount = count(1) from dbo.Complaint											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.Complaint with NOCHECK add constraint ck_Complaint
			check
				(
				dbo.fComplaint#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.Complaint'
		and
			br.BusinessRuleStatus <> 'x'
