...
GO
CREATE PROCEDURE [dbo].[pReinstatementStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ReinstatementStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ReinstatementStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ReinstatementStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fReinstatementStatus#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pReinstatementStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ReinstatementStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ReinstatementStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ReinstatementStatus'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ReinstatementStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ReinstatementStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ReinstatementStatus'
		)
		begin
			exec sp_executesql N'alter table dbo.ReinstatementStatus drop constraint ck_ReinstatementStatus'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ReinstatementStatus with CHECK add constraint ck_ReinstatementStatus
			check
				(
				dbo.fReinstatementStatus#Check
					(
			...
			select @RowCount = count(1) from dbo.ReinstatementStatus						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ReinstatementStatus'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fReinstatementStatus#Check
							(
							 x.ReinstatementStatusSID
							,x.ReinstatementSID
							,x.FormStatusSID
							,x.ReinstatementStatusXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.ReinstatementStatus  x
					where
						x.ReinstatementStatusSID = @nextSID
			...
			alter table dbo.ReinstatementStatus with NOCHECK add constraint ck_ReinstatementStatus
			check
				(
				dbo.fReinstatementStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ReinstatementStatus'
		and
			br.BusinessRuleStatus <> 'x'
