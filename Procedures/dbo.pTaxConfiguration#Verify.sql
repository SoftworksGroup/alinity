...
GO
CREATE PROCEDURE [dbo].[pTaxConfiguration#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.TaxConfiguration
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.TaxConfiguration for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.TaxConfiguration table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fTaxConfiguration#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pTaxConfiguration#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.TaxConfiguration'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.TaxConfiguration'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.TaxConfiguration'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.TaxConfiguration'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.TaxConfiguration'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_TaxConfiguration'
		)
		begin
			exec sp_executesql N'alter table dbo.TaxConfiguration drop constraint ck_TaxConfiguration'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.TaxConfiguration with CHECK add constraint ck_TaxConfiguration
			check
				(
				dbo.fTaxConfiguration#Check
					(
					 TaxConfigurationSID
					,TaxSID
					,TaxRate
			...
			select @RowCount = count(1) from dbo.TaxConfiguration								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'TaxConfiguration'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.TaxConfigurationSID
				,x.RowGUID
			from
				dbo.TaxConfiguration x
			order by
				x.TaxConfigurationSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fTaxConfiguration#Check
							(
							 x.TaxConfigurationSID
							,x.TaxSID
							,x.TaxRate
			...
			alter table dbo.TaxConfiguration with NOCHECK add constraint ck_TaxConfiguration
			check
				(
				dbo.fTaxConfiguration#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.TaxConfiguration'
		and
			br.BusinessRuleStatus <> 'x'
