...
GO
CREATE PROCEDURE [sf].[pCrossWalkPair#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pCrossWalkPair#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.CrossWalkPair records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.CrossWalkPair table. When a new record is to be added from the UI, this procedure
...
columns of the vCrossWalkPair entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pCrossWalkPair
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pCrossWalkPair of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@crossWalkPairSID   int               = -1														-- specific default required by EF - do not override
		,@crossWalkSID       int               = null													-- no default provided from DB constraint - OK to override
		,@sourceValue        nvarchar(100)     = null													-- no default provided from DB constraint - OK to override
		,@targetValue        nvarchar(100)     = null													-- no default provided from DB constraint - OK to override
		,@userDefinedColumns xml               = null													-- no default provided from DB constraint - OK to override
		,@crossWalkPairXID   varchar(150)      = null													-- no default provided from DB constraint - OK to override
		...
		,@crossWalkLabel     nvarchar(35)      = N'?'													-- not a base table column (default ignored)
		,@crossWalkIsDefault bit               = 0														-- not a base table column (default ignored)
		,@crossWalkRowGUID   uniqueidentifier  = newid()											-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @crossWalkSID      = isnull(context.node.value('@CrossWalkSID'    ,'int'          ),@crossWalkSID)
			,@sourceValue       = isnull(context.node.value('@SourceValue'     ,'nvarchar(100)'),@sourceValue)
			,@targetValue       = isnull(context.node.value('@TargetValue'     ,'nvarchar(100)'),@targetValue)
			,@crossWalkPairXID  = isnull(context.node.value('@CrossWalkPairXID','varchar(150)' ),@crossWalkPairXID)
		...
		if isnull(@crossWalkSID,0) = 0 select @crossWalkSID = x.CrossWalkSID from sf.CrossWalk  x where x.IsDefault = @ON
		...
		select
			...
			,@rowGUID RowGUID
			,@rowStamp RowStamp
			,@crossWalkLabel CrossWalkLabel
			,@crossWalkIsDefault CrossWalkIsDefault
			,@crossWalkRowGUID CrossWalkRowGUID
