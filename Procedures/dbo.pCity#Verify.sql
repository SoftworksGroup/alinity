...
GO
CREATE PROCEDURE [dbo].[pCity#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.City
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.City for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.City table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fCity#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pCity#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.City'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.City'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.City'															-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.City'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.City'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_City'
		)
		begin
			exec sp_executesql N'alter table dbo.City drop constraint ck_City'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.City with CHECK add constraint ck_City
			check
				(
				dbo.fCity#Check
					(
			...
			select @RowCount = count(1) from dbo.City														-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.City with NOCHECK add constraint ck_City
			check
				(
				dbo.fCity#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.City'
		and
			br.BusinessRuleStatus <> 'x'
