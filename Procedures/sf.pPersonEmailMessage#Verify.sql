...
GO
CREATE PROCEDURE [sf].[pPersonEmailMessage#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.PersonEmailMessage
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.PersonEmailMessage for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.PersonEmailMessage table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fPersonEmailMessage#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pPersonEmailMessage#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.PersonEmailMessage'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.PersonEmailMessage'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.PersonEmailMessage'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.PersonEmailMessage'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.PersonEmailMessage'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_PersonEmailMessage'
		)
		begin
			exec sp_executesql N'alter table sf.PersonEmailMessage drop constraint ck_PersonEmailMessage'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.PersonEmailMessage with CHECK add constraint ck_PersonEmailMessage
			check
				(
				sf.fPersonEmailMessage#Check
					(
			...
			select @RowCount = count(1) from sf.PersonEmailMessage							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PersonEmailMessage'
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fPersonEmailMessage#Check
							(
							 x.PersonEmailMessageSID
							,x.PersonSID
							,x.EmailMessageSID
					...
					from
						sf.PersonEmailMessage  x
					where
						x.PersonEmailMessageSID = @nextSID
			...
			alter table sf.PersonEmailMessage with NOCHECK add constraint ck_PersonEmailMessage
			check
				(
				sf.fPersonEmailMessage#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.PersonEmailMessage'
		and
			br.BusinessRuleStatus <> 'x'
