...
GO
CREATE PROCEDURE [dbo].[pRegistrationSchedule#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrationSchedule
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrationSchedule for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrationSchedule table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrationSchedule#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrationSchedule#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrationSchedule'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrationSchedule'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationSchedule'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationSchedule'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrationSchedule'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrationSchedule'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrationSchedule drop constraint ck_RegistrationSchedule'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrationSchedule with CHECK add constraint ck_RegistrationSchedule
			check
				(
				dbo.fRegistrationSchedule#Check
					(
			...
			select @RowCount = count(1) from dbo.RegistrationSchedule						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrationSchedule'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrationSchedule#Check
							(
							 x.RegistrationScheduleSID
							,x.RegistrationScheduleLabel
							,x.IsDefault
							,x.IsActive
							,x.RegistrationScheduleXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrationSchedule  x
					where
						x.RegistrationScheduleSID = @nextSID
			...
			alter table dbo.RegistrationSchedule with NOCHECK add constraint ck_RegistrationSchedule
			check
				(
				dbo.fRegistrationSchedule#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrationSchedule'
		and
			br.BusinessRuleStatus <> 'x'
