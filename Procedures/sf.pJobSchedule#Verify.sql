...
GO
CREATE PROCEDURE [sf].[pJobSchedule#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.JobSchedule
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.JobSchedule for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.JobSchedule table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fJobSchedule#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pJobSchedule#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.JobSchedule'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.JobSchedule'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.JobSchedule'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.JobSchedule'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.JobSchedule'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_JobSchedule'
		)
		begin
			exec sp_executesql N'alter table sf.JobSchedule drop constraint ck_JobSchedule'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.JobSchedule with CHECK add constraint ck_JobSchedule
			check
				(
				sf.fJobSchedule#Check
					(
					 JobScheduleSID
					,JobScheduleLabel
					,IsEnabled
			...
			select @RowCount = count(1) from sf.JobSchedule											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.JobSchedule with NOCHECK add constraint ck_JobSchedule
			check
				(
				sf.fJobSchedule#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.JobSchedule'
		and
			br.BusinessRuleStatus <> 'x'
