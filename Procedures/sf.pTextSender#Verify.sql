...
GO
CREATE PROCEDURE [sf].[pTextSender#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.TextSender
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.TextSender for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.TextSender table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fTextSender#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pTextSender#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.TextSender'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.TextSender'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.TextSender'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.TextSender'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.TextSender'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_TextSender'
		)
		begin
			exec sp_executesql N'alter table sf.TextSender drop constraint ck_TextSender'									-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.TextSender with CHECK add constraint ck_TextSender
			check
				(
				sf.fTextSender#Check
					(
					 TextSenderSID
					,SenderPhone
					,SenderDisplayName
			...
			select @RowCount = count(1) from sf.TextSender											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.TextSender with NOCHECK add constraint ck_TextSender
			check
				(
				sf.fTextSender#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.TextSender'
		and
			br.BusinessRuleStatus <> 'x'
