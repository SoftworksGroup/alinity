...
GO
CREATE PROCEDURE [sf].[pQuery#Delete]
	 @QuerySID                 int               = null -- required! id of row to delete - must be set in custom logic if not passed
	...
	,@QueryCategoryIsActive    bit               = null
	,@QueryCategoryIsDefault   bit               = null
	,@QueryCategoryRowGUID     uniqueidentifier  = null
...
as
/*********************************************************************************************************************************
Procedure : sf.pQuery#Delete
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : deletes 1 row in the sf.Query table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.Query table. The procedure requires a primary key value to locate the record
to delete.
...
and in the ext.pQuery procedure for client-specific deletion rules.
...
begin
	...
	begin try
		...
		if @RowStamp is null select @RowStamp = x.RowStamp from sf.Query x where x.QuerySID = @QuerySID
    ...
    -- the deletion option based on the value returned from the IsDeleteEnabled function
		-- in the #Ext view.
    set @IsDeleteEnabled = sf.fQuery#IsDeleteEnabled(@QuerySID)

    if isnull(@IsDeleteEnabled, cast(0 as bit)) = cast(0 as bit)
    begin

      exec sf.pMessage#Get   @MessageSCD    = 'DeleteNotPermitted'
        ,@MessageText   = @errorText output
