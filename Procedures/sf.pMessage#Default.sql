...
GO
CREATE PROCEDURE [sf].[pMessage#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pMessage#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.Message records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.Message table. When a new record is to be added from the UI, this procedure
...
columns of the vMessage entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pMessage
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pMessage of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@messageSID            int               = -1												-- specific default required by EF - do not override
		,@messageSCD            varchar(128)      = null											-- no default provided from DB constraint - OK to override
		,@messageName           nvarchar(100)     = null											-- no default provided from DB constraint - OK to override
		,@messageText           nvarchar(1000)    = null											-- no default provided from DB constraint - OK to override
		,@messageTextUpdateTime datetimeoffset(7) = null											-- no default provided from DB constraint - OK to override
		,@defaultText           nvarchar(1000)    = null											-- no default provided from DB constraint - OK to override
		,@defaultTextUpdateTime datetimeoffset(7) = sysdatetimeoffset()				-- default provided from DB constraint - OK to override
		,@userDefinedColumns    xml               = null											-- no default provided from DB constraint - OK to override
		,@messageXID            varchar(150)      = null											-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			...
			,@messageName            = isnull(context.node.value('@MessageName'          ,'nvarchar(100)'    ),@messageName)
			,@messageText            = isnull(context.node.value('@MessageText'          ,'nvarchar(1000)'   ),@messageText)
			,@messageTextUpdateTime  = isnull(context.node.value('@MessageTextUpdateTime','datetimeoffset(7)'),@messageTextUpdateTime)
			,@defaultText            = isnull(context.node.value('@DefaultText'          ,'nvarchar(1000)'   ),@defaultText)
			,@defaultTextUpdateTime  = isnull(context.node.value('@DefaultTextUpdateTime','datetimeoffset(7)'),@defaultTextUpdateTime)
			,@messageXID             = isnull(context.node.value('@MessageXID'           ,'varchar(150)'     ),@messageXID)
