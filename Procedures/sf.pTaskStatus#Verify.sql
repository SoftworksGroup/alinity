...
GO
CREATE PROCEDURE [sf].[pTaskStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.TaskStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.TaskStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.TaskStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fTaskStatus#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pTaskStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.TaskStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.TaskStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.TaskStatus'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.TaskStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.TaskStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_TaskStatus'
		)
		begin
			exec sp_executesql N'alter table sf.TaskStatus drop constraint ck_TaskStatus'									-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.TaskStatus with CHECK add constraint ck_TaskStatus
			check
				(
				sf.fTaskStatus#Check
					(
					 TaskStatusSID
					,TaskStatusSCD
					,TaskStatusLabel
			...
			select @RowCount = count(1) from sf.TaskStatus											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.TaskStatus with NOCHECK add constraint ck_TaskStatus
			check
				(
				sf.fTaskStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.TaskStatus'
		and
			br.BusinessRuleStatus <> 'x'
