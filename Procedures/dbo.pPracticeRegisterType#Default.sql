...
GO
CREATE PROCEDURE [dbo].[pPracticeRegisterType#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPracticeRegisterType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.PracticeRegisterType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PracticeRegisterType table. When a new record is to be added from the UI, this procedure
...
columns of the vPracticeRegisterType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pPracticeRegisterType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pPracticeRegisterType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@practiceRegisterTypeSID      int               = -1									-- specific default required by EF - do not override
		,@practiceRegisterTypeSCD      varchar(15)       = null								-- no default provided from DB constraint - OK to override
		,@practiceRegisterTypeLabel    nvarchar(35)      = null								-- no default provided from DB constraint - OK to override
		,@practiceRegisterTypeCategory nvarchar(65)      = null								-- no default provided from DB constraint - OK to override
		...
		,@practiceRegisterTypeXID      varchar(150)      = null								-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @practiceRegisterTypeSCD       = isnull(context.node.value('@PracticeRegisterTypeSCD'     ,'varchar(15)'  ),@practiceRegisterTypeSCD)
			,@practiceRegisterTypeLabel     = isnull(context.node.value('@PracticeRegisterTypeLabel'   ,'nvarchar(35)' ),@practiceRegisterTypeLabel)
			,@practiceRegisterTypeCategory  = isnull(context.node.value('@PracticeRegisterTypeCategory','nvarchar(65)' ),@practiceRegisterTypeCategory)
			...
			,@practiceRegisterTypeXID       = isnull(context.node.value('@PracticeRegisterTypeXID'     ,'varchar(150)' ),@practiceRegisterTypeXID)
