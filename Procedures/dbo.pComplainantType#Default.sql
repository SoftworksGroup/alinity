...
GO
CREATE PROCEDURE [dbo].[pComplainantType#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pComplainantType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.ComplainantType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.ComplainantType table. When a new record is to be added from the UI, this procedure
...
columns of the vComplainantType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pComplainantType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pComplainantType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@complainantTypeSID      int               = -1											-- specific default required by EF - do not override
		,@complainantTypeLabel    nvarchar(35)      = null										-- no default provided from DB constraint - OK to override
		,@complainantTypeCategory nvarchar(65)      = null										-- no default provided from DB constraint - OK to override
		...
		,@complainantTypeXID      varchar(150)      = null										-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @complainantTypeLabel     = isnull(context.node.value('@ComplainantTypeLabel'   ,'nvarchar(35)' ),@complainantTypeLabel)
			,@complainantTypeCategory  = isnull(context.node.value('@ComplainantTypeCategory','nvarchar(65)' ),@complainantTypeCategory)
			...
			,@complainantTypeXID       = isnull(context.node.value('@ComplainantTypeXID'     ,'varchar(150)' ),@complainantTypeXID)
