...
GO
CREATE PROCEDURE [dbo].[pRegistrantAuditReview#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantAuditReview
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantAuditReview for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantAuditReview table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantAuditReview#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantAuditReview#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantAuditReview'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantAuditReview'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAuditReview'						-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAuditReview'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantAuditReview'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantAuditReview'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantAuditReview drop constraint ck_RegistrantAuditReview'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantAuditReview with CHECK add constraint ck_RegistrantAuditReview
			check
				(
				dbo.fRegistrantAuditReview#Check
					(
			...
			select @RowCount = count(1) from dbo.RegistrantAuditReview					-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantAuditReview'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantAuditReview#Check
							(
							 x.RegistrantAuditReviewSID
					...
					from
						dbo.RegistrantAuditReview  x
					where
						x.RegistrantAuditReviewSID = @nextSID
			...
			alter table dbo.RegistrantAuditReview with NOCHECK add constraint ck_RegistrantAuditReview
			check
				(
				dbo.fRegistrantAuditReview#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantAuditReview'
		and
			br.BusinessRuleStatus <> 'x'
