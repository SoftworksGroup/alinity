...
GO
CREATE PROCEDURE dbo.pQuery#Execute$Registration
 ...
 ,@LatestRegistration dbo.LatestRegistration readonly -- table storing keys of registration record for selected year 
 ,@ReturnPersonSID		bit = 0													-- pass as 1 when called from PersonList screen
as
/*********************************************************************************************************************************
Sproc    : Query Search - Registration
Notice   : Copyright © 2018 Softworks Group Inc.
Summary  : This procedure executes searches (queries) to support management of Registration records
----------------------------------------------------------------------------------------------------------------------------------
History	: Author							| Month Year	| Change Summary
				: ------------------- + ----------- + ------------------------------------------------------------------------------------
 				: Tim Edlund          | Sep 2018		|	Initial version
				: Cory Ng							| Nov 2018		| Added culture support to various renewal, application and reinstatement queries
				: Tim Edlund					| Jan 2019		| Added @LatestRegistration parameter to improve performance
				: Tim Edlund					| Jan 2019		| Modified logic to write results directly to #selected (temp table declared by parent)
				: Tim Edlund					| Mar 2019		| Added ReturnPersonSID parameter to support calling from PersonList page

Comments	
--------
This procedure is a subroutine called from pQuery#Execute. It provides the syntax for executing queries in support of Registration
...
begin
  ...
  <Test Name = "TermBased" Description="Executes the procedure to return all term-based registrations for the previous registration year">
    <SQLScript>
      <![CDATA[
declare
	@parameters					dbo.Parameter
...
begin
	...
	exec dbo.pQuery#Execute$Registration
		@QueryCode = 'S!REG.ALL.TERM.BASED'
	 ,@Parameters = @parameters
	 ,@MaxRows = 9999999
	 ,@RegistrationYear = @registrationYear
	 ,@LatestRegistration = @latestRegistration;
...
exec sf.pUnitTest#Execute
	 @ObjectName = 'dbo.pQuery#Execute$Registration'
	,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
	...
	begin try
		...
		begin
			...
			insert
				#selected (EntitySID)
			select top (@MaxRows)
				(case when @ReturnPersonSID = @ON then r.PersonSID else lReg.RegistrationSID end)
			from
		...
		begin
			...
			insert
				#selected (EntitySID)
			select top (@MaxRows)
				(case when @ReturnPersonSID = @ON then lReg.PersonSID else lReg.RegistrationSID end)
			from
		...
		begin
			...
			insert
				#selected (EntitySID)
			select top (@MaxRows)
				(case when @ReturnPersonSID = @ON then r.PersonSID else fs.RegistrationSID end)
			from
				dbo.fRegistration#FormStatus(@LatestRegistration) fs
			join
				dbo.Registrant																		r on fs.RegistrantSID			= r.RegistrantSID
			join
		...
		begin
			...
			insert
				#selected (EntitySID)
			select	(case when @ReturnPersonSID = @ON then r.PersonSID else fs.RegistrationSID end)
			from
				dbo.fRegistration#FormStatus(@LatestRegistration) fs
			join
				dbo.Registrant																		r on fs.RegistrantSID			= r.RegistrantSID
		...
		begin
			...
			insert
				#selected (EntitySID)
			select	(case when @ReturnPersonSID = @ON then r.PersonSID else lReg.RegistrationSID end)
			from
				@LatestRegistration					lReg
			join
				dbo.Registration						reg on lReg.RegistrationSID						= reg.RegistrationSID
		...
		begin
			...
			begin
				...
				insert
					#selected (EntitySID)
				select	(case when @ReturnPersonSID = @ON then re.PersonSID else cs.RegistrationSID end)
				from
			...
			begin
				...
				insert
					#selected (EntitySID)
				select	(case when @ReturnPersonSID = @ON then x.PersonSID else x.RegistrationSID end)
				from
				(
					select
						cs.RegistrationSID
		...
		begin
			...
			insert
				#selected (EntitySID)
			select top (@MaxRows)
				(case when @ReturnPersonSID = @ON then rs.PersonSID else rs.RegistrationSIDFrom end)
			from
		...
		begin
			...
			insert
				#selected (EntitySID)
			select	(case when @ReturnPersonSID = @ON then r.PersonSID else rnw.RegistrationSID end)
			from
			(
				select
			...
			)																																																	pad on r.PersonSID = pad.PersonSID and pad.RowNo = 1
			left outer join
		...
		begin
			...
			insert
				#selected (EntitySID)
			select	(case when @ReturnPersonSID = @ON then rs.PersonSID else rs.RegistrationSIDFrom end)
			from
				dbo.fRegistration#RenewalStatus(@RegistrationYear) rs
			where
		...
		begin
			...
			insert
				#selected (EntitySID)
			select top (@MaxRows)
				(case when @ReturnPersonSID = @ON then rnw.PersonSID else rnw.RegistrationSIDFrom end)
			from
		...
		begin
			...
			insert
				#selected (EntitySID)
			select	(case when @ReturnPersonSID = @ON then re.PersonSID else app.RegistrationSID end)
			from
