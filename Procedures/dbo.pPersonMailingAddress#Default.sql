...
GO
CREATE PROCEDURE [dbo].[pPersonMailingAddress#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPersonMailingAddress#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.PersonMailingAddress records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PersonMailingAddress table. When a new record is to be added from the UI, this procedure
...
columns of the vPersonMailingAddress entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pPersonMailingAddress
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pPersonMailingAddress of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@personMailingAddressSID   int               = -1										-- specific default required by EF - do not override
		...
		,@personMailingAddressXID   varchar(150)      = null									-- no default provided from DB constraint - OK to override
		...
		,@htmlAddress               nvarchar(512)															-- not a base table column (default ignored)
		...
		,@isCurrentAddress          bit																				-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@personMailingAddressXID  = isnull(context.node.value('@PersonMailingAddressXID','varchar(150)'     ),@personMailingAddressXID)
		...
		-- Christian T | Dec 2015
		-- Override effectiveTime to right now. Ensures db time is retrieved and not
		-- browser time
