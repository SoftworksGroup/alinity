...
GO
CREATE PROCEDURE [stg].[pOrgProfile#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for stg.OrgProfile
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks stg.OrgProfile for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the stg.OrgProfile table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, stg.fOrgProfile#Check(), on each row. If an error is
...
Example:
	...
	exec stg.pOrgProfile#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'stg.OrgProfile'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'stg.OrgProfile'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'stg.OrgProfile'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'stg.OrgProfile'
			)
		...
		where
			brx.ApplicationEntitySCD = 'stg.OrgProfile'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'stg'
		and
			cc.ConstraintName = 'ck_OrgProfile'
		)
		begin
			exec sp_executesql N'alter table stg.OrgProfile drop constraint ck_OrgProfile'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table stg.OrgProfile with CHECK add constraint ck_OrgProfile
			check
				(
			...
			select @RowCount = count(1) from stg.OrgProfile											-- rules validated - calculate row count to return
		...
		begin
				...
				begin try
					...
					select
						@isValid = stg.fOrgProfile#Check
							...
							,x.LastVerifiedTime
							,x.IsEmployer
							,x.IsEducationInstitution
							,x.IsActive
							,x.ParentOrgProfileSID
							,x.CitySID
							,x.StateProvinceSID
							,x.CountrySID
							,x.RegionSID
			...
			alter table stg.OrgProfile with NOCHECK add constraint ck_OrgProfile
			check
				(
				stg.fOrgProfile#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'stg.OrgProfile'
		and
			br.BusinessRuleStatus <> 'x'
