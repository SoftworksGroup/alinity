...
GO
CREATE PROCEDURE [sf].[pDefaultEmailTemplate#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pDefaultEmailTemplate#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.DefaultEmailTemplate records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.DefaultEmailTemplate table. When a new record is to be added from the UI, this procedure
...
columns of the vDefaultEmailTemplate entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pDefaultEmailTemplate
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pDefaultEmailTemplate of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@defaultEmailTemplateSID   int               = -1										-- specific default required by EF - do not override
		,@defaultEmailTemplateSCD   varchar(10)       = null									-- no default provided from DB constraint - OK to override
		,@defaultEmailTemplateLabel nvarchar(35)      = null									-- no default provided from DB constraint - OK to override
		...
		,@defaultEmailTemplateXID   varchar(150)      = null									-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @defaultEmailTemplateSCD    = isnull(context.node.value('@DefaultEmailTemplateSCD'  ,'varchar(10)' ),@defaultEmailTemplateSCD)
			,@defaultEmailTemplateLabel  = isnull(context.node.value('@DefaultEmailTemplateLabel','nvarchar(35)'),@defaultEmailTemplateLabel)
			,@emailTemplateSID           = isnull(context.node.value('@EmailTemplateSID'         ,'int'         ),@emailTemplateSID)
			,@defaultEmailTemplateXID    = isnull(context.node.value('@DefaultEmailTemplateXID'  ,'varchar(150)'),@defaultEmailTemplateXID)
