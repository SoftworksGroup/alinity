...
GO
CREATE PROCEDURE [sf].[pFormOwner#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.FormOwner
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.FormOwner for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.FormOwner table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fFormOwner#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pFormOwner#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.FormOwner'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.FormOwner'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.FormOwner'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.FormOwner'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.FormOwner'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_FormOwner'
		)
		begin
			exec sp_executesql N'alter table sf.FormOwner drop constraint ck_FormOwner'										-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.FormOwner with CHECK add constraint ck_FormOwner
			check
				(
				sf.fFormOwner#Check
					(
			...
			select @RowCount = count(1) from sf.FormOwner												-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.FormOwner with NOCHECK add constraint ck_FormOwner
			check
				(
				sf.fFormOwner#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.FormOwner'
		and
			br.BusinessRuleStatus <> 'x'
