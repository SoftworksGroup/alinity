...
GO
CREATE PROCEDURE [sf].[pDefaultEmailTemplate#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.DefaultEmailTemplate
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.DefaultEmailTemplate for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.DefaultEmailTemplate table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fDefaultEmailTemplate#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pDefaultEmailTemplate#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.DefaultEmailTemplate'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.DefaultEmailTemplate'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.DefaultEmailTemplate'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.DefaultEmailTemplate'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.DefaultEmailTemplate'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_DefaultEmailTemplate'
		)
		begin
			exec sp_executesql N'alter table sf.DefaultEmailTemplate drop constraint ck_DefaultEmailTemplate'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.DefaultEmailTemplate with CHECK add constraint ck_DefaultEmailTemplate
			check
				(
				sf.fDefaultEmailTemplate#Check
					(
			...
			select @RowCount = count(1) from sf.DefaultEmailTemplate						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'DefaultEmailTemplate'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.DefaultEmailTemplateSID
				,x.RowGUID
			from
				sf.DefaultEmailTemplate x
			order by
				x.DefaultEmailTemplateSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fDefaultEmailTemplate#Check
							(
							 x.DefaultEmailTemplateSID
							,x.DefaultEmailTemplateSCD
							,x.DefaultEmailTemplateLabel
							,x.EmailTemplateSID
							,x.DefaultEmailTemplateXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						sf.DefaultEmailTemplate  x
					where
						x.DefaultEmailTemplateSID = @nextSID
			...
			alter table sf.DefaultEmailTemplate with NOCHECK add constraint ck_DefaultEmailTemplate
			check
				(
				sf.fDefaultEmailTemplate#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.DefaultEmailTemplate'
		and
			br.BusinessRuleStatus <> 'x'
