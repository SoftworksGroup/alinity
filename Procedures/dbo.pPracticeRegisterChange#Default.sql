...
GO
CREATE PROCEDURE [dbo].[pPracticeRegisterChange#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPracticeRegisterChange#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.PracticeRegisterChange records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PracticeRegisterChange table. When a new record is to be added from the UI, this procedure
...
columns of the vPracticeRegisterChange entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pPracticeRegisterChange
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pPracticeRegisterChange of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@practiceRegisterChangeSID                  int               = -1		-- specific default required by EF - do not override
		...
		,@isEnabledForRegistrant                     bit               = CONVERT(bit,(0))								-- default provided from DB constraint - OK to override
		,@userDefinedColumns                         xml               = null	-- no default provided from DB constraint - OK to override
		,@practiceRegisterChangeXID                  varchar(150)      = null	-- no default provided from DB constraint - OK to override
		...
		,@practiceRegisterSectionPracticeRegisterSID int               = 0		-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@isEnabledForRegistrant      = isnull(context.node.value('@IsEnabledForRegistrant'    ,'bit'          ),@isEnabledForRegistrant)
			,@practiceRegisterChangeXID   = isnull(context.node.value('@PracticeRegisterChangeXID' ,'varchar(150)' ),@practiceRegisterChangeXID)
