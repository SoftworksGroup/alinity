...
GO
CREATE PROCEDURE [dbo].[pLearningClaimType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.LearningClaimType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.LearningClaimType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.LearningClaimType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fLearningClaimType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pLearningClaimType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.LearningClaimType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.LearningClaimType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.LearningClaimType'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.LearningClaimType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.LearningClaimType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_LearningClaimType'
		)
		begin
			exec sp_executesql N'alter table dbo.LearningClaimType drop constraint ck_LearningClaimType'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.LearningClaimType with CHECK add constraint ck_LearningClaimType
			check
				(
				dbo.fLearningClaimType#Check
					(
			...
			select @RowCount = count(1) from dbo.LearningClaimType							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'LearningClaimType'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fLearningClaimType#Check
							(
							 x.LearningClaimTypeSID
							,x.LearningClaimTypeLabel
							,x.LearningClaimTypeCategory
							,x.IsValidForRenewal
							,x.IsComplete
							,x.IsWithdrawn
							,x.IsDefault
							,x.IsActive
							,x.LearningClaimTypeXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.LearningClaimType  x
					where
						x.LearningClaimTypeSID = @nextSID
			...
			alter table dbo.LearningClaimType with NOCHECK add constraint ck_LearningClaimType
			check
				(
				dbo.fLearningClaimType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.LearningClaimType'
		and
			br.BusinessRuleStatus <> 'x'
