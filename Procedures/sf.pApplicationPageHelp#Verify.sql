...
GO
CREATE PROCEDURE [sf].[pApplicationPageHelp#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ApplicationPageHelp
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ApplicationPageHelp for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ApplicationPageHelp table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fApplicationPageHelp#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pApplicationPageHelp#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ApplicationPageHelp'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ApplicationPageHelp'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationPageHelp'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationPageHelp'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ApplicationPageHelp'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ApplicationPageHelp'
		)
		begin
			exec sp_executesql N'alter table sf.ApplicationPageHelp drop constraint ck_ApplicationPageHelp'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ApplicationPageHelp with CHECK add constraint ck_ApplicationPageHelp
			check
				(
				sf.fApplicationPageHelp#Check
					(
			...
			select @RowCount = count(1) from sf.ApplicationPageHelp							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ApplicationPageHelp'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.ApplicationPageHelpSID
				,x.RowGUID
			from
				sf.ApplicationPageHelp x
			order by
				x.ApplicationPageHelpSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fApplicationPageHelp#Check
							(
							 x.ApplicationPageHelpSID
							,x.ApplicationPageSID
							,x.ApplicationPageHelpID
							,x.StepSequence
							,x.ApplicationPageHelpXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						sf.ApplicationPageHelp  x
					where
						x.ApplicationPageHelpSID = @nextSID
			...
			alter table sf.ApplicationPageHelp with NOCHECK add constraint ck_ApplicationPageHelp
			check
				(
				sf.fApplicationPageHelp#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ApplicationPageHelp'
		and
			br.BusinessRuleStatus <> 'x'
