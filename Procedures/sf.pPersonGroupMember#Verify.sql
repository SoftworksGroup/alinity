...
GO
CREATE PROCEDURE [sf].[pPersonGroupMember#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.PersonGroupMember
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.PersonGroupMember for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.PersonGroupMember table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fPersonGroupMember#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pPersonGroupMember#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.PersonGroupMember'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.PersonGroupMember'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.PersonGroupMember'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.PersonGroupMember'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.PersonGroupMember'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_PersonGroupMember'
		)
		begin
			exec sp_executesql N'alter table sf.PersonGroupMember drop constraint ck_PersonGroupMember'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.PersonGroupMember with CHECK add constraint ck_PersonGroupMember
			check
				(
			...
			select @RowCount = count(1) from sf.PersonGroupMember								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PersonGroupMember'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.PersonGroupMemberSID
				,x.RowGUID
			from
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fPersonGroupMember#Check
							(
							 x.PersonGroupMemberSID
							,x.PersonGroupSID
							,x.PersonSID
							...
							,x.EffectiveTime
							,x.ExpiryTime
							,x.IsReplacementRequiredAfterTerm
							,x.ReplacementClearedDate
							,x.PersonGroupMemberXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						sf.PersonGroupMember  x
					where
						x.PersonGroupMemberSID = @nextSID
			...
			alter table sf.PersonGroupMember with NOCHECK add constraint ck_PersonGroupMember
			check
				(
				sf.fPersonGroupMember#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.PersonGroupMember'
		and
			br.BusinessRuleStatus <> 'x'
