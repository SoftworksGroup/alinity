...
GO
CREATE PROCEDURE [dbo].[pExamOffering#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ExamOffering
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ExamOffering for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ExamOffering table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fExamOffering#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pExamOffering#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ExamOffering'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ExamOffering'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ExamOffering'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ExamOffering'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ExamOffering'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ExamOffering'
		)
		begin
			exec sp_executesql N'alter table dbo.ExamOffering drop constraint ck_ExamOffering'						-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ExamOffering with CHECK add constraint ck_ExamOffering
			check
				(
			...
			select @RowCount = count(1) from dbo.ExamOffering										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fExamOffering#Check
							(
							 x.ExamOfferingSID
							,x.ExamSID
							,x.OrgSID
			...
			alter table dbo.ExamOffering with NOCHECK add constraint ck_ExamOffering
			check
				(
				dbo.fExamOffering#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ExamOffering'
		and
			br.BusinessRuleStatus <> 'x'
