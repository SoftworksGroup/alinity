...
GO
CREATE PROCEDURE [sf].[pExportFile#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ExportFile
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ExportFile for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ExportFile table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fExportFile#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pExportFile#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ExportFile'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ExportFile'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ExportFile'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ExportFile'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ExportFile'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ExportFile'
		)
		begin
			exec sp_executesql N'alter table sf.ExportFile drop constraint ck_ExportFile'									-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ExportFile with CHECK add constraint ck_ExportFile
			check
				(
				sf.fExportFile#Check
					(
					 ExportFileSID
					,ExportSourceGUID
					,FileFormatSID
			...
			select @RowCount = count(1) from sf.ExportFile											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.ExportFile with NOCHECK add constraint ck_ExportFile
			check
				(
				sf.fExportFile#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ExportFile'
		and
			br.BusinessRuleStatus <> 'x'
