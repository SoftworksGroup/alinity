...
GO
CREATE PROCEDURE [sf].[pClearedAnnouncement#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pClearedAnnouncement#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.ClearedAnnouncement records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.ClearedAnnouncement table. When a new record is to be added from the UI, this procedure
...
columns of the vClearedAnnouncement entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pClearedAnnouncement
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pClearedAnnouncement of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@clearedAnnouncementSID           int               = -1							-- specific default required by EF - do not override
		...
		,@clearedAnnouncementXID           varchar(150)      = null						-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			...
			,@clearedAnnouncementXID  = isnull(context.node.value('@ClearedAnnouncementXID','varchar(150)'),@clearedAnnouncementXID)
