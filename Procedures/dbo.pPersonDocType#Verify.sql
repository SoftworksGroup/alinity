...
GO
CREATE PROCEDURE [dbo].[pPersonDocType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PersonDocType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PersonDocType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PersonDocType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPersonDocType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPersonDocType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PersonDocType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PersonDocType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PersonDocType'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PersonDocType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PersonDocType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PersonDocType'
		)
		begin
			exec sp_executesql N'alter table dbo.PersonDocType drop constraint ck_PersonDocType'					-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PersonDocType with CHECK add constraint ck_PersonDocType
			check
				(
				dbo.fPersonDocType#Check
					(
					 PersonDocTypeSID
					,PersonDocTypeSCD
					,PersonDocTypeLabel
			...
			select @RowCount = count(1) from dbo.PersonDocType									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PersonDocType'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPersonDocType#Check
							(
							 x.PersonDocTypeSID
							,x.PersonDocTypeSCD
							,x.PersonDocTypeLabel
							,x.PersonDocTypeCategory
							,x.IsDefault
							,x.IsActive
							,x.PersonDocTypeXID
							,x.LegacyKey
			...
			alter table dbo.PersonDocType with NOCHECK add constraint ck_PersonDocType
			check
				(
				dbo.fPersonDocType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PersonDocType'
		and
			br.BusinessRuleStatus <> 'x'
