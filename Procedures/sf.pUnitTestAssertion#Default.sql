...
GO
CREATE PROCEDURE [sf].[pUnitTestAssertion#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pUnitTestAssertion#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.UnitTestAssertion records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.UnitTestAssertion table. When a new record is to be added from the UI, this procedure
...
columns of the vUnitTestAssertion entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pUnitTestAssertion
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pUnitTestAssertion of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@unitTestAssertionSID int               = -1													-- specific default required by EF - do not override
		,@unitTestSID          int               = null												-- no default provided from DB constraint - OK to override
		,@assertionType        varchar(25)       = null												-- no default provided from DB constraint - OK to override
		,@value                nvarchar(1000)    = null												-- no default provided from DB constraint - OK to override
		,@resultSet            tinyint           = (0)												-- default provided from DB constraint - OK to override
		,@rowNo                int               = (0)												-- default provided from DB constraint - OK to override
		,@columnNo             int               = (0)												-- default provided from DB constraint - OK to override
		,@userDefinedColumns   xml               = null												-- no default provided from DB constraint - OK to override
		,@unitTestAssertionXID varchar(150)      = null												-- no default provided from DB constraint - OK to override
		...
		,@schemaName           nvarchar(128)     = N'?'												-- not a base table column (default ignored)
		,@objectName           nvarchar(128)     = N'?'												-- not a base table column (default ignored)
		,@testName             nvarchar(35)      = N'?'												-- not a base table column (default ignored)
		,@objectType           varchar(10)       = '?'												-- not a base table column (default ignored)
		,@unitTestIsDefault    bit               = 0													-- not a base table column (default ignored)
		,@unitTestRowGUID      uniqueidentifier  = newid()										-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @unitTestSID           = isnull(context.node.value('@UnitTestSID'         ,'int'           ),@unitTestSID)
			,@assertionType         = isnull(context.node.value('@AssertionType'       ,'varchar(25)'   ),@assertionType)
			,@value                 = isnull(context.node.value('@Value'               ,'nvarchar(1000)'),@value)
			,@resultSet             = isnull(context.node.value('@ResultSet'           ,'tinyint'       ),@resultSet)
			,@rowNo                 = isnull(context.node.value('@RowNo'               ,'int'           ),@rowNo)
			,@columnNo              = isnull(context.node.value('@ColumnNo'            ,'int'           ),@columnNo)
			,@unitTestAssertionXID  = isnull(context.node.value('@UnitTestAssertionXID','varchar(150)'  ),@unitTestAssertionXID)
		...
		if isnull(@unitTestSID,0) = 0 select @unitTestSID = x.UnitTestSID from sf.UnitTest  x where x.IsDefault = @ON and x.SchemaName = @schemaName and x.ObjectName = @objectName
		...
		select
			...
			,@unitTestIsDefault UnitTestIsDefault
			,@unitTestRowGUID UnitTestRowGUID
