...
GO
CREATE PROCEDURE [dbo].[pRegistrantAuditResponse#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantAuditResponse
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantAuditResponse for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantAuditResponse table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantAuditResponse#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantAuditResponse#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantAuditResponse'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantAuditResponse'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAuditResponse'					-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAuditResponse'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantAuditResponse'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantAuditResponse'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantAuditResponse drop constraint ck_RegistrantAuditResponse'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantAuditResponse with CHECK add constraint ck_RegistrantAuditResponse
			check
				(
				dbo.fRegistrantAuditResponse#Check
					(
					 RegistrantAuditResponseSID
					,RegistrantAuditSID
					,FormOwnerSID
					,RegistrantAuditResponseXID
					,LegacyKey
					,IsDeleted
					,CreateUser
					,CreateTime
			...
			select @RowCount = count(1) from dbo.RegistrantAuditResponse				-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantAuditResponse'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrantAuditResponseSID
				,x.RowGUID
			from
				dbo.RegistrantAuditResponse x
			order by
				x.RegistrantAuditResponseSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantAuditResponse#Check
							(
							 x.RegistrantAuditResponseSID
							,x.RegistrantAuditSID
							,x.FormOwnerSID
							,x.RegistrantAuditResponseXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantAuditResponse  x
					where
						x.RegistrantAuditResponseSID = @nextSID
			...
			alter table dbo.RegistrantAuditResponse with NOCHECK add constraint ck_RegistrantAuditResponse
			check
				(
				dbo.fRegistrantAuditResponse#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantAuditResponse'
		and
			br.BusinessRuleStatus <> 'x'
