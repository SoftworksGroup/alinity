...
GO
CREATE PROCEDURE [dbo].[pOrgOtherName#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.OrgOtherName
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.OrgOtherName for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.OrgOtherName table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fOrgOtherName#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pOrgOtherName#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.OrgOtherName'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.OrgOtherName'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.OrgOtherName'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.OrgOtherName'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.OrgOtherName'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_OrgOtherName'
		)
		begin
			exec sp_executesql N'alter table dbo.OrgOtherName drop constraint ck_OrgOtherName'						-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.OrgOtherName with CHECK add constraint ck_OrgOtherName
			check
				(
				dbo.fOrgOtherName#Check
					(
					 OrgOtherNameSID
					,OrgSID
					,OrgName
			...
			select @RowCount = count(1) from dbo.OrgOtherName										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fOrgOtherName#Check
							(
							 x.OrgOtherNameSID
							,x.OrgSID
							,x.OrgName
			...
			alter table dbo.OrgOtherName with NOCHECK add constraint ck_OrgOtherName
			check
				(
				dbo.fOrgOtherName#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.OrgOtherName'
		and
			br.BusinessRuleStatus <> 'x'
