...
GO
CREATE PROCEDURE [dbo].[pProfileUpdateResponse#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pProfileUpdateResponse#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.ProfileUpdateResponse records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.ProfileUpdateResponse table. When a new record is to be added from the UI, this procedure
...
columns of the vProfileUpdateResponse entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pProfileUpdateResponse
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pProfileUpdateResponse of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@profileUpdateResponseSID int               = -1											-- specific default required by EF - do not override
		,@profileUpdateSID         int               = null										-- no default provided from DB constraint - OK to override
		...
		,@profileUpdateResponseXID varchar(150)      = null										-- no default provided from DB constraint - OK to override
		...
		,@profileUpdateRowGUID     uniqueidentifier  = newid()								-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @profileUpdateSID          = isnull(context.node.value('@ProfileUpdateSID'        ,'int'         ),@profileUpdateSID)
			,@formOwnerSID              = isnull(context.node.value('@FormOwnerSID'            ,'int'         ),@formOwnerSID)
			,@profileUpdateResponseXID  = isnull(context.node.value('@ProfileUpdateResponseXID','varchar(150)'),@profileUpdateResponseXID)
