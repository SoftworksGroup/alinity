...
GO
CREATE PROCEDURE [dbo].[pPracticeRegisterCatalogItem#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PracticeRegisterCatalogItem
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PracticeRegisterCatalogItem for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PracticeRegisterCatalogItem table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPracticeRegisterCatalogItem#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPracticeRegisterCatalogItem#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PracticeRegisterCatalogItem'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PracticeRegisterCatalogItem'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRegisterCatalogItem'			-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRegisterCatalogItem'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PracticeRegisterCatalogItem'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PracticeRegisterCatalogItem'
		)
		begin
			exec sp_executesql N'alter table dbo.PracticeRegisterCatalogItem drop constraint ck_PracticeRegisterCatalogItem'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PracticeRegisterCatalogItem with CHECK add constraint ck_PracticeRegisterCatalogItem
			check
				(
			...
			select @RowCount = count(1) from dbo.PracticeRegisterCatalogItem		-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PracticeRegisterCatalogItem'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.PracticeRegisterCatalogItemSID
				,x.RowGUID
			from
				dbo.PracticeRegisterCatalogItem x
			order by
				x.PracticeRegisterCatalogItemSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPracticeRegisterCatalogItem#Check
							(
							 x.PracticeRegisterCatalogItemSID
							,x.PracticeRegisterSID
							,x.CatalogItemSID
							,x.IsAppliedOnApplication
							,x.IsAppliedOnApplicationApproval
							,x.IsAppliedOnRenewal
							,x.IsAppliedOnReinstatement
							,x.IsAppliedOnRegChange
							,x.IsAppliedToPAPSubscribers
							,x.PracticeRegisterSectionSID
							,x.PracticeRegisterChangeSID
							,x.FeeSequence
							,x.EffectiveTime
							,x.ExpiryTime
							,x.PracticeRegisterCatalogItemXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.PracticeRegisterCatalogItem  x
					where
						x.PracticeRegisterCatalogItemSID = @nextSID
			...
			alter table dbo.PracticeRegisterCatalogItem with NOCHECK add constraint ck_PracticeRegisterCatalogItem
			check
				(
				dbo.fPracticeRegisterCatalogItem#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PracticeRegisterCatalogItem'
		and
			br.BusinessRuleStatus <> 'x'
