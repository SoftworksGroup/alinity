...
GO
CREATE PROCEDURE [dbo].[pOrgType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.OrgType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.OrgType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.OrgType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fOrgType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pOrgType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.OrgType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.OrgType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.OrgType'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.OrgType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.OrgType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_OrgType'
		)
		begin
			exec sp_executesql N'alter table dbo.OrgType drop constraint ck_OrgType'											-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.OrgType with CHECK add constraint ck_OrgType
			check
				(
				dbo.fOrgType#Check
					(
			...
			select @RowCount = count(1) from dbo.OrgType												-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.OrgType with NOCHECK add constraint ck_OrgType
			check
				(
				dbo.fOrgType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.OrgType'
		and
			br.BusinessRuleStatus <> 'x'
