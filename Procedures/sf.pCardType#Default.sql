...
GO
CREATE PROCEDURE [sf].[pCardType#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pCardType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.CardType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.CardType table. When a new record is to be added from the UI, this procedure
...
columns of the vCardType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pCardType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pCardType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@cardTypeSID        int               = -1														-- specific default required by EF - do not override
		,@cardTypeSCD        varchar(25)       = null													-- no default provided from DB constraint - OK to override
		,@cardTypeLabel      nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		...
		,@cardTypeXID        varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @cardTypeSCD    = isnull(context.node.value('@CardTypeSCD'  ,'varchar(25)'  ),@cardTypeSCD)
			,@cardTypeLabel  = isnull(context.node.value('@CardTypeLabel','nvarchar(35)' ),@cardTypeLabel)
			,@usageNotes     = isnull(context.node.value('@UsageNotes'   ,'nvarchar(max)'),@usageNotes)
			,@cardTypeXID    = isnull(context.node.value('@CardTypeXID'  ,'varchar(150)' ),@cardTypeXID)
