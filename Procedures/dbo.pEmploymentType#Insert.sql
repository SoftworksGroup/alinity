...
GO
CREATE PROCEDURE [dbo].[pEmploymentType#Insert]
	 @EmploymentTypeSID      int               = null output								-- identity value assigned to the new record
	,@EmploymentTypeName     nvarchar(50)      = null												-- required! if not passed value must be set in custom logic prior to insert
	,@EmploymentTypeCode     varchar(20)       = null												-- required! if not passed value must be set in custom logic prior to insert
...
as
/*********************************************************************************************************************************
Procedure : dbo.pEmploymentType#Insert
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : inserts 1 row into the dbo.EmploymentType table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.EmploymentType table with parameter values represent columns of the table. Additional
parameters are provided for all columns in the vEmploymentType entity view, however, the base logic of the procedure inserts the
...
begin
	...
	set @EmploymentTypeSID = null																						-- initialize output parameter
	...
	begin try
		...
		begin
			...
			select @recordSID = x.EmploymentTypeSID from dbo.EmploymentType x where x.IsDefault = @ON
		...
		select
			...
			,@EmploymentTypeSID = scope_identity()															-- capture key value of new row
