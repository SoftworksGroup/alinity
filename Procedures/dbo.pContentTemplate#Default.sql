...
GO
CREATE PROCEDURE [dbo].[pContentTemplate#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pContentTemplate#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.ContentTemplate records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.ContentTemplate table. When a new record is to be added from the UI, this procedure
...
columns of the vContentTemplate entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pContentTemplate
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pContentTemplate of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@contentTemplateSID   int               = -1													-- specific default required by EF - do not override
		,@contentTemplateSCD   varchar(15)       = null												-- no default provided from DB constraint - OK to override
		,@contentTemplateLabel nvarchar(35)      = null												-- no default provided from DB constraint - OK to override
		,@contentTemplateData  xml               = null												-- no default provided from DB constraint - OK to override
		,@userDefinedColumns   xml               = null												-- no default provided from DB constraint - OK to override
		,@contentTemplateXID   varchar(150)      = null												-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @contentTemplateSCD    = isnull(context.node.value('@ContentTemplateSCD'  ,'varchar(15)' ),@contentTemplateSCD)
			,@contentTemplateLabel  = isnull(context.node.value('@ContentTemplateLabel','nvarchar(35)'),@contentTemplateLabel)
			,@contentTemplateXID    = isnull(context.node.value('@ContentTemplateXID'  ,'varchar(150)'),@contentTemplateXID)
