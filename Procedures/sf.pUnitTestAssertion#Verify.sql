...
GO
CREATE PROCEDURE [sf].[pUnitTestAssertion#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.UnitTestAssertion
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.UnitTestAssertion for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.UnitTestAssertion table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fUnitTestAssertion#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pUnitTestAssertion#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.UnitTestAssertion'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.UnitTestAssertion'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.UnitTestAssertion'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.UnitTestAssertion'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.UnitTestAssertion'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_UnitTestAssertion'
		)
		begin
			exec sp_executesql N'alter table sf.UnitTestAssertion drop constraint ck_UnitTestAssertion'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.UnitTestAssertion with CHECK add constraint ck_UnitTestAssertion
			check
				(
				sf.fUnitTestAssertion#Check
					(
			...
			select @RowCount = count(1) from sf.UnitTestAssertion								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'UnitTestAssertion'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.UnitTestAssertionSID
				,x.RowGUID
			from
				sf.UnitTestAssertion x
			order by
				x.UnitTestAssertionSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fUnitTestAssertion#Check
							(
							 x.UnitTestAssertionSID
							,x.UnitTestSID
							,x.AssertionType
					...
					from
						sf.UnitTestAssertion  x
					where
						x.UnitTestAssertionSID = @nextSID
			...
			alter table sf.UnitTestAssertion with NOCHECK add constraint ck_UnitTestAssertion
			check
				(
				sf.fUnitTestAssertion#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.UnitTestAssertion'
		and
			br.BusinessRuleStatus <> 'x'
