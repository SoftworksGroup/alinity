...
GO
CREATE PROCEDURE [dbo].[pPracticeRegisterChangeRequirement#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PracticeRegisterChangeRequirement
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PracticeRegisterChangeRequirement for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PracticeRegisterChangeRequirement table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPracticeRegisterChangeRequirement#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPracticeRegisterChangeRequirement#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PracticeRegisterChangeRequirement'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PracticeRegisterChangeRequirement'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRegisterChangeRequirement'-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRegisterChangeRequirement'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PracticeRegisterChangeRequirement'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PracticeRegisterChangeRequirement'
		)
		begin
			exec sp_executesql N'alter table dbo.PracticeRegisterChangeRequirement drop constraint ck_PracticeRegisterChangeRequirement'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PracticeRegisterChangeRequirement with CHECK add constraint ck_PracticeRegisterChangeRequirement
			check
				(
				dbo.fPracticeRegisterChangeRequirement#Check
					(
			...
			select @RowCount = count(1) from dbo.PracticeRegisterChangeRequirement												-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PracticeRegisterChangeRequirement'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPracticeRegisterChangeRequirement#Check
							(
							 x.PracticeRegisterChangeRequirementSID
							,x.PracticeRegisterChangeSID
							,x.RegistrationRequirementSID
							,x.IsMandatory
							,x.ExpiryMonths
							,x.IsActive
							,x.RequirementSequence
							,x.PracticeRegisterChangeRequirementXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.PracticeRegisterChangeRequirement  x
					where
						x.PracticeRegisterChangeRequirementSID = @nextSID
			...
			alter table dbo.PracticeRegisterChangeRequirement with NOCHECK add constraint ck_PracticeRegisterChangeRequirement
			check
				(
				dbo.fPracticeRegisterChangeRequirement#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PracticeRegisterChangeRequirement'
		and
			br.BusinessRuleStatus <> 'x'
