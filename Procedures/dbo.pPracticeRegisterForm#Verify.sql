...
GO
CREATE PROCEDURE [dbo].[pPracticeRegisterForm#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PracticeRegisterForm
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PracticeRegisterForm for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PracticeRegisterForm table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPracticeRegisterForm#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPracticeRegisterForm#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PracticeRegisterForm'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PracticeRegisterForm'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRegisterForm'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRegisterForm'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PracticeRegisterForm'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PracticeRegisterForm'
		)
		begin
			exec sp_executesql N'alter table dbo.PracticeRegisterForm drop constraint ck_PracticeRegisterForm'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PracticeRegisterForm with CHECK add constraint ck_PracticeRegisterForm
			check
				(
				dbo.fPracticeRegisterForm#Check
					(
			...
			select @RowCount = count(1) from dbo.PracticeRegisterForm						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PracticeRegisterForm'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.PracticeRegisterFormSID
				,x.RowGUID
			from
				dbo.PracticeRegisterForm x
			order by
				x.PracticeRegisterFormSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPracticeRegisterForm#Check
							(
							 x.PracticeRegisterFormSID
							,x.PracticeRegisterSID
							,x.FormSID
							,x.PracticeRegisterFormXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.PracticeRegisterForm  x
					where
						x.PracticeRegisterFormSID = @nextSID
			...
			alter table dbo.PracticeRegisterForm with NOCHECK add constraint ck_PracticeRegisterForm
			check
				(
				dbo.fPracticeRegisterForm#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PracticeRegisterForm'
		and
			br.BusinessRuleStatus <> 'x'
