...
GO
CREATE PROCEDURE [sf].[pAnnouncement#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.Announcement
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.Announcement for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.Announcement table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fAnnouncement#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pAnnouncement#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.Announcement'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.Announcement'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.Announcement'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.Announcement'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.Announcement'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_Announcement'
		)
		begin
			exec sp_executesql N'alter table sf.Announcement drop constraint ck_Announcement'							-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.Announcement with CHECK add constraint ck_Announcement
			check
				(
				sf.fAnnouncement#Check
					(
					 AnnouncementSID
					,Title
					,AnnouncementText
			...
			select @RowCount = count(1) from sf.Announcement										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fAnnouncement#Check
							(
							 x.AnnouncementSID
							,x.Title
							,x.AnnouncementText
			...
			alter table sf.Announcement with NOCHECK add constraint ck_Announcement
			check
				(
				sf.fAnnouncement#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.Announcement'
		and
			br.BusinessRuleStatus <> 'x'
