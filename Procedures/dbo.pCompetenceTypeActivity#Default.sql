...
GO
CREATE PROCEDURE [dbo].[pCompetenceTypeActivity#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pCompetenceTypeActivity#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.CompetenceTypeActivity records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.CompetenceTypeActivity table. When a new record is to be added from the UI, this procedure
...
columns of the vCompetenceTypeActivity entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pCompetenceTypeActivity
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pCompetenceTypeActivity of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@competenceTypeActivitySID  int               = -1										-- specific default required by EF - do not override
		,@competenceTypeSID          int               = null									-- no default provided from DB constraint - OK to override
		,@competenceActivitySID      int               = null									-- no default provided from DB constraint - OK to override
		...
		,@competenceTypeActivityXID  varchar(150)      = null									-- no default provided from DB constraint - OK to override
		...
		,@competenceActivityLabel    nvarchar(35)      = N'?'									-- not a base table column (default ignored)
		,@competenceActivityName     nvarchar(65)      = N'?'									-- not a base table column (default ignored)
		,@unitValue                  decimal(5,2)      = convert(decimal(5,2), 0.0)											-- not a base table column (default ignored)
		,@competenceActivityIsActive bit               = 0										-- not a base table column (default ignored)
		,@competenceActivityRowGUID  uniqueidentifier  = newid()							-- not a base table column (default ignored)
		...
		,@competenceActivityXID      varchar(150)															-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@competenceActivitySID      = isnull(context.node.value('@CompetenceActivitySID'    ,'int'         ),@competenceActivitySID)
			...
			,@competenceTypeActivityXID  = isnull(context.node.value('@CompetenceTypeActivityXID','varchar(150)'),@competenceTypeActivityXID)
