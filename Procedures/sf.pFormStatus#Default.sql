...
GO
CREATE PROCEDURE [sf].[pFormStatus#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pFormStatus#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.FormStatus records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.FormStatus table. When a new record is to be added from the UI, this procedure
...
columns of the vFormStatus entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pFormStatus
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pFormStatus of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@formStatusSID      int               = -1														-- specific default required by EF - do not override
		,@formStatusSCD      varchar(25)       = null													-- no default provided from DB constraint - OK to override
		,@formStatusLabel    nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		...
		,@formStatusSequence int               = (0)													-- default provided from DB constraint - OK to override
		...
		,@formStatusXID      varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @formStatusSCD       = isnull(context.node.value('@FormStatusSCD'     ,'varchar(25)'  ),@formStatusSCD)
			,@formStatusLabel     = isnull(context.node.value('@FormStatusLabel'   ,'nvarchar(35)' ),@formStatusLabel)
			...
			,@formStatusSequence  = isnull(context.node.value('@FormStatusSequence','int'          ),@formStatusSequence)
			,@formOwnerSID        = isnull(context.node.value('@FormOwnerSID'      ,'int'          ),@formOwnerSID)
			,@formStatusXID       = isnull(context.node.value('@FormStatusXID'     ,'varchar(150)' ),@formStatusXID)
