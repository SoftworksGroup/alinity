...
GO
CREATE PROCEDURE [dbo].[pOrg#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.Org
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.Org for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.Org table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fOrg#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pOrg#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.Org'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.Org'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.Org'															-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.Org'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.Org'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_Org'
		)
		begin
			exec sp_executesql N'alter table dbo.Org drop constraint ck_Org'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.Org with CHECK add constraint ck_Org
			check
				(
				dbo.fOrg#Check
					(
			...
			select @RowCount = count(1) from dbo.Org														-- rules validated - calculate row count to return
		...
		begin
				...
				begin try
					...
					select
						@isValid = dbo.fOrg#Check
							...
							,x.IsInsurer
							,x.IsInsuranceCertificateRequired
							,x.IsPublic
							,x.IsActive
			...
			alter table dbo.Org with NOCHECK add constraint ck_Org
			check
				(
				dbo.fOrg#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.Org'
		and
			br.BusinessRuleStatus <> 'x'
