...
GO
CREATE PROCEDURE [sf].[pMessage#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.Message
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.Message for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.Message table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fMessage#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pMessage#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.Message'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.Message'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.Message'														-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.Message'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.Message'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_Message'
		)
		begin
			exec sp_executesql N'alter table sf.Message drop constraint ck_Message'												-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.Message with CHECK add constraint ck_Message
			check
				(
				sf.fMessage#Check
					(
			...
			select @RowCount = count(1) from sf.Message													-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fMessage#Check
							(
							 x.MessageSID
							,x.MessageSCD
							,x.MessageName
							,x.MessageText
							,x.MessageTextUpdateTime
							,x.DefaultText
							,x.DefaultTextUpdateTime
							,x.MessageXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
			...
			alter table sf.Message with NOCHECK add constraint ck_Message
			check
				(
				sf.fMessage#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.Message'
		and
			br.BusinessRuleStatus <> 'x'
