...
GO
CREATE PROCEDURE [dbo].[pExamQuestionReference#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ExamQuestionReference
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ExamQuestionReference for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ExamQuestionReference table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fExamQuestionReference#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pExamQuestionReference#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ExamQuestionReference'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ExamQuestionReference'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ExamQuestionReference'						-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ExamQuestionReference'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ExamQuestionReference'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ExamQuestionReference'
		)
		begin
			exec sp_executesql N'alter table dbo.ExamQuestionReference drop constraint ck_ExamQuestionReference'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ExamQuestionReference with CHECK add constraint ck_ExamQuestionReference
			check
				(
				dbo.fExamQuestionReference#Check
					(
					 ExamQuestionReferenceSID
					,ExamQuestionSID
					,ReferenceTitle
					,ReferenceURL
					,ExamQuestionReferenceXID
					,LegacyKey
					,IsDeleted
					,CreateUser
					,CreateTime
			...
			select @RowCount = count(1) from dbo.ExamQuestionReference					-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ExamQuestionReference'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.ExamQuestionReferenceSID
				,x.RowGUID
			from
				dbo.ExamQuestionReference x
			order by
				x.ExamQuestionReferenceSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fExamQuestionReference#Check
							(
							 x.ExamQuestionReferenceSID
							,x.ExamQuestionSID
							,x.ReferenceTitle
							,x.ReferenceURL
							,x.ExamQuestionReferenceXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.ExamQuestionReference  x
					where
						x.ExamQuestionReferenceSID = @nextSID
			...
			alter table dbo.ExamQuestionReference with NOCHECK add constraint ck_ExamQuestionReference
			check
				(
				dbo.fExamQuestionReference#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ExamQuestionReference'
		and
			br.BusinessRuleStatus <> 'x'
