...
GO
CREATE PROCEDURE [dbo].[pRegistrantAppReview#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantAppReview
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantAppReview for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantAppReview table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantAppReview#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantAppReview#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantAppReview'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantAppReview'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAppReview'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAppReview'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantAppReview'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantAppReview'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantAppReview drop constraint ck_RegistrantAppReview'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantAppReview with CHECK add constraint ck_RegistrantAppReview
			check
				(
			...
			select @RowCount = count(1) from dbo.RegistrantAppReview						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantAppReview'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantAppReview#Check
							(
							 x.RegistrantAppReviewSID
							,x.RegistrantAppSID
							,x.FormVersionSID
					...
					from
						dbo.RegistrantAppReview  x
					where
						x.RegistrantAppReviewSID = @nextSID
			...
			alter table dbo.RegistrantAppReview with NOCHECK add constraint ck_RegistrantAppReview
			check
				(
				dbo.fRegistrantAppReview#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantAppReview'
		and
			br.BusinessRuleStatus <> 'x'
