...
GO
CREATE PROCEDURE [sf].[pApplicationUserProfileProperty#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pApplicationUserProfileProperty#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.ApplicationUserProfileProperty records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.ApplicationUserProfileProperty table. When a new record is to be added from the UI, this procedure
...
columns of the vApplicationUserProfileProperty entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pApplicationUserProfileProperty
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pApplicationUserProfileProperty of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@applicationUserProfilePropertySID int               = -1						-- specific default required by EF - do not override
		,@applicationUserSID                int               = null					-- no default provided from DB constraint - OK to override
		,@propertyName                      varchar(100)      = null					-- no default provided from DB constraint - OK to override
		,@propertyValue                     xml               = null					-- no default provided from DB constraint - OK to override
		,@userDefinedColumns                xml               = null					-- no default provided from DB constraint - OK to override
		,@applicationUserProfilePropertyXID varchar(150)      = null					-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			...
			,@propertyName                       = isnull(context.node.value('@PropertyName'                     ,'varchar(100)'),@propertyName)
			,@applicationUserProfilePropertyXID  = isnull(context.node.value('@ApplicationUserProfilePropertyXID','varchar(150)'),@applicationUserProfilePropertyXID)
