...
GO
CREATE PROCEDURE [dbo].[pPaymentType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PaymentType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PaymentType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PaymentType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPaymentType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPaymentType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PaymentType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PaymentType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PaymentType'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PaymentType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PaymentType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PaymentType'
		)
		begin
			exec sp_executesql N'alter table dbo.PaymentType drop constraint ck_PaymentType'							-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PaymentType with CHECK add constraint ck_PaymentType
			check
				(
			...
			select @RowCount = count(1) from dbo.PaymentType										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPaymentType#Check
							(
							 x.PaymentTypeSID
							,x.PaymentTypeSCD
							,x.PaymentTypeLabel
							,x.PaymentTypeCategory
							,x.GLAccountSID
							,x.PaymentStatusSID
							,x.IsReferenceRequired
							,x.DepositDateLagDays
							,x.IsRefundExcludedFromGL
							,x.ExcludeDepositFromGLBefore
							,x.IsDefault
							,x.IsActive
							,x.PaymentTypeXID
							,x.LegacyKey
			...
			alter table dbo.PaymentType with NOCHECK add constraint ck_PaymentType
			check
				(
				dbo.fPaymentType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PaymentType'
		and
			br.BusinessRuleStatus <> 'x'
