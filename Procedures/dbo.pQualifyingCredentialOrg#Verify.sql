...
GO
CREATE PROCEDURE [dbo].[pQualifyingCredentialOrg#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.QualifyingCredentialOrg
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.QualifyingCredentialOrg for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.QualifyingCredentialOrg table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fQualifyingCredentialOrg#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pQualifyingCredentialOrg#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.QualifyingCredentialOrg'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.QualifyingCredentialOrg'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.QualifyingCredentialOrg'					-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.QualifyingCredentialOrg'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.QualifyingCredentialOrg'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_QualifyingCredentialOrg'
		)
		begin
			exec sp_executesql N'alter table dbo.QualifyingCredentialOrg drop constraint ck_QualifyingCredentialOrg'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.QualifyingCredentialOrg with CHECK add constraint ck_QualifyingCredentialOrg
			check
				(
				dbo.fQualifyingCredentialOrg#Check
					(
			...
			select @RowCount = count(1) from dbo.QualifyingCredentialOrg				-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'QualifyingCredentialOrg'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.QualifyingCredentialOrgSID
				,x.RowGUID
			from
				dbo.QualifyingCredentialOrg x
			order by
				x.QualifyingCredentialOrgSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fQualifyingCredentialOrg#Check
							(
							 x.QualifyingCredentialOrgSID
							,x.CredentialSID
							,x.OrgSID
							,x.IsActive
							,x.QualifyingCredentialOrgXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.QualifyingCredentialOrg  x
					where
						x.QualifyingCredentialOrgSID = @nextSID
			...
			alter table dbo.QualifyingCredentialOrg with NOCHECK add constraint ck_QualifyingCredentialOrg
			check
				(
				dbo.fQualifyingCredentialOrg#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.QualifyingCredentialOrg'
		and
			br.BusinessRuleStatus <> 'x'
