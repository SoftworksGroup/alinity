...
GO
CREATE PROCEDURE [dbo].[pPracticeRegisterRestriction#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPracticeRegisterRestriction#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.PracticeRegisterRestriction records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PracticeRegisterRestriction table. When a new record is to be added from the UI, this procedure
...
columns of the vPracticeRegisterRestriction entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pPracticeRegisterRestriction
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pPracticeRegisterRestriction of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@practiceRegisterRestrictionSID              int               = -1	-- specific default required by EF - do not override
		,@practiceRegisterSID                         int               = null-- no default provided from DB constraint - OK to override
		,@practiceRestrictionSID                      int               = null-- no default provided from DB constraint - OK to override
		...
		,@practiceRegisterRestrictionXID              varchar(150)      = null-- no default provided from DB constraint - OK to override
		...
		,@practiceRegisterIsSupervisionRequired       bit               = 0		-- not a base table column (default ignored)
		...
		,@practiceRestrictionLabel                    nvarchar(35)      = N'?'-- not a base table column (default ignored)
		,@practiceRestrictionIsDisplayedOnLicense     bit               = 0		-- not a base table column (default ignored)
		,@practiceRestrictionIsActive                 bit               = 0		-- not a base table column (default ignored)
		,@practiceRestrictionIsSupervisionRequired    bit               = 0		-- not a base table column (default ignored)
		,@practiceRestrictionRowGUID                  uniqueidentifier  = newid()												-- not a base table column (default ignored)
		...
		,@practiceRegisterSectionIsDisplayedOnLicense bit											-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@practiceRestrictionSID          = isnull(context.node.value('@PracticeRestrictionSID'        ,'int'         ),@practiceRestrictionSID)
			...
			,@practiceRegisterRestrictionXID  = isnull(context.node.value('@PracticeRegisterRestrictionXID','varchar(150)'),@practiceRegisterRestrictionXID)
