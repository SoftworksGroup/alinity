...
GO
CREATE PROCEDURE [dbo].[pRegistrantInactivePeriod#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantInactivePeriod
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantInactivePeriod for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantInactivePeriod table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantInactivePeriod#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantInactivePeriod#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantInactivePeriod'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantInactivePeriod'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantInactivePeriod'					-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantInactivePeriod'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantInactivePeriod'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantInactivePeriod'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantInactivePeriod drop constraint ck_RegistrantInactivePeriod'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantInactivePeriod with CHECK add constraint ck_RegistrantInactivePeriod
			check
				(
			...
			select @RowCount = count(1) from dbo.RegistrantInactivePeriod				-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantInactivePeriod'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrantInactivePeriodSID
				,x.RowGUID
			from
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantInactivePeriod#Check
							(
							 x.RegistrantInactivePeriodSID
							,x.RegistrantSID
					...
					from
						dbo.RegistrantInactivePeriod  x
					where
						x.RegistrantInactivePeriodSID = @nextSID
			...
			alter table dbo.RegistrantInactivePeriod with NOCHECK add constraint ck_RegistrantInactivePeriod
			check
				(
				dbo.fRegistrantInactivePeriod#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantInactivePeriod'
		and
			br.BusinessRuleStatus <> 'x'
