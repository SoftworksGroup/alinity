...
GO
CREATE PROCEDURE [sf].[pUnexpectedError#Delete]
	 @UnexpectedErrorSID int               = null -- required! id of row to delete - must be set in custom logic if not passed
...
as
/*********************************************************************************************************************************
Procedure : sf.pUnexpectedError#Delete
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : deletes 1 row in the sf.UnexpectedError table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.UnexpectedError table. The procedure requires a primary key value to locate the record
to delete.
...
and in the ext.pUnexpectedError procedure for client-specific deletion rules.
...
begin
	...
	begin try
		...
		if @RowStamp is null select @RowStamp = x.RowStamp from sf.UnexpectedError x where x.UnexpectedErrorSID = @UnexpectedErrorSID
    ...
    -- the deletion option based on the value returned from the IsDeleteEnabled function
		-- in the #Ext view.
    set @IsDeleteEnabled = sf.fUnexpectedError#IsDeleteEnabled(@UnexpectedErrorSID)
