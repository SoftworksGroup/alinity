...
GO
CREATE PROCEDURE [dbo].[pCredentialType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.CredentialType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.CredentialType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.CredentialType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fCredentialType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pCredentialType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.CredentialType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.CredentialType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.CredentialType'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.CredentialType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.CredentialType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_CredentialType'
		)
		begin
			exec sp_executesql N'alter table dbo.CredentialType drop constraint ck_CredentialType'				-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.CredentialType with CHECK add constraint ck_CredentialType
			check
				(
				dbo.fCredentialType#Check
					(
					 CredentialTypeSID
					,CredentialTypeLabel
			...
			select @RowCount = count(1) from dbo.CredentialType									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'CredentialType'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fCredentialType#Check
							(
							 x.CredentialTypeSID
							,x.CredentialTypeLabel
							,x.CredentialTypeCategory
							,x.IsDefault
							,x.IsActive
							,x.CredentialTypeXID
							,x.LegacyKey
			...
			alter table dbo.CredentialType with NOCHECK add constraint ck_CredentialType
			check
				(
				dbo.fCredentialType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.CredentialType'
		and
			br.BusinessRuleStatus <> 'x'
