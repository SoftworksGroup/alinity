...
GO
CREATE PROCEDURE [sf].[pConfigParam#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ConfigParam
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ConfigParam for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ConfigParam table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fConfigParam#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pConfigParam#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ConfigParam'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ConfigParam'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ConfigParam'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ConfigParam'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ConfigParam'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ConfigParam'
		)
		begin
			exec sp_executesql N'alter table sf.ConfigParam drop constraint ck_ConfigParam'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ConfigParam with CHECK add constraint ck_ConfigParam
			check
				(
				sf.fConfigParam#Check
					(
					 ConfigParamSID
					,ConfigParamSCD
					,ConfigParamName
			...
			select @RowCount = count(1) from sf.ConfigParam											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.ConfigParam with NOCHECK add constraint ck_ConfigParam
			check
				(
				sf.fConfigParam#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ConfigParam'
		and
			br.BusinessRuleStatus <> 'x'
