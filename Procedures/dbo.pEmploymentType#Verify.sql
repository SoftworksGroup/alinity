...
GO
CREATE PROCEDURE [dbo].[pEmploymentType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.EmploymentType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.EmploymentType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.EmploymentType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fEmploymentType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pEmploymentType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.EmploymentType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.EmploymentType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.EmploymentType'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.EmploymentType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.EmploymentType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_EmploymentType'
		)
		begin
			exec sp_executesql N'alter table dbo.EmploymentType drop constraint ck_EmploymentType'				-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.EmploymentType with CHECK add constraint ck_EmploymentType
			check
				(
				dbo.fEmploymentType#Check
					(
					 EmploymentTypeSID
					,EmploymentTypeName
					,EmploymentTypeCode
			...
			select @RowCount = count(1) from dbo.EmploymentType									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'EmploymentType'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fEmploymentType#Check
							(
							 x.EmploymentTypeSID
							,x.EmploymentTypeName
							,x.EmploymentTypeCode
							,x.EmploymentTypeCategory
							,x.IsDefault
							,x.IsActive
							,x.EmploymentTypeXID
							,x.LegacyKey
			...
			alter table dbo.EmploymentType with NOCHECK add constraint ck_EmploymentType
			check
				(
				dbo.fEmploymentType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.EmploymentType'
		and
			br.BusinessRuleStatus <> 'x'
