...
GO
CREATE PROCEDURE [dbo].[pRegistrationChange#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrationChange
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrationChange for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrationChange table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrationChange#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrationChange#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrationChange'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrationChange'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationChange'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationChange'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrationChange'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrationChange'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrationChange drop constraint ck_RegistrationChange'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrationChange with CHECK add constraint ck_RegistrationChange
			check
				(
			...
			select @RowCount = count(1) from dbo.RegistrationChange							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrationChange'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrationChange#Check
							(
							 x.RegistrationChangeSID
							,x.RegistrationSID
							...
							,x.RegistrationYear
							,x.NextFollowUp
							,x.RegistrationEffective
							,x.ReservedRegistrantNo
							,x.ReasonSID
							,x.InvoiceSID
							,x.ComplaintSID
							,x.RegistrationChangeXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrationChange  x
					where
						x.RegistrationChangeSID = @nextSID
			...
			alter table dbo.RegistrationChange with NOCHECK add constraint ck_RegistrationChange
			check
				(
				dbo.fRegistrationChange#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrationChange'
		and
			br.BusinessRuleStatus <> 'x'
