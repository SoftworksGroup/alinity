...
GO
CREATE PROCEDURE dbo.[pSetup$SF#Query$Payment]
	@SetupUser nvarchar(75) -- user assigned to audit columns
as
/*********************************************************************************************************************************
Sproc    : Setup Query for Payment management
Notice   : Copyright © 2018 Softworks Group Inc.
Summary  : Synchronizes the sf.Query table for product queries used in management of Payments
----------------------------------------------------------------------------------------------------------------------------------
...
Comments  
--------
This procedure sets up product queries used to manage Payments.  The data is setup in the sf.Query table. Product queries
...
exec sf.pUnitTest#Execute
	 @ObjectName = 'dbo.pSetup$SF#Query$Payment'
	,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
...
begin
	...
	begin try
		...
		where
			ap.ApplicationPageURI = 'PaymentList';
		...
		values
		(
			@defaultCategorySID, 'S!PMT.ALL', @parametersDoc, 'All', @OFF, N'Returns all payments for the selected date range'
		);
		...
		values
		(
			@defaultCategorySID, 'S!PMT.ALL.LATEST.WK', @parametersDoc, 'Latest week', @ON, N'Returns all payments for the latest week of payment activity'
		);
		...
		values
		(
			@defaultCategorySID, 'S!PMT.DEPOSIT', @parametersDoc, 'Deposit', @OFF, N'Returns payments for a selected range of deposit dates and bank account'
		);
		...
		values
		(
			@defaultCategorySID, 'S!PMT.GLPOSTING', @parametersDoc, 'GL Posting', @OFF, N'Returns payments for a selected range of GL posting dates'
		);
		...
		values
		(
			@defaultCategorySID, 'S!PMT.ALL.PAP', @parametersDoc, 'All pre-authorized', @OFF, N'Returns all pre-authorized payments in a date range'
		);
		...
		exec dbo.pQuery#SetParametersDoc
			@ParameterID = 'PaymentTypeSID'
		 ,@ParametersDoc = @parametersDoc output;

		exec dbo.pQuery#SetParametersDoc
			@ParameterID = 'PaymentStatusSID'
		 ,@Label = 'Status'
		 ,@IsMandatory = @OFF		 
		 ,@Cell = 2
		 ,@ParametersDoc = @parametersDoc output;
		...
		values
		(
			@defaultCategorySID, 'S!PMT.BY.TYPE', @parametersDoc, 'By type', @OFF, N'Returns payments of a selected type (and status) within date range'
		);
		...
		values
		(
			@formsCategorySID, 'S!PMT.RENEWAL', @parametersDoc, 'Renewal payments', @OFF
		 ,N'Returns payments applied against renewal invoices for the selected date range'
		);
		...
		values
		(
			@formsCategorySID, 'S!PMT.REINSTATEMENT', @parametersDoc, 'Reinstatement payments', @OFF
		 ,N'Returns payments applied against reinstatement invoices for the selected date range'
		);
		...
		values
		(
			@formsCategorySID, 'S!PMT.APPLICATION', @parametersDoc, 'Application payments', @OFF
		 ,N'Returns payments applied against application invoices for the selected date range'
		);
		...
		values
		(
			@formsCategorySID, 'S!PMT.REGCHG', @parametersDoc, 'Registration change payments', @OFF
		 ,N'Returns payments applied against registration change invoices for the selected date range'
		);
		...
		values
		(
			@defaultCategorySID, 'S!PMT.UNAPPLIED', @parametersDoc, 'Unapplied', @OFF, N'Returns payments where the total is not fully applied to invoices'
		);
		...
		values
		(
			@defaultCategorySID, 'S!PMT.PENDING', @parametersDoc, 'Pending', @OFF, N'Returns payments in a pending status (with and without processor response)'
		);
		...
		values
		(
			@defaultCategorySID, 'S!PMT.CANCELLED', @parametersDoc, 'Declined/Cancelled', @OFF, N'Returns payments with a status of declined or cancelled'
		);
		...
		values
		(
			@defaultCategorySID, 'S!PMT.FIND.BY.PHONE', @parametersDoc, 'Find by phone', @OFF
		...
		values
		(
			@defaultCategorySID, 'S!PMT.FIND.BY.ADDRESS', @parametersDoc, 'Find by address', @OFF
		...
		values
		(
			@defaultCategorySID, 'S!PMT.RECENTLY.UPDATED', @parametersDoc, 'Recently updated', @OFF
		...
		exec sf.pQuery#CheckCodes
			@ApplicationEntitySCD = 'dbo.Payment'
		 ,@SchemaAndRoutineName = 'dbo.pQuery#Execute$Payment'
