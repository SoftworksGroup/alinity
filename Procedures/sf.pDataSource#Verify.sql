...
GO
CREATE PROCEDURE [sf].[pDataSource#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.DataSource
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.DataSource for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.DataSource table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fDataSource#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pDataSource#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.DataSource'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.DataSource'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.DataSource'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.DataSource'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.DataSource'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_DataSource'
		)
		begin
			exec sp_executesql N'alter table sf.DataSource drop constraint ck_DataSource'									-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.DataSource with CHECK add constraint ck_DataSource
			check
				(
				sf.fDataSource#Check
					(
					 DataSourceSID
					,DataSourceLabel
					,DBObjectName
			...
			select @RowCount = count(1) from sf.DataSource											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.DataSource with NOCHECK add constraint ck_DataSource
			check
				(
				sf.fDataSource#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.DataSource'
		and
			br.BusinessRuleStatus <> 'x'
