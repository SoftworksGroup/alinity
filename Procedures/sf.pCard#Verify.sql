...
GO
CREATE PROCEDURE [sf].[pCard#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.Card
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.Card for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.Card table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fCard#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pCard#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.Card'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.Card'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.Card'															-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.Card'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.Card'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_Card'
		)
		begin
			exec sp_executesql N'alter table sf.Card drop constraint ck_Card'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.Card with CHECK add constraint ck_Card
			check
				(
				sf.fCard#Check
					(
			...
			select @RowCount = count(1) from sf.Card														-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.Card with NOCHECK add constraint ck_Card
			check
				(
				sf.fCard#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.Card'
		and
			br.BusinessRuleStatus <> 'x'
