...
GO
CREATE PROCEDURE [dbo].[pComplaintProcess#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ComplaintProcess
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ComplaintProcess for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ComplaintProcess table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fComplaintProcess#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pComplaintProcess#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ComplaintProcess'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ComplaintProcess'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ComplaintProcess'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ComplaintProcess'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ComplaintProcess'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ComplaintProcess'
		)
		begin
			exec sp_executesql N'alter table dbo.ComplaintProcess drop constraint ck_ComplaintProcess'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ComplaintProcess with CHECK add constraint ck_ComplaintProcess
			check
				(
				dbo.fComplaintProcess#Check
					(
					 ComplaintProcessSID
					,ComplaintProcessLabel
					,IsDefault
					,ParentComplaintProcessSID
					,ComplaintProcessXID
					,LegacyKey
					,IsDeleted
					,CreateUser
			...
			select @RowCount = count(1) from dbo.ComplaintProcess								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ComplaintProcess'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fComplaintProcess#Check
							(
							 x.ComplaintProcessSID
							,x.ComplaintProcessLabel
							,x.IsDefault
							,x.ParentComplaintProcessSID
							,x.ComplaintProcessXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
			...
			alter table dbo.ComplaintProcess with NOCHECK add constraint ck_ComplaintProcess
			check
				(
				dbo.fComplaintProcess#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ComplaintProcess'
		and
			br.BusinessRuleStatus <> 'x'
