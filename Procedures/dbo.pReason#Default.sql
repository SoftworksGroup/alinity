...
GO
CREATE PROCEDURE [dbo].[pReason#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pReason#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.Reason records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.Reason table. When a new record is to be added from the UI, this procedure
...
columns of the vReason entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pReason
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pReason of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@reasonSID          int               = -1														-- specific default required by EF - do not override
		,@reasonGroupSID     int               = null													-- no default provided from DB constraint - OK to override
		,@reasonName         nvarchar(50)      = null													-- no default provided from DB constraint - OK to override
		,@reasonCode         varchar(25)       = null													-- no default provided from DB constraint - OK to override
		,@reasonSequence     smallint          = (0)													-- default provided from DB constraint - OK to override
		...
		,@reasonXID          varchar(150)      = null													-- no default provided from DB constraint - OK to override
		...
		,@reasonGroupLabel   nvarchar(35)      = N'?'													-- not a base table column (default ignored)
		,@isLockedGroup      bit               = 0														-- not a base table column (default ignored)
		,@reasonGroupRowGUID uniqueidentifier  = newid()											-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@reasonName      = isnull(context.node.value('@ReasonName'    ,'nvarchar(50)' ),@reasonName)
			,@reasonCode      = isnull(context.node.value('@ReasonCode'    ,'varchar(25)'  ),@reasonCode)
			,@reasonSequence  = isnull(context.node.value('@ReasonSequence','smallint'     ),@reasonSequence)
			,@toolTip         = isnull(context.node.value('@ToolTip'       ,'nvarchar(500)'),@toolTip)
			,@isActive        = isnull(context.node.value('@IsActive'      ,'bit'          ),@isActive)
			,@reasonXID       = isnull(context.node.value('@ReasonXID'     ,'varchar(150)' ),@reasonXID)
