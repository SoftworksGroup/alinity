...
GO
CREATE PROCEDURE [dbo].[pContentTemplate#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ContentTemplate
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ContentTemplate for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ContentTemplate table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fContentTemplate#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pContentTemplate#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ContentTemplate'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ContentTemplate'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ContentTemplate'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ContentTemplate'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ContentTemplate'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ContentTemplate'
		)
		begin
			exec sp_executesql N'alter table dbo.ContentTemplate drop constraint ck_ContentTemplate'			-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ContentTemplate with CHECK add constraint ck_ContentTemplate
			check
				(
				dbo.fContentTemplate#Check
					(
					 ContentTemplateSID
					,ContentTemplateSCD
			...
			select @RowCount = count(1) from dbo.ContentTemplate								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ContentTemplate'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.ContentTemplateSID
				,x.RowGUID
			from
				dbo.ContentTemplate x
			order by
				x.ContentTemplateSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fContentTemplate#Check
							(
							 x.ContentTemplateSID
							,x.ContentTemplateSCD
							,x.ContentTemplateLabel
							,x.ContentTemplateXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
			...
			alter table dbo.ContentTemplate with NOCHECK add constraint ck_ContentTemplate
			check
				(
				dbo.fContentTemplate#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ContentTemplate'
		and
			br.BusinessRuleStatus <> 'x'
