...
GO
CREATE PROCEDURE [dbo].[pRegistrationChangeStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrationChangeStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrationChangeStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrationChangeStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrationChangeStatus#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrationChangeStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrationChangeStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrationChangeStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationChangeStatus'					-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationChangeStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrationChangeStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrationChangeStatus'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrationChangeStatus drop constraint ck_RegistrationChangeStatus'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrationChangeStatus with CHECK add constraint ck_RegistrationChangeStatus
			check
				(
				dbo.fRegistrationChangeStatus#Check
					(
					 RegistrationChangeStatusSID
					,RegistrationChangeSID
					,FormStatusSID
					,RegistrationChangeStatusXID
					,LegacyKey
					,IsDeleted
					,CreateUser
					,CreateTime
			...
			select @RowCount = count(1) from dbo.RegistrationChangeStatus				-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrationChangeStatus'
			...
			select
				...
				,x.RowGUID
			from
				dbo.RegistrationChangeStatus x
			order by
				x.RegistrationChangeStatusSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrationChangeStatus#Check
							(
							 x.RegistrationChangeStatusSID
							,x.RegistrationChangeSID
							,x.FormStatusSID
							,x.RegistrationChangeStatusXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrationChangeStatus  x
					where
						x.RegistrationChangeStatusSID = @nextSID
			...
			alter table dbo.RegistrationChangeStatus with NOCHECK add constraint ck_RegistrationChangeStatus
			check
				(
				dbo.fRegistrationChangeStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrationChangeStatus'
		and
			br.BusinessRuleStatus <> 'x'
