...
GO
CREATE PROCEDURE [sf].[pCard#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pCard#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.Card records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.Card table. When a new record is to be added from the UI, this procedure
...
columns of the vCard entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pCard
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pCard of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@cardSID               int               = -1												-- specific default required by EF - do not override
		,@cardTypeSID           int               = null											-- no default provided from DB constraint - OK to override
		,@cardLabel             nvarchar(35)      = null											-- no default provided from DB constraint - OK to override
		,@cardContext           varchar(25)       = 'DEFAULT'									-- default provided from DB constraint - OK to override
		,@cSSDefinition         varchar(150)      = 'col-xs-12 col-sm-6 col-md-6 col-lg-4'							-- default provided from DB constraint - OK to override
		,@summaryCardDefinition nvarchar(max)     = null											-- no default provided from DB constraint - OK to override
		,@detailCardDefinition  nvarchar(max)     = null											-- no default provided from DB constraint - OK to override
		,@headerDefinition      nvarchar(max)     = null											-- no default provided from DB constraint - OK to override
		,@footerDefinition      nvarchar(max)     = null											-- no default provided from DB constraint - OK to override
		,@gridRowDefinition     nvarchar(max)     = null											-- no default provided from DB constraint - OK to override
		,@isActive              bit               = (1)												-- default provided from DB constraint - OK to override
		,@cardHelp              nvarchar(max)     = null											-- no default provided from DB constraint - OK to override
		,@userDefinedColumns    xml               = null											-- no default provided from DB constraint - OK to override
		,@cardXID               varchar(150)      = null											-- no default provided from DB constraint - OK to override
		...
		,@cardTypeSCD           varchar(25)       = '?'												-- not a base table column (default ignored)
		,@cardTypeLabel         nvarchar(35)      = N'?'											-- not a base table column (default ignored)
		,@cardTypeRowGUID       uniqueidentifier  = newid()										-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @cardTypeSID            = isnull(context.node.value('@CardTypeSID'          ,'int'          ),@cardTypeSID)
			,@cardLabel              = isnull(context.node.value('@CardLabel'            ,'nvarchar(35)' ),@cardLabel)
			,@cardContext            = isnull(context.node.value('@CardContext'          ,'varchar(25)'  ),@cardContext)
			,@cSSDefinition          = isnull(context.node.value('@CSSDefinition'        ,'varchar(150)' ),@cSSDefinition)
			,@summaryCardDefinition  = isnull(context.node.value('@SummaryCardDefinition','nvarchar(max)'),@summaryCardDefinition)
			,@detailCardDefinition   = isnull(context.node.value('@DetailCardDefinition' ,'nvarchar(max)'),@detailCardDefinition)
			,@headerDefinition       = isnull(context.node.value('@HeaderDefinition'     ,'nvarchar(max)'),@headerDefinition)
			,@footerDefinition       = isnull(context.node.value('@FooterDefinition'     ,'nvarchar(max)'),@footerDefinition)
			,@gridRowDefinition      = isnull(context.node.value('@GridRowDefinition'    ,'nvarchar(max)'),@gridRowDefinition)
			,@isActive               = isnull(context.node.value('@IsActive'             ,'bit'          ),@isActive)
			,@cardHelp               = isnull(context.node.value('@CardHelp'             ,'nvarchar(max)'),@cardHelp)
			,@cardXID                = isnull(context.node.value('@CardXID'              ,'varchar(150)' ),@cardXID)
