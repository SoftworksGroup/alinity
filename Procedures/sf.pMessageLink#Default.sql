...
GO
CREATE PROCEDURE [sf].[pMessageLink#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pMessageLink#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.MessageLink records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.MessageLink table. When a new record is to be added from the UI, this procedure
...
columns of the vMessageLink entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pMessageLink
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pMessageLink of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@messageLinkSID         int               = -1												-- specific default required by EF - do not override
		,@messageLinkSCD         varchar(30)       = null											-- no default provided from DB constraint - OK to override
		,@messageLinkLabel       nvarchar(35)      = null											-- no default provided from DB constraint - OK to override
		...
		,@messageLinkXID         varchar(150)      = null											-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @messageLinkSCD      = isnull(context.node.value('@MessageLinkSCD'    ,'varchar(30)'  ),@messageLinkSCD)
			,@messageLinkLabel    = isnull(context.node.value('@MessageLinkLabel'  ,'nvarchar(35)' ),@messageLinkLabel)
			...
			,@messageLinkXID      = isnull(context.node.value('@MessageLinkXID'    ,'varchar(150)' ),@messageLinkXID)
