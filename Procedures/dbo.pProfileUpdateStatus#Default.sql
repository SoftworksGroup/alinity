...
GO
CREATE PROCEDURE [dbo].[pProfileUpdateStatus#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pProfileUpdateStatus#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.ProfileUpdateStatus records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.ProfileUpdateStatus table. When a new record is to be added from the UI, this procedure
...
columns of the vProfileUpdateStatus entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pProfileUpdateStatus
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pProfileUpdateStatus of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@profileUpdateStatusSID int               = -1												-- specific default required by EF - do not override
		...
		,@profileUpdateStatusXID varchar(150)      = null											-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			...
			,@profileUpdateStatusXID  = isnull(context.node.value('@ProfileUpdateStatusXID','varchar(150)'),@profileUpdateStatusXID)
