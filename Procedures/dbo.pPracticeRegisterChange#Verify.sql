...
GO
CREATE PROCEDURE [dbo].[pPracticeRegisterChange#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PracticeRegisterChange
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PracticeRegisterChange for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PracticeRegisterChange table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPracticeRegisterChange#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPracticeRegisterChange#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PracticeRegisterChange'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PracticeRegisterChange'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRegisterChange'						-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRegisterChange'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PracticeRegisterChange'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PracticeRegisterChange'
		)
		begin
			exec sp_executesql N'alter table dbo.PracticeRegisterChange drop constraint ck_PracticeRegisterChange'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PracticeRegisterChange with CHECK add constraint ck_PracticeRegisterChange
			check
				(
				dbo.fPracticeRegisterChange#Check
					(
			...
			select @RowCount = count(1) from dbo.PracticeRegisterChange					-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PracticeRegisterChange'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPracticeRegisterChange#Check
							(
							 x.PracticeRegisterChangeSID
							...
							,x.IsActive
							,x.ToolTip
							,x.IsEnabledForRegistrant
							,x.PracticeRegisterChangeXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.PracticeRegisterChange  x
					where
						x.PracticeRegisterChangeSID = @nextSID
			...
			alter table dbo.PracticeRegisterChange with NOCHECK add constraint ck_PracticeRegisterChange
			check
				(
				dbo.fPracticeRegisterChange#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PracticeRegisterChange'
		and
			br.BusinessRuleStatus <> 'x'
