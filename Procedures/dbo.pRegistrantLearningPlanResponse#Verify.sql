...
GO
CREATE PROCEDURE [dbo].[pRegistrantLearningPlanResponse#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantLearningPlanResponse
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantLearningPlanResponse for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantLearningPlanResponse table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantLearningPlanResponse#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantLearningPlanResponse#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantLearningPlanResponse'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantLearningPlanResponse'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantLearningPlanResponse'		-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantLearningPlanResponse'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantLearningPlanResponse'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantLearningPlanResponse'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantLearningPlanResponse drop constraint ck_RegistrantLearningPlanResponse'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantLearningPlanResponse with CHECK add constraint ck_RegistrantLearningPlanResponse
			check
				(
				dbo.fRegistrantLearningPlanResponse#Check
					(
					 RegistrantLearningPlanResponseSID
					,RegistrantLearningPlanSID
					,FormOwnerSID
					,RegistrantLearningPlanResponseXID
					,LegacyKey
					,IsDeleted
					,CreateUser
					,CreateTime
			...
			select @RowCount = count(1) from dbo.RegistrantLearningPlanResponse	-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantLearningPlanResponse'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrantLearningPlanResponseSID
				,x.RowGUID
			from
				dbo.RegistrantLearningPlanResponse x
			order by
				x.RegistrantLearningPlanResponseSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantLearningPlanResponse#Check
							(
							 x.RegistrantLearningPlanResponseSID
							,x.RegistrantLearningPlanSID
							,x.FormOwnerSID
							,x.RegistrantLearningPlanResponseXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantLearningPlanResponse  x
					where
						x.RegistrantLearningPlanResponseSID = @nextSID
			...
			alter table dbo.RegistrantLearningPlanResponse with NOCHECK add constraint ck_RegistrantLearningPlanResponse
			check
				(
				dbo.fRegistrantLearningPlanResponse#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantLearningPlanResponse'
		and
			br.BusinessRuleStatus <> 'x'
