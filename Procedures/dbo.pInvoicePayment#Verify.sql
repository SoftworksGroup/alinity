...
GO
CREATE PROCEDURE [dbo].[pInvoicePayment#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.InvoicePayment
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.InvoicePayment for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.InvoicePayment table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fInvoicePayment#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pInvoicePayment#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.InvoicePayment'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.InvoicePayment'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.InvoicePayment'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.InvoicePayment'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.InvoicePayment'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_InvoicePayment'
		)
		begin
			exec sp_executesql N'alter table dbo.InvoicePayment drop constraint ck_InvoicePayment'				-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.InvoicePayment with CHECK add constraint ck_InvoicePayment
			check
				(
			...
			select @RowCount = count(1) from dbo.InvoicePayment									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'InvoicePayment'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fInvoicePayment#Check
							(
							 x.InvoicePaymentSID
							,x.InvoiceSID
							,x.PaymentSID
			...
			alter table dbo.InvoicePayment with NOCHECK add constraint ck_InvoicePayment
			check
				(
				dbo.fInvoicePayment#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.InvoicePayment'
		and
			br.BusinessRuleStatus <> 'x'
