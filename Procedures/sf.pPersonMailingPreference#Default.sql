...
GO
CREATE PROCEDURE [sf].[pPersonMailingPreference#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pPersonMailingPreference#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.PersonMailingPreference records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.PersonMailingPreference table. When a new record is to be added from the UI, this procedure
...
columns of the vPersonMailingPreference entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pPersonMailingPreference
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pPersonMailingPreference of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@personMailingPreferenceSID int               = -1										-- specific default required by EF - do not override
		,@personSID                  int               = null									-- no default provided from DB constraint - OK to override
		,@mailingPreferenceSID       int               = null									-- no default provided from DB constraint - OK to override
		...
		,@personMailingPreferenceXID varchar(150)      = null									-- no default provided from DB constraint - OK to override
		...
		,@mailingPreferenceLabel     nvarchar(35)      = N'?'									-- not a base table column (default ignored)
		,@isAutoOptIn                bit               = 0										-- not a base table column (default ignored)
		,@mailingPreferenceIsActive  bit               = 0										-- not a base table column (default ignored)
		,@mailingPreferenceRowGUID   uniqueidentifier  = newid()							-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@mailingPreferenceSID        = isnull(context.node.value('@MailingPreferenceSID'      ,'int'          ),@mailingPreferenceSID)
			...
			,@personMailingPreferenceXID  = isnull(context.node.value('@PersonMailingPreferenceXID','varchar(150)' ),@personMailingPreferenceXID)
