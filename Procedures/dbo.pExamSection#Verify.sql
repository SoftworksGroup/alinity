...
GO
CREATE PROCEDURE [dbo].[pExamSection#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ExamSection
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ExamSection for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ExamSection table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fExamSection#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pExamSection#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ExamSection'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ExamSection'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ExamSection'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ExamSection'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ExamSection'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ExamSection'
		)
		begin
			exec sp_executesql N'alter table dbo.ExamSection drop constraint ck_ExamSection'							-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ExamSection with CHECK add constraint ck_ExamSection
			check
				(
				dbo.fExamSection#Check
					(
					 ExamSectionSID
					,ExamSID
					,Sequence
			...
			select @RowCount = count(1) from dbo.ExamSection										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fExamSection#Check
							(
							 x.ExamSectionSID
							,x.ExamSID
							,x.Sequence
							,x.SectionTitle
							,x.RandomQuestionCount
							,x.WeightPerQuestion
							,x.MinimumCorrect
							,x.ExamSectionXID
							,x.LegacyKey
			...
			alter table dbo.ExamSection with NOCHECK add constraint ck_ExamSection
			check
				(
				dbo.fExamSection#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ExamSection'
		and
			br.BusinessRuleStatus <> 'x'
