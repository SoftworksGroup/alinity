...
GO
CREATE PROCEDURE [dbo].[pRegistrationScheduleYear#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrationScheduleYear
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrationScheduleYear for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrationScheduleYear table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrationScheduleYear#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrationScheduleYear#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrationScheduleYear'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrationScheduleYear'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationScheduleYear'					-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationScheduleYear'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrationScheduleYear'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrationScheduleYear'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrationScheduleYear drop constraint ck_RegistrationScheduleYear'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrationScheduleYear with CHECK add constraint ck_RegistrationScheduleYear
			check
				(
			...
			select @RowCount = count(1) from dbo.RegistrationScheduleYear				-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrationScheduleYear'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrationScheduleYearSID
				,x.RowGUID
			from
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrationScheduleYear#Check
							(
							 x.RegistrationScheduleYearSID
							...
							,x.YearStartTime
							,x.YearEndTime
							,x.RenewalVerificationOpenTime
							,x.RenewalGeneralOpenTime
							,x.RenewalLateFeeStartTime
							,x.RenewalEndTime
							,x.ReinstatementVerificationOpenTime
							,x.ReinstatementGeneralOpenTime
							,x.ReinstatementEndTime
							,x.CECollectionStartTime
							,x.CECollectionEndTime
							,x.PAPBlockStartTime
							,x.PAPBlockEndTime
							,x.RegistrationScheduleYearXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrationScheduleYear  x
					where
						x.RegistrationScheduleYearSID = @nextSID
			...
			alter table dbo.RegistrationScheduleYear with NOCHECK add constraint ck_RegistrationScheduleYear
			check
				(
				dbo.fRegistrationScheduleYear#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrationScheduleYear'
		and
			br.BusinessRuleStatus <> 'x'
