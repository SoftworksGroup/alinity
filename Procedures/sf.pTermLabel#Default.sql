...
GO
CREATE PROCEDURE [sf].[pTermLabel#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pTermLabel#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.TermLabel records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.TermLabel table. When a new record is to be added from the UI, this procedure
...
columns of the vTermLabel entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pTermLabel
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pTermLabel of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@termLabelSID           int               = -1												-- specific default required by EF - do not override
		,@termLabelSCD           varchar(35)       = null											-- no default provided from DB constraint - OK to override
		,@termLabel              nvarchar(35)      = null											-- no default provided from DB constraint - OK to override
		,@usageNotes             nvarchar(max)     = null											-- no default provided from DB constraint - OK to override
		,@termLabelUpdateTime    datetimeoffset(7) = null											-- no default provided from DB constraint - OK to override
		,@defaultLabel           nvarchar(35)      = null											-- no default provided from DB constraint - OK to override
		,@defaultLabelUpdateTime datetimeoffset(7) = sysdatetimeoffset()			-- default provided from DB constraint - OK to override
		,@userDefinedColumns     xml               = null											-- no default provided from DB constraint - OK to override
		,@termLabelXID           varchar(150)      = null											-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @termLabelSCD            = isnull(context.node.value('@TermLabelSCD'          ,'varchar(35)'      ),@termLabelSCD)
			,@termLabel               = isnull(context.node.value('@TermLabel'             ,'nvarchar(35)'     ),@termLabel)
			,@usageNotes              = isnull(context.node.value('@UsageNotes'            ,'nvarchar(max)'    ),@usageNotes)
			,@termLabelUpdateTime     = isnull(context.node.value('@TermLabelUpdateTime'   ,'datetimeoffset(7)'),@termLabelUpdateTime)
			,@defaultLabel            = isnull(context.node.value('@DefaultLabel'          ,'nvarchar(35)'     ),@defaultLabel)
			,@defaultLabelUpdateTime  = isnull(context.node.value('@DefaultLabelUpdateTime','datetimeoffset(7)'),@defaultLabelUpdateTime)
			,@termLabelXID            = isnull(context.node.value('@TermLabelXID'          ,'varchar(150)'     ),@termLabelXID)
