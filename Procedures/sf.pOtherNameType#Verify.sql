...
GO
CREATE PROCEDURE [sf].[pOtherNameType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.OtherNameType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.OtherNameType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.OtherNameType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fOtherNameType#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pOtherNameType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.OtherNameType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.OtherNameType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.OtherNameType'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.OtherNameType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.OtherNameType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_OtherNameType'
		)
		begin
			exec sp_executesql N'alter table sf.OtherNameType drop constraint ck_OtherNameType'						-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.OtherNameType with CHECK add constraint ck_OtherNameType
			check
				(
				sf.fOtherNameType#Check
					(
					 OtherNameTypeSID
					,OtherNameTypeLabel
					,IsDefault
			...
			select @RowCount = count(1) from sf.OtherNameType										-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'OtherNameType'
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fOtherNameType#Check
							(
							 x.OtherNameTypeSID
							,x.OtherNameTypeLabel
							,x.IsDefault
							,x.IsActive
							,x.OtherNameTypeXID
							,x.LegacyKey
			...
			alter table sf.OtherNameType with NOCHECK add constraint ck_OtherNameType
			check
				(
				sf.fOtherNameType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.OtherNameType'
		and
			br.BusinessRuleStatus <> 'x'
