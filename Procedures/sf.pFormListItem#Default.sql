...
GO
CREATE PROCEDURE [sf].[pFormListItem#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pFormListItem#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.FormListItem records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.FormListItem table. When a new record is to be added from the UI, this procedure
...
columns of the vFormListItem entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pFormListItem
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pFormListItem of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@formListItemSID      int               = -1													-- specific default required by EF - do not override
		,@formListSID          int               = null												-- no default provided from DB constraint - OK to override
		,@formListItemCode     varchar(15)       = null												-- no default provided from DB constraint - OK to override
		,@formListItemLabel    nvarchar(35)      = null												-- no default provided from DB constraint - OK to override
		,@formListItemSequence smallint          = (0)												-- default provided from DB constraint - OK to override
		...
		,@formListItemXID      varchar(150)      = null												-- no default provided from DB constraint - OK to override
		...
		,@formListCode         varchar(15)       = '?'												-- not a base table column (default ignored)
		,@formListLabel        nvarchar(35)      = N'?'												-- not a base table column (default ignored)
		,@formListToolTip      nvarchar(500)																	-- not a base table column (default ignored)
		,@formListRowGUID      uniqueidentifier  = newid()										-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @formListSID           = isnull(context.node.value('@FormListSID'         ,'int'          ),@formListSID)
			,@formListItemCode      = isnull(context.node.value('@FormListItemCode'    ,'varchar(15)'  ),@formListItemCode)
			,@formListItemLabel     = isnull(context.node.value('@FormListItemLabel'   ,'nvarchar(35)' ),@formListItemLabel)
			,@formListItemSequence  = isnull(context.node.value('@FormListItemSequence','smallint'     ),@formListItemSequence)
			,@toolTip               = isnull(context.node.value('@ToolTip'             ,'nvarchar(500)'),@toolTip)
			,@isActive              = isnull(context.node.value('@IsActive'            ,'bit'          ),@isActive)
			,@formListItemXID       = isnull(context.node.value('@FormListItemXID'     ,'varchar(150)' ),@formListItemXID)
