...
GO
CREATE PROCEDURE [dbo].[pRegistrantExam#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantExam
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantExam for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantExam table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantExam#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantExam#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantExam'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantExam'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantExam'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantExam'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantExam'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantExam'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantExam drop constraint ck_RegistrantExam'				-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantExam with CHECK add constraint ck_RegistrantExam
			check
				(
			...
			select @RowCount = count(1) from dbo.RegistrantExam									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantExam'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantExam#Check
							(
							 x.RegistrantExamSID
							,x.RegistrantSID
							,x.ExamSID
			...
			alter table dbo.RegistrantExam with NOCHECK add constraint ck_RegistrantExam
			check
				(
				dbo.fRegistrantExam#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantExam'
		and
			br.BusinessRuleStatus <> 'x'
