...
GO
CREATE PROCEDURE [sf].[pExportFile#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pExportFile#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.ExportFile records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.ExportFile table. When a new record is to be added from the UI, this procedure
...
columns of the vExportFile entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pExportFile
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pExportFile of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@exportFileSID       int               = -1													-- specific default required by EF - do not override
		,@exportSourceGUID    uniqueidentifier  = null												-- no default provided from DB constraint - OK to override
		...
		,@exportSpecification xml               = null												-- no default provided from DB constraint - OK to override
		...
		,@exportFileXID       varchar(150)      = null												-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @exportSourceGUID  = isnull(context.node.value('@ExportSourceGUID','uniqueidentifier' ),@exportSourceGUID)
			...
			,@exportFileXID     = isnull(context.node.value('@ExportFileXID'   ,'varchar(150)'     ),@exportFileXID)
