...
GO
CREATE PROCEDURE [sf].[pPersonEmailAddress#Delete]
	 @PersonEmailAddressSID  int               = null -- required! id of row to delete - must be set in custom logic if not passed
	...
	,@IsEmailUsedForLogin    bit               = null
as
/*********************************************************************************************************************************
Procedure : sf.pPersonEmailAddress#Delete
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : deletes 1 row in the sf.PersonEmailAddress table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.PersonEmailAddress table. The procedure requires a primary key value to locate the record
to delete.
...
and in the ext.pPersonEmailAddress procedure for client-specific deletion rules.
...
begin
	...
	begin try
		...
		if @RowStamp is null select @RowStamp = x.RowStamp from sf.PersonEmailAddress x where x.PersonEmailAddressSID = @PersonEmailAddressSID
    ...
    -- the deletion option based on the value returned from the IsDeleteEnabled function		-- in the #Ext view.
    set @IsDeleteEnabled = sf.fPersonEmailAddress#IsDeleteEnabled(@PersonEmailAddressSID)

    if isnull(@IsDeleteEnabled, cast(0 as bit)) = cast(0 as bit)begin
      ...
      raiserror(@errorText, 16, 1) end    -- Tim Edlund | December 2012
		-- If the row stamp is not provided by the caller, look it up here
		-- (TODO - needs to go into generator for #Delete procedures)
