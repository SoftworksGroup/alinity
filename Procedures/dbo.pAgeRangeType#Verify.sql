...
GO
CREATE PROCEDURE [dbo].[pAgeRangeType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.AgeRangeType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.AgeRangeType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.AgeRangeType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fAgeRangeType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pAgeRangeType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.AgeRangeType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.AgeRangeType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.AgeRangeType'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.AgeRangeType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.AgeRangeType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_AgeRangeType'
		)
		begin
			exec sp_executesql N'alter table dbo.AgeRangeType drop constraint ck_AgeRangeType'						-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.AgeRangeType with CHECK add constraint ck_AgeRangeType
			check
				(
				dbo.fAgeRangeType#Check
					(
					 AgeRangeTypeSID
					,AgeRangeTypeLabel
					,AgeRangeTypeCode
			...
			select @RowCount = count(1) from dbo.AgeRangeType										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fAgeRangeType#Check
							(
							 x.AgeRangeTypeSID
							,x.AgeRangeTypeLabel
							,x.AgeRangeTypeCode
			...
			alter table dbo.AgeRangeType with NOCHECK add constraint ck_AgeRangeType
			check
				(
				dbo.fAgeRangeType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.AgeRangeType'
		and
			br.BusinessRuleStatus <> 'x'
