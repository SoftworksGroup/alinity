...
GO
CREATE PROCEDURE [sf].[pAuditAction#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pAuditAction#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.AuditAction records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.AuditAction table. When a new record is to be added from the UI, this procedure
...
columns of the vAuditAction entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pAuditAction
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pAuditAction of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@auditActionSID     int               = -1														-- specific default required by EF - do not override
		,@auditActionSCD     varchar(25)       = null													-- no default provided from DB constraint - OK to override
		,@auditActionName    nvarchar(50)      = null													-- no default provided from DB constraint - OK to override
		...
		,@auditActionXID     varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @auditActionSCD   = isnull(context.node.value('@AuditActionSCD' ,'varchar(25)'  ),@auditActionSCD)
			,@auditActionName  = isnull(context.node.value('@AuditActionName','nvarchar(50)' ),@auditActionName)
			...
			,@auditActionXID   = isnull(context.node.value('@AuditActionXID' ,'varchar(150)' ),@auditActionXID)
