...
GO
CREATE PROCEDURE [sf].[pApplicationGrant#Delete]
	 @ApplicationGrantSID  int               = null -- required! id of row to delete - must be set in custom logic if not passed
	...
	,@ModuleSCD            varchar(15)       = null
	,@ModuleName           nvarchar(100)     = null
	,@TotalLicenses        int               = null
	,@RemainingLicenses    int               = null
as
/*********************************************************************************************************************************
Procedure : sf.pApplicationGrant#Delete
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : deletes 1 row in the sf.ApplicationGrant table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.ApplicationGrant table. The procedure requires a primary key value to locate the record
to delete.
...
and in the ext.pApplicationGrant procedure for client-specific deletion rules.
...
begin
	...
	begin try
		...
		if @RowStamp is null select @RowStamp = x.RowStamp from sf.ApplicationGrant x where x.ApplicationGrantSID = @ApplicationGrantSID
    ...
    -- the deletion option based on the value returned from the IsDeleteEnabled function
		-- in the #Ext view.
 set @IsDeleteEnabled = sf.fApplicationGrant#IsDeleteEnabled(@ApplicationGrantSID)
    ...
    -- Ji Zhou | Feb 2013
		...
		if exists (select 1 from sf.ApplicationGrant x where x.ApplicationGrantSID = @ApplicationGrantSID and x.IsDefault = cast(1 as bit))
		begin
