...
GO
CREATE PROCEDURE [dbo].[pRegistrantRenewalResponse#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantRenewalResponse
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantRenewalResponse for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantRenewalResponse table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantRenewalResponse#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantRenewalResponse#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantRenewalResponse'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantRenewalResponse'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantRenewalResponse'				-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantRenewalResponse'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantRenewalResponse'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantRenewalResponse'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantRenewalResponse drop constraint ck_RegistrantRenewalResponse'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantRenewalResponse with CHECK add constraint ck_RegistrantRenewalResponse
			check
				(
				dbo.fRegistrantRenewalResponse#Check
					(
					 RegistrantRenewalResponseSID
					,RegistrantRenewalSID
					,FormOwnerSID
					,RegistrantRenewalResponseXID
					,LegacyKey
					,IsDeleted
					,CreateUser
					,CreateTime
			...
			select @RowCount = count(1) from dbo.RegistrantRenewalResponse			-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantRenewalResponse'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrantRenewalResponseSID
				,x.RowGUID
			from
				dbo.RegistrantRenewalResponse x
			order by
				x.RegistrantRenewalResponseSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantRenewalResponse#Check
							(
							 x.RegistrantRenewalResponseSID
							,x.RegistrantRenewalSID
							,x.FormOwnerSID
							,x.RegistrantRenewalResponseXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantRenewalResponse  x
					where
						x.RegistrantRenewalResponseSID = @nextSID
			...
			alter table dbo.RegistrantRenewalResponse with NOCHECK add constraint ck_RegistrantRenewalResponse
			check
				(
				dbo.fRegistrantRenewalResponse#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantRenewalResponse'
		and
			br.BusinessRuleStatus <> 'x'
