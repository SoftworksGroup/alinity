...
GO
CREATE PROCEDURE [sf].[pFormVersion#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.FormVersion
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.FormVersion for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.FormVersion table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fFormVersion#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pFormVersion#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.FormVersion'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.FormVersion'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.FormVersion'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.FormVersion'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.FormVersion'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_FormVersion'
		)
		begin
			exec sp_executesql N'alter table sf.FormVersion drop constraint ck_FormVersion'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.FormVersion with CHECK add constraint ck_FormVersion
			check
				(
				sf.fFormVersion#Check
					(
					 FormVersionSID
					,FormSID
					,VersionNo
			...
			select @RowCount = count(1) from sf.FormVersion											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.FormVersion with NOCHECK add constraint ck_FormVersion
			check
				(
				sf.fFormVersion#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.FormVersion'
		and
			br.BusinessRuleStatus <> 'x'
