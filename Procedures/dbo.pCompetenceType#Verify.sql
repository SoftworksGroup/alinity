...
GO
CREATE PROCEDURE [dbo].[pCompetenceType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.CompetenceType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.CompetenceType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.CompetenceType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fCompetenceType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pCompetenceType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.CompetenceType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.CompetenceType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.CompetenceType'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.CompetenceType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.CompetenceType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_CompetenceType'
		)
		begin
			exec sp_executesql N'alter table dbo.CompetenceType drop constraint ck_CompetenceType'				-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.CompetenceType with CHECK add constraint ck_CompetenceType
			check
				(
				dbo.fCompetenceType#Check
					(
					 CompetenceTypeSID
					,CompetenceTypeLabel
			...
			select @RowCount = count(1) from dbo.CompetenceType									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'CompetenceType'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fCompetenceType#Check
							(
							 x.CompetenceTypeSID
							,x.CompetenceTypeLabel
							,x.CompetenceTypeCategory
							,x.IsDefault
							,x.IsActive
							,x.CompetenceTypeXID
							,x.LegacyKey
			...
			alter table dbo.CompetenceType with NOCHECK add constraint ck_CompetenceType
			check
				(
				dbo.fCompetenceType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.CompetenceType'
		and
			br.BusinessRuleStatus <> 'x'
