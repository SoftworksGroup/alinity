...
GO
CREATE PROCEDURE [sf].[pApplicationUserSession#Delete]
	 @ApplicationUserSessionSID        int               = null -- required! id of row to delete - must be set in custom logic if not passed
...
as
/*********************************************************************************************************************************
Procedure : sf.pApplicationUserSession#Delete
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : deletes 1 row in the sf.ApplicationUserSession table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.ApplicationUserSession table. The procedure requires a primary key value to locate the record
to delete.
...
and in the ext.pApplicationUserSession procedure for client-specific deletion rules.
...
begin
	...
	begin try
		...
		if @RowStamp is null select @RowStamp = x.RowStamp from sf.ApplicationUserSession x where x.ApplicationUserSessionSID = @ApplicationUserSessionSID
    ...
    set @IsDeleteEnabled = sf.fApplicationUserSession#IsDeleteEnabled(@ApplicationUserSessionSID)if isnull(@IsDeleteEnabled, cast(0 as bit)) = cast(0 as bit)
    begin
    ...
    --! </PreDelete>
		...
		update																																-- set audit details on sf.ApplicationUserSessionProperty rows that will delete through CASCADE
