...
GO
CREATE PROCEDURE [dbo].[pLanguage#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pLanguage#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.Language records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.Language table. When a new record is to be added from the UI, this procedure
...
columns of the vLanguage entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pLanguage
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pLanguage of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@languageSID        int               = -1														-- specific default required by EF - do not override
		,@languageLabel      nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		...
		,@languageXID        varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @languageLabel  = isnull(context.node.value('@LanguageLabel','nvarchar(35)'),@languageLabel)
			,@isDefault      = isnull(context.node.value('@IsDefault'    ,'bit'         ),@isDefault)
			,@isActive       = isnull(context.node.value('@IsActive'     ,'bit'         ),@isActive)
			,@languageXID    = isnull(context.node.value('@LanguageXID'  ,'varchar(150)'),@languageXID)
