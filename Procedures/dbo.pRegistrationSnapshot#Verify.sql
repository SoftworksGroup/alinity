...
GO
CREATE PROCEDURE [dbo].[pRegistrationSnapshot#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrationSnapshot
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrationSnapshot for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrationSnapshot table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrationSnapshot#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrationSnapshot#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrationSnapshot'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrationSnapshot'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationSnapshot'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationSnapshot'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrationSnapshot'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrationSnapshot'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrationSnapshot drop constraint ck_RegistrationSnapshot'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrationSnapshot with CHECK add constraint ck_RegistrationSnapshot
			check
				(
			...
			select @RowCount = count(1) from dbo.RegistrationSnapshot						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrationSnapshot'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrationSnapshot#Check
							(
							 x.RegistrationSnapshotSID
							,x.RegistrationSnapshotTypeSID
							,x.RegistrationSnapshotLabel
							,x.RegistrationYear
							,x.QueuedTime
							,x.LockedTime
							,x.LastCodeUpdateTime
							,x.LastVerifiedTime
							,x.JobRunSID
							,x.RegistrationSnapshotXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrationSnapshot  x
					where
						x.RegistrationSnapshotSID = @nextSID
			...
			alter table dbo.RegistrationSnapshot with NOCHECK add constraint ck_RegistrationSnapshot
			check
				(
				dbo.fRegistrationSnapshot#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrationSnapshot'
		and
			br.BusinessRuleStatus <> 'x'
