...
GO
CREATE PROCEDURE [sf].[pFormVersion#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pFormVersion#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.FormVersion records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.FormVersion table. When a new record is to be added from the UI, this procedure
...
columns of the vFormVersion entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pFormVersion
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pFormVersion of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@formVersionSID     int               = -1														-- specific default required by EF - do not override
		,@formSID            int               = null													-- no default provided from DB constraint - OK to override
		,@versionNo          smallint          = (0)													-- default provided from DB constraint - OK to override
		,@revisionNo         smallint          = (1001)												-- default provided from DB constraint - OK to override
		,@formDefinition     xml               = null													-- no default provided from DB constraint - OK to override
		,@isSaveDisplayed    bit               = CONVERT(bit,(1))							-- default provided from DB constraint - OK to override
		,@approvedTime       datetimeoffset(7) = null													-- no default provided from DB constraint - OK to override
		,@changeNotes        nvarchar(max)     = null													-- no default provided from DB constraint - OK to override
		,@userDefinedColumns xml               = null													-- no default provided from DB constraint - OK to override
		,@formVersionXID     varchar(150)      = null													-- no default provided from DB constraint - OK to override
		...
		,@isApproved         bit																							-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@versionNo        = isnull(context.node.value('@VersionNo'      ,'smallint'         ),@versionNo)
			,@revisionNo       = isnull(context.node.value('@RevisionNo'     ,'smallint'         ),@revisionNo)
			,@isSaveDisplayed  = isnull(context.node.value('@IsSaveDisplayed','bit'              ),@isSaveDisplayed)
			,@approvedTime     = isnull(context.node.value('@ApprovedTime'   ,'datetimeoffset(7)'),@approvedTime)
			,@changeNotes      = isnull(context.node.value('@ChangeNotes'    ,'nvarchar(max)'    ),@changeNotes)
			,@formVersionXID   = isnull(context.node.value('@FormVersionXID' ,'varchar(150)'     ),@formVersionXID)
