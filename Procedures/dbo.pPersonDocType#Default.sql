...
GO
CREATE PROCEDURE [dbo].[pPersonDocType#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPersonDocType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.PersonDocType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PersonDocType table. When a new record is to be added from the UI, this procedure
...
columns of the vPersonDocType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pPersonDocType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pPersonDocType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@personDocTypeSID      int               = -1												-- specific default required by EF - do not override
		,@personDocTypeSCD      varchar(15)       = null											-- no default provided from DB constraint - OK to override
		,@personDocTypeLabel    nvarchar(35)      = null											-- no default provided from DB constraint - OK to override
		,@personDocTypeCategory nvarchar(65)      = null											-- no default provided from DB constraint - OK to override
		...
		,@personDocTypeXID      varchar(150)      = null											-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @personDocTypeSCD       = isnull(context.node.value('@PersonDocTypeSCD'     ,'varchar(15)' ),@personDocTypeSCD)
			,@personDocTypeLabel     = isnull(context.node.value('@PersonDocTypeLabel'   ,'nvarchar(35)'),@personDocTypeLabel)
			,@personDocTypeCategory  = isnull(context.node.value('@PersonDocTypeCategory','nvarchar(65)'),@personDocTypeCategory)
			,@isDefault              = isnull(context.node.value('@IsDefault'            ,'bit'         ),@isDefault)
			,@isActive               = isnull(context.node.value('@IsActive'             ,'bit'         ),@isActive)
			,@personDocTypeXID       = isnull(context.node.value('@PersonDocTypeXID'     ,'varchar(150)'),@personDocTypeXID)
