...
GO
CREATE PROCEDURE [sf].[pUnitTest#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pUnitTest#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.UnitTest records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.UnitTest table. When a new record is to be added from the UI, this procedure
...
columns of the vUnitTest entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pUnitTest
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pUnitTest of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@unitTestSID        int               = -1														-- specific default required by EF - do not override
		,@schemaName         nvarchar(128)     = N'dbo'												-- default provided from DB constraint - OK to override
		,@objectName         nvarchar(128)     = null													-- no default provided from DB constraint - OK to override
		,@testName           nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		,@usageNotes         nvarchar(max)     = null													-- no default provided from DB constraint - OK to override
		,@sQLScript          nvarchar(max)     = null													-- no default provided from DB constraint - OK to override
		,@objectType         varchar(10)       = null													-- no default provided from DB constraint - OK to override
		...
		,@unitTestXID        varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @schemaName   = isnull(context.node.value('@SchemaName' ,'nvarchar(128)'),@schemaName)
			,@objectName   = isnull(context.node.value('@ObjectName' ,'nvarchar(128)'),@objectName)
			,@testName     = isnull(context.node.value('@TestName'   ,'nvarchar(35)' ),@testName)
			,@usageNotes   = isnull(context.node.value('@UsageNotes' ,'nvarchar(max)'),@usageNotes)
			,@sQLScript    = isnull(context.node.value('@SQLScript'  ,'nvarchar(max)'),@sQLScript)
			,@objectType   = isnull(context.node.value('@ObjectType' ,'varchar(10)'  ),@objectType)
			,@isDefault    = isnull(context.node.value('@IsDefault'  ,'bit'          ),@isDefault)
			,@unitTestXID  = isnull(context.node.value('@UnitTestXID','varchar(150)' ),@unitTestXID)
