...
GO
CREATE PROCEDURE [sf].[pApplicationPageHelp#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pApplicationPageHelp#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.ApplicationPageHelp records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.ApplicationPageHelp table. When a new record is to be added from the UI, this procedure
...
columns of the vApplicationPageHelp entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pApplicationPageHelp
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pApplicationPageHelp of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@applicationPageHelpSID int               = -1												-- specific default required by EF - do not override
		,@applicationPageSID     int               = null											-- no default provided from DB constraint - OK to override
		,@applicationPageHelpID  varchar(100)      = null											-- no default provided from DB constraint - OK to override
		,@helpContent            nvarchar(max)     = null											-- no default provided from DB constraint - OK to override
		,@stepSequence           int               = (0)											-- default provided from DB constraint - OK to override
		,@userDefinedColumns     xml               = null											-- no default provided from DB constraint - OK to override
		,@applicationPageHelpXID varchar(150)      = null											-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			...
			,@applicationPageHelpID   = isnull(context.node.value('@ApplicationPageHelpID' ,'varchar(100)' ),@applicationPageHelpID)
			,@helpContent             = isnull(context.node.value('@HelpContent'           ,'nvarchar(max)'),@helpContent)
			,@stepSequence            = isnull(context.node.value('@StepSequence'          ,'int'          ),@stepSequence)
			,@applicationPageHelpXID  = isnull(context.node.value('@ApplicationPageHelpXID','varchar(150)' ),@applicationPageHelpXID)
