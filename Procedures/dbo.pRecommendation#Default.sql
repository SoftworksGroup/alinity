...
GO
CREATE PROCEDURE [dbo].[pRecommendation#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pRecommendation#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.Recommendation records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.Recommendation table. When a new record is to be added from the UI, this procedure
...
columns of the vRecommendation entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pRecommendation
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pRecommendation of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@recommendationSID          int               = -1										-- specific default required by EF - do not override
		,@recommendationGroupSID     int               = null									-- no default provided from DB constraint - OK to override
		,@buttonLabel                nvarchar(20)      = null									-- no default provided from DB constraint - OK to override
		,@recommendationSequence     smallint          = (0)									-- default provided from DB constraint - OK to override
		...
		,@recommendationXID          varchar(150)      = null									-- no default provided from DB constraint - OK to override
		...
		,@recommendationGroupSCD     varchar(15)       = '?'									-- not a base table column (default ignored)
		,@recommendationGroupLabel   nvarchar(35)      = N'?'									-- not a base table column (default ignored)
		,@recommendationGroupRowGUID uniqueidentifier  = newid()							-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @recommendationGroupSID  = isnull(context.node.value('@RecommendationGroupSID','int'          ),@recommendationGroupSID)
			,@buttonLabel             = isnull(context.node.value('@ButtonLabel'           ,'nvarchar(20)' ),@buttonLabel)
			,@recommendationSequence  = isnull(context.node.value('@RecommendationSequence','smallint'     ),@recommendationSequence)
			,@toolTip                 = isnull(context.node.value('@ToolTip'               ,'nvarchar(500)'),@toolTip)
			,@isActive                = isnull(context.node.value('@IsActive'              ,'bit'          ),@isActive)
			,@recommendationXID       = isnull(context.node.value('@RecommendationXID'     ,'varchar(150)' ),@recommendationXID)
