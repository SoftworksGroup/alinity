...
GO
CREATE PROCEDURE [dbo].[pCatalogItemPriceProration#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.CatalogItemPriceProration
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.CatalogItemPriceProration for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.CatalogItemPriceProration table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fCatalogItemPriceProration#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pCatalogItemPriceProration#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.CatalogItemPriceProration'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.CatalogItemPriceProration'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.CatalogItemPriceProration'				-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.CatalogItemPriceProration'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.CatalogItemPriceProration'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_CatalogItemPriceProration'
		)
		begin
			exec sp_executesql N'alter table dbo.CatalogItemPriceProration drop constraint ck_CatalogItemPriceProration'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.CatalogItemPriceProration with CHECK add constraint ck_CatalogItemPriceProration
			check
				(
			...
			select @RowCount = count(1) from dbo.CatalogItemPriceProration			-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'CatalogItemPriceProration'
			...
			select
				...
				,x.RowGUID
			from
				dbo.CatalogItemPriceProration x
			order by
				x.CatalogItemPriceProrationSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fCatalogItemPriceProration#Check
							(
							 x.CatalogItemPriceProrationSID
							,x.CatalogItemPriceSID
							,x.StartMonthDay
							,x.Price
							,x.PercentageOfCurrentPrice
							,x.CatalogItemPriceProrationXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.CatalogItemPriceProration  x
					where
						x.CatalogItemPriceProrationSID = @nextSID
			...
			alter table dbo.CatalogItemPriceProration with NOCHECK add constraint ck_CatalogItemPriceProration
			check
				(
				dbo.fCatalogItemPriceProration#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.CatalogItemPriceProration'
		and
			br.BusinessRuleStatus <> 'x'
