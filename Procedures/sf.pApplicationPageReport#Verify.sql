...
GO
CREATE PROCEDURE [sf].[pApplicationPageReport#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ApplicationPageReport
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ApplicationPageReport for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ApplicationPageReport table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fApplicationPageReport#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pApplicationPageReport#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ApplicationPageReport'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ApplicationPageReport'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationPageReport'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationPageReport'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ApplicationPageReport'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ApplicationPageReport'
		)
		begin
			exec sp_executesql N'alter table sf.ApplicationPageReport drop constraint ck_ApplicationPageReport'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ApplicationPageReport with CHECK add constraint ck_ApplicationPageReport
			check
				(
				sf.fApplicationPageReport#Check
					(
			...
			select @RowCount = count(1) from sf.ApplicationPageReport						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ApplicationPageReport'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.ApplicationPageReportSID
				,x.RowGUID
			from
				sf.ApplicationPageReport x
			order by
				x.ApplicationPageReportSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fApplicationPageReport#Check
							(
							 x.ApplicationPageReportSID
							,x.ApplicationPageSID
							,x.ApplicationReportSID
							,x.ApplicationPageReportXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						sf.ApplicationPageReport  x
					where
						x.ApplicationPageReportSID = @nextSID
			...
			alter table sf.ApplicationPageReport with NOCHECK add constraint ck_ApplicationPageReport
			check
				(
				sf.fApplicationPageReport#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ApplicationPageReport'
		and
			br.BusinessRuleStatus <> 'x'
