...
GO
CREATE PROCEDURE [dbo].[pCompetenceType#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pCompetenceType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.CompetenceType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.CompetenceType table. When a new record is to be added from the UI, this procedure
...
columns of the vCompetenceType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pCompetenceType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pCompetenceType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@competenceTypeSID      int               = -1												-- specific default required by EF - do not override
		,@competenceTypeLabel    nvarchar(35)      = null											-- no default provided from DB constraint - OK to override
		,@competenceTypeCategory nvarchar(65)      = null											-- no default provided from DB constraint - OK to override
		...
		,@competenceTypeXID      varchar(150)      = null											-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @competenceTypeLabel     = isnull(context.node.value('@CompetenceTypeLabel'   ,'nvarchar(35)' ),@competenceTypeLabel)
			,@competenceTypeCategory  = isnull(context.node.value('@CompetenceTypeCategory','nvarchar(65)' ),@competenceTypeCategory)
			...
			,@competenceTypeXID       = isnull(context.node.value('@CompetenceTypeXID'     ,'varchar(150)' ),@competenceTypeXID)
