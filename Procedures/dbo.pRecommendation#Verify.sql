...
GO
CREATE PROCEDURE [dbo].[pRecommendation#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.Recommendation
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.Recommendation for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.Recommendation table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRecommendation#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRecommendation#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.Recommendation'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.Recommendation'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.Recommendation'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.Recommendation'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.Recommendation'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_Recommendation'
		)
		begin
			exec sp_executesql N'alter table dbo.Recommendation drop constraint ck_Recommendation'				-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.Recommendation with CHECK add constraint ck_Recommendation
			check
				(
				dbo.fRecommendation#Check
					(
					 RecommendationSID
			...
			select @RowCount = count(1) from dbo.Recommendation									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'Recommendation'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRecommendation#Check
							(
							 x.RecommendationSID
							,x.RecommendationGroupSID
							,x.ButtonLabel
							,x.RecommendationSequence
							,x.ToolTip
							,x.IsActive
							,x.RecommendationXID
							,x.LegacyKey
			...
			alter table dbo.Recommendation with NOCHECK add constraint ck_Recommendation
			check
				(
				dbo.fRecommendation#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.Recommendation'
		and
			br.BusinessRuleStatus <> 'x'
