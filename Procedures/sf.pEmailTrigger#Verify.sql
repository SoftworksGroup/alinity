...
GO
CREATE PROCEDURE [sf].[pEmailTrigger#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.EmailTrigger
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.EmailTrigger for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.EmailTrigger table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fEmailTrigger#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pEmailTrigger#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.EmailTrigger'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.EmailTrigger'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.EmailTrigger'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.EmailTrigger'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.EmailTrigger'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_EmailTrigger'
		)
		begin
			exec sp_executesql N'alter table sf.EmailTrigger drop constraint ck_EmailTrigger'							-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.EmailTrigger with CHECK add constraint ck_EmailTrigger
			check
				(
			...
			select @RowCount = count(1) from sf.EmailTrigger										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fEmailTrigger#Check
							(
							 x.EmailTriggerSID
							,x.EmailTriggerLabel
							,x.EmailTemplateSID
			...
			alter table sf.EmailTrigger with NOCHECK add constraint ck_EmailTrigger
			check
				(
				sf.fEmailTrigger#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.EmailTrigger'
		and
			br.BusinessRuleStatus <> 'x'
