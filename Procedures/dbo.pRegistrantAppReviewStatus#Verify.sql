...
GO
CREATE PROCEDURE [dbo].[pRegistrantAppReviewStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantAppReviewStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantAppReviewStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantAppReviewStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantAppReviewStatus#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantAppReviewStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantAppReviewStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantAppReviewStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAppReviewStatus'				-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAppReviewStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantAppReviewStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantAppReviewStatus'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantAppReviewStatus drop constraint ck_RegistrantAppReviewStatus'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantAppReviewStatus with CHECK add constraint ck_RegistrantAppReviewStatus
			check
				(
				dbo.fRegistrantAppReviewStatus#Check
					(
			...
			select @RowCount = count(1) from dbo.RegistrantAppReviewStatus			-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantAppReviewStatus'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrantAppReviewStatusSID
				,x.RowGUID
			from
				dbo.RegistrantAppReviewStatus x
			order by
				x.RegistrantAppReviewStatusSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantAppReviewStatus#Check
							(
							 x.RegistrantAppReviewStatusSID
							,x.RegistrantAppReviewSID
							,x.FormStatusSID
							,x.RegistrantAppReviewStatusXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantAppReviewStatus  x
					where
						x.RegistrantAppReviewStatusSID = @nextSID
			...
			alter table dbo.RegistrantAppReviewStatus with NOCHECK add constraint ck_RegistrantAppReviewStatus
			check
				(
				dbo.fRegistrantAppReviewStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantAppReviewStatus'
		and
			br.BusinessRuleStatus <> 'x'
