...
GO
CREATE PROCEDURE [dbo].[pRegistrantLearningPlanStatus#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pRegistrantLearningPlanStatus#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.RegistrantLearningPlanStatus records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.RegistrantLearningPlanStatus table. When a new record is to be added from the UI, this procedure
...
columns of the vRegistrantLearningPlanStatus entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pRegistrantLearningPlanStatus
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pRegistrantLearningPlanStatus of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@registrantLearningPlanStatusSID int               = -1							-- specific default required by EF - do not override
		...
		,@registrantLearningPlanStatusXID varchar(150)      = null						-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			...
			,@registrantLearningPlanStatusXID  = isnull(context.node.value('@RegistrantLearningPlanStatusXID','varchar(150)'),@registrantLearningPlanStatusXID)
