...
GO
CREATE PROCEDURE [dbo].[pAuditTypeForm#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.AuditTypeForm
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.AuditTypeForm for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.AuditTypeForm table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fAuditTypeForm#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pAuditTypeForm#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.AuditTypeForm'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.AuditTypeForm'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.AuditTypeForm'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.AuditTypeForm'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.AuditTypeForm'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_AuditTypeForm'
		)
		begin
			exec sp_executesql N'alter table dbo.AuditTypeForm drop constraint ck_AuditTypeForm'					-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.AuditTypeForm with CHECK add constraint ck_AuditTypeForm
			check
				(
				dbo.fAuditTypeForm#Check
					(
					 AuditTypeFormSID
					,AuditTypeSID
					,FormSID
			...
			select @RowCount = count(1) from dbo.AuditTypeForm									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'AuditTypeForm'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fAuditTypeForm#Check
							(
							 x.AuditTypeFormSID
							,x.AuditTypeSID
							,x.FormSID
			...
			alter table dbo.AuditTypeForm with NOCHECK add constraint ck_AuditTypeForm
			check
				(
				dbo.fAuditTypeForm#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.AuditTypeForm'
		and
			br.BusinessRuleStatus <> 'x'
