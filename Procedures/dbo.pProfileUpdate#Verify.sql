...
GO
CREATE PROCEDURE [dbo].[pProfileUpdate#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ProfileUpdate
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ProfileUpdate for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ProfileUpdate table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fProfileUpdate#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pProfileUpdate#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ProfileUpdate'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ProfileUpdate'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ProfileUpdate'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ProfileUpdate'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ProfileUpdate'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ProfileUpdate'
		)
		begin
			exec sp_executesql N'alter table dbo.ProfileUpdate drop constraint ck_ProfileUpdate'					-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ProfileUpdate with CHECK add constraint ck_ProfileUpdate
			check
				(
			...
			select @RowCount = count(1) from dbo.ProfileUpdate									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ProfileUpdate'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fProfileUpdate#Check
							(
							 x.ProfileUpdateSID
							,x.PersonSID
							,x.RegistrationYear
			...
			alter table dbo.ProfileUpdate with NOCHECK add constraint ck_ProfileUpdate
			check
				(
				dbo.fProfileUpdate#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ProfileUpdate'
		and
			br.BusinessRuleStatus <> 'x'
