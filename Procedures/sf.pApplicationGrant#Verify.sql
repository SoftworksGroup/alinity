...
GO
CREATE PROCEDURE [sf].[pApplicationGrant#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ApplicationGrant
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ApplicationGrant for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ApplicationGrant table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fApplicationGrant#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pApplicationGrant#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ApplicationGrant'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ApplicationGrant'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationGrant'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationGrant'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ApplicationGrant'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ApplicationGrant'
		)
		begin
			exec sp_executesql N'alter table sf.ApplicationGrant drop constraint ck_ApplicationGrant'			-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ApplicationGrant with CHECK add constraint ck_ApplicationGrant
			check
				(
				sf.fApplicationGrant#Check
					(
					 ApplicationGrantSID
					,ApplicationGrantSCD
			...
			select @RowCount = count(1) from sf.ApplicationGrant								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ApplicationGrant'
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fApplicationGrant#Check
							(
							 x.ApplicationGrantSID
							,x.ApplicationGrantSCD
							,x.ApplicationGrantName
							,x.IsDefault
							,x.ApplicationGrantXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
			...
			alter table sf.ApplicationGrant with NOCHECK add constraint ck_ApplicationGrant
			check
				(
				sf.fApplicationGrant#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ApplicationGrant'
		and
			br.BusinessRuleStatus <> 'x'
