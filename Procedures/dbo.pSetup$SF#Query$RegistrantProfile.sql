...
GO
CREATE PROCEDURE dbo.pSetup$SF#Query$RegistrantProfile
	@SetupUser nvarchar(75) -- user assigned to audit columns
as
/*********************************************************************************************************************************
Sproc    : Setup Query for Registrant Profile management
Notice   : Copyright © 2019 Softworks Group Inc.
Summary  : Synchronizes the sf.Query table for product queries used in management of Registrant Profiles
----------------------------------------------------------------------------------------------------------------------------------
...
procedure: dbo.pQuery#Execute#RegistrantProfile.
...
begin
	...
	where
		ae.ApplicationEntitySCD = 'stg.RegistrantProfile'
	order by
		q.QueryLabel;
...
exec sf.pUnitTest#Execute
	 @ObjectName = 'dbo.pSetup$SF#Query$RegistrantProfile'
	,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
...
begin
	...
	begin try
		...
		values
		(
			@defaultCategorySID, 'S!REGP.ALL', @parametersDoc, 'All', @OFF, N'Returns records loaded in a date range or from a given import file'
		);
		...
		values
		(
			@defaultCategorySID, 'S!REGP.LAST.IMPORT', @parametersDoc, 'Last import', @ON, N'Returns records from the last import file loaded'
		);
		...
		values
		(
			@defaultCategorySID, 'S!REGP.UNPROCESSED', @parametersDoc, 'Unprocessed', @OFF, N'Returns records not yet applied (staged records only)'
		);
		...
		values
		(
			@defaultCategorySID, 'S!REGP.BY.STATUS', @parametersDoc, 'By status', @OFF, N'Returns records for selected processing status'
		);
		...
		values
		(
			@defaultCategorySID, 'S!REGP.BY.FILE', @parametersDoc, 'By file', @OFF, N'Returns records for selected import file'
		);
		...
		values
		(
			@defaultCategorySID, 'S!REGP.CANCELLED', @parametersDoc, 'Cancelled', @OFF, N'Returns records with a status of cancelled'
		);
		...
		values
		(
			@defaultCategorySID, 'S!REGP.FIND.BY.PHONE', @parametersDoc, 'Find by phone', @OFF
		 ,N'Returns records where the registrant''s home or mobile phone matches any portion of a number provided'
		);
		...
		values
		(
			@defaultCategorySID, 'S!REGP.FIND.BY.ADDRESS', @parametersDoc, 'Find by address', @OFF
		 ,N'Returns records where the registrant''s address matches any portion of a street address provided'
		);
		...
		values
		(
			@defaultCategorySID, 'S!REGP.RECENTLY.UPDATED', @parametersDoc, 'Recently updated', @OFF
		...
		exec sf.pQuery#CheckCodes
			@ApplicationEntitySCD = 'stg.RegistrantProfile'
		 ,@SchemaAndRoutineName = 'dbo.pQuery#Execute$RegistrantProfile'
