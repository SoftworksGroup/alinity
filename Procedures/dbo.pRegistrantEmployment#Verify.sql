...
GO
CREATE PROCEDURE [dbo].[pRegistrantEmployment#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantEmployment
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantEmployment for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantEmployment table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantEmployment#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantEmployment#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantEmployment'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantEmployment'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantEmployment'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantEmployment'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantEmployment'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantEmployment'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantEmployment drop constraint ck_RegistrantEmployment'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantEmployment with CHECK add constraint ck_RegistrantEmployment
			check
				(
			...
			select @RowCount = count(1) from dbo.RegistrantEmployment						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantEmployment'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantEmployment#Check
							(
							 x.RegistrantEmploymentSID
							,x.RegistrantSID
							,x.OrgSID
					...
					from
						dbo.RegistrantEmployment  x
					where
						x.RegistrantEmploymentSID = @nextSID
			...
			alter table dbo.RegistrantEmployment with NOCHECK add constraint ck_RegistrantEmployment
			check
				(
				dbo.fRegistrantEmployment#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantEmployment'
		and
			br.BusinessRuleStatus <> 'x'
