...
GO
CREATE PROCEDURE [dbo].[pTax#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pTax#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.Tax records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.Tax table. When a new record is to be added from the UI, this procedure
...
columns of the vTax entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pTax
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pTax of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@taxSID                int               = -1												-- specific default required by EF - do not override
		,@taxLabel              nvarchar(35)      = null											-- no default provided from DB constraint - OK to override
		,@taxSequence           tinyint           = null											-- no default provided from DB constraint - OK to override
		,@userDefinedColumns    xml               = null											-- no default provided from DB constraint - OK to override
		,@taxXID                varchar(150)      = null											-- no default provided from DB constraint - OK to override
		...
		,@currentTaxRate        decimal(4,4)																	-- not a base table column (default ignored)
		,@currentGLAccountSID   int																						-- not a base table column (default ignored)
		,@currentGLAccountCode  varchar(50)																		-- not a base table column (default ignored)
		,@currentGLAccountLabel nvarchar(35)																	-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @taxLabel     = isnull(context.node.value('@TaxLabel'   ,'nvarchar(35)'),@taxLabel)
			,@taxSequence  = isnull(context.node.value('@TaxSequence','tinyint'     ),@taxSequence)
			,@taxXID       = isnull(context.node.value('@TaxXID'     ,'varchar(150)'),@taxXID)
