...
GO
CREATE PROCEDURE [dbo].[pOrgContact#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.OrgContact
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.OrgContact for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.OrgContact table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fOrgContact#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pOrgContact#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.OrgContact'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.OrgContact'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.OrgContact'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.OrgContact'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.OrgContact'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_OrgContact'
		)
		begin
			exec sp_executesql N'alter table dbo.OrgContact drop constraint ck_OrgContact'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.OrgContact with CHECK add constraint ck_OrgContact
			check
				(
			...
			select @RowCount = count(1) from dbo.OrgContact											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.OrgContact with NOCHECK add constraint ck_OrgContact
			check
				(
				dbo.fOrgContact#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.OrgContact'
		and
			br.BusinessRuleStatus <> 'x'
