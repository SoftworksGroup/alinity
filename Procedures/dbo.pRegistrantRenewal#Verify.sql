...
GO
CREATE PROCEDURE [dbo].[pRegistrantRenewal#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantRenewal
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantRenewal for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantRenewal table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantRenewal#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantRenewal#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantRenewal'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantRenewal'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantRenewal'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantRenewal'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantRenewal'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantRenewal'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantRenewal drop constraint ck_RegistrantRenewal'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantRenewal with CHECK add constraint ck_RegistrantRenewal
			check
				(
			...
			select @RowCount = count(1) from dbo.RegistrantRenewal							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantRenewal'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantRenewal#Check
							(
							 x.RegistrantRenewalSID
							,x.RegistrationSID
					...
					from
						dbo.RegistrantRenewal  x
					where
						x.RegistrantRenewalSID = @nextSID
			...
			alter table dbo.RegistrantRenewal with NOCHECK add constraint ck_RegistrantRenewal
			check
				(
				dbo.fRegistrantRenewal#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantRenewal'
		and
			br.BusinessRuleStatus <> 'x'
