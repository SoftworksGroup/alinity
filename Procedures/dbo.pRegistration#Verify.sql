...
GO
CREATE PROCEDURE [dbo].[pRegistration#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.Registration
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.Registration for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.Registration table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistration#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistration#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.Registration'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.Registration'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.Registration'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.Registration'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.Registration'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_Registration'
		)
		begin
			exec sp_executesql N'alter table dbo.Registration drop constraint ck_Registration'						-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			select @RowCount = count(1) from dbo.Registration										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistration#Check
							(
							 x.RegistrationSID
							,x.RegistrantSID
			...
			alter table dbo.Registration with NOCHECK add constraint ck_Registration
			check
				(
				dbo.fRegistration#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.Registration'
		and
			br.BusinessRuleStatus <> 'x'
