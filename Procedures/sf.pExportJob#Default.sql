...
GO
CREATE PROCEDURE [sf].[pExportJob#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pExportJob#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.ExportJob records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.ExportJob table. When a new record is to be added from the UI, this procedure
...
columns of the vExportJob entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pExportJob
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pExportJob of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@exportJobSID          int               = -1												-- specific default required by EF - do not override
		,@exportJobName         nvarchar(65)      = null											-- no default provided from DB constraint - OK to override
		,@exportJobCode         varchar(15)       = null											-- no default provided from DB constraint - OK to override
		...
		,@fileFormatSID         int               = null											-- no default provided from DB constraint - OK to override
		,@bodySpecification     nvarchar(max)     = null											-- no default provided from DB constraint - OK to override
		,@lineSpecification     nvarchar(max)     = null											-- no default provided from DB constraint - OK to override
		,@xMLTransformation     xml               = null											-- no default provided from DB constraint - OK to override
		,@jobScheduleSID        int               = null											-- no default provided from DB constraint - OK to override
		,@endPoint              xml               = null											-- no default provided from DB constraint - OK to override
		...
		,@exportJobXID          varchar(150)      = null											-- no default provided from DB constraint - OK to override
		...
		,@fileFormatSCD         varchar(10)       = '?'												-- not a base table column (default ignored)
		,@fileFormatLabel       nvarchar(35)      = N'?'											-- not a base table column (default ignored)
		,@fileFormatIsDefault   bit               = 0													-- not a base table column (default ignored)
		,@fileFormatRowGUID     uniqueidentifier  = newid()										-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @exportJobName      = isnull(context.node.value('@ExportJobName'    ,'nvarchar(65)'     ),@exportJobName)
			,@exportJobCode      = isnull(context.node.value('@ExportJobCode'    ,'varchar(15)'      ),@exportJobCode)
			,@querySQL           = isnull(context.node.value('@QuerySQL'         ,'nvarchar(max)'    ),@querySQL)
			,@fileFormatSID      = isnull(context.node.value('@FileFormatSID'    ,'int'              ),@fileFormatSID)
			,@bodySpecification  = isnull(context.node.value('@BodySpecification','nvarchar(max)'    ),@bodySpecification)
			,@lineSpecification  = isnull(context.node.value('@LineSpecification','nvarchar(max)'    ),@lineSpecification)
			...
			,@exportJobXID       = isnull(context.node.value('@ExportJobXID'     ,'varchar(150)'     ),@exportJobXID)
		...
		if isnull(@fileFormatSID,0) = 0 select @fileFormatSID = x.FileFormatSID from sf.FileFormat  x where x.IsDefault = @ON
