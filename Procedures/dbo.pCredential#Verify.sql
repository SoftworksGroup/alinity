...
GO
CREATE PROCEDURE [dbo].[pCredential#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.Credential
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.Credential for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.Credential table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fCredential#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pCredential#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.Credential'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.Credential'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.Credential'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.Credential'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.Credential'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_Credential'
		)
		begin
			exec sp_executesql N'alter table dbo.Credential drop constraint ck_Credential'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.Credential with CHECK add constraint ck_Credential
			check
				(
				dbo.fCredential#Check
					(
					 CredentialSID
					,CredentialTypeSID
					,CredentialLabel
			...
			select @RowCount = count(1) from dbo.Credential											-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fCredential#Check
							(
							 x.CredentialSID
							,x.CredentialTypeSID
							,x.CredentialLabel
							,x.ToolTip
							,x.IsRelatedToProfession
							,x.IsProgramRequired
							,x.IsSpecialization
							,x.IsActive
							,x.CredentialCode
			...
			alter table dbo.Credential with NOCHECK add constraint ck_Credential
			check
				(
				dbo.fCredential#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.Credential'
		and
			br.BusinessRuleStatus <> 'x'
