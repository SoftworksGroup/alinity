...
GO
CREATE PROCEDURE [sf].[pAuditAction#Delete]
	 @AuditActionSID     int               = null -- required! id of row to delete - must be set in custom logic if not passed
...
as
/*********************************************************************************************************************************
Procedure : sf.pAuditAction#Delete
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : deletes 1 row in the sf.AuditAction table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.AuditAction table. The procedure requires a primary key value to locate the record
to delete.
...
and in the ext.pAuditAction procedure for client-specific deletion rules.
...
begin
	...
	begin try
		...
		if @RowStamp is null select @RowStamp = x.RowStamp from sf.AuditAction x where x.AuditActionSID = @AuditActionSID
    ...
    -- the deletion option based on the value returned from the IsDeleteEnabled function
		-- in the #Ext view.
    set @IsDeleteEnabled = sf.fAuditAction#IsDeleteEnabled(@AuditActionSID)
    ...
    begin
      ...
      raiserror(@errorText, 16, 1)
		...
		if exists (select 1 from sf.AuditAction x where x.AuditActionSID = @AuditActionSID and x.IsDefault = cast(1 as bit))
		begin
