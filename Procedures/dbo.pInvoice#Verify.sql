...
GO
CREATE PROCEDURE [dbo].[pInvoice#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.Invoice
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.Invoice for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.Invoice table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fInvoice#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pInvoice#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.Invoice'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.Invoice'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.Invoice'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.Invoice'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.Invoice'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_Invoice'
		)
		begin
			exec sp_executesql N'alter table dbo.Invoice drop constraint ck_Invoice'											-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.Invoice with CHECK add constraint ck_Invoice
			check
				(
				dbo.fInvoice#Check
					(
			...
			select @RowCount = count(1) from dbo.Invoice												-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.Invoice with NOCHECK add constraint ck_Invoice
			check
				(
				dbo.fInvoice#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.Invoice'
		and
			br.BusinessRuleStatus <> 'x'
