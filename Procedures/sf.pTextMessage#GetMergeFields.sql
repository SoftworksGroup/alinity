...
GO
CREATE PROCEDURE [sf].[pTextMessage#GetMergeFields]
	 @TextMessageSID													int					= null						-- key of text message to return values for
...
as
/*********************************************************************************************************************************
Procedure : Text Message - Get Merge Fields
Notice    : Copyright © 2015 Softworks Group Inc.
Summary   : Returns merge field tokens to the user interface for composing text
----------------------------------------------------------------------------------------------------------------------------------
History	: Author(s)  	| Month Year	| Change Summary
				: ------------|-------------|---------------------------------------------------------------------------------------------
				: Cory Ng			| Jun		2016	| Initial Version
        : Russ Poirier| Feb   2017  | Query now only brings in fields from Application Entity that vPersonTextMessage
                                      is lacking. Query excludes xml and unique identifiers.

Comments
--------
This procedure is called by text composition screens to provide a list of merge fields that can be used in the text text.
Merge fields are replaced during the sending process so that, for example, [@DisplayName] is replaced with "Tim Edlund" before
the message is sent.

This procedure allow either @TextMessageSID to be passed to lookup a replace source Application Entity - e.g. dbo.vEpisode,
...
for no parameter to be provided. This is because an Application Entity on the text is optional but may be used to specify an
additional source for replacement of merge fields. All texts automatically receive the fields from sf.vPersonTextMessage as a
replacement source.
...
<TestHarness>
	<Test Name="Random" IsDefault="true" Description="Selects 1 text message at random and calls
...
select top (1)
	@TextMessageSID = em.TextMessageSID
from
	sf.TextMessage em
order by
	newid()
...
begin
	...
	exec sf.pTextMessage#GetMergeFields
...
exec sf.pUnitTest#Execute
	@ObjectName = 'sf.pTextMessage#GetMergeFields'
...
begin
	...
	declare
		...
		,@environmentVariableLabel				nvarchar(128)																									-- label to use with envrionment variables
		,@schemaAndViewName								nvarchar(257)																									-- entity to search for

	begin try

    if @TextMessageSID is not null
    begin

			-- check for a merge data source other than the default
			-- person text message entity (not an error if not defined)
			...
			where
				em.TextMessageSID = @TextMessageSID
		...
		set @schemaAndViewName				= isnull(replace(@ApplicationEntitySCD, '.', '.v'), N'?')
        ...
        left outer join
          ...
          and vc2.SchemaAndViewName = 'sf.vPersonTextMessage'
        where
        ...
        from
          sf.vViewColumn vc
        where
          vc.SchemaAndViewName = N'sf.vPersonTextMessage'
        and
          vc.DataType <> 'timestamp'
				and 
					vc.DataType <> 'varbinary'
