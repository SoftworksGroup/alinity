...
GO
CREATE PROCEDURE [dbo].[pRegistrantPractice#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantPractice
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantPractice for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantPractice table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantPractice#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantPractice#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantPractice'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantPractice'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantPractice'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantPractice'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantPractice'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantPractice'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantPractice drop constraint ck_RegistrantPractice'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantPractice with CHECK add constraint ck_RegistrantPractice
			check
				(
			...
			select @RowCount = count(1) from dbo.RegistrantPractice							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantPractice'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantPractice#Check
							(
							 x.RegistrantPracticeSID
							,x.RegistrantSID
							,x.RegistrationYear
							,x.EmploymentStatusSID
							,x.PlannedRetirementDate
							,x.OtherJurisdiction
							,x.OtherJurisdictionHours
							,x.TotalPracticeHours
							,x.OrgSID
							,x.InsurancePolicyNo
							,x.InsuranceAmount
							,x.InsuranceCertificateNo
							,x.RegistrantPracticeXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantPractice  x
					where
						x.RegistrantPracticeSID = @nextSID
			...
			alter table dbo.RegistrantPractice with NOCHECK add constraint ck_RegistrantPractice
			check
				(
				dbo.fRegistrantPractice#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantPractice'
		and
			br.BusinessRuleStatus <> 'x'
