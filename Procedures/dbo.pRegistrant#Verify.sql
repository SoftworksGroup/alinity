...
GO
CREATE PROCEDURE [dbo].[pRegistrant#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.Registrant
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.Registrant for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.Registrant table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrant#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrant#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.Registrant'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.Registrant'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.Registrant'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.Registrant'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.Registrant'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_Registrant'
		)
		begin
			exec sp_executesql N'alter table dbo.Registrant drop constraint ck_Registrant'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.Registrant with CHECK add constraint ck_Registrant
			check
				(
			...
			select @RowCount = count(1) from dbo.Registrant											-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrant#Check
							(
							 x.RegistrantSID
							,x.PersonSID
							,x.RegistrantNo
							,x.YearOfInitialEmployment
							,x.IsOnPublicRegistry
							,x.CityNameOfBirth
							,x.CountrySID
							,x.DirectedAuditYearCompetence
							,x.DirectedAuditYearPracticeHours
							,x.LateFeeExclusionYear
							,x.IsRenewalAutoApprovalBlocked
							,x.RenewalExtensionExpiryTime
							,x.ArchivedTime
							,x.RegistrantXID
							,x.LegacyKey
							,x.IsDeleted
			...
			alter table dbo.Registrant with NOCHECK add constraint ck_Registrant
			check
				(
				dbo.fRegistrant#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.Registrant'
		and
			br.BusinessRuleStatus <> 'x'
