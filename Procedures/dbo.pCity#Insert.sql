...
GO
CREATE PROCEDURE [dbo].[pCity#Insert]
	 @CitySID                            int               = null output		-- identity value assigned to the new record
	,@CityName                           nvarchar(30)      = null						-- required! if not passed value must be set in custom logic prior to insert
	,@StateProvinceSID                   int               = null						-- required! if not passed value must be set in custom logic prior to insert
	...
	,@CitySearch                         nvarchar(112)     = null						-- not a base table column (default ignored)
as
/*********************************************************************************************************************************
Procedure : dbo.pCity#Insert
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : inserts 1 row into the dbo.City table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.City table with parameter values represent columns of the table. Additional
parameters are provided for all columns in the vCity entity view, however, the base logic of the procedure inserts the
...
begin
	...
	set @CitySID = null																											-- initialize output parameter
	...
	begin try
		...
		if @StateProvinceSID  is null select @StateProvinceSID  = x.StateProvinceSID from dbo.StateProvince x where x.IsDefault = @ON and x.CountrySID = @CountrySID
		...
		begin
			...
			select @recordSID = x.CitySID from dbo.City x where x.IsDefault = @ON
		...
		select
			...
			,@CitySID = scope_identity()																				-- capture key value of new row
