...
GO
CREATE PROCEDURE [dbo].[pRecommendationGroup#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RecommendationGroup
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RecommendationGroup for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RecommendationGroup table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRecommendationGroup#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRecommendationGroup#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RecommendationGroup'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RecommendationGroup'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RecommendationGroup'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RecommendationGroup'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RecommendationGroup'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RecommendationGroup'
		)
		begin
			exec sp_executesql N'alter table dbo.RecommendationGroup drop constraint ck_RecommendationGroup'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RecommendationGroup with CHECK add constraint ck_RecommendationGroup
			check
				(
				dbo.fRecommendationGroup#Check
					(
			...
			select @RowCount = count(1) from dbo.RecommendationGroup						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RecommendationGroup'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RecommendationGroupSID
				,x.RowGUID
			from
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRecommendationGroup#Check
							(
							 x.RecommendationGroupSID
							,x.RecommendationGroupSCD
							,x.RecommendationGroupLabel
							,x.RecommendationGroupXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RecommendationGroup  x
					where
						x.RecommendationGroupSID = @nextSID
			...
			alter table dbo.RecommendationGroup with NOCHECK add constraint ck_RecommendationGroup
			check
				(
				dbo.fRecommendationGroup#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RecommendationGroup'
		and
			br.BusinessRuleStatus <> 'x'
