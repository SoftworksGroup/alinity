...
GO
CREATE PROCEDURE [dbo].[pProfileUpdate#GetMissingPDF]
...
Summary  : This procedure returns a list of profile updates that require PDF generation.
----------------------------------------------------------------------------------------------------------------------------------
...
Comments	
--------
This procedure returns all approved profile updates where a PDF has not yet been generated.  The procedure is called from the UI
...
<TestHarness>
	<Test Name="Simple" IsDefault="true" Description="Returns all approved profile updates where a PDF is not yet generated.">
		<SQLScript>
			<![CDATA[

exec dbo.pProfileUpdate#GetMissingPDF
...
exec sf.pUnitTest#Execute
	@ObjectName = 'dbo.pProfileUpdate#GetMissingPDF'
------------------------------------------------------------------------------------------------------------------------------- */
...
begin
	...
	begin try
		...
		join
			sf.Person																								p on cs.PersonSID							 = p.PersonSID
		join
			dbo.Registrant																					r on p.PersonSID							 = r.PersonSID
		join
			sf.ApplicationEntity																		ae on ae.ApplicationEntitySCD	 = 'dbo.ProfileUpdate'
		left outer join
			dbo.PersonDocContext																		pdc on ae.ApplicationEntitySID = pdc.ApplicationEntitySID and cs.ProfileUpdateSID = pdc.EntitySID and pdc.IsPrimary = @ON
		where
