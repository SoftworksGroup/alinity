...
GO
CREATE PROCEDURE [sf].[pFormSubForm#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pFormSubForm#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.FormSubForm records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.FormSubForm table. When a new record is to be added from the UI, this procedure
...
columns of the vFormSubForm entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pFormSubForm
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pFormSubForm of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@formSubFormSID            int               = -1										-- specific default required by EF - do not override
		,@formSID                   int               = null									-- no default provided from DB constraint - OK to override
		,@subFormSID                int               = null									-- no default provided from DB constraint - OK to override
		,@subFormSequence           int               = (5)										-- default provided from DB constraint - OK to override
		,@userDefinedColumns        xml               = null									-- no default provided from DB constraint - OK to override
		,@formSubFormXID            varchar(150)      = null									-- no default provided from DB constraint - OK to override
		...
		,@formFormTypeSID           int               = 0											-- not a base table column (default ignored)
		,@formFormName              nvarchar(65)      = N'?'									-- not a base table column (default ignored)
		,@formFormLabel             nvarchar(35)      = N'?'									-- not a base table column (default ignored)
		,@formFormContext           varchar(25)																-- not a base table column (default ignored)
		,@formAuthorCredit          nvarchar(500)     = N'?'									-- not a base table column (default ignored)
		,@formIsActive              bit               = 0											-- not a base table column (default ignored)
		,@formApplicationUserSID    int               = 0											-- not a base table column (default ignored)
		,@formRowGUID               uniqueidentifier  = newid()								-- not a base table column (default ignored)
		,@formSubFormTypeSID        int               = 0											-- not a base table column (default ignored)
		,@formSubFormName           nvarchar(65)      = N'?'									-- not a base table column (default ignored)
		,@formSubFormLabel          nvarchar(35)      = N'?'									-- not a base table column (default ignored)
		,@formSubFormContext        varchar(25)																-- not a base table column (default ignored)
		,@formSubAuthorCredit       nvarchar(500)     = N'?'									-- not a base table column (default ignored)
		,@formSubIsActive           bit               = 0											-- not a base table column (default ignored)
		,@formSubApplicationUserSID int               = 0											-- not a base table column (default ignored)
		,@formSubRowGUID            uniqueidentifier  = newid()								-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@subFormSID       = isnull(context.node.value('@SubFormSID'     ,'int'         ),@subFormSID)
			,@subFormSequence  = isnull(context.node.value('@SubFormSequence','int'         ),@subFormSequence)
			,@formSubFormXID   = isnull(context.node.value('@FormSubFormXID' ,'varchar(150)'),@formSubFormXID)
