...
GO
CREATE PROCEDURE [stg].[pCredentialProfile#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for stg.CredentialProfile
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks stg.CredentialProfile for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the stg.CredentialProfile table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, stg.fCredentialProfile#Check(), on each row. If an error is
...
Example:
	...
	exec stg.pCredentialProfile#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'stg.CredentialProfile'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'stg.CredentialProfile'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'stg.CredentialProfile'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'stg.CredentialProfile'
			)
		...
		where
			brx.ApplicationEntitySCD = 'stg.CredentialProfile'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'stg'
		and
			cc.ConstraintName = 'ck_CredentialProfile'
		)
		begin
			exec sp_executesql N'alter table stg.CredentialProfile drop constraint ck_CredentialProfile'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table stg.CredentialProfile with CHECK add constraint ck_CredentialProfile
			check
				(
			...
			select @RowCount = count(1) from stg.CredentialProfile							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'CredentialProfile'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.CredentialProfileSID
				,x.RowGUID
			from
				stg.CredentialProfile x
			order by
				x.CredentialProfileSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = stg.fCredentialProfile#Check
							(
							 x.CredentialProfileSID
							...
							,x.RegionLabel
							,x.RegionName
							,x.CredentialTypeLabel
							,x.RegistrantSID
							,x.CredentialSID
							,x.CredentialTypeSID
							,x.OrgSID
							,x.RegionSID
							,x.CredentialProfileXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						stg.CredentialProfile  x
					where
						x.CredentialProfileSID = @nextSID
			...
			alter table stg.CredentialProfile with NOCHECK add constraint ck_CredentialProfile
			check
				(
				stg.fCredentialProfile#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'stg.CredentialProfile'
		and
			br.BusinessRuleStatus <> 'x'
