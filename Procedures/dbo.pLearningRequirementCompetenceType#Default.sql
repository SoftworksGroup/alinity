...
GO
CREATE PROCEDURE [dbo].[pLearningRequirementCompetenceType#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pLearningRequirementCompetenceType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.LearningRequirementCompetenceType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.LearningRequirementCompetenceType table. When a new record is to be added from the UI, this procedure
...
columns of the vLearningRequirementCompetenceType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pLearningRequirementCompetenceType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pLearningRequirementCompetenceType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@learningRequirementCompetenceTypeSID int               = -1					-- specific default required by EF - do not override
		,@learningRequirementSID               int               = null				-- no default provided from DB constraint - OK to override
		,@competenceTypeSID                    int               = null				-- no default provided from DB constraint - OK to override
		,@userDefinedColumns                   xml               = null				-- no default provided from DB constraint - OK to override
		,@learningRequirementCompetenceTypeXID varchar(150)      = null				-- no default provided from DB constraint - OK to override
		...
		,@competenceTypeLabel                  nvarchar(35)      = N'?'				-- not a base table column (default ignored)
		,@competenceTypeCategory               nvarchar(65)										-- not a base table column (default ignored)
		,@competenceTypeIsDefault              bit               = 0					-- not a base table column (default ignored)
		,@competenceTypeIsActive               bit               = 0					-- not a base table column (default ignored)
		,@competenceTypeRowGUID                uniqueidentifier  = newid()		-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@competenceTypeSID                     = isnull(context.node.value('@CompetenceTypeSID'                   ,'int'         ),@competenceTypeSID)
			,@learningRequirementCompetenceTypeXID  = isnull(context.node.value('@LearningRequirementCompetenceTypeXID','varchar(150)'),@learningRequirementCompetenceTypeXID)
		...
		if isnull(@competenceTypeSID,0) = 0 select @competenceTypeSID = x.CompetenceTypeSID from dbo.CompetenceType x where x.IsDefault = @ON
