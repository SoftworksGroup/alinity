...
GO
CREATE PROCEDURE [dbo].[pRegistrantLanguage#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pRegistrantLanguage#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.RegistrantLanguage records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.RegistrantLanguage table. When a new record is to be added from the UI, this procedure
...
columns of the vRegistrantLanguage entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pRegistrantLanguage
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pRegistrantLanguage of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@registrantLanguageSID          int               = -1								-- specific default required by EF - do not override
		,@registrantSID                  int               = null							-- no default provided from DB constraint - OK to override
		,@languageSID                    int               = null							-- no default provided from DB constraint - OK to override
		,@isSpoken                       bit               = (1)							-- default provided from DB constraint - OK to override
		,@isWritten                      bit               = (1)							-- default provided from DB constraint - OK to override
		,@userDefinedColumns             xml               = null							-- no default provided from DB constraint - OK to override
		,@registrantLanguageXID          varchar(150)      = null							-- no default provided from DB constraint - OK to override
		...
		,@languageLabel                  nvarchar(35)      = N'?'							-- not a base table column (default ignored)
		,@languageIsDefault              bit               = 0								-- not a base table column (default ignored)
		,@languageIsActive               bit               = 0								-- not a base table column (default ignored)
		,@languageRowGUID                uniqueidentifier  = newid()					-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@languageSID            = isnull(context.node.value('@LanguageSID'          ,'int'         ),@languageSID)
			,@isSpoken               = isnull(context.node.value('@IsSpoken'             ,'bit'         ),@isSpoken)
			,@isWritten              = isnull(context.node.value('@IsWritten'            ,'bit'         ),@isWritten)
			,@registrantLanguageXID  = isnull(context.node.value('@RegistrantLanguageXID','varchar(150)'),@registrantLanguageXID)
		...
		if isnull(@languageSID,0) = 0 select @languageSID = x.LanguageSID from dbo.Language x where x.IsDefault = @ON
