...
GO
CREATE PROCEDURE [sf].[pMessageLinkStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.MessageLinkStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.MessageLinkStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.MessageLinkStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fMessageLinkStatus#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pMessageLinkStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.MessageLinkStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.MessageLinkStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.MessageLinkStatus'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.MessageLinkStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.MessageLinkStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_MessageLinkStatus'
		)
		begin
			exec sp_executesql N'alter table sf.MessageLinkStatus drop constraint ck_MessageLinkStatus'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.MessageLinkStatus with CHECK add constraint ck_MessageLinkStatus
			check
				(
				sf.fMessageLinkStatus#Check
					(
			...
			select @RowCount = count(1) from sf.MessageLinkStatus								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'MessageLinkStatus'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.MessageLinkStatusSID
				,x.RowGUID
			from
				sf.MessageLinkStatus x
			order by
				x.MessageLinkStatusSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fMessageLinkStatus#Check
							(
							 x.MessageLinkStatusSID
					...
					from
						sf.MessageLinkStatus  x
					where
						x.MessageLinkStatusSID = @nextSID
			...
			alter table sf.MessageLinkStatus with NOCHECK add constraint ck_MessageLinkStatus
			check
				(
				sf.fMessageLinkStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.MessageLinkStatus'
		and
			br.BusinessRuleStatus <> 'x'
