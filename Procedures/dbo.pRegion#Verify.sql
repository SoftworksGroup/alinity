...
GO
CREATE PROCEDURE [dbo].[pRegion#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.Region
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.Region for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.Region table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegion#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegion#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.Region'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.Region'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.Region'														-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.Region'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.Region'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_Region'
		)
		begin
			exec sp_executesql N'alter table dbo.Region drop constraint ck_Region'												-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.Region with CHECK add constraint ck_Region
			check
				(
				dbo.fRegion#Check
					(
			...
			select @RowCount = count(1) from dbo.Region													-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.Region with NOCHECK add constraint ck_Region
			check
				(
				dbo.fRegion#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.Region'
		and
			br.BusinessRuleStatus <> 'x'
