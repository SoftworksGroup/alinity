...
GO
CREATE PROCEDURE [sf].[pMessage#Delete]
	 @MessageSID            int               = null -- required! id of row to delete - must be set in custom logic if not passed
...
as
/*********************************************************************************************************************************
Procedure : sf.pMessage#Delete
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : deletes 1 row in the sf.Message table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.Message table. The procedure requires a primary key value to locate the record
to delete.
...
and in the ext.pMessage procedure for client-specific deletion rules.
...
begin
	...
	begin try
		...
		if @RowStamp is null select @RowStamp = x.RowStamp from sf.Message x where x.MessageSID = @MessageSID
    ...
    -- the deletion option based on the value returned from the IsDeleteEnabled function
		-- in the #Ext view.
    set @IsDeleteEnabled = sf.fMessage#IsDeleteEnabled(@MessageSID)
