...
GO
CREATE PROCEDURE [sf].[pApplicationUserGrant#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ApplicationUserGrant
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ApplicationUserGrant for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ApplicationUserGrant table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fApplicationUserGrant#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pApplicationUserGrant#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ApplicationUserGrant'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ApplicationUserGrant'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationUserGrant'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationUserGrant'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ApplicationUserGrant'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ApplicationUserGrant'
		)
		begin
			exec sp_executesql N'alter table sf.ApplicationUserGrant drop constraint ck_ApplicationUserGrant'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ApplicationUserGrant with CHECK add constraint ck_ApplicationUserGrant
			check
				(
				sf.fApplicationUserGrant#Check
					(
			...
			select @RowCount = count(1) from sf.ApplicationUserGrant						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ApplicationUserGrant'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.ApplicationUserGrantSID
				,x.RowGUID
			from
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fApplicationUserGrant#Check
							(
							 x.ApplicationUserGrantSID
					...
					from
						sf.ApplicationUserGrant  x
					where
						x.ApplicationUserGrantSID = @nextSID
			...
			alter table sf.ApplicationUserGrant with NOCHECK add constraint ck_ApplicationUserGrant
			check
				(
				sf.fApplicationUserGrant#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ApplicationUserGrant'
		and
			br.BusinessRuleStatus <> 'x'
