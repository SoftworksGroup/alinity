...
GO
CREATE PROCEDURE [dbo].[pLearningRequirementCompetenceType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.LearningRequirementCompetenceType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.LearningRequirementCompetenceType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.LearningRequirementCompetenceType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fLearningRequirementCompetenceType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pLearningRequirementCompetenceType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.LearningRequirementCompetenceType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.LearningRequirementCompetenceType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.LearningRequirementCompetenceType'-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.LearningRequirementCompetenceType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.LearningRequirementCompetenceType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_LearningRequirementCompetenceType'
		)
		begin
			exec sp_executesql N'alter table dbo.LearningRequirementCompetenceType drop constraint ck_LearningRequirementCompetenceType'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.LearningRequirementCompetenceType with CHECK add constraint ck_LearningRequirementCompetenceType
			check
				(
				dbo.fLearningRequirementCompetenceType#Check
					(
			...
			select @RowCount = count(1) from dbo.LearningRequirementCompetenceType												-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'LearningRequirementCompetenceType'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.LearningRequirementCompetenceTypeSID
				,x.RowGUID
			from
				dbo.LearningRequirementCompetenceType x
			order by
				x.LearningRequirementCompetenceTypeSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fLearningRequirementCompetenceType#Check
							(
							 x.LearningRequirementCompetenceTypeSID
							,x.LearningRequirementSID
							,x.CompetenceTypeSID
							,x.LearningRequirementCompetenceTypeXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.LearningRequirementCompetenceType  x
					where
						x.LearningRequirementCompetenceTypeSID = @nextSID
			...
			alter table dbo.LearningRequirementCompetenceType with NOCHECK add constraint ck_LearningRequirementCompetenceType
			check
				(
				dbo.fLearningRequirementCompetenceType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.LearningRequirementCompetenceType'
		and
			br.BusinessRuleStatus <> 'x'
