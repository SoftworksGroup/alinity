...
GO
CREATE PROCEDURE [dbo].[pPAPSubscription#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PAPSubscription
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PAPSubscription for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PAPSubscription table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPAPSubscription#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPAPSubscription#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PAPSubscription'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PAPSubscription'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PAPSubscription'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PAPSubscription'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PAPSubscription'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PAPSubscription'
		)
		begin
			exec sp_executesql N'alter table dbo.PAPSubscription drop constraint ck_PAPSubscription'			-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PAPSubscription with CHECK add constraint ck_PAPSubscription
			check
				(
				dbo.fPAPSubscription#Check
					(
					 PAPSubscriptionSID
					,PersonSID
					,InstitutionNo
			...
			select @RowCount = count(1) from dbo.PAPSubscription								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PAPSubscription'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.PAPSubscriptionSID
				,x.RowGUID
			from
				dbo.PAPSubscription x
			order by
				x.PAPSubscriptionSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPAPSubscription#Check
							(
							 x.PAPSubscriptionSID
							,x.PersonSID
							,x.InstitutionNo
			...
			alter table dbo.PAPSubscription with NOCHECK add constraint ck_PAPSubscription
			check
				(
				dbo.fPAPSubscription#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PAPSubscription'
		and
			br.BusinessRuleStatus <> 'x'
