...
GO
CREATE PROCEDURE [dbo].[pAgeRange#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.AgeRange
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.AgeRange for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.AgeRange table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fAgeRange#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pAgeRange#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.AgeRange'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.AgeRange'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.AgeRange'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.AgeRange'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.AgeRange'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_AgeRange'
		)
		begin
			exec sp_executesql N'alter table dbo.AgeRange drop constraint ck_AgeRange'										-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.AgeRange with CHECK add constraint ck_AgeRange
			check
				(
				dbo.fAgeRange#Check
					(
			...
			select @RowCount = count(1) from dbo.AgeRange												-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.AgeRange with NOCHECK add constraint ck_AgeRange
			check
				(
				dbo.fAgeRange#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.AgeRange'
		and
			br.BusinessRuleStatus <> 'x'
