...
GO
CREATE PROCEDURE [stg].[pRegistrantExamProfile#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for stg.RegistrantExamProfile
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks stg.RegistrantExamProfile for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the stg.RegistrantExamProfile table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, stg.fRegistrantExamProfile#Check(), on each row. If an error is
...
Example:
	...
	exec stg.pRegistrantExamProfile#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'stg.RegistrantExamProfile'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'stg.RegistrantExamProfile'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'stg.RegistrantExamProfile'						-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'stg.RegistrantExamProfile'
			)
		...
		where
			brx.ApplicationEntitySCD = 'stg.RegistrantExamProfile'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'stg'
		and
			cc.ConstraintName = 'ck_RegistrantExamProfile'
		)
		begin
			exec sp_executesql N'alter table stg.RegistrantExamProfile drop constraint ck_RegistrantExamProfile'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table stg.RegistrantExamProfile with CHECK add constraint ck_RegistrantExamProfile
			check
				(
			...
			select @RowCount = count(1) from stg.RegistrantExamProfile					-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantExamProfile'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrantExamProfileSID
				,x.RowGUID
			from
			...
			begin
				...
				begin try
					...
					select
						@isValid = stg.fRegistrantExamProfile#Check
							(
							 x.RegistrantExamProfileSID
							,x.ImportFileSID
					...
					from
						stg.RegistrantExamProfile  x
					where
						x.RegistrantExamProfileSID = @nextSID
			...
			alter table stg.RegistrantExamProfile with NOCHECK add constraint ck_RegistrantExamProfile
			check
				(
				stg.fRegistrantExamProfile#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'stg.RegistrantExamProfile'
		and
			br.BusinessRuleStatus <> 'x'
