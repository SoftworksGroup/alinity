...
GO
CREATE PROCEDURE [sf].[pTextTemplate#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.TextTemplate
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.TextTemplate for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.TextTemplate table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fTextTemplate#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pTextTemplate#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.TextTemplate'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.TextTemplate'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.TextTemplate'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.TextTemplate'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.TextTemplate'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_TextTemplate'
		)
		begin
			exec sp_executesql N'alter table sf.TextTemplate drop constraint ck_TextTemplate'							-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.TextTemplate with CHECK add constraint ck_TextTemplate
			check
				(
				sf.fTextTemplate#Check
					(
					 TextTemplateSID
					,TextTemplateLabel
					,PriorityLevel
			...
			select @RowCount = count(1) from sf.TextTemplate										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fTextTemplate#Check
							(
							 x.TextTemplateSID
							,x.TextTemplateLabel
							,x.PriorityLevel
			...
			alter table sf.TextTemplate with NOCHECK add constraint ck_TextTemplate
			check
				(
				sf.fTextTemplate#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.TextTemplate'
		and
			br.BusinessRuleStatus <> 'x'
