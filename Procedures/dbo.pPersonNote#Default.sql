...
GO
CREATE PROCEDURE [dbo].[pPersonNote#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPersonNote#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.PersonNote records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PersonNote table. When a new record is to be added from the UI, this procedure
...
columns of the vPersonNote entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pPersonNote
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pPersonNote of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@personNoteSID             int               = -1										-- specific default required by EF - do not override
		,@personSID                 int               = null									-- no default provided from DB constraint - OK to override
		,@personNoteTypeSID         int               = null									-- no default provided from DB constraint - OK to override
		,@noteTitle                 nvarchar(65)      = null									-- no default provided from DB constraint - OK to override
		,@noteContent               nvarchar(max)     = null									-- no default provided from DB constraint - OK to override
		,@showToRegistrant          bit               = (0)										-- default provided from DB constraint - OK to override
		...
		,@personNoteXID             varchar(150)      = null									-- no default provided from DB constraint - OK to override
		...
		,@personNoteTypeLabel       nvarchar(35)      = N'?'									-- not a base table column (default ignored)
		,@personNoteTypeCategory    nvarchar(65)															-- not a base table column (default ignored)
		,@personNoteTypeIsDefault   bit               = 0											-- not a base table column (default ignored)
		,@personNoteTypeIsActive    bit               = 0											-- not a base table column (default ignored)
		,@personNoteTypeRowGUID     uniqueidentifier  = newid()								-- not a base table column (default ignored)
		...
		,@noteDisplayTitle          nvarchar(65)															-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@personNoteTypeSID    = isnull(context.node.value('@PersonNoteTypeSID'  ,'int'          ),@personNoteTypeSID)
			,@noteTitle            = isnull(context.node.value('@NoteTitle'          ,'nvarchar(65)' ),@noteTitle)
			,@noteContent          = isnull(context.node.value('@NoteContent'        ,'nvarchar(max)'),@noteContent)
			...
			,@personNoteXID        = isnull(context.node.value('@PersonNoteXID'      ,'varchar(150)' ),@personNoteXID)
		...
		if isnull(@personNoteTypeSID  ,0) = 0 select @personNoteTypeSID   = x.PersonNoteTypeSID   from dbo.PersonNoteType   x where x.IsDefault = @ON
