...
GO
CREATE PROCEDURE [sf].[pForm#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.Form
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.Form for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.Form table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fForm#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pForm#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.Form'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.Form'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.Form'															-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.Form'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.Form'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_Form'
		)
		begin
			exec sp_executesql N'alter table sf.Form drop constraint ck_Form'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.Form with CHECK add constraint ck_Form
			check
				(
				sf.fForm#Check
					(
			...
			select @RowCount = count(1) from sf.Form														-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.Form with NOCHECK add constraint ck_Form
			check
				(
				sf.fForm#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.Form'
		and
			br.BusinessRuleStatus <> 'x'
