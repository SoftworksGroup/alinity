...
GO
CREATE PROCEDURE [sf].[pRecordAudit#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.RecordAudit
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.RecordAudit for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.RecordAudit table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fRecordAudit#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pRecordAudit#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.RecordAudit'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.RecordAudit'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.RecordAudit'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.RecordAudit'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.RecordAudit'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_RecordAudit'
		)
		begin
			exec sp_executesql N'alter table sf.RecordAudit drop constraint ck_RecordAudit'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.RecordAudit with CHECK add constraint ck_RecordAudit
			check
				(
				sf.fRecordAudit#Check
					(
					 RecordAuditSID
			...
			select @RowCount = count(1) from sf.RecordAudit											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.RecordAudit with NOCHECK add constraint ck_RecordAudit
			check
				(
				sf.fRecordAudit#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.RecordAudit'
		and
			br.BusinessRuleStatus <> 'x'
