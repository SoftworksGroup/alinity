...
GO
CREATE PROCEDURE dbo.pStateProvince#Merge
	@StateProvinceSIDFrom int			-- record key to unassign 
 ,@StateProvinceSIDTo		int			-- replacement record key
 ,@ReturnSelect					bit = 0 -- when 1 result (count) of updates are returned in a message (1 row dataset)
as
/*********************************************************************************************************************************
Sproc    : StateProvince - Merge
Notice   : Copyright © 2019 Softworks Group Inc.
Summary  : Replaces assignments of one StateProvince key value with another to support master table data clean-up
----------------------------------------------------------------------------------------------------------------------------------
...
Comments	
--------
This is a utility procedure called to replace the assignment of one State-Province with another. The procedure is used when 
...
logic that may be configured to apply when State-Province assignments are changed. In most cases application of additional logic is 
...
select top (1)
	@toSID = x.StateProvinceSID
from
	dbo.StateProvince x
where
	x.StateProvinceSID <> @fromSID
order by
	newid();
...
begin
	...
	select
		@updateRequested = y.StateProvinceName + N' -> '
	from
		dbo.StateProvince y
	where
		y.StateProvinceSID = @fromSID;

	select
		@updateRequested += y.StateProvinceName
	from
		dbo.StateProvince y
	where
		y.StateProvinceSID = @toSID;

	select @updateRequested	 UpdateRequested;

	begin transaction;

	exec dbo.pStateProvince#Merge
		@StateProvinceSIDFrom = @fromSID
	 ,@StateProvinceSIDTo = @toSID
	 ,@ReturnSelect = 1;
...
exec sf.pUnitTest#Execute
	 @ObjectName = 'dbo.pStateProvince#Merge'
	,@DefaultTestOnly = 1
------------------------------------------------------------------------------------------------------------------------------- */
...
begin
	...
	begin try
		...
		select @fromLabel =  x.StateProvinceName from	dbo.StateProvince x where x.StateProvinceSID = @StateProvinceSIDFrom
		...
		select @toLabel =  x.StateProvinceName from	dbo.StateProvince x where x.StateProvinceSID = @StateProvinceSIDTo
   ...
   Execute statement below to generate UPDATE syntax for affected tables
	 ...
	 this comment and reformat.
		...
		where
			kcu.COLUMN_NAME = 'StateProvinceSID'; 
		*/

		update
			dbo.City
		set
			StateProvinceSID = @StateProvinceSIDTo
		 ,UpdateUser = @updateUser
		 ,UpdateTime = @now
		where
			StateProvinceSID = @StateProvinceSIDFrom;
