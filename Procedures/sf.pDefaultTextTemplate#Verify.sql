...
GO
CREATE PROCEDURE [sf].[pDefaultTextTemplate#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.DefaultTextTemplate
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.DefaultTextTemplate for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.DefaultTextTemplate table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fDefaultTextTemplate#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pDefaultTextTemplate#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.DefaultTextTemplate'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.DefaultTextTemplate'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.DefaultTextTemplate'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.DefaultTextTemplate'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.DefaultTextTemplate'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_DefaultTextTemplate'
		)
		begin
			exec sp_executesql N'alter table sf.DefaultTextTemplate drop constraint ck_DefaultTextTemplate'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.DefaultTextTemplate with CHECK add constraint ck_DefaultTextTemplate
			check
				(
				sf.fDefaultTextTemplate#Check
					(
			...
			select @RowCount = count(1) from sf.DefaultTextTemplate							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'DefaultTextTemplate'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.DefaultTextTemplateSID
				,x.RowGUID
			from
				sf.DefaultTextTemplate x
			order by
				x.DefaultTextTemplateSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fDefaultTextTemplate#Check
							(
							 x.DefaultTextTemplateSID
							,x.DefaultTextTemplateSCD
							,x.DefaultTextTemplateLabel
							,x.TextTemplateSID
							,x.DefaultTextTemplateXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						sf.DefaultTextTemplate  x
					where
						x.DefaultTextTemplateSID = @nextSID
			...
			alter table sf.DefaultTextTemplate with NOCHECK add constraint ck_DefaultTextTemplate
			check
				(
				sf.fDefaultTextTemplate#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.DefaultTextTemplate'
		and
			br.BusinessRuleStatus <> 'x'
