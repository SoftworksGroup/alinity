...
GO
CREATE PROCEDURE [sf].[pDataSourceApplicationPage#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.DataSourceApplicationPage
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.DataSourceApplicationPage for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.DataSourceApplicationPage table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fDataSourceApplicationPage#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pDataSourceApplicationPage#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.DataSourceApplicationPage'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.DataSourceApplicationPage'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.DataSourceApplicationPage'					-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.DataSourceApplicationPage'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.DataSourceApplicationPage'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_DataSourceApplicationPage'
		)
		begin
			exec sp_executesql N'alter table sf.DataSourceApplicationPage drop constraint ck_DataSourceApplicationPage'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.DataSourceApplicationPage with CHECK add constraint ck_DataSourceApplicationPage
			check
				(
				sf.fDataSourceApplicationPage#Check
					(
			...
			select @RowCount = count(1) from sf.DataSourceApplicationPage				-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'DataSourceApplicationPage'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.DataSourceApplicationPageSID
				,x.RowGUID
			from
				sf.DataSourceApplicationPage x
			order by
				x.DataSourceApplicationPageSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fDataSourceApplicationPage#Check
							(
							 x.DataSourceApplicationPageSID
							,x.DataSourceSID
							,x.ApplicationPageSID
							,x.DataSourceApplicationPageXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						sf.DataSourceApplicationPage  x
					where
						x.DataSourceApplicationPageSID = @nextSID
			...
			alter table sf.DataSourceApplicationPage with NOCHECK add constraint ck_DataSourceApplicationPage
			check
				(
				sf.fDataSourceApplicationPage#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.DataSourceApplicationPage'
		and
			br.BusinessRuleStatus <> 'x'
