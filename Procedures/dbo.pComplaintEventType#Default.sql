...
GO
CREATE PROCEDURE [dbo].[pComplaintEventType#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pComplaintEventType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.ComplaintEventType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.ComplaintEventType table. When a new record is to be added from the UI, this procedure
...
columns of the vComplaintEventType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pComplaintEventType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pComplaintEventType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@complaintEventTypeSID   int               = -1											-- specific default required by EF - do not override
		,@complaintEventTypeSCD   varchar(20)       = null										-- no default provided from DB constraint - OK to override
		,@complaintEventTypeLabel nvarchar(35)      = null										-- no default provided from DB constraint - OK to override
		...
		,@complaintEventTypeXID   varchar(150)      = null										-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @complaintEventTypeSCD    = isnull(context.node.value('@ComplaintEventTypeSCD'  ,'varchar(20)' ),@complaintEventTypeSCD)
			,@complaintEventTypeLabel  = isnull(context.node.value('@ComplaintEventTypeLabel','nvarchar(35)'),@complaintEventTypeLabel)
			,@isDefault                = isnull(context.node.value('@IsDefault'              ,'bit'         ),@isDefault)
			,@complaintEventTypeXID    = isnull(context.node.value('@ComplaintEventTypeXID'  ,'varchar(150)'),@complaintEventTypeXID)
