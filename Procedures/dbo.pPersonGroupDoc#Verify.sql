...
GO
CREATE PROCEDURE [dbo].[pPersonGroupDoc#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PersonGroupDoc
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PersonGroupDoc for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PersonGroupDoc table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPersonGroupDoc#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPersonGroupDoc#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PersonGroupDoc'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PersonGroupDoc'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PersonGroupDoc'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PersonGroupDoc'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PersonGroupDoc'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PersonGroupDoc'
		)
		begin
			exec sp_executesql N'alter table dbo.PersonGroupDoc drop constraint ck_PersonGroupDoc'				-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PersonGroupDoc with CHECK add constraint ck_PersonGroupDoc
			check
				(
				dbo.fPersonGroupDoc#Check
					(
					 PersonGroupDocSID
			...
			select @RowCount = count(1) from dbo.PersonGroupDoc									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PersonGroupDoc'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPersonGroupDoc#Check
							(
							 x.PersonGroupDocSID
			...
			alter table dbo.PersonGroupDoc with NOCHECK add constraint ck_PersonGroupDoc
			check
				(
				dbo.fPersonGroupDoc#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PersonGroupDoc'
		and
			br.BusinessRuleStatus <> 'x'
