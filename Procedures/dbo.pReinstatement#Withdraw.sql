...
GO
CREATE PROCEDURE [dbo].[pReinstatement#Withdraw]
	@ReinstatementSID int	-- key of the change form to withdraw
as
/*********************************************************************************************************************************
Procedure : Reinstatement - Withdraw
Notice    : Copyright © 2018 Softworks Group Inc.
Summary   : Withdraws a reinstatement (reinstatement) form
----------------------------------------------------------------------------------------------------------------------------------
...
Comments	
--------
This procedure withdraws the reinstatement form (typically a reinstatement) and cancels any pending invoices, payments
and registration if it has not yet gone into effect. Note that only System Administrators can withdraw a change form that has been 
paid.  Even then, a reinstatement form cannot be withdrawn after its associated registration has gone active. The 
registration must be terminated manually first.

If the change has been approved, registrants (clients) are not allowed to withdraw it.  "REINSTATEMENT" admin's can perform
...
begin
	...
	where
		x.ReinstatementSID = @ReinstatementSID and x.ReinstatementStatusSCD = 'WITHDRAWN';
...
exec sf.pUnitTest#Execute
	 @ObjectName			= 'dbo.pReinstatement#Withdraw'
	,@DefaultTestOnly = 1
-------------------------------------------------------------------------------------------------------------------------------- */
...
-- SQL Prompt formatting on
		...
		from
			dbo.Reinstatement																									rc
		join
			dbo.Registration																										rl on rc.RegistrationSID = rl.RegistrationSID
		left outer join
			dbo.Registration																										rlNext on rc.RowGUID = rlNext.FormGUID
		outer apply dbo.fReinstatement#CurrentStatus(@ReinstatementSID, -1) cs
		where
			rc.ReinstatementSID = @ReinstatementSID;
		...
		if @FormStatusSCD = 'APPROVED' and sf.fIsGranted('ADMIN.REINSTATEMENT') = @OFF
		begin
