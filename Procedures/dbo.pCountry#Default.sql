...
GO
CREATE PROCEDURE [dbo].[pCountry#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pCountry#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.Country records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.Country table. When a new record is to be added from the UI, this procedure
...
columns of the vCountry entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pCountry
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pCountry of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@countrySID              int               = -1											-- specific default required by EF - do not override
		,@countryName             nvarchar(50)      = null										-- no default provided from DB constraint - OK to override
		,@iSOA2                   char(2)           = null										-- no default provided from DB constraint - OK to override
		,@iSOA3                   char(3)           = null										-- no default provided from DB constraint - OK to override
		,@iSONumber               smallint          = null										-- no default provided from DB constraint - OK to override
		,@isStateProvinceRequired bit               = (0)											-- default provided from DB constraint - OK to override
		...
		,@countryXID              varchar(150)      = null										-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			...
			,@iSOA2                    = isnull(context.node.value('@ISOA2'                  ,'char(2)'     ),@iSOA2)
			,@iSOA3                    = isnull(context.node.value('@ISOA3'                  ,'char(3)'     ),@iSOA3)
			,@iSONumber                = isnull(context.node.value('@ISONumber'              ,'smallint'    ),@iSONumber)
			,@isStateProvinceRequired  = isnull(context.node.value('@IsStateProvinceRequired','bit'         ),@isStateProvinceRequired)
			,@isDefault                = isnull(context.node.value('@IsDefault'              ,'bit'         ),@isDefault)
			,@isActive                 = isnull(context.node.value('@IsActive'               ,'bit'         ),@isActive)
			,@countryXID               = isnull(context.node.value('@CountryXID'             ,'varchar(150)'),@countryXID)
