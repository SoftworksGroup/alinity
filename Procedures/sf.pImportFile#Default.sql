...
GO
CREATE PROCEDURE [sf].[pImportFile#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pImportFile#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.ImportFile records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.ImportFile table. When a new record is to be added from the UI, this procedure
...
columns of the vImportFile entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pImportFile
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pImportFile of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@importFileSID            int               = -1											-- specific default required by EF - do not override
		...
		,@fileName                 nvarchar(100)     = null										-- no default provided from DB constraint - OK to override
		,@fileContent              varbinary(max)    = null										-- no default provided from DB constraint - OK to override
		,@loadStartTime            datetimeoffset(7) = null										-- no default provided from DB constraint - OK to override
		,@loadEndTime              datetimeoffset(7) = null										-- no default provided from DB constraint - OK to override
		,@isFailed                 bit               = (0)										-- default provided from DB constraint - OK to override
		...
		,@importFileXID            varchar(150)      = null										-- no default provided from DB constraint - OK to override
		...
		,@isInProcess              bit																				-- not a base table column (default ignored)
		,@minutesInProcess         int																				-- not a base table column (default ignored)
		,@fileLength               bigint																			-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@fileName              = isnull(context.node.value('@FileName'            ,'nvarchar(100)'    ),@fileName)
			,@fileContent           = isnull(context.node.value('@FileContent'         ,'varbinary(max)'   ),@fileContent)
			,@loadStartTime         = isnull(context.node.value('@LoadStartTime'       ,'datetimeoffset(7)'),@loadStartTime)
			,@loadEndTime           = isnull(context.node.value('@LoadEndTime'         ,'datetimeoffset(7)'),@loadEndTime)
			...
			,@importFileXID         = isnull(context.node.value('@ImportFileXID'       ,'varchar(150)'     ),@importFileXID)
