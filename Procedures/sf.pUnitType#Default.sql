...
GO
CREATE PROCEDURE [sf].[pUnitType#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pUnitType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.UnitType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.UnitType table. When a new record is to be added from the UI, this procedure
...
columns of the vUnitType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pUnitType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pUnitType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@unitTypeSID        int               = -1														-- specific default required by EF - do not override
		,@unitTypeLabel      nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		...
		,@unitTypeXID        varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @unitTypeLabel  = isnull(context.node.value('@UnitTypeLabel','nvarchar(35)'),@unitTypeLabel)
			,@isDefault      = isnull(context.node.value('@IsDefault'    ,'bit'         ),@isDefault)
			,@isActive       = isnull(context.node.value('@IsActive'     ,'bit'         ),@isActive)
			,@unitTypeXID    = isnull(context.node.value('@UnitTypeXID'  ,'varchar(150)'),@unitTypeXID)
