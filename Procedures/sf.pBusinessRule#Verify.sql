...
GO
CREATE PROCEDURE [sf].[pBusinessRule#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.BusinessRule
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.BusinessRule for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.BusinessRule table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fBusinessRule#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pBusinessRule#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.BusinessRule'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.BusinessRule'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.BusinessRule'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.BusinessRule'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.BusinessRule'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_BusinessRule'
		)
		begin
			exec sp_executesql N'alter table sf.BusinessRule drop constraint ck_BusinessRule'							-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.BusinessRule with CHECK add constraint ck_BusinessRule
			check
				(
				sf.fBusinessRule#Check
					(
					 BusinessRuleSID
			...
			select @RowCount = count(1) from sf.BusinessRule										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fBusinessRule#Check
							(
							 x.BusinessRuleSID
							,x.ApplicationEntitySID
							,x.MessageSID
							,x.ColumnName
							,x.BusinessRuleStatus
							,x.BusinessRuleXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
			...
			alter table sf.BusinessRule with NOCHECK add constraint ck_BusinessRule
			check
				(
				sf.fBusinessRule#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.BusinessRule'
		and
			br.BusinessRuleStatus <> 'x'
