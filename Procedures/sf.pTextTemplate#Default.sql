...
GO
CREATE PROCEDURE [sf].[pTextTemplate#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pTextTemplate#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.TextTemplate records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.TextTemplate table. When a new record is to be added from the UI, this procedure
...
columns of the vTextTemplate entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pTextTemplate
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pTextTemplate of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@textTemplateSID           int               = -1										-- specific default required by EF - do not override
		,@textTemplateLabel         nvarchar(35)      = null									-- no default provided from DB constraint - OK to override
		...
		,@textTemplateXID           varchar(150)      = null									-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @textTemplateLabel          = isnull(context.node.value('@TextTemplateLabel'        ,'nvarchar(35)'  ),@textTemplateLabel)
			...
			,@textTemplateXID            = isnull(context.node.value('@TextTemplateXID'          ,'varchar(150)'  ),@textTemplateXID)
