...
GO
CREATE PROCEDURE [dbo].[pLearningRequirementProration#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.LearningRequirementProration
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.LearningRequirementProration for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.LearningRequirementProration table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fLearningRequirementProration#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pLearningRequirementProration#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.LearningRequirementProration'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.LearningRequirementProration'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.LearningRequirementProration'			-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.LearningRequirementProration'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.LearningRequirementProration'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_LearningRequirementProration'
		)
		begin
			exec sp_executesql N'alter table dbo.LearningRequirementProration drop constraint ck_LearningRequirementProration'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.LearningRequirementProration with CHECK add constraint ck_LearningRequirementProration
			check
				(
			...
			select @RowCount = count(1) from dbo.LearningRequirementProration		-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'LearningRequirementProration'
			...
			select
				...
				,x.RowGUID
			from
				dbo.LearningRequirementProration x
			order by
				x.LearningRequirementProrationSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fLearningRequirementProration#Check
							(
							 x.LearningRequirementProrationSID
							,x.LearningRequirementSID
							,x.StartMonthDay
							,x.LearningRequirementMinimum
							,x.PercentageOfFullMinimum
							,x.LearningRequirementProrationXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.LearningRequirementProration  x
					where
						x.LearningRequirementProrationSID = @nextSID
			...
			alter table dbo.LearningRequirementProration with NOCHECK add constraint ck_LearningRequirementProration
			check
				(
				dbo.fLearningRequirementProration#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.LearningRequirementProration'
		and
			br.BusinessRuleStatus <> 'x'
