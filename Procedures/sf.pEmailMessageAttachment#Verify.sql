...
GO
CREATE PROCEDURE [sf].[pEmailMessageAttachment#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.EmailMessageAttachment
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.EmailMessageAttachment for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.EmailMessageAttachment table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fEmailMessageAttachment#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pEmailMessageAttachment#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.EmailMessageAttachment'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.EmailMessageAttachment'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.EmailMessageAttachment'						-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.EmailMessageAttachment'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.EmailMessageAttachment'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_EmailMessageAttachment'
		)
		begin
			exec sp_executesql N'alter table sf.EmailMessageAttachment drop constraint ck_EmailMessageAttachment'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.EmailMessageAttachment with CHECK add constraint ck_EmailMessageAttachment
			check
				(
				sf.fEmailMessageAttachment#Check
					(
			...
			select @RowCount = count(1) from sf.EmailMessageAttachment					-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'EmailMessageAttachment'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.EmailMessageAttachmentSID
				,x.RowGUID
			from
				sf.EmailMessageAttachment x
			order by
				x.EmailMessageAttachmentSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fEmailMessageAttachment#Check
							(
							 x.EmailMessageAttachmentSID
							,x.EmailMessageSID
							,x.DocumentTitle
					...
					from
						sf.EmailMessageAttachment  x
					where
						x.EmailMessageAttachmentSID = @nextSID
			...
			alter table sf.EmailMessageAttachment with NOCHECK add constraint ck_EmailMessageAttachment
			check
				(
				sf.fEmailMessageAttachment#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.EmailMessageAttachment'
		and
			br.BusinessRuleStatus <> 'x'
