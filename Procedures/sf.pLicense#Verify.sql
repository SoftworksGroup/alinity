...
GO
CREATE PROCEDURE [sf].[pLicense#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.License
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.License for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.License table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fLicense#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pLicense#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.License'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.License'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.License'														-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.License'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.License'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_License'
		)
		begin
			exec sp_executesql N'alter table sf.License drop constraint ck_License'												-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.License with CHECK add constraint ck_License
			check
				(
				sf.fLicense#Check
					(
			...
			select @RowCount = count(1) from sf.License													-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.License with NOCHECK add constraint ck_License
			check
				(
				sf.fLicense#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.License'
		and
			br.BusinessRuleStatus <> 'x'
