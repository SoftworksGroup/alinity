...
GO
CREATE PROCEDURE [dbo].[pRegistrationRequirement#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pRegistrationRequirement#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.RegistrationRequirement records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.RegistrationRequirement table. When a new record is to be added from the UI, this procedure
...
columns of the vRegistrationRequirement entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pRegistrationRequirement
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pRegistrationRequirement of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@registrationRequirementSID           int               = -1					-- specific default required by EF - do not override
		,@registrationRequirementTypeSID       int               = null				-- no default provided from DB constraint - OK to override
		,@registrationRequirementLabel         nvarchar(35)      = null				-- no default provided from DB constraint - OK to override
		,@requirementDescription               varbinary(max)    = null				-- no default provided from DB constraint - OK to override
		,@adminGuidance                        varbinary(max)    = null				-- no default provided from DB constraint - OK to override
		...
		,@registrationRequirementXID           varchar(150)      = null				-- no default provided from DB constraint - OK to override
		...
		,@registrationRequirementTypeLabel     nvarchar(35)      = N'?'				-- not a base table column (default ignored)
		,@registrationRequirementTypeCode      varchar(20)       = '?'				-- not a base table column (default ignored)
		,@registrationRequirementTypeCategory  nvarchar(65)										-- not a base table column (default ignored)
		,@isAppliedToPeople                    bit               = 0					-- not a base table column (default ignored)
		,@isAppliedToOrganizations             bit               = 0					-- not a base table column (default ignored)
		,@registrationRequirementTypeIsDefault bit               = 0					-- not a base table column (default ignored)
		,@registrationRequirementTypeIsActive  bit               = 0					-- not a base table column (default ignored)
		,@registrationRequirementTypeRowGUID   uniqueidentifier  = newid()		-- not a base table column (default ignored)
		...
		,@personDocTypeLabel                   nvarchar(35)										-- not a base table column (default ignored)
		,@personDocTypeCategory                nvarchar(65)										-- not a base table column (default ignored)
		,@personDocTypeIsDefault               bit														-- not a base table column (default ignored)
		,@personDocTypeIsActive                bit														-- not a base table column (default ignored)
		,@personDocTypeRowGUID                 uniqueidentifier								-- not a base table column (default ignored)
		...
		,@isDeclaration                        bit														-- not a base table column (default ignored)
		,@isExam                               bit														-- not a base table column (default ignored)
		,@isDocument                           bit														-- not a base table column (default ignored)
	...
	begin try
		...
		if @SetFKDefaults = @ON
		begin
			set @registrationRequirementTypeSID = -1
		end
		...
		select
			 @registrationRequirementTypeSID  = isnull(context.node.value('@RegistrationRequirementTypeSID','int'           ),@registrationRequirementTypeSID)
			,@registrationRequirementLabel    = isnull(context.node.value('@RegistrationRequirementLabel'  ,'nvarchar(35)'  ),@registrationRequirementLabel)
			,@requirementDescription          = isnull(context.node.value('@RequirementDescription'        ,'varbinary(max)'),@requirementDescription)
			,@adminGuidance                   = isnull(context.node.value('@AdminGuidance'                 ,'varbinary(max)'),@adminGuidance)
			...
			,@registrationRequirementXID      = isnull(context.node.value('@RegistrationRequirementXID'    ,'varchar(150)'  ),@registrationRequirementXID)
		...
		if isnull(@registrationRequirementTypeSID,0) = 0 select @registrationRequirementTypeSID = x.RegistrationRequirementTypeSID from dbo.RegistrationRequirementType x where x.IsDefault = @ON
