...
GO
CREATE PROCEDURE [sf].[pUnitTest#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.UnitTest
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.UnitTest for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.UnitTest table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fUnitTest#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pUnitTest#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.UnitTest'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.UnitTest'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.UnitTest'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.UnitTest'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.UnitTest'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_UnitTest'
		)
		begin
			exec sp_executesql N'alter table sf.UnitTest drop constraint ck_UnitTest'											-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.UnitTest with CHECK add constraint ck_UnitTest
			check
				(
				sf.fUnitTest#Check
					(
			...
			select @RowCount = count(1) from sf.UnitTest												-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.UnitTest with NOCHECK add constraint ck_UnitTest
			check
				(
				sf.fUnitTest#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.UnitTest'
		and
			br.BusinessRuleStatus <> 'x'
