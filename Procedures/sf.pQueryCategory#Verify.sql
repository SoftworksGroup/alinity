...
GO
CREATE PROCEDURE [sf].[pQueryCategory#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.QueryCategory
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.QueryCategory for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.QueryCategory table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fQueryCategory#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pQueryCategory#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.QueryCategory'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.QueryCategory'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.QueryCategory'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.QueryCategory'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.QueryCategory'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_QueryCategory'
		)
		begin
			exec sp_executesql N'alter table sf.QueryCategory drop constraint ck_QueryCategory'						-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.QueryCategory with CHECK add constraint ck_QueryCategory
			check
				(
				sf.fQueryCategory#Check
					(
					 QueryCategorySID
					,QueryCategoryLabel
					,QueryCategoryCode
			...
			select @RowCount = count(1) from sf.QueryCategory										-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'QueryCategory'
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fQueryCategory#Check
							(
							 x.QueryCategorySID
			...
			alter table sf.QueryCategory with NOCHECK add constraint ck_QueryCategory
			check
				(
				sf.fQueryCategory#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.QueryCategory'
		and
			br.BusinessRuleStatus <> 'x'
