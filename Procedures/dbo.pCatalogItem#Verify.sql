...
GO
CREATE PROCEDURE [dbo].[pCatalogItem#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.CatalogItem
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.CatalogItem for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.CatalogItem table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fCatalogItem#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pCatalogItem#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.CatalogItem'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.CatalogItem'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.CatalogItem'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.CatalogItem'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.CatalogItem'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_CatalogItem'
		)
		begin
			exec sp_executesql N'alter table dbo.CatalogItem drop constraint ck_CatalogItem'							-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.CatalogItem with CHECK add constraint ck_CatalogItem
			check
				(
			...
			select @RowCount = count(1) from dbo.CatalogItem										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fCatalogItem#Check
							(
							 x.CatalogItemSID
							,x.CatalogItemLabel
							,x.InvoiceItemDescription
							,x.IsLateFee
							,x.ImageAlternateText
							,x.IsAvailableOnClientPortal
							,x.IsComplaintPenalty
							,x.GLAccountSID
							,x.IsTaxRate1Applied
							,x.IsTaxRate2Applied
							,x.IsTaxRate3Applied
			...
			alter table dbo.CatalogItem with NOCHECK add constraint ck_CatalogItem
			check
				(
				dbo.fCatalogItem#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.CatalogItem'
		and
			br.BusinessRuleStatus <> 'x'
