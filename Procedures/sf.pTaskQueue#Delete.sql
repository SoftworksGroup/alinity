...
GO
CREATE PROCEDURE [sf].[pTaskQueue#Delete]
	 @TaskQueueSID                     int               = null -- required! id of row to delete - must be set in custom logic if not passed
...
as
/*********************************************************************************************************************************
Procedure : sf.pTaskQueue#Delete
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : deletes 1 row in the sf.TaskQueue table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.TaskQueue table. The procedure requires a primary key value to locate the record
to delete.
...
and in the ext.pTaskQueue procedure for client-specific deletion rules.
...
begin
	...
	begin try
		...
		if @RowStamp is null select @RowStamp = x.RowStamp from sf.TaskQueue x where x.TaskQueueSID = @TaskQueueSID
		...
		update																																-- set audit details on sf.Task rows that will delete through CASCADE
			sf.Task
		set
