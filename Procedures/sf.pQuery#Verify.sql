...
GO
CREATE PROCEDURE [sf].[pQuery#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.Query
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.Query for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.Query table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fQuery#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pQuery#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.Query'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.Query'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.Query'															-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.Query'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.Query'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_Query'
		)
		begin
			exec sp_executesql N'alter table sf.Query drop constraint ck_Query'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.Query with CHECK add constraint ck_Query
			check
				(
				sf.fQuery#Check
					(
			...
			select @RowCount = count(1) from sf.Query														-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.Query with NOCHECK add constraint ck_Query
			check
				(
				sf.fQuery#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.Query'
		and
			br.BusinessRuleStatus <> 'x'
