...
GO
CREATE PROCEDURE [dbo].[pFieldOfStudy#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pFieldOfStudy#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.FieldOfStudy records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.FieldOfStudy table. When a new record is to be added from the UI, this procedure
...
columns of the vFieldOfStudy entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pFieldOfStudy
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pFieldOfStudy of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@fieldOfStudySID      int               = -1													-- specific default required by EF - do not override
		,@fieldOfStudyName     nvarchar(50)      = null												-- no default provided from DB constraint - OK to override
		,@fieldOfStudyCode     varchar(20)       = null												-- no default provided from DB constraint - OK to override
		,@fieldOfStudyCategory nvarchar(65)      = null												-- no default provided from DB constraint - OK to override
		...
		,@fieldOfStudyXID      varchar(150)      = null												-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @fieldOfStudyName      = isnull(context.node.value('@FieldOfStudyName'    ,'nvarchar(50)'),@fieldOfStudyName)
			,@fieldOfStudyCode      = isnull(context.node.value('@FieldOfStudyCode'    ,'varchar(20)' ),@fieldOfStudyCode)
			,@fieldOfStudyCategory  = isnull(context.node.value('@FieldOfStudyCategory','nvarchar(65)'),@fieldOfStudyCategory)
			,@isDefault             = isnull(context.node.value('@IsDefault'           ,'bit'         ),@isDefault)
			,@isActive              = isnull(context.node.value('@IsActive'            ,'bit'         ),@isActive)
			,@fieldOfStudyXID       = isnull(context.node.value('@FieldOfStudyXID'     ,'varchar(150)'),@fieldOfStudyXID)
