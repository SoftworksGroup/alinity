...
GO
CREATE PROCEDURE [sf].[pMessageLink#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.MessageLink
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.MessageLink for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.MessageLink table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fMessageLink#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pMessageLink#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.MessageLink'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.MessageLink'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.MessageLink'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.MessageLink'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.MessageLink'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_MessageLink'
		)
		begin
			exec sp_executesql N'alter table sf.MessageLink drop constraint ck_MessageLink'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.MessageLink with CHECK add constraint ck_MessageLink
			check
				(
				sf.fMessageLink#Check
					(
					 MessageLinkSID
					,MessageLinkSCD
					,MessageLinkLabel
			...
			select @RowCount = count(1) from sf.MessageLink											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.MessageLink with NOCHECK add constraint ck_MessageLink
			check
				(
				sf.fMessageLink#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.MessageLink'
		and
			br.BusinessRuleStatus <> 'x'
