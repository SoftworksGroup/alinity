...
GO
CREATE PROCEDURE [dbo].[pRegistrationSnapshotType#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pRegistrationSnapshotType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.RegistrationSnapshotType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.RegistrationSnapshotType table. When a new record is to be added from the UI, this procedure
...
columns of the vRegistrationSnapshotType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pRegistrationSnapshotType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pRegistrationSnapshotType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@registrationSnapshotTypeSID       int               = -1						-- specific default required by EF - do not override
		,@registrationSnapshotTypeLabel     nvarchar(35)      = null					-- no default provided from DB constraint - OK to override
		,@registrationSnapshotTypeSCD       varchar(15)       = null					-- no default provided from DB constraint - OK to override
		,@registrationSnapshotLabelTemplate nvarchar(50)      = null					-- no default provided from DB constraint - OK to override
		...
		,@registrationSnapshotTypeXID       varchar(150)      = null					-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @registrationSnapshotTypeLabel      = isnull(context.node.value('@RegistrationSnapshotTypeLabel'    ,'nvarchar(35)'),@registrationSnapshotTypeLabel)
			,@registrationSnapshotTypeSCD        = isnull(context.node.value('@RegistrationSnapshotTypeSCD'      ,'varchar(15)' ),@registrationSnapshotTypeSCD)
			,@registrationSnapshotLabelTemplate  = isnull(context.node.value('@RegistrationSnapshotLabelTemplate','nvarchar(50)'),@registrationSnapshotLabelTemplate)
			,@isDefault                          = isnull(context.node.value('@IsDefault'                        ,'bit'         ),@isDefault)
			,@isActive                           = isnull(context.node.value('@IsActive'                         ,'bit'         ),@isActive)
			,@registrationSnapshotTypeXID        = isnull(context.node.value('@RegistrationSnapshotTypeXID'      ,'varchar(150)'),@registrationSnapshotTypeXID)
