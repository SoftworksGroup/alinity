...
GO
CREATE PROCEDURE [dbo].[pExamStatus#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pExamStatus#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.ExamStatus records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.ExamStatus table. When a new record is to be added from the UI, this procedure
...
columns of the vExamStatus entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pExamStatus
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pExamStatus of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@examStatusSID      int               = -1														-- specific default required by EF - do not override
		,@examStatusSCD      varchar(15)       = null													-- no default provided from DB constraint - OK to override
		,@examStatusLabel    nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		...
		,@examStatusXID      varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @examStatusSCD    = isnull(context.node.value('@ExamStatusSCD'  ,'varchar(15)' ),@examStatusSCD)
			,@examStatusLabel  = isnull(context.node.value('@ExamStatusLabel','nvarchar(35)'),@examStatusLabel)
			,@sequence         = isnull(context.node.value('@Sequence'       ,'int'         ),@sequence)
			,@isDefault        = isnull(context.node.value('@IsDefault'      ,'bit'         ),@isDefault)
			,@examStatusXID    = isnull(context.node.value('@ExamStatusXID'  ,'varchar(150)'),@examStatusXID)
