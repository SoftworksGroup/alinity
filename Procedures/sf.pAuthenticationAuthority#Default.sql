...
GO
CREATE PROCEDURE [sf].[pAuthenticationAuthority#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pAuthenticationAuthority#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.AuthenticationAuthority records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.AuthenticationAuthority table. When a new record is to be added from the UI, this procedure
...
columns of the vAuthenticationAuthority entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pAuthenticationAuthority
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pAuthenticationAuthority of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@authenticationAuthoritySID   int               = -1									-- specific default required by EF - do not override
		,@authenticationAuthoritySCD   varchar(10)       = null								-- no default provided from DB constraint - OK to override
		,@authenticationAuthorityLabel nvarchar(35)      = null								-- no default provided from DB constraint - OK to override
		...
		,@authenticationAuthorityXID   varchar(150)      = null								-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @authenticationAuthoritySCD    = isnull(context.node.value('@AuthenticationAuthoritySCD'  ,'varchar(10)'  ),@authenticationAuthoritySCD)
			,@authenticationAuthorityLabel  = isnull(context.node.value('@AuthenticationAuthorityLabel','nvarchar(35)' ),@authenticationAuthorityLabel)
			...
			,@authenticationAuthorityXID    = isnull(context.node.value('@AuthenticationAuthorityXID'  ,'varchar(150)' ),@authenticationAuthorityXID)
