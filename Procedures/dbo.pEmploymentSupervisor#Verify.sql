...
GO
CREATE PROCEDURE [dbo].[pEmploymentSupervisor#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.EmploymentSupervisor
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.EmploymentSupervisor for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.EmploymentSupervisor table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fEmploymentSupervisor#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pEmploymentSupervisor#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.EmploymentSupervisor'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.EmploymentSupervisor'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.EmploymentSupervisor'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.EmploymentSupervisor'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.EmploymentSupervisor'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_EmploymentSupervisor'
		)
		begin
			exec sp_executesql N'alter table dbo.EmploymentSupervisor drop constraint ck_EmploymentSupervisor'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.EmploymentSupervisor with CHECK add constraint ck_EmploymentSupervisor
			check
				(
				dbo.fEmploymentSupervisor#Check
					(
			...
			select @RowCount = count(1) from dbo.EmploymentSupervisor						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'EmploymentSupervisor'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.EmploymentSupervisorSID
				,x.RowGUID
			from
				dbo.EmploymentSupervisor x
			order by
				x.EmploymentSupervisorSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fEmploymentSupervisor#Check
							(
							 x.EmploymentSupervisorSID
							,x.RegistrantEmploymentSID
							,x.PersonSID
							,x.ExpiryTime
							,x.EmploymentSupervisorXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.EmploymentSupervisor  x
					where
						x.EmploymentSupervisorSID = @nextSID
			...
			alter table dbo.EmploymentSupervisor with NOCHECK add constraint ck_EmploymentSupervisor
			check
				(
				dbo.fEmploymentSupervisor#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.EmploymentSupervisor'
		and
			br.BusinessRuleStatus <> 'x'
