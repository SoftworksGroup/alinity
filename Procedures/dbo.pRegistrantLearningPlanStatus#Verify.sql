...
GO
CREATE PROCEDURE [dbo].[pRegistrantLearningPlanStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantLearningPlanStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantLearningPlanStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantLearningPlanStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantLearningPlanStatus#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantLearningPlanStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantLearningPlanStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantLearningPlanStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantLearningPlanStatus'			-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantLearningPlanStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantLearningPlanStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantLearningPlanStatus'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantLearningPlanStatus drop constraint ck_RegistrantLearningPlanStatus'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantLearningPlanStatus with CHECK add constraint ck_RegistrantLearningPlanStatus
			check
				(
				dbo.fRegistrantLearningPlanStatus#Check
					(
					 RegistrantLearningPlanStatusSID
					,RegistrantLearningPlanSID
					,FormStatusSID
					,RegistrantLearningPlanStatusXID
					,LegacyKey
					,IsDeleted
					,CreateUser
					,CreateTime
			...
			select @RowCount = count(1) from dbo.RegistrantLearningPlanStatus		-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantLearningPlanStatus'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrantLearningPlanStatusSID
				,x.RowGUID
			from
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantLearningPlanStatus#Check
							(
							 x.RegistrantLearningPlanStatusSID
							,x.RegistrantLearningPlanSID
							,x.FormStatusSID
							,x.RegistrantLearningPlanStatusXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantLearningPlanStatus  x
					where
						x.RegistrantLearningPlanStatusSID = @nextSID
			...
			alter table dbo.RegistrantLearningPlanStatus with NOCHECK add constraint ck_RegistrantLearningPlanStatus
			check
				(
				dbo.fRegistrantLearningPlanStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantLearningPlanStatus'
		and
			br.BusinessRuleStatus <> 'x'
