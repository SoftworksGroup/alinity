...
GO
CREATE PROCEDURE [dbo].[pLearningPlanActivity#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.LearningPlanActivity
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.LearningPlanActivity for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.LearningPlanActivity table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fLearningPlanActivity#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pLearningPlanActivity#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.LearningPlanActivity'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.LearningPlanActivity'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.LearningPlanActivity'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.LearningPlanActivity'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.LearningPlanActivity'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_LearningPlanActivity'
		)
		begin
			exec sp_executesql N'alter table dbo.LearningPlanActivity drop constraint ck_LearningPlanActivity'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.LearningPlanActivity with CHECK add constraint ck_LearningPlanActivity
			check
				(
			...
			select @RowCount = count(1) from dbo.LearningPlanActivity						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'LearningPlanActivity'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.LearningPlanActivitySID
				,x.RowGUID
			from
				dbo.LearningPlanActivity x
			order by
				x.LearningPlanActivitySID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fLearningPlanActivity#Check
							(
							 x.LearningPlanActivitySID
							,x.RegistrantLearningPlanSID
							,x.CompetenceTypeActivitySID
							,x.UnitValue
							,x.CarryOverUnitValue
							,x.ActivityDate
							,x.LearningClaimTypeSID
							,x.LearningPlanActivityCategory
							,x.PlannedCompletion
							,x.OrgSID
							,x.IsSubjectToReview
							,x.IsArchived
							,x.LearningPlanActivityXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.LearningPlanActivity  x
					where
						x.LearningPlanActivitySID = @nextSID
			...
			alter table dbo.LearningPlanActivity with NOCHECK add constraint ck_LearningPlanActivity
			check
				(
				dbo.fLearningPlanActivity#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.LearningPlanActivity'
		and
			br.BusinessRuleStatus <> 'x'
