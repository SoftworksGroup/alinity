...
GO
CREATE PROCEDURE [sf].[pAuthenticationAuthority#Insert]
	 @AuthenticationAuthoritySID   int               = null output					-- identity value assigned to the new record
	,@AuthenticationAuthoritySCD   varchar(10)       = null									-- required! if not passed value must be set in custom logic prior to insert
	,@AuthenticationAuthorityLabel nvarchar(35)      = null									-- required! if not passed value must be set in custom logic prior to insert
...
as
/*********************************************************************************************************************************
Procedure : sf.pAuthenticationAuthority#Insert
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : inserts 1 row into the sf.AuthenticationAuthority table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.AuthenticationAuthority table with parameter values represent columns of the table. Additional
parameters are provided for all columns in the vAuthenticationAuthority entity view, however, the base logic of the procedure inserts the
...
begin
	...
	set @AuthenticationAuthoritySID = null																	-- initialize output parameter
	...
	begin try
		...
		begin
			...
			select @recordSID = x.AuthenticationAuthoritySID from sf.AuthenticationAuthority x where x.IsDefault = @ON
		...
		select
			...
			,@AuthenticationAuthoritySID = scope_identity()											-- capture key value of new row
