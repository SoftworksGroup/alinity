...
GO
CREATE PROCEDURE [dbo].[pRegistrantAppReviewStatus#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pRegistrantAppReviewStatus#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.RegistrantAppReviewStatus records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.RegistrantAppReviewStatus table. When a new record is to be added from the UI, this procedure
...
columns of the vRegistrantAppReviewStatus entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pRegistrantAppReviewStatus
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pRegistrantAppReviewStatus of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@registrantAppReviewStatusSID int               = -1									-- specific default required by EF - do not override
		,@registrantAppReviewSID       int               = null								-- no default provided from DB constraint - OK to override
		...
		,@registrantAppReviewStatusXID varchar(150)      = null								-- no default provided from DB constraint - OK to override
		...
		,@registrantAppSID             int               = 0									-- not a base table column (default ignored)
		...
		,@registrantAppReviewRowGUID   uniqueidentifier  = newid()						-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @registrantAppReviewSID        = isnull(context.node.value('@RegistrantAppReviewSID'      ,'int'         ),@registrantAppReviewSID)
			,@formStatusSID                 = isnull(context.node.value('@FormStatusSID'               ,'int'         ),@formStatusSID)
			,@registrantAppReviewStatusXID  = isnull(context.node.value('@RegistrantAppReviewStatusXID','varchar(150)'),@registrantAppReviewStatusXID)
