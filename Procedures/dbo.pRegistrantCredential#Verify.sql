...
GO
CREATE PROCEDURE [dbo].[pRegistrantCredential#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantCredential
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantCredential for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantCredential table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantCredential#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantCredential#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantCredential'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantCredential'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantCredential'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantCredential'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantCredential'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantCredential'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantCredential drop constraint ck_RegistrantCredential'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantCredential with CHECK add constraint ck_RegistrantCredential
			check
				(
			...
			select @RowCount = count(1) from dbo.RegistrantCredential						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantCredential'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrantCredentialSID
				,x.RowGUID
			from
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantCredential#Check
							(
							 x.RegistrantCredentialSID
							,x.RegistrantSID
							,x.CredentialSID
							...
							,x.ProgramName
							,x.ProgramStartDate
							,x.ProgramTargetCompletionDate
							,x.EffectiveTime
							,x.ExpiryTime
							,x.FieldOfStudySID
							,x.RegistrantCredentialXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantCredential  x
					where
						x.RegistrantCredentialSID = @nextSID
			...
			alter table dbo.RegistrantCredential with NOCHECK add constraint ck_RegistrantCredential
			check
				(
				dbo.fRegistrantCredential#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantCredential'
		and
			br.BusinessRuleStatus <> 'x'
