...
GO
CREATE PROCEDURE [dbo].[pIdentifierType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.IdentifierType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.IdentifierType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.IdentifierType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fIdentifierType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pIdentifierType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.IdentifierType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.IdentifierType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.IdentifierType'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.IdentifierType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.IdentifierType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_IdentifierType'
		)
		begin
			exec sp_executesql N'alter table dbo.IdentifierType drop constraint ck_IdentifierType'				-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.IdentifierType with CHECK add constraint ck_IdentifierType
			check
				(
				dbo.fIdentifierType#Check
					(
					 IdentifierTypeSID
					,OrgSID
					,IdentifierTypeLabel
			...
			select @RowCount = count(1) from dbo.IdentifierType									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'IdentifierType'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fIdentifierType#Check
							(
							 x.IdentifierTypeSID
							,x.OrgSID
							,x.IdentifierTypeLabel
							,x.IdentifierTypeCategory
							,x.IsOtherRegistration
							,x.DisplayRank
							,x.EditMask
							,x.IdentifierCode
							,x.IsDefault
			...
			alter table dbo.IdentifierType with NOCHECK add constraint ck_IdentifierType
			check
				(
				dbo.fIdentifierType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.IdentifierType'
		and
			br.BusinessRuleStatus <> 'x'
