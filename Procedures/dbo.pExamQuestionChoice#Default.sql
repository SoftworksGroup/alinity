...
GO
CREATE PROCEDURE [dbo].[pExamQuestionChoice#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pExamQuestionChoice#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.ExamQuestionChoice records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.ExamQuestionChoice table. When a new record is to be added from the UI, this procedure
...
columns of the vExamQuestionChoice entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pExamQuestionChoice
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pExamQuestionChoice of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@examQuestionChoiceSID int               = -1												-- specific default required by EF - do not override
		,@examQuestionSID       int               = null											-- no default provided from DB constraint - OK to override
		,@choiceText            nvarchar(max)     = null											-- no default provided from DB constraint - OK to override
		,@sequence              tinyint           = (0)												-- default provided from DB constraint - OK to override
		,@isCorrectAnswer       bit               = CONVERT(bit,(0))					-- default provided from DB constraint - OK to override
		...
		,@examQuestionChoiceXID varchar(150)      = null											-- no default provided from DB constraint - OK to override
		...
		,@examQuestionSequence  smallint          = 0													-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@choiceText             = isnull(context.node.value('@ChoiceText'           ,'nvarchar(max)'),@choiceText)
			,@sequence               = isnull(context.node.value('@Sequence'             ,'tinyint'      ),@sequence)
			,@isCorrectAnswer        = isnull(context.node.value('@IsCorrectAnswer'      ,'bit'          ),@isCorrectAnswer)
			,@isActive               = isnull(context.node.value('@IsActive'             ,'bit'          ),@isActive)
			,@examQuestionChoiceXID  = isnull(context.node.value('@ExamQuestionChoiceXID','varchar(150)' ),@examQuestionChoiceXID)
