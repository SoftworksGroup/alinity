...
GO
CREATE PROCEDURE [dbo].[pPracticeScope#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPracticeScope#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.PracticeScope records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PracticeScope table. When a new record is to be added from the UI, this procedure
...
columns of the vPracticeScope entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pPracticeScope
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pPracticeScope of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@practiceScopeSID   int               = -1														-- specific default required by EF - do not override
		,@practiceScopeName  nvarchar(50)      = null													-- no default provided from DB constraint - OK to override
		,@practiceScopeCode  varchar(20)       = null													-- no default provided from DB constraint - OK to override
		...
		,@practiceScopeXID   varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @practiceScopeName  = isnull(context.node.value('@PracticeScopeName','nvarchar(50)'),@practiceScopeName)
			,@practiceScopeCode  = isnull(context.node.value('@PracticeScopeCode','varchar(20)' ),@practiceScopeCode)
			,@isDefault          = isnull(context.node.value('@IsDefault'        ,'bit'         ),@isDefault)
			,@isActive           = isnull(context.node.value('@IsActive'         ,'bit'         ),@isActive)
			,@practiceScopeXID   = isnull(context.node.value('@PracticeScopeXID' ,'varchar(150)'),@practiceScopeXID)
