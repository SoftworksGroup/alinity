...
GO
CREATE PROCEDURE [sf].[pProcessingStatus#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pProcessingStatus#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.ProcessingStatus records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.ProcessingStatus table. When a new record is to be added from the UI, this procedure
...
columns of the vProcessingStatus entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pProcessingStatus
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pProcessingStatus of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@processingStatusSID   int               = -1												-- specific default required by EF - do not override
		,@processingStatusSCD   varchar(10)       = null											-- no default provided from DB constraint - OK to override
		,@processingStatusLabel nvarchar(35)      = null											-- no default provided from DB constraint - OK to override
		...
		,@processingStatusXID   varchar(150)      = null											-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @processingStatusSCD    = isnull(context.node.value('@ProcessingStatusSCD'  ,'varchar(10)'  ),@processingStatusSCD)
			,@processingStatusLabel  = isnull(context.node.value('@ProcessingStatusLabel','nvarchar(35)' ),@processingStatusLabel)
			...
			,@processingStatusXID    = isnull(context.node.value('@ProcessingStatusXID'  ,'varchar(150)' ),@processingStatusXID)
