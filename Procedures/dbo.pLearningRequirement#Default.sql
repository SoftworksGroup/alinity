...
GO
CREATE PROCEDURE [dbo].[pLearningRequirement#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pLearningRequirement#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.LearningRequirement records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.LearningRequirement table. When a new record is to be added from the UI, this procedure
...
columns of the vLearningRequirement entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pLearningRequirement
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pLearningRequirement of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@learningRequirementSID          int               = -1							-- specific default required by EF - do not override
		,@practiceRegisterSID             int               = null						-- no default provided from DB constraint - OK to override
		,@learningRequirementLabel        nvarchar(35)      = null						-- no default provided from DB constraint - OK to override
		,@startingRegistrationYear        smallint          = sf.fTodayYear()	-- default provided from DB constraint - OK to override
		,@minimum                         decimal(5,2)      = (1.0)						-- default provided from DB constraint - OK to override
		,@maximum                         decimal(5,2)      = (999.9)					-- default provided from DB constraint - OK to override
		,@maximumCarryOver                decimal(5,2)      = (999.9)					-- default provided from DB constraint - OK to override
		,@userDefinedColumns              xml               = null						-- no default provided from DB constraint - OK to override
		,@learningRequirementXID          varchar(150)      = null						-- no default provided from DB constraint - OK to override
		...
		,@unitTypeLabel                   nvarchar(35)												-- not a base table column (default ignored)
		,@learningRequirementProrationSID int																	-- not a base table column (default ignored)
		,@currentRequiredUnits            decimal(5,2)												-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@learningRequirementLabel  = isnull(context.node.value('@LearningRequirementLabel','nvarchar(35)'),@learningRequirementLabel)
			,@startingRegistrationYear  = isnull(context.node.value('@StartingRegistrationYear','smallint'    ),@startingRegistrationYear)
			,@minimum                   = isnull(context.node.value('@Minimum'                 ,'decimal(5,2)'),@minimum)
			,@maximum                   = isnull(context.node.value('@Maximum'                 ,'decimal(5,2)'),@maximum)
			,@maximumCarryOver          = isnull(context.node.value('@MaximumCarryOver'        ,'decimal(5,2)'),@maximumCarryOver)
			,@learningRequirementXID    = isnull(context.node.value('@LearningRequirementXID'  ,'varchar(150)'),@learningRequirementXID)
