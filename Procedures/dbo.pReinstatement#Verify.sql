...
GO
CREATE PROCEDURE [dbo].[pReinstatement#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.Reinstatement
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.Reinstatement for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.Reinstatement table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fReinstatement#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pReinstatement#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.Reinstatement'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.Reinstatement'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.Reinstatement'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.Reinstatement'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.Reinstatement'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_Reinstatement'
		)
		begin
			exec sp_executesql N'alter table dbo.Reinstatement drop constraint ck_Reinstatement'					-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.Reinstatement with CHECK add constraint ck_Reinstatement
			check
				(
			...
			select @RowCount = count(1) from dbo.Reinstatement									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'Reinstatement'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fReinstatement#Check
							(
							 x.ReinstatementSID
							,x.RegistrationSID
			...
			alter table dbo.Reinstatement with NOCHECK add constraint ck_Reinstatement
			check
				(
				dbo.fReinstatement#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.Reinstatement'
		and
			br.BusinessRuleStatus <> 'x'
