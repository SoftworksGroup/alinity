...
GO
CREATE PROCEDURE [dbo].[pRegistrantAudit#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantAudit
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantAudit for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantAudit table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantAudit#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantAudit#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantAudit'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantAudit'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAudit'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAudit'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantAudit'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantAudit'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantAudit drop constraint ck_RegistrantAudit'			-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantAudit with CHECK add constraint ck_RegistrantAudit
			check
				(
			...
			select @RowCount = count(1) from dbo.RegistrantAudit								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantAudit'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantAudit#Check
							(
							 x.RegistrantAuditSID
							,x.RegistrantSID
							,x.AuditTypeSID
			...
			alter table dbo.RegistrantAudit with NOCHECK add constraint ck_RegistrantAudit
			check
				(
				dbo.fRegistrantAudit#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantAudit'
		and
			br.BusinessRuleStatus <> 'x'
