...
GO
CREATE PROCEDURE [dbo].[pPersonNoteContext#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PersonNoteContext
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PersonNoteContext for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PersonNoteContext table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPersonNoteContext#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPersonNoteContext#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PersonNoteContext'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PersonNoteContext'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PersonNoteContext'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PersonNoteContext'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PersonNoteContext'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PersonNoteContext'
		)
		begin
			exec sp_executesql N'alter table dbo.PersonNoteContext drop constraint ck_PersonNoteContext'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PersonNoteContext with CHECK add constraint ck_PersonNoteContext
			check
				(
				dbo.fPersonNoteContext#Check
					(
					 PersonNoteContextSID
					,PersonNoteSID
					,ApplicationEntitySID
					,EntitySID
					,PersonNoteContextXID
					,LegacyKey
					,IsDeleted
					,CreateUser
					,CreateTime
			...
			select @RowCount = count(1) from dbo.PersonNoteContext							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PersonNoteContext'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.PersonNoteContextSID
				,x.RowGUID
			from
				dbo.PersonNoteContext x
			order by
				x.PersonNoteContextSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPersonNoteContext#Check
							(
							 x.PersonNoteContextSID
							,x.PersonNoteSID
							,x.ApplicationEntitySID
							,x.EntitySID
							,x.PersonNoteContextXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.PersonNoteContext  x
					where
						x.PersonNoteContextSID = @nextSID
			...
			alter table dbo.PersonNoteContext with NOCHECK add constraint ck_PersonNoteContext
			check
				(
				dbo.fPersonNoteContext#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PersonNoteContext'
		and
			br.BusinessRuleStatus <> 'x'
