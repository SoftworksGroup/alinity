...
GO
CREATE PROCEDURE [dbo].[pPracticeRegisterSection#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PracticeRegisterSection
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PracticeRegisterSection for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PracticeRegisterSection table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPracticeRegisterSection#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPracticeRegisterSection#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PracticeRegisterSection'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PracticeRegisterSection'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRegisterSection'					-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRegisterSection'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PracticeRegisterSection'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PracticeRegisterSection'
		)
		begin
			exec sp_executesql N'alter table dbo.PracticeRegisterSection drop constraint ck_PracticeRegisterSection'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PracticeRegisterSection with CHECK add constraint ck_PracticeRegisterSection
			check
				(
				dbo.fPracticeRegisterSection#Check
					(
			...
			select @RowCount = count(1) from dbo.PracticeRegisterSection				-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PracticeRegisterSection'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPracticeRegisterSection#Check
							(
							 x.PracticeRegisterSectionSID
					...
					from
						dbo.PracticeRegisterSection  x
					where
						x.PracticeRegisterSectionSID = @nextSID
			...
			alter table dbo.PracticeRegisterSection with NOCHECK add constraint ck_PracticeRegisterSection
			check
				(
				dbo.fPracticeRegisterSection#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PracticeRegisterSection'
		and
			br.BusinessRuleStatus <> 'x'
