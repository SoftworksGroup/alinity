...
GO
CREATE PROCEDURE [sf].[pEmailTemplate#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.EmailTemplate
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.EmailTemplate for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.EmailTemplate table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fEmailTemplate#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pEmailTemplate#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.EmailTemplate'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.EmailTemplate'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.EmailTemplate'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.EmailTemplate'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.EmailTemplate'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_EmailTemplate'
		)
		begin
			exec sp_executesql N'alter table sf.EmailTemplate drop constraint ck_EmailTemplate'						-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.EmailTemplate with CHECK add constraint ck_EmailTemplate
			check
				(
				sf.fEmailTemplate#Check
					(
					 EmailTemplateSID
					,EmailTemplateLabel
					,PriorityLevel
			...
			select @RowCount = count(1) from sf.EmailTemplate										-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'EmailTemplate'
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fEmailTemplate#Check
							(
							 x.EmailTemplateSID
							,x.EmailTemplateLabel
							,x.PriorityLevel
							,x.Subject
							,x.IsApplicationUserRequired
							,x.LinkExpiryHours
			...
			alter table sf.EmailTemplate with NOCHECK add constraint ck_EmailTemplate
			check
				(
				sf.fEmailTemplate#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.EmailTemplate'
		and
			br.BusinessRuleStatus <> 'x'
