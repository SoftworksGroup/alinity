...
GO
CREATE PROCEDURE [dbo].[pExamStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ExamStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ExamStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ExamStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fExamStatus#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pExamStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ExamStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ExamStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ExamStatus'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ExamStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ExamStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ExamStatus'
		)
		begin
			exec sp_executesql N'alter table dbo.ExamStatus drop constraint ck_ExamStatus'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ExamStatus with CHECK add constraint ck_ExamStatus
			check
				(
				dbo.fExamStatus#Check
					(
					 ExamStatusSID
					,ExamStatusSCD
					,ExamStatusLabel
			...
			select @RowCount = count(1) from dbo.ExamStatus											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.ExamStatus with NOCHECK add constraint ck_ExamStatus
			check
				(
				dbo.fExamStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ExamStatus'
		and
			br.BusinessRuleStatus <> 'x'
