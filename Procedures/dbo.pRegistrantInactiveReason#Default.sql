...
GO
CREATE PROCEDURE [dbo].[pRegistrantInactiveReason#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pRegistrantInactiveReason#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.RegistrantInactiveReason records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.RegistrantInactiveReason table. When a new record is to be added from the UI, this procedure
...
columns of the vRegistrantInactiveReason entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pRegistrantInactiveReason
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pRegistrantInactiveReason of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@registrantInactiveReasonSID   int               = -1								-- specific default required by EF - do not override
		,@registrantInactiveReasonLabel nvarchar(35)      = null							-- no default provided from DB constraint - OK to override
		...
		,@registrantInactiveReasonXID   varchar(150)      = null							-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @registrantInactiveReasonLabel  = isnull(context.node.value('@RegistrantInactiveReasonLabel','nvarchar(35)'),@registrantInactiveReasonLabel)
			,@isActive                       = isnull(context.node.value('@IsActive'                     ,'bit'         ),@isActive)
			,@registrantInactiveReasonXID    = isnull(context.node.value('@RegistrantInactiveReasonXID'  ,'varchar(150)'),@registrantInactiveReasonXID)
