...
GO
CREATE PROCEDURE [sf].[pNamePrefix#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.NamePrefix
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.NamePrefix for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.NamePrefix table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fNamePrefix#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pNamePrefix#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.NamePrefix'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.NamePrefix'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.NamePrefix'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.NamePrefix'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.NamePrefix'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_NamePrefix'
		)
		begin
			exec sp_executesql N'alter table sf.NamePrefix drop constraint ck_NamePrefix'									-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.NamePrefix with CHECK add constraint ck_NamePrefix
			check
				(
				sf.fNamePrefix#Check
					(
					 NamePrefixSID
					,NamePrefixLabel
					,IsActive
			...
			select @RowCount = count(1) from sf.NamePrefix											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.NamePrefix with NOCHECK add constraint ck_NamePrefix
			check
				(
				sf.fNamePrefix#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.NamePrefix'
		and
			br.BusinessRuleStatus <> 'x'
