...
GO
CREATE PROCEDURE [dbo].[pEmploymentRole#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.EmploymentRole
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.EmploymentRole for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.EmploymentRole table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fEmploymentRole#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pEmploymentRole#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.EmploymentRole'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.EmploymentRole'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.EmploymentRole'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.EmploymentRole'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.EmploymentRole'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_EmploymentRole'
		)
		begin
			exec sp_executesql N'alter table dbo.EmploymentRole drop constraint ck_EmploymentRole'				-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.EmploymentRole with CHECK add constraint ck_EmploymentRole
			check
				(
				dbo.fEmploymentRole#Check
					(
					 EmploymentRoleSID
					,EmploymentRoleName
					,EmploymentRoleCode
			...
			select @RowCount = count(1) from dbo.EmploymentRole									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'EmploymentRole'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fEmploymentRole#Check
							(
							 x.EmploymentRoleSID
							,x.EmploymentRoleName
							,x.EmploymentRoleCode
							,x.IsDefault
							,x.IsActive
							,x.EmploymentRoleXID
							,x.LegacyKey
			...
			alter table dbo.EmploymentRole with NOCHECK add constraint ck_EmploymentRole
			check
				(
				dbo.fEmploymentRole#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.EmploymentRole'
		and
			br.BusinessRuleStatus <> 'x'
