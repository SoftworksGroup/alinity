...
GO
CREATE PROCEDURE [dbo].[pPersonNoteContext#Insert]
	 @PersonNoteContextSID     int               = null output							-- identity value assigned to the new record
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPersonNoteContext#Insert
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : inserts 1 row into the dbo.PersonNoteContext table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PersonNoteContext table with parameter values represent columns of the table. Additional
parameters are provided for all columns in the vPersonNoteContext entity view, however, the base logic of the procedure inserts the
...
begin
	...
	set @PersonNoteContextSID = null																				-- initialize output parameter
	...
	begin try
		...
		select
			...
			,@PersonNoteContextSID = scope_identity()														-- capture key value of new row
		...
		-- note to the admin role for that module.
		...
		begin
			...
			(
				...
				from
					dbo.PersonNote pn
				where
					pn.PersonNoteSID = @PersonNoteSID and pn.ApplicationGrantSID is null -- no grant currently assigned to the note
			)
			begin
				select
				...
				where
					PersonNoteSID = @PersonNoteSID; -- apply the grant to the document
			end;
		end;
		--! </PostInsert>
