...
GO
CREATE PROCEDURE [dbo].[pPracticeRegisterChangeRequirement#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPracticeRegisterChangeRequirement#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.PracticeRegisterChangeRequirement records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PracticeRegisterChangeRequirement table. When a new record is to be added from the UI, this procedure
...
columns of the vPracticeRegisterChangeRequirement entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pPracticeRegisterChangeRequirement
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pPracticeRegisterChangeRequirement of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@practiceRegisterChangeRequirementSID int               = -1					-- specific default required by EF - do not override
		...
		,@isMandatory                          bit               = CONVERT(bit,(0))											-- default provided from DB constraint - OK to override
		...
		,@practiceRegisterChangeRequirementXID varchar(150)      = null				-- no default provided from DB constraint - OK to override
		...
		,@practiceRegisterChangeIsActive       bit               = 0					-- not a base table column (default ignored)
		,@toolTip                              nvarchar(500)									-- not a base table column (default ignored)
		,@isEnabledForRegistrant               bit               = 0					-- not a base table column (default ignored)
		,@practiceRegisterChangeRowGUID        uniqueidentifier  = newid()		-- not a base table column (default ignored)
		...
		,@personDocTypeSID                     int														-- not a base table column (default ignored)
		,@examSID                              int														-- not a base table column (default ignored)
		...
		,@practiceRegisterChangeLabel          nvarchar(110)									-- not a base table column (default ignored)
	...
	begin try
		...
		if @SetFKDefaults = @ON
		begin
			set @practiceRegisterChangeSID = -1
		...
		select
			...
			,@isMandatory                           = isnull(context.node.value('@IsMandatory'                         ,'bit'         ),@isMandatory)
			...
			,@practiceRegisterChangeRequirementXID  = isnull(context.node.value('@PracticeRegisterChangeRequirementXID','varchar(150)'),@practiceRegisterChangeRequirementXID)
		...
		if @practiceRegisterChangeSID is not null and @requirementSequence = 10
		begin

			select
				@requirementSequence = max(prcr.RequirementSequence)
			from
				dbo.PracticeRegisterChangeRequirement prcr
			where
				prcr.PracticeRegisterChangeSID = @practiceRegisterChangeSID
