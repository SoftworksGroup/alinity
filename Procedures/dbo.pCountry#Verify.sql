...
GO
CREATE PROCEDURE [dbo].[pCountry#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.Country
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.Country for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.Country table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fCountry#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pCountry#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.Country'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.Country'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.Country'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.Country'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.Country'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_Country'
		)
		begin
			exec sp_executesql N'alter table dbo.Country drop constraint ck_Country'											-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.Country with CHECK add constraint ck_Country
			check
				(
				dbo.fCountry#Check
					(
			...
			select @RowCount = count(1) from dbo.Country												-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.Country with NOCHECK add constraint ck_Country
			check
				(
				dbo.fCountry#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.Country'
		and
			br.BusinessRuleStatus <> 'x'
