...
GO
CREATE PROCEDURE [sf].[pPersonMailingPreference#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.PersonMailingPreference
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.PersonMailingPreference for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.PersonMailingPreference table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fPersonMailingPreference#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pPersonMailingPreference#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.PersonMailingPreference'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.PersonMailingPreference'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.PersonMailingPreference'						-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.PersonMailingPreference'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.PersonMailingPreference'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_PersonMailingPreference'
		)
		begin
			exec sp_executesql N'alter table sf.PersonMailingPreference drop constraint ck_PersonMailingPreference'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.PersonMailingPreference with CHECK add constraint ck_PersonMailingPreference
			check
				(
				sf.fPersonMailingPreference#Check
					(
			...
			select @RowCount = count(1) from sf.PersonMailingPreference					-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PersonMailingPreference'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.PersonMailingPreferenceSID
				,x.RowGUID
			from
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fPersonMailingPreference#Check
							(
							 x.PersonMailingPreferenceSID
							,x.PersonSID
							,x.MailingPreferenceSID
							,x.EffectiveTime
							,x.ExpiryTime
							,x.PersonMailingPreferenceXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						sf.PersonMailingPreference  x
					where
						x.PersonMailingPreferenceSID = @nextSID
			...
			alter table sf.PersonMailingPreference with NOCHECK add constraint ck_PersonMailingPreference
			check
				(
				sf.fPersonMailingPreference#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.PersonMailingPreference'
		and
			br.BusinessRuleStatus <> 'x'
