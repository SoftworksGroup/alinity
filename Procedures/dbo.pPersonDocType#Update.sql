...
GO
CREATE PROCEDURE [dbo].[pPersonDocType#Update]
	 @PersonDocTypeSID      int               = null -- required! id of row to update - must be set in custom logic if not passed
	,@PersonDocTypeSCD      varchar(15)       = null -- table column values to update:
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPersonDocType#Update
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : updates 1 row in the dbo.PersonDocType table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to update the dbo.PersonDocType table. The procedure requires a primary key to locate the record to update.
Additional parameters are provided for all columns in the vPersonDocType entity view, however, the base logic of the procedure
...
begin
	...
	begin try
		...
		begin
			...
			select
				 @PersonDocTypeSCD      = isnull(@PersonDocTypeSCD,pdt.PersonDocTypeSCD)
				,@PersonDocTypeLabel    = isnull(@PersonDocTypeLabel,pdt.PersonDocTypeLabel)
				,@PersonDocTypeCategory = isnull(@PersonDocTypeCategory,pdt.PersonDocTypeCategory)
				,@IsDefault             = isnull(@IsDefault,pdt.IsDefault)
				,@IsActive              = isnull(@IsActive,pdt.IsActive)
				,@UserDefinedColumns    = isnull(@UserDefinedColumns,pdt.UserDefinedColumns)
				,@PersonDocTypeXID      = isnull(@PersonDocTypeXID,pdt.PersonDocTypeXID)
				,@LegacyKey             = isnull(@LegacyKey,pdt.LegacyKey)
				,@UpdateUser            = isnull(@UpdateUser,pdt.UpdateUser)
				,@IsReselected          = isnull(@IsReselected,pdt.IsReselected)
				,@IsNullApplied         = isnull(@IsNullApplied,pdt.IsNullApplied)
				,@zContext              = isnull(@zContext,pdt.zContext)
				,@IsDeleteEnabled       = isnull(@IsDeleteEnabled,pdt.IsDeleteEnabled)
			from
				dbo.vPersonDocType pdt
			where
				pdt.PersonDocTypeSID = @PersonDocTypeSID
		...
		begin
			...
			select @recordSID = x.PersonDocTypeSID from dbo.PersonDocType x where x.IsDefault = @ON and x.PersonDocTypeSID <> @PersonDocTypeSID
		...
		begin
			...
			if exists (select 1 from dbo.PersonDocType where PersonDocTypeSID = @personDocTypeSID)
			begin
		...
		if not exists
		(
			select 1 from	dbo.PersonDocType x where x.IsDefault = @ON
		)
		begin
