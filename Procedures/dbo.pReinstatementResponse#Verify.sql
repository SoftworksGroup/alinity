...
GO
CREATE PROCEDURE [dbo].[pReinstatementResponse#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ReinstatementResponse
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ReinstatementResponse for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ReinstatementResponse table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fReinstatementResponse#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pReinstatementResponse#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ReinstatementResponse'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ReinstatementResponse'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ReinstatementResponse'						-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ReinstatementResponse'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ReinstatementResponse'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ReinstatementResponse'
		)
		begin
			exec sp_executesql N'alter table dbo.ReinstatementResponse drop constraint ck_ReinstatementResponse'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ReinstatementResponse with CHECK add constraint ck_ReinstatementResponse
			check
				(
				dbo.fReinstatementResponse#Check
					(
					 ReinstatementResponseSID
					,ReinstatementSID
					,FormOwnerSID
					,ReinstatementResponseXID
					,LegacyKey
					,IsDeleted
					,CreateUser
					,CreateTime
			...
			select @RowCount = count(1) from dbo.ReinstatementResponse					-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ReinstatementResponse'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.ReinstatementResponseSID
				,x.RowGUID
			from
				dbo.ReinstatementResponse x
			order by
				x.ReinstatementResponseSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fReinstatementResponse#Check
							(
							 x.ReinstatementResponseSID
							,x.ReinstatementSID
							,x.FormOwnerSID
							,x.ReinstatementResponseXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.ReinstatementResponse  x
					where
						x.ReinstatementResponseSID = @nextSID
			...
			alter table dbo.ReinstatementResponse with NOCHECK add constraint ck_ReinstatementResponse
			check
				(
				dbo.fReinstatementResponse#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ReinstatementResponse'
		and
			br.BusinessRuleStatus <> 'x'
