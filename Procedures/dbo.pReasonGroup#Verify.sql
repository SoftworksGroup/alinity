...
GO
CREATE PROCEDURE [dbo].[pReasonGroup#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ReasonGroup
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ReasonGroup for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ReasonGroup table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fReasonGroup#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pReasonGroup#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ReasonGroup'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ReasonGroup'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ReasonGroup'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ReasonGroup'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ReasonGroup'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ReasonGroup'
		)
		begin
			exec sp_executesql N'alter table dbo.ReasonGroup drop constraint ck_ReasonGroup'							-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ReasonGroup with CHECK add constraint ck_ReasonGroup
			check
				(
				dbo.fReasonGroup#Check
					(
					 ReasonGroupSID
					,ReasonGroupSCD
					,ReasonGroupLabel
			...
			select @RowCount = count(1) from dbo.ReasonGroup										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fReasonGroup#Check
							(
							 x.ReasonGroupSID
							,x.ReasonGroupSCD
							,x.ReasonGroupLabel
			...
			alter table dbo.ReasonGroup with NOCHECK add constraint ck_ReasonGroup
			check
				(
				dbo.fReasonGroup#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ReasonGroup'
		and
			br.BusinessRuleStatus <> 'x'
