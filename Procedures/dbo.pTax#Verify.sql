...
GO
CREATE PROCEDURE [dbo].[pTax#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.Tax
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.Tax for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.Tax table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fTax#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pTax#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.Tax'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.Tax'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.Tax'															-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.Tax'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.Tax'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_Tax'
		)
		begin
			exec sp_executesql N'alter table dbo.Tax drop constraint ck_Tax'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.Tax with CHECK add constraint ck_Tax
			check
				(
				dbo.fTax#Check
					(
			...
			select @RowCount = count(1) from dbo.Tax														-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.Tax with NOCHECK add constraint ck_Tax
			check
				(
				dbo.fTax#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.Tax'
		and
			br.BusinessRuleStatus <> 'x'
