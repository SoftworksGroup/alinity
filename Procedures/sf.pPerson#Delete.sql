...
GO
CREATE PROCEDURE [sf].[pPerson#Delete]
	 @PersonSID              int               = null -- required! id of row to delete - must be set in custom logic if not passed
...
as
/*********************************************************************************************************************************
Procedure : sf.pPerson#Delete
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : deletes 1 row in the sf.Person table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.Person table. The procedure requires a primary key value to locate the record
to delete.
...
and in the ext.pPerson procedure for client-specific deletion rules.
...
begin
	...
	begin try
		...
		if @RowStamp is null select @RowStamp = x.RowStamp from sf.Person x where x.PersonSID = @PersonSID
	...
	-- Ensure the row can be deleted. This error should not appear if the UI is enabling
		...
		-- in the #Ext view.
		set @IsDeleteEnabled = sf.fPerson#IsDeleteEnabled(@PersonSID)
		...
		begin
			exec sf.pMessage#Get
				...
				,@MessageText   = @errorText output
				,@DefaultText   = N'Deletion of this %1 record is not allowed because this %1 is an user or you do not have sufficient grant.'
				,@Arg1          = N'person'
		...
		update																																-- set audit details on sf.PersonEmailAddress rows that will delete through CASCADE
