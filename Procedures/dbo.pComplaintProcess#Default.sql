...
GO
CREATE PROCEDURE [dbo].[pComplaintProcess#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pComplaintProcess#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.ComplaintProcess records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.ComplaintProcess table. When a new record is to be added from the UI, this procedure
...
columns of the vComplaintProcess entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pComplaintProcess
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pComplaintProcess of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@complaintProcessSID       int               = -1										-- specific default required by EF - do not override
		,@complaintProcessLabel     nvarchar(35)      = null									-- no default provided from DB constraint - OK to override
		...
		,@parentComplaintProcessSID int               = null									-- no default provided from DB constraint - OK to override
		,@userDefinedColumns        xml               = null									-- no default provided from DB constraint - OK to override
		,@complaintProcessXID       varchar(150)      = null									-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @complaintProcessLabel      = isnull(context.node.value('@ComplaintProcessLabel'    ,'nvarchar(35)' ),@complaintProcessLabel)
			...
			,@parentComplaintProcessSID  = isnull(context.node.value('@ParentComplaintProcessSID','int'          ),@parentComplaintProcessSID)
			,@complaintProcessXID        = isnull(context.node.value('@ComplaintProcessXID'      ,'varchar(150)' ),@complaintProcessXID)
		...
		if isnull(@parentComplaintProcessSID,0) = 0 select @parentComplaintProcessSID = x.ComplaintProcessSID from dbo.ComplaintProcess x where x.IsDefault = @ON
