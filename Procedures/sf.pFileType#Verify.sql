...
GO
CREATE PROCEDURE [sf].[pFileType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.FileType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.FileType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.FileType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fFileType#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pFileType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.FileType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.FileType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.FileType'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.FileType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.FileType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_FileType'
		)
		begin
			exec sp_executesql N'alter table sf.FileType drop constraint ck_FileType'											-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.FileType with CHECK add constraint ck_FileType
			check
				(
				sf.fFileType#Check
					(
			...
			select @RowCount = count(1) from sf.FileType												-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.FileType with NOCHECK add constraint ck_FileType
			check
				(
				sf.fFileType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.FileType'
		and
			br.BusinessRuleStatus <> 'x'
