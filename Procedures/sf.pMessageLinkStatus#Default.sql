...
GO
CREATE PROCEDURE [sf].[pMessageLinkStatus#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pMessageLinkStatus#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.MessageLinkStatus records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.MessageLinkStatus table. When a new record is to be added from the UI, this procedure
...
columns of the vMessageLinkStatus entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pMessageLinkStatus
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pMessageLinkStatus of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@messageLinkStatusSID   int               = -1												-- specific default required by EF - do not override
		,@messageLinkStatusSCD   varchar(10)       = null											-- no default provided from DB constraint - OK to override
		,@messageLinkStatusLabel nvarchar(35)      = null											-- no default provided from DB constraint - OK to override
		,@usageNotes             nvarchar(max)     = null											-- no default provided from DB constraint - OK to override
		,@isResendEnabled        bit               = (0)											-- default provided from DB constraint - OK to override
		...
		,@messageLinkStatusXID   varchar(150)      = null											-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @messageLinkStatusSCD    = isnull(context.node.value('@MessageLinkStatusSCD'  ,'varchar(10)'  ),@messageLinkStatusSCD)
			,@messageLinkStatusLabel  = isnull(context.node.value('@MessageLinkStatusLabel','nvarchar(35)' ),@messageLinkStatusLabel)
			,@usageNotes              = isnull(context.node.value('@UsageNotes'            ,'nvarchar(max)'),@usageNotes)
			,@isResendEnabled         = isnull(context.node.value('@IsResendEnabled'       ,'bit'          ),@isResendEnabled)
			,@isActive                = isnull(context.node.value('@IsActive'              ,'bit'          ),@isActive)
			,@isDefault               = isnull(context.node.value('@IsDefault'             ,'bit'          ),@isDefault)
			,@messageLinkStatusXID    = isnull(context.node.value('@MessageLinkStatusXID'  ,'varchar(150)' ),@messageLinkStatusXID)
