...
GO
CREATE PROCEDURE [sf].[pPersonTextMessage#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.PersonTextMessage
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.PersonTextMessage for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.PersonTextMessage table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fPersonTextMessage#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pPersonTextMessage#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.PersonTextMessage'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.PersonTextMessage'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.PersonTextMessage'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.PersonTextMessage'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.PersonTextMessage'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_PersonTextMessage'
		)
		begin
			exec sp_executesql N'alter table sf.PersonTextMessage drop constraint ck_PersonTextMessage'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.PersonTextMessage with CHECK add constraint ck_PersonTextMessage
			check
				(
			...
			select @RowCount = count(1) from sf.PersonTextMessage								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PersonTextMessage'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.PersonTextMessageSID
				,x.RowGUID
			from
				sf.PersonTextMessage x
			order by
				x.PersonTextMessageSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fPersonTextMessage#Check
							(
							 x.PersonTextMessageSID
							,x.PersonSID
							,x.TextMessageSID
					...
					from
						sf.PersonTextMessage  x
					where
						x.PersonTextMessageSID = @nextSID
			...
			alter table sf.PersonTextMessage with NOCHECK add constraint ck_PersonTextMessage
			check
				(
				sf.fPersonTextMessage#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.PersonTextMessage'
		and
			br.BusinessRuleStatus <> 'x'
