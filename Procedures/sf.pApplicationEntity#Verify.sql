...
GO
CREATE PROCEDURE [sf].[pApplicationEntity#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ApplicationEntity
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ApplicationEntity for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ApplicationEntity table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fApplicationEntity#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pApplicationEntity#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ApplicationEntity'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ApplicationEntity'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationEntity'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationEntity'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ApplicationEntity'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ApplicationEntity'
		)
		begin
			exec sp_executesql N'alter table sf.ApplicationEntity drop constraint ck_ApplicationEntity'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ApplicationEntity with CHECK add constraint ck_ApplicationEntity
			check
				(
				sf.fApplicationEntity#Check
					(
			...
			select @RowCount = count(1) from sf.ApplicationEntity								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ApplicationEntity'
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fApplicationEntity#Check
							(
							 x.ApplicationEntitySID
							,x.ApplicationEntitySCD
							,x.ApplicationEntityName
							,x.IsMergeDataSource
							,x.ApplicationEntityXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						sf.ApplicationEntity  x
					where
						x.ApplicationEntitySID = @nextSID
			...
			alter table sf.ApplicationEntity with NOCHECK add constraint ck_ApplicationEntity
			check
				(
				sf.fApplicationEntity#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ApplicationEntity'
		and
			br.BusinessRuleStatus <> 'x'
