...
GO
CREATE PROCEDURE [sf].[pAuthenticationAuthority#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.AuthenticationAuthority
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.AuthenticationAuthority for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.AuthenticationAuthority table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fAuthenticationAuthority#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pAuthenticationAuthority#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.AuthenticationAuthority'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.AuthenticationAuthority'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.AuthenticationAuthority'						-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.AuthenticationAuthority'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.AuthenticationAuthority'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_AuthenticationAuthority'
		)
		begin
			exec sp_executesql N'alter table sf.AuthenticationAuthority drop constraint ck_AuthenticationAuthority'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.AuthenticationAuthority with CHECK add constraint ck_AuthenticationAuthority
			check
				(
				sf.fAuthenticationAuthority#Check
					(
			...
			select @RowCount = count(1) from sf.AuthenticationAuthority					-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'AuthenticationAuthority'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.AuthenticationAuthoritySID
				,x.RowGUID
			from
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fAuthenticationAuthority#Check
							(
							 x.AuthenticationAuthoritySID
					...
					from
						sf.AuthenticationAuthority  x
					where
						x.AuthenticationAuthoritySID = @nextSID
			...
			alter table sf.AuthenticationAuthority with NOCHECK add constraint ck_AuthenticationAuthority
			check
				(
				sf.fAuthenticationAuthority#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.AuthenticationAuthority'
		and
			br.BusinessRuleStatus <> 'x'
