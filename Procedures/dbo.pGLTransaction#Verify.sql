...
GO
CREATE PROCEDURE [dbo].[pGLTransaction#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.GLTransaction
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.GLTransaction for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.GLTransaction table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fGLTransaction#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pGLTransaction#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.GLTransaction'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.GLTransaction'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.GLTransaction'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.GLTransaction'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.GLTransaction'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_GLTransaction'
		)
		begin
			exec sp_executesql N'alter table dbo.GLTransaction drop constraint ck_GLTransaction'					-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.GLTransaction with CHECK add constraint ck_GLTransaction
			check
				(
			...
			select @RowCount = count(1) from dbo.GLTransaction									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'GLTransaction'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fGLTransaction#Check
							(
							 x.GLTransactionSID
							,x.PaymentSID
							,x.InvoicePaymentSID
							,x.DebitGLAccountCode
							,x.CreditGLAccountCode
							,x.Amount
							,x.GLPostingDate
							,x.PaymentCheckSum
							,x.InvoicePaymentCheckSum
							,x.ReversedGLTransactionSID
							,x.IsExcluded
							,x.GLTransactionXID
							,x.LegacyKey
							,x.IsDeleted
			...
			alter table dbo.GLTransaction with NOCHECK add constraint ck_GLTransaction
			check
				(
				dbo.fGLTransaction#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.GLTransaction'
		and
			br.BusinessRuleStatus <> 'x'
