...
GO
CREATE PROCEDURE [dbo].[pPracticeRestriction#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPracticeRestriction#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.PracticeRestriction records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PracticeRestriction table. When a new record is to be added from the UI, this procedure
...
columns of the vPracticeRestriction entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pPracticeRestriction
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pPracticeRestriction of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@practiceRestrictionSID   int               = -1											-- specific default required by EF - do not override
		,@practiceRestrictionLabel nvarchar(35)      = null										-- no default provided from DB constraint - OK to override
		...
		,@practiceRestrictionXID   varchar(150)      = null										-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @practiceRestrictionLabel  = isnull(context.node.value('@PracticeRestrictionLabel','nvarchar(35)'  ),@practiceRestrictionLabel)
			...
			,@practiceRestrictionXID    = isnull(context.node.value('@PracticeRestrictionXID'  ,'varchar(150)'  ),@practiceRestrictionXID)
