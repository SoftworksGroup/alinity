...
GO
CREATE PROCEDURE [dbo].[pPAPBatch#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PAPBatch
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PAPBatch for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PAPBatch table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPAPBatch#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPAPBatch#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PAPBatch'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PAPBatch'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PAPBatch'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PAPBatch'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PAPBatch'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PAPBatch'
		)
		begin
			exec sp_executesql N'alter table dbo.PAPBatch drop constraint ck_PAPBatch'										-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PAPBatch with CHECK add constraint ck_PAPBatch
			check
				(
				dbo.fPAPBatch#Check
					(
			...
			select @RowCount = count(1) from dbo.PAPBatch												-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.PAPBatch with NOCHECK add constraint ck_PAPBatch
			check
				(
				dbo.fPAPBatch#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PAPBatch'
		and
			br.BusinessRuleStatus <> 'x'
