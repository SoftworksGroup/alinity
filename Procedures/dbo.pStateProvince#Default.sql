...
GO
CREATE PROCEDURE [dbo].[pStateProvince#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pStateProvince#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.StateProvince records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.StateProvince table. When a new record is to be added from the UI, this procedure
...
columns of the vStateProvince entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pStateProvince
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pStateProvince of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@stateProvinceSID        int               = -1											-- specific default required by EF - do not override
		...
		,@iSONumber               smallint          = null										-- no default provided from DB constraint - OK to override
		,@isDisplayed             bit               = (1)											-- default provided from DB constraint - OK to override
		...
		,@stateProvinceXID        varchar(150)      = null										-- no default provided from DB constraint - OK to override
		...
		,@isStateProvinceRequired bit               = 0												-- not a base table column (default ignored)
		,@countryIsDefault        bit               = 0												-- not a base table column (default ignored)
		,@countryIsActive         bit               = 0												-- not a base table column (default ignored)
		,@countryRowGUID          uniqueidentifier  = newid()									-- not a base table column (default ignored)
		...
		,@stateProvinceSearch     nvarchar(81)      = N'?'										-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@iSONumber              = isnull(context.node.value('@ISONumber'            ,'smallint'    ),@iSONumber)
			,@isDisplayed            = isnull(context.node.value('@IsDisplayed'          ,'bit'         ),@isDisplayed)
			...
			,@stateProvinceXID       = isnull(context.node.value('@StateProvinceXID'     ,'varchar(150)'),@stateProvinceXID)
    ...
    --! <Overrides>
    -- Ji Zhou / Feb 2013
    -- Return the default Country if identified.
