...
GO
CREATE PROCEDURE [dbo].[pEmploymentType#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pEmploymentType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.EmploymentType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.EmploymentType table. When a new record is to be added from the UI, this procedure
...
columns of the vEmploymentType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pEmploymentType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pEmploymentType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@employmentTypeSID      int               = -1												-- specific default required by EF - do not override
		,@employmentTypeName     nvarchar(50)      = null											-- no default provided from DB constraint - OK to override
		,@employmentTypeCode     varchar(20)       = null											-- no default provided from DB constraint - OK to override
		,@employmentTypeCategory nvarchar(65)      = null											-- no default provided from DB constraint - OK to override
		...
		,@employmentTypeXID      varchar(150)      = null											-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @employmentTypeName      = isnull(context.node.value('@EmploymentTypeName'    ,'nvarchar(50)'),@employmentTypeName)
			,@employmentTypeCode      = isnull(context.node.value('@EmploymentTypeCode'    ,'varchar(20)' ),@employmentTypeCode)
			,@employmentTypeCategory  = isnull(context.node.value('@EmploymentTypeCategory','nvarchar(65)'),@employmentTypeCategory)
			,@isDefault               = isnull(context.node.value('@IsDefault'             ,'bit'         ),@isDefault)
			,@isActive                = isnull(context.node.value('@IsActive'              ,'bit'         ),@isActive)
			,@employmentTypeXID       = isnull(context.node.value('@EmploymentTypeXID'     ,'varchar(150)'),@employmentTypeXID)
