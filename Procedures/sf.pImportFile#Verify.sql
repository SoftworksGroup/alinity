...
GO
CREATE PROCEDURE [sf].[pImportFile#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ImportFile
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ImportFile for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ImportFile table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fImportFile#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pImportFile#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ImportFile'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ImportFile'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ImportFile'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ImportFile'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ImportFile'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ImportFile'
		)
		begin
			exec sp_executesql N'alter table sf.ImportFile drop constraint ck_ImportFile'									-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ImportFile with CHECK add constraint ck_ImportFile
			check
				(
				sf.fImportFile#Check
					(
					 ImportFileSID
					,FileFormatSID
			...
			select @RowCount = count(1) from sf.ImportFile											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.ImportFile with NOCHECK add constraint ck_ImportFile
			check
				(
				sf.fImportFile#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ImportFile'
		and
			br.BusinessRuleStatus <> 'x'
