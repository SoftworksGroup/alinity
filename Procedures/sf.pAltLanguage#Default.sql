...
GO
CREATE PROCEDURE [sf].[pAltLanguage#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pAltLanguage#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.AltLanguage records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.AltLanguage table. When a new record is to be added from the UI, this procedure
...
columns of the vAltLanguage entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pAltLanguage
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pAltLanguage of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@altLanguageSID     int               = -1														-- specific default required by EF - do not override
		,@sourceGUID         uniqueidentifier  = null													-- no default provided from DB constraint - OK to override
		,@fieldID            nvarchar(128)     = null													-- no default provided from DB constraint - OK to override
		,@cultureSID         int               = null													-- no default provided from DB constraint - OK to override
		,@altLanguageText    nvarchar(max)     = null													-- no default provided from DB constraint - OK to override
		,@userDefinedColumns xml               = null													-- no default provided from DB constraint - OK to override
		,@altLanguageXID     varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			...
			,@fieldID          = isnull(context.node.value('@FieldID'        ,'nvarchar(128)'   ),@fieldID)
			,@cultureSID       = isnull(context.node.value('@CultureSID'     ,'int'             ),@cultureSID)
			,@altLanguageText  = isnull(context.node.value('@AltLanguageText','nvarchar(max)'   ),@altLanguageText)
			,@altLanguageXID   = isnull(context.node.value('@AltLanguageXID' ,'varchar(150)'    ),@altLanguageXID)
