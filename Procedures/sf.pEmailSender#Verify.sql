...
GO
CREATE PROCEDURE [sf].[pEmailSender#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.EmailSender
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.EmailSender for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.EmailSender table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fEmailSender#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pEmailSender#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.EmailSender'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.EmailSender'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.EmailSender'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.EmailSender'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.EmailSender'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_EmailSender'
		)
		begin
			exec sp_executesql N'alter table sf.EmailSender drop constraint ck_EmailSender'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.EmailSender with CHECK add constraint ck_EmailSender
			check
				(
				sf.fEmailSender#Check
					(
					 EmailSenderSID
					,SenderEmailAddress
					,SenderDisplayName
			...
			select @RowCount = count(1) from sf.EmailSender											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.EmailSender with NOCHECK add constraint ck_EmailSender
			check
				(
				sf.fEmailSender#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.EmailSender'
		and
			br.BusinessRuleStatus <> 'x'
