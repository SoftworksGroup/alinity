...
GO
CREATE PROCEDURE [sf].[pEmailTemplate#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pEmailTemplate#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.EmailTemplate records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.EmailTemplate table. When a new record is to be added from the UI, this procedure
...
columns of the vEmailTemplate entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pEmailTemplate
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pEmailTemplate of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@emailTemplateSID          int               = -1										-- specific default required by EF - do not override
		,@emailTemplateLabel        nvarchar(35)      = null									-- no default provided from DB constraint - OK to override
		...
		,@changeLogSummary          varbinary(max)    = null									-- no default provided from DB constraint - OK to override
		...
		,@emailTemplateXID          varchar(150)      = null									-- no default provided from DB constraint - OK to override
		...
		,@isDefaultEmailTemplate    bit																				-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @emailTemplateLabel         = isnull(context.node.value('@EmailTemplateLabel'       ,'nvarchar(35)'  ),@emailTemplateLabel)
			...
			,@changeLogSummary           = isnull(context.node.value('@ChangeLogSummary'         ,'varbinary(max)'),@changeLogSummary)
			...
			,@emailTemplateXID           = isnull(context.node.value('@EmailTemplateXID'         ,'varchar(150)'  ),@emailTemplateXID)
