...
GO
CREATE PROCEDURE [dbo].[pPracticeRestriction#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PracticeRestriction
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PracticeRestriction for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PracticeRestriction table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPracticeRestriction#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPracticeRestriction#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PracticeRestriction'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PracticeRestriction'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRestriction'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRestriction'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PracticeRestriction'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PracticeRestriction'
		)
		begin
			exec sp_executesql N'alter table dbo.PracticeRestriction drop constraint ck_PracticeRestriction'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PracticeRestriction with CHECK add constraint ck_PracticeRestriction
			check
				(
				dbo.fPracticeRestriction#Check
					(
			...
			select @RowCount = count(1) from dbo.PracticeRestriction						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PracticeRestriction'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPracticeRestriction#Check
							(
							 x.PracticeRestrictionSID
							,x.PracticeRestrictionLabel
							,x.IsDisplayedOnLicense
							,x.IsActive
							,x.IsSupervisionRequired
							,x.PracticeRestrictionXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.PracticeRestriction  x
					where
						x.PracticeRestrictionSID = @nextSID
			...
			alter table dbo.PracticeRestriction with NOCHECK add constraint ck_PracticeRestriction
			check
				(
				dbo.fPracticeRestriction#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PracticeRestriction'
		and
			br.BusinessRuleStatus <> 'x'
