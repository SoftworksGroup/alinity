...
GO
CREATE PROCEDURE [sf].[pPersonGroup#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.PersonGroup
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.PersonGroup for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.PersonGroup table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fPersonGroup#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pPersonGroup#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.PersonGroup'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.PersonGroup'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.PersonGroup'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.PersonGroup'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.PersonGroup'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_PersonGroup'
		)
		begin
			exec sp_executesql N'alter table sf.PersonGroup drop constraint ck_PersonGroup'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.PersonGroup with CHECK add constraint ck_PersonGroup
			check
				(
			...
			select @RowCount = count(1) from sf.PersonGroup											-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fPersonGroup#Check
							(
							 x.PersonGroupSID
							,x.PersonGroupName
							,x.PersonGroupLabel
							,x.PersonGroupCategory
							,x.Description
							,x.ApplicationUserSID
							,x.IsPreference
							,x.IsDocumentLibraryEnabled
							,x.QuerySID
							,x.LastReviewUser
							,x.LastReviewTime
							,x.SmartGroupCount
							,x.SmartGroupCountTime
							,x.IsActive
							,x.PersonGroupXID
							,x.LegacyKey
							,x.IsDeleted
			...
			alter table sf.PersonGroup with NOCHECK add constraint ck_PersonGroup
			check
				(
				sf.fPersonGroup#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.PersonGroup'
		and
			br.BusinessRuleStatus <> 'x'
