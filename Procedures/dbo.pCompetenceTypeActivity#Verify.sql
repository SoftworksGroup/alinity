...
GO
CREATE PROCEDURE [dbo].[pCompetenceTypeActivity#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.CompetenceTypeActivity
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.CompetenceTypeActivity for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.CompetenceTypeActivity table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fCompetenceTypeActivity#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pCompetenceTypeActivity#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.CompetenceTypeActivity'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.CompetenceTypeActivity'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.CompetenceTypeActivity'						-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.CompetenceTypeActivity'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.CompetenceTypeActivity'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_CompetenceTypeActivity'
		)
		begin
			exec sp_executesql N'alter table dbo.CompetenceTypeActivity drop constraint ck_CompetenceTypeActivity'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.CompetenceTypeActivity with CHECK add constraint ck_CompetenceTypeActivity
			check
				(
				dbo.fCompetenceTypeActivity#Check
					(
			...
			select @RowCount = count(1) from dbo.CompetenceTypeActivity					-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'CompetenceTypeActivity'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.CompetenceTypeActivitySID
				,x.RowGUID
			from
				dbo.CompetenceTypeActivity x
			order by
				x.CompetenceTypeActivitySID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fCompetenceTypeActivity#Check
							(
							 x.CompetenceTypeActivitySID
							,x.CompetenceTypeSID
					...
					from
						dbo.CompetenceTypeActivity  x
					where
						x.CompetenceTypeActivitySID = @nextSID
			...
			alter table dbo.CompetenceTypeActivity with NOCHECK add constraint ck_CompetenceTypeActivity
			check
				(
				dbo.fCompetenceTypeActivity#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.CompetenceTypeActivity'
		and
			br.BusinessRuleStatus <> 'x'
