...
GO
CREATE PROCEDURE [dbo].[pRegistrantAuditReviewStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantAuditReviewStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantAuditReviewStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantAuditReviewStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantAuditReviewStatus#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantAuditReviewStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantAuditReviewStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantAuditReviewStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAuditReviewStatus'			-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAuditReviewStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantAuditReviewStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantAuditReviewStatus'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantAuditReviewStatus drop constraint ck_RegistrantAuditReviewStatus'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantAuditReviewStatus with CHECK add constraint ck_RegistrantAuditReviewStatus
			check
				(
			...
			select @RowCount = count(1) from dbo.RegistrantAuditReviewStatus		-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantAuditReviewStatus'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrantAuditReviewStatusSID
				,x.RowGUID
			from
				dbo.RegistrantAuditReviewStatus x
			order by
				x.RegistrantAuditReviewStatusSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantAuditReviewStatus#Check
							(
							 x.RegistrantAuditReviewStatusSID
							,x.RegistrantAuditReviewSID
							,x.FormStatusSID
							,x.RegistrantAuditReviewStatusXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantAuditReviewStatus  x
					where
						x.RegistrantAuditReviewStatusSID = @nextSID
			...
			alter table dbo.RegistrantAuditReviewStatus with NOCHECK add constraint ck_RegistrantAuditReviewStatus
			check
				(
				dbo.fRegistrantAuditReviewStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantAuditReviewStatus'
		and
			br.BusinessRuleStatus <> 'x'
