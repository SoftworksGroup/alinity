...
GO
CREATE PROCEDURE [dbo].[pRegistrantIdentifier#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantIdentifier
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantIdentifier for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantIdentifier table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantIdentifier#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantIdentifier#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantIdentifier'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantIdentifier'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantIdentifier'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantIdentifier'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantIdentifier'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantIdentifier'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantIdentifier drop constraint ck_RegistrantIdentifier'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantIdentifier with CHECK add constraint ck_RegistrantIdentifier
			check
				(
			...
			select @RowCount = count(1) from dbo.RegistrantIdentifier						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantIdentifier'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrantIdentifierSID
				,x.RowGUID
			from
				dbo.RegistrantIdentifier x
			order by
				x.RegistrantIdentifierSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantIdentifier#Check
							(
							 x.RegistrantIdentifierSID
							,x.RegistrantSID
							,x.IdentifierValue
					...
					from
						dbo.RegistrantIdentifier  x
					where
						x.RegistrantIdentifierSID = @nextSID
			...
			alter table dbo.RegistrantIdentifier with NOCHECK add constraint ck_RegistrantIdentifier
			check
				(
				dbo.fRegistrantIdentifier#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantIdentifier'
		and
			br.BusinessRuleStatus <> 'x'
