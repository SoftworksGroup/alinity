...
GO
CREATE PROCEDURE [dbo].[pRegistrantRenewalStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantRenewalStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantRenewalStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantRenewalStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantRenewalStatus#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantRenewalStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantRenewalStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantRenewalStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantRenewalStatus'					-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantRenewalStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantRenewalStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantRenewalStatus'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantRenewalStatus drop constraint ck_RegistrantRenewalStatus'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantRenewalStatus with CHECK add constraint ck_RegistrantRenewalStatus
			check
				(
				dbo.fRegistrantRenewalStatus#Check
					(
					 RegistrantRenewalStatusSID
					,RegistrantRenewalSID
					,FormStatusSID
					,RegistrantRenewalStatusXID
					,LegacyKey
					,IsDeleted
					,CreateUser
					,CreateTime
			...
			select @RowCount = count(1) from dbo.RegistrantRenewalStatus				-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantRenewalStatus'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantRenewalStatus#Check
							(
							 x.RegistrantRenewalStatusSID
							,x.RegistrantRenewalSID
							,x.FormStatusSID
							,x.RegistrantRenewalStatusXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantRenewalStatus  x
					where
						x.RegistrantRenewalStatusSID = @nextSID
			...
			alter table dbo.RegistrantRenewalStatus with NOCHECK add constraint ck_RegistrantRenewalStatus
			check
				(
				dbo.fRegistrantRenewalStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantRenewalStatus'
		and
			br.BusinessRuleStatus <> 'x'
