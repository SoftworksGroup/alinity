...
GO
CREATE PROCEDURE [sf].[pApplicationUserProfileProperty#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ApplicationUserProfileProperty
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ApplicationUserProfileProperty for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ApplicationUserProfileProperty table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fApplicationUserProfileProperty#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pApplicationUserProfileProperty#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ApplicationUserProfileProperty'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ApplicationUserProfileProperty'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationUserProfileProperty'		-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationUserProfileProperty'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ApplicationUserProfileProperty'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ApplicationUserProfileProperty'
		)
		begin
			exec sp_executesql N'alter table sf.ApplicationUserProfileProperty drop constraint ck_ApplicationUserProfileProperty'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ApplicationUserProfileProperty with CHECK add constraint ck_ApplicationUserProfileProperty
			check
				(
				sf.fApplicationUserProfileProperty#Check
					(
					 ApplicationUserProfilePropertySID
					,ApplicationUserSID
					,PropertyName
					,ApplicationUserProfilePropertyXID
					,LegacyKey
					,IsDeleted
					,CreateUser
					,CreateTime
			...
			select @RowCount = count(1) from sf.ApplicationUserProfileProperty	-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ApplicationUserProfileProperty'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.ApplicationUserProfilePropertySID
				,x.RowGUID
			from
				sf.ApplicationUserProfileProperty x
			order by
				x.ApplicationUserProfilePropertySID
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fApplicationUserProfileProperty#Check
							(
							 x.ApplicationUserProfilePropertySID
							,x.ApplicationUserSID
							,x.PropertyName
							,x.ApplicationUserProfilePropertyXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						sf.ApplicationUserProfileProperty  x
					where
						x.ApplicationUserProfilePropertySID = @nextSID
			...
			alter table sf.ApplicationUserProfileProperty with NOCHECK add constraint ck_ApplicationUserProfileProperty
			check
				(
				sf.fApplicationUserProfileProperty#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ApplicationUserProfileProperty'
		and
			br.BusinessRuleStatus <> 'x'
