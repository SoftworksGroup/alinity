...
GO
CREATE PROCEDURE [sf].[pBusinessRuleError#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pBusinessRuleError#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.BusinessRuleError records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.BusinessRuleError table. When a new record is to be added from the UI, this procedure
...
columns of the vBusinessRuleError entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pBusinessRuleError
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pBusinessRuleError of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@businessRuleErrorSID int               = -1													-- specific default required by EF - do not override
		,@businessRuleSID      int               = null												-- no default provided from DB constraint - OK to override
		,@messageText          nvarchar(4000)    = null												-- no default provided from DB constraint - OK to override
		,@sourceSID            int               = null												-- no default provided from DB constraint - OK to override
		,@sourceGUID           uniqueidentifier  = null												-- no default provided from DB constraint - OK to override
		,@userDefinedColumns   xml               = null												-- no default provided from DB constraint - OK to override
		,@businessRuleErrorXID varchar(150)      = null												-- no default provided from DB constraint - OK to override
		...
		,@messageSID           int               = 0													-- not a base table column (default ignored)
		,@columnName           nvarchar(128)     = N'?'												-- not a base table column (default ignored)
		,@businessRuleStatus   char(1)           = '?'												-- not a base table column (default ignored)
		,@businessRuleRowGUID  uniqueidentifier  = newid()										-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @businessRuleSID       = isnull(context.node.value('@BusinessRuleSID'     ,'int'             ),@businessRuleSID)
			,@messageText           = isnull(context.node.value('@MessageText'         ,'nvarchar(4000)'  ),@messageText)
			,@sourceSID             = isnull(context.node.value('@SourceSID'           ,'int'             ),@sourceSID)
			,@sourceGUID            = isnull(context.node.value('@SourceGUID'          ,'uniqueidentifier'),@sourceGUID)
			,@businessRuleErrorXID  = isnull(context.node.value('@BusinessRuleErrorXID','varchar(150)'    ),@businessRuleErrorXID)
		...
		select
			...
			,@businessRuleRowGUID BusinessRuleRowGUID
