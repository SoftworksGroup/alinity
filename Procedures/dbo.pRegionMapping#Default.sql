...
GO
CREATE PROCEDURE [dbo].[pRegionMapping#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pRegionMapping#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.RegionMapping records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.RegionMapping table. When a new record is to be added from the UI, this procedure
...
columns of the vRegionMapping entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pRegionMapping
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pRegionMapping of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@regionMappingSID   int               = -1														-- specific default required by EF - do not override
		,@postalCodeMask     varchar(10)       = null													-- no default provided from DB constraint - OK to override
		...
		,@regionMappingXID   varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @postalCodeMask    = isnull(context.node.value('@PostalCodeMask'  ,'varchar(10)' ),@postalCodeMask)
			,@regionSID         = isnull(context.node.value('@RegionSID'       ,'int'         ),@regionSID)
			,@regionMappingXID  = isnull(context.node.value('@RegionMappingXID','varchar(150)'),@regionMappingXID)
