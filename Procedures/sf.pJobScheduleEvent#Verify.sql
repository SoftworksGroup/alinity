...
GO
CREATE PROCEDURE [sf].[pJobScheduleEvent#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.JobScheduleEvent
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.JobScheduleEvent for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.JobScheduleEvent table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fJobScheduleEvent#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pJobScheduleEvent#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.JobScheduleEvent'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.JobScheduleEvent'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.JobScheduleEvent'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.JobScheduleEvent'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.JobScheduleEvent'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_JobScheduleEvent'
		)
		begin
			exec sp_executesql N'alter table sf.JobScheduleEvent drop constraint ck_JobScheduleEvent'			-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.JobScheduleEvent with CHECK add constraint ck_JobScheduleEvent
			check
				(
				sf.fJobScheduleEvent#Check
					(
					 JobScheduleEventSID
					,EventName
					,JobScheduleEventXID
			...
			select @RowCount = count(1) from sf.JobScheduleEvent								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'JobScheduleEvent'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.JobScheduleEventSID
				,x.RowGUID
			from
				sf.JobScheduleEvent x
			order by
				x.JobScheduleEventSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fJobScheduleEvent#Check
							(
							 x.JobScheduleEventSID
							,x.EventName
							,x.JobScheduleEventXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
			...
			alter table sf.JobScheduleEvent with NOCHECK add constraint ck_JobScheduleEvent
			check
				(
				sf.fJobScheduleEvent#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.JobScheduleEvent'
		and
			br.BusinessRuleStatus <> 'x'
