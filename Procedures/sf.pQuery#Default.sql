...
GO
CREATE PROCEDURE [sf].[pQuery#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pQuery#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.Query records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.Query table. When a new record is to be added from the UI, this procedure
...
columns of the vQuery entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pQuery
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pQuery of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@querySID                 int               = -1											-- specific default required by EF - do not override
		,@queryCategorySID         int               = null										-- no default provided from DB constraint - OK to override
		,@applicationPageSID       int               = null										-- no default provided from DB constraint - OK to override
		,@queryLabel               nvarchar(35)      = null										-- no default provided from DB constraint - OK to override
		,@toolTip                  nvarchar(250)     = null										-- no default provided from DB constraint - OK to override
		,@lastExecuteTime          datetimeoffset(7) = sysdatetimeoffset()		-- default provided from DB constraint - OK to override
		,@lastExecuteUser          nvarchar(75)      = suser_sname()					-- default provided from DB constraint - OK to override
		,@executeCount             int               = (0)										-- default provided from DB constraint - OK to override
		,@querySQL                 nvarchar(max)     = null										-- no default provided from DB constraint - OK to override
		,@queryParameters          xml               = null										-- no default provided from DB constraint - OK to override
		,@queryCode                varchar(30)       = 'None'									-- default provided from DB constraint - OK to override
		,@isActive                 bit               = (1)										-- default provided from DB constraint - OK to override
		,@isApplicationPageDefault bit               = (0)										-- default provided from DB constraint - OK to override
		,@userDefinedColumns       xml               = null										-- no default provided from DB constraint - OK to override
		,@queryXID                 varchar(150)      = null										-- no default provided from DB constraint - OK to override
		...
		,@queryCategoryLabel       nvarchar(35)      = N'?'										-- not a base table column (default ignored)
		,@queryCategoryCode        varchar(30)       = '?'										-- not a base table column (default ignored)
		,@displayOrder             int               = 0											-- not a base table column (default ignored)
		,@queryCategoryIsActive    bit               = 0											-- not a base table column (default ignored)
		,@queryCategoryIsDefault   bit               = 0											-- not a base table column (default ignored)
		,@queryCategoryRowGUID     uniqueidentifier  = newid()								-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @queryCategorySID          = isnull(context.node.value('@QueryCategorySID'        ,'int'              ),@queryCategorySID)
			,@applicationPageSID        = isnull(context.node.value('@ApplicationPageSID'      ,'int'              ),@applicationPageSID)
			,@queryLabel                = isnull(context.node.value('@QueryLabel'              ,'nvarchar(35)'     ),@queryLabel)
			,@toolTip                   = isnull(context.node.value('@ToolTip'                 ,'nvarchar(250)'    ),@toolTip)
			,@lastExecuteTime           = isnull(context.node.value('@LastExecuteTime'         ,'datetimeoffset(7)'),@lastExecuteTime)
			,@lastExecuteUser           = isnull(context.node.value('@LastExecuteUser'         ,'nvarchar(75)'     ),@lastExecuteUser)
			,@executeCount              = isnull(context.node.value('@ExecuteCount'            ,'int'              ),@executeCount)
			,@querySQL                  = isnull(context.node.value('@QuerySQL'                ,'nvarchar(max)'    ),@querySQL)
			,@queryCode                 = isnull(context.node.value('@QueryCode'               ,'varchar(30)'      ),@queryCode)
			,@isActive                  = isnull(context.node.value('@IsActive'                ,'bit'              ),@isActive)
			,@isApplicationPageDefault  = isnull(context.node.value('@IsApplicationPageDefault','bit'              ),@isApplicationPageDefault)
			,@queryXID                  = isnull(context.node.value('@QueryXID'                ,'varchar(150)'     ),@queryXID)
		...
		if isnull(@queryCategorySID,0) = 0 select @queryCategorySID = x.QueryCategorySID from sf.QueryCategory  x where x.IsDefault = @ON
