...
GO
CREATE PROCEDURE [sf].[pFormSubForm#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.FormSubForm
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.FormSubForm for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.FormSubForm table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fFormSubForm#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pFormSubForm#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.FormSubForm'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.FormSubForm'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.FormSubForm'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.FormSubForm'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.FormSubForm'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_FormSubForm'
		)
		begin
			exec sp_executesql N'alter table sf.FormSubForm drop constraint ck_FormSubForm'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.FormSubForm with CHECK add constraint ck_FormSubForm
			check
				(
			...
			select @RowCount = count(1) from sf.FormSubForm											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.FormSubForm with NOCHECK add constraint ck_FormSubForm
			check
				(
				sf.fFormSubForm#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.FormSubForm'
		and
			br.BusinessRuleStatus <> 'x'
