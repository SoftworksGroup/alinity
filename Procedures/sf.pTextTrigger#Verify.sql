...
GO
CREATE PROCEDURE [sf].[pTextTrigger#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.TextTrigger
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.TextTrigger for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.TextTrigger table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fTextTrigger#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pTextTrigger#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.TextTrigger'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.TextTrigger'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.TextTrigger'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.TextTrigger'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.TextTrigger'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_TextTrigger'
		)
		begin
			exec sp_executesql N'alter table sf.TextTrigger drop constraint ck_TextTrigger'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.TextTrigger with CHECK add constraint ck_TextTrigger
			check
				(
			...
			select @RowCount = count(1) from sf.TextTrigger											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.TextTrigger with NOCHECK add constraint ck_TextTrigger
			check
				(
				sf.fTextTrigger#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.TextTrigger'
		and
			br.BusinessRuleStatus <> 'x'
