...
GO
CREATE PROCEDURE [dbo].[pRegistrationRequirementType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrationRequirementType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrationRequirementType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrationRequirementType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrationRequirementType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrationRequirementType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrationRequirementType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrationRequirementType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationRequirementType'			-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationRequirementType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrationRequirementType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrationRequirementType'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrationRequirementType drop constraint ck_RegistrationRequirementType'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrationRequirementType with CHECK add constraint ck_RegistrationRequirementType
			check
				(
				dbo.fRegistrationRequirementType#Check
					(
			...
			select @RowCount = count(1) from dbo.RegistrationRequirementType		-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrationRequirementType'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrationRequirementTypeSID
				,x.RowGUID
			from
				dbo.RegistrationRequirementType x
			order by
				x.RegistrationRequirementTypeSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrationRequirementType#Check
							(
							 x.RegistrationRequirementTypeSID
							,x.RegistrationRequirementTypeLabel
							,x.RegistrationRequirementTypeCode
							,x.RegistrationRequirementTypeCategory
							,x.IsAppliedToPeople
							,x.IsAppliedToOrganizations
							,x.IsDefault
							,x.IsActive
							,x.RegistrationRequirementTypeXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrationRequirementType  x
					where
						x.RegistrationRequirementTypeSID = @nextSID
			...
			alter table dbo.RegistrationRequirementType with NOCHECK add constraint ck_RegistrationRequirementType
			check
				(
				dbo.fRegistrationRequirementType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrationRequirementType'
		and
			br.BusinessRuleStatus <> 'x'
