...
GO
CREATE PROCEDURE [sf].[pFormStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.FormStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.FormStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.FormStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fFormStatus#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pFormStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.FormStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.FormStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.FormStatus'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.FormStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.FormStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_FormStatus'
		)
		begin
			exec sp_executesql N'alter table sf.FormStatus drop constraint ck_FormStatus'									-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.FormStatus with CHECK add constraint ck_FormStatus
			check
				(
				sf.fFormStatus#Check
					(
					 FormStatusSID
					,FormStatusSCD
					,FormStatusLabel
			...
			select @RowCount = count(1) from sf.FormStatus											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.FormStatus with NOCHECK add constraint ck_FormStatus
			check
				(
				sf.fFormStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.FormStatus'
		and
			br.BusinessRuleStatus <> 'x'
