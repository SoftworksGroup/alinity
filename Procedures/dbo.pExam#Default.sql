...
GO
CREATE PROCEDURE [dbo].[pExam#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pExam#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.Exam records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.Exam table. When a new record is to be added from the UI, this procedure
...
columns of the vExam entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pExam
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pExam of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@examSID                   int               = -1										-- specific default required by EF - do not override
		,@examName                  nvarchar(50)      = null									-- no default provided from DB constraint - OK to override
		,@examCategory              nvarchar(65)      = null									-- no default provided from DB constraint - OK to override
		...
		,@isOnlineExam              bit               = CONVERT(bit,(0))			-- default provided from DB constraint - OK to override
		,@isEnabledOnPortal         bit               = CONVERT(bit,(0))			-- default provided from DB constraint - OK to override
		,@sequence                  int               = (0)										-- default provided from DB constraint - OK to override
		,@cultureSID                int               = null									-- no default provided from DB constraint - OK to override
		,@instructionText           nvarchar(max)     = null									-- no default provided from DB constraint - OK to override
		,@lastVerifiedTime          datetimeoffset(7) = null									-- no default provided from DB constraint - OK to override
		,@minLagDaysBetweenAttempts smallint          = (0)										-- default provided from DB constraint - OK to override
		,@maxAttemptsPerYear        tinyint           = (99)									-- default provided from DB constraint - OK to override
		,@vendorExamID              varchar(25)       = null									-- no default provided from DB constraint - OK to override
		,@userDefinedColumns        xml               = null									-- no default provided from DB constraint - OK to override
		,@examXID                   varchar(150)      = null									-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @examName                   = isnull(context.node.value('@ExamName'                 ,'nvarchar(50)'     ),@examName)
			,@examCategory               = isnull(context.node.value('@ExamCategory'             ,'nvarchar(65)'     ),@examCategory)
			...
			,@isOnlineExam               = isnull(context.node.value('@IsOnlineExam'             ,'bit'              ),@isOnlineExam)
			,@isEnabledOnPortal          = isnull(context.node.value('@IsEnabledOnPortal'        ,'bit'              ),@isEnabledOnPortal)
			,@sequence                   = isnull(context.node.value('@Sequence'                 ,'int'              ),@sequence)
			,@cultureSID                 = isnull(context.node.value('@CultureSID'               ,'int'              ),@cultureSID)
			,@instructionText            = isnull(context.node.value('@InstructionText'          ,'nvarchar(max)'    ),@instructionText)
			,@lastVerifiedTime           = isnull(context.node.value('@LastVerifiedTime'         ,'datetimeoffset(7)'),@lastVerifiedTime)
			,@minLagDaysBetweenAttempts  = isnull(context.node.value('@MinLagDaysBetweenAttempts','smallint'         ),@minLagDaysBetweenAttempts)
			,@maxAttemptsPerYear         = isnull(context.node.value('@MaxAttemptsPerYear'       ,'tinyint'          ),@maxAttemptsPerYear)
			,@vendorExamID               = isnull(context.node.value('@VendorExamID'             ,'varchar(25)'      ),@vendorExamID)
			,@examXID                    = isnull(context.node.value('@ExamXID'                  ,'varchar(150)'     ),@examXID)
