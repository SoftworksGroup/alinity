...
GO
CREATE PROCEDURE [stg].[pPersonProfile#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for stg.PersonProfile
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks stg.PersonProfile for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the stg.PersonProfile table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, stg.fPersonProfile#Check(), on each row. If an error is
...
Example:
	...
	exec stg.pPersonProfile#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'stg.PersonProfile'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'stg.PersonProfile'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'stg.PersonProfile'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'stg.PersonProfile'
			)
		...
		where
			brx.ApplicationEntitySCD = 'stg.PersonProfile'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'stg'
		and
			cc.ConstraintName = 'ck_PersonProfile'
		)
		begin
			exec sp_executesql N'alter table stg.PersonProfile drop constraint ck_PersonProfile'					-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table stg.PersonProfile with CHECK add constraint ck_PersonProfile
			check
				(
			...
			select @RowCount = count(1) from stg.PersonProfile									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PersonProfile'
			...
			begin
				...
				begin try
					...
					select
						@isValid = stg.fPersonProfile#Check
							(
							 x.PersonProfileSID
			...
			alter table stg.PersonProfile with NOCHECK add constraint ck_PersonProfile
			check
				(
				stg.fPersonProfile#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'stg.PersonProfile'
		and
			br.BusinessRuleStatus <> 'x'
