...
GO
CREATE PROCEDURE [dbo].[pPracticeArea#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPracticeArea#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.PracticeArea records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PracticeArea table. When a new record is to be added from the UI, this procedure
...
columns of the vPracticeArea entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pPracticeArea
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pPracticeArea of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@practiceAreaSID         int               = -1											-- specific default required by EF - do not override
		,@practiceAreaName        nvarchar(50)      = null										-- no default provided from DB constraint - OK to override
		,@practiceAreaCode        varchar(20)       = null										-- no default provided from DB constraint - OK to override
		,@practiceAreaCategory    nvarchar(65)      = null										-- no default provided from DB constraint - OK to override
		,@isPracticeScopeRequired bit               = (0)											-- default provided from DB constraint - OK to override
		...
		,@practiceAreaXID         varchar(150)      = null										-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @practiceAreaName         = isnull(context.node.value('@PracticeAreaName'       ,'nvarchar(50)'),@practiceAreaName)
			,@practiceAreaCode         = isnull(context.node.value('@PracticeAreaCode'       ,'varchar(20)' ),@practiceAreaCode)
			,@practiceAreaCategory     = isnull(context.node.value('@PracticeAreaCategory'   ,'nvarchar(65)'),@practiceAreaCategory)
			,@isPracticeScopeRequired  = isnull(context.node.value('@IsPracticeScopeRequired','bit'         ),@isPracticeScopeRequired)
			,@isDefault                = isnull(context.node.value('@IsDefault'              ,'bit'         ),@isDefault)
			,@isActive                 = isnull(context.node.value('@IsActive'               ,'bit'         ),@isActive)
			,@practiceAreaXID          = isnull(context.node.value('@PracticeAreaXID'        ,'varchar(150)'),@practiceAreaXID)
