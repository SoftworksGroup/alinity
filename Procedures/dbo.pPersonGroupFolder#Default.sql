...
GO
CREATE PROCEDURE [dbo].[pPersonGroupFolder#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPersonGroupFolder#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.PersonGroupFolder records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PersonGroupFolder table. When a new record is to be added from the UI, this procedure
...
columns of the vPersonGroupFolder entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pPersonGroupFolder
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pPersonGroupFolder of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@personGroupFolderSID       int               = -1										-- specific default required by EF - do not override
		,@parentPersonGroupFolderSID int               = null									-- no default provided from DB constraint - OK to override
		,@personGroupSID             int               = null									-- no default provided from DB constraint - OK to override
		,@folderName                 nvarchar(35)      = null									-- no default provided from DB constraint - OK to override
		,@isRoot                     bit               = (0)									-- default provided from DB constraint - OK to override
		,@userDefinedColumns         xml               = null									-- no default provided from DB constraint - OK to override
		,@personGroupFolderXID       varchar(150)      = null									-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @parentPersonGroupFolderSID  = isnull(context.node.value('@ParentPersonGroupFolderSID','int'         ),@parentPersonGroupFolderSID)
			,@personGroupSID              = isnull(context.node.value('@PersonGroupSID'            ,'int'         ),@personGroupSID)
			,@folderName                  = isnull(context.node.value('@FolderName'                ,'nvarchar(35)'),@folderName)
			,@isRoot                      = isnull(context.node.value('@IsRoot'                    ,'bit'         ),@isRoot)
			,@personGroupFolderXID        = isnull(context.node.value('@PersonGroupFolderXID'      ,'varchar(150)'),@personGroupFolderXID)
