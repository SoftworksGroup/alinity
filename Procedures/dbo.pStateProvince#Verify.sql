...
GO
CREATE PROCEDURE [dbo].[pStateProvince#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.StateProvince
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.StateProvince for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.StateProvince table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fStateProvince#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pStateProvince#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.StateProvince'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.StateProvince'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.StateProvince'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.StateProvince'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.StateProvince'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_StateProvince'
		)
		begin
			exec sp_executesql N'alter table dbo.StateProvince drop constraint ck_StateProvince'					-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.StateProvince with CHECK add constraint ck_StateProvince
			check
				(
				dbo.fStateProvince#Check
					(
					 StateProvinceSID
					,StateProvinceName
					,StateProvinceCode
			...
			select @RowCount = count(1) from dbo.StateProvince									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'StateProvince'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fStateProvince#Check
							(
							 x.StateProvinceSID
							,x.StateProvinceName
							,x.StateProvinceCode
			...
			alter table dbo.StateProvince with NOCHECK add constraint ck_StateProvince
			check
				(
				dbo.fStateProvince#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.StateProvince'
		and
			br.BusinessRuleStatus <> 'x'
