...
GO
CREATE PROCEDURE [sf].[pPersonOtherName#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.PersonOtherName
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.PersonOtherName for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.PersonOtherName table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fPersonOtherName#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pPersonOtherName#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.PersonOtherName'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.PersonOtherName'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.PersonOtherName'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.PersonOtherName'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.PersonOtherName'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_PersonOtherName'
		)
		begin
			exec sp_executesql N'alter table sf.PersonOtherName drop constraint ck_PersonOtherName'				-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.PersonOtherName with CHECK add constraint ck_PersonOtherName
			check
				(
				sf.fPersonOtherName#Check
					(
					 PersonOtherNameSID
					,PersonSID
					,OtherNameTypeSID
			...
			select @RowCount = count(1) from sf.PersonOtherName									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PersonOtherName'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.PersonOtherNameSID
				,x.RowGUID
			from
				sf.PersonOtherName x
			order by
				x.PersonOtherNameSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fPersonOtherName#Check
							(
							 x.PersonOtherNameSID
							,x.PersonSID
							,x.OtherNameTypeSID
			...
			alter table sf.PersonOtherName with NOCHECK add constraint ck_PersonOtherName
			check
				(
				sf.fPersonOtherName#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.PersonOtherName'
		and
			br.BusinessRuleStatus <> 'x'
