...
GO
CREATE PROCEDURE [sf].[pQueryCategory#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pQueryCategory#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.QueryCategory records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.QueryCategory table. When a new record is to be added from the UI, this procedure
...
columns of the vQueryCategory entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pQueryCategory
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pQueryCategory of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@queryCategorySID   int               = -1														-- specific default required by EF - do not override
		,@queryCategoryLabel nvarchar(35)      = N'Default'										-- default provided from DB constraint - OK to override
		,@queryCategoryCode  varchar(30)       = null													-- no default provided from DB constraint - OK to override
		,@usageNotes         nvarchar(max)     = null													-- no default provided from DB constraint - OK to override
		,@displayOrder       int               = (0)													-- default provided from DB constraint - OK to override
		...
		,@queryCategoryXID   varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @queryCategoryLabel  = isnull(context.node.value('@QueryCategoryLabel','nvarchar(35)' ),@queryCategoryLabel)
			,@queryCategoryCode   = isnull(context.node.value('@QueryCategoryCode' ,'varchar(30)'  ),@queryCategoryCode)
			,@usageNotes          = isnull(context.node.value('@UsageNotes'        ,'nvarchar(max)'),@usageNotes)
			,@displayOrder        = isnull(context.node.value('@DisplayOrder'      ,'int'          ),@displayOrder)
			,@isActive            = isnull(context.node.value('@IsActive'          ,'bit'          ),@isActive)
			,@isDefault           = isnull(context.node.value('@IsDefault'         ,'bit'          ),@isDefault)
			,@queryCategoryXID    = isnull(context.node.value('@QueryCategoryXID'  ,'varchar(150)' ),@queryCategoryXID)
