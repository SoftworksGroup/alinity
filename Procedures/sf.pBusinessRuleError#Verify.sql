...
GO
CREATE PROCEDURE [sf].[pBusinessRuleError#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.BusinessRuleError
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.BusinessRuleError for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.BusinessRuleError table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fBusinessRuleError#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pBusinessRuleError#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.BusinessRuleError'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.BusinessRuleError'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.BusinessRuleError'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.BusinessRuleError'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.BusinessRuleError'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_BusinessRuleError'
		)
		begin
			exec sp_executesql N'alter table sf.BusinessRuleError drop constraint ck_BusinessRuleError'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.BusinessRuleError with CHECK add constraint ck_BusinessRuleError
			check
				(
				sf.fBusinessRuleError#Check
					(
					 BusinessRuleErrorSID
					,BusinessRuleSID
					,MessageText
			...
			select @RowCount = count(1) from sf.BusinessRuleError								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'BusinessRuleError'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.BusinessRuleErrorSID
				,x.RowGUID
			from
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fBusinessRuleError#Check
							(
							 x.BusinessRuleErrorSID
							,x.BusinessRuleSID
							,x.MessageText
					...
					from
						sf.BusinessRuleError  x
					where
						x.BusinessRuleErrorSID = @nextSID
			...
			alter table sf.BusinessRuleError with NOCHECK add constraint ck_BusinessRuleError
			check
				(
				sf.fBusinessRuleError#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.BusinessRuleError'
		and
			br.BusinessRuleStatus <> 'x'
