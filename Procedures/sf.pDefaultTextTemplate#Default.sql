...
GO
CREATE PROCEDURE [sf].[pDefaultTextTemplate#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pDefaultTextTemplate#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.DefaultTextTemplate records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.DefaultTextTemplate table. When a new record is to be added from the UI, this procedure
...
columns of the vDefaultTextTemplate entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pDefaultTextTemplate
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pDefaultTextTemplate of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@defaultTextTemplateSID    int               = -1										-- specific default required by EF - do not override
		,@defaultTextTemplateSCD    varchar(10)       = null									-- no default provided from DB constraint - OK to override
		,@defaultTextTemplateLabel  nvarchar(35)      = null									-- no default provided from DB constraint - OK to override
		...
		,@defaultTextTemplateXID    varchar(150)      = null									-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @defaultTextTemplateSCD    = isnull(context.node.value('@DefaultTextTemplateSCD'  ,'varchar(10)' ),@defaultTextTemplateSCD)
			,@defaultTextTemplateLabel  = isnull(context.node.value('@DefaultTextTemplateLabel','nvarchar(35)'),@defaultTextTemplateLabel)
			,@textTemplateSID           = isnull(context.node.value('@TextTemplateSID'         ,'int'         ),@textTemplateSID)
			,@defaultTextTemplateXID    = isnull(context.node.value('@DefaultTextTemplateXID'  ,'varchar(150)'),@defaultTextTemplateXID)
