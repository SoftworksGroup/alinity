...
GO
CREATE PROCEDURE [dbo].[pReinstatement#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pReinstatement#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.Reinstatement records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.Reinstatement table. When a new record is to be added from the UI, this procedure
...
columns of the vReinstatement entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pReinstatement
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pReinstatement of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@reinstatementSID                       int               = -1				-- specific default required by EF - do not override
		...
		,@reinstatementXID                       varchar(150)      = null			-- no default provided from DB constraint - OK to override
		...
		,@reinstatementLabel                     nvarchar(80)									-- not a base table column (default ignored)
		,@isRegisterChange                       bit													-- not a base table column (default ignored)
		,@hasOpenAudit                           bit													-- not a base table column (default ignored)
		,@isReinstatementOpen                    bit													-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@reinstatementXID            = isnull(context.node.value('@ReinstatementXID'          ,'varchar(150)'     ),@reinstatementXID)
		...
		begin																																														
			...
			join				sf.Form											f on prf.FormSID					= f.FormSID
			join
