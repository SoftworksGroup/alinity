...
GO
CREATE PROCEDURE [sf].[pApplicationGrant#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pApplicationGrant#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.ApplicationGrant records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.ApplicationGrant table. When a new record is to be added from the UI, this procedure
...
columns of the vApplicationGrant entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pApplicationGrant
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pApplicationGrant of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@applicationGrantSID  int               = -1													-- specific default required by EF - do not override
		,@applicationGrantSCD  varchar(30)       = null												-- no default provided from DB constraint - OK to override
		,@applicationGrantName nvarchar(150)     = null												-- no default provided from DB constraint - OK to override
		...
		,@applicationGrantXID  varchar(150)      = null												-- no default provided from DB constraint - OK to override
		...
		,@moduleSCD            varchar(15)																		-- not a base table column (default ignored)
		,@moduleName           nvarchar(100)																	-- not a base table column (default ignored)
		,@totalLicenses        int																						-- not a base table column (default ignored)
		,@remainingLicenses    int																						-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @applicationGrantSCD   = isnull(context.node.value('@ApplicationGrantSCD' ,'varchar(30)'  ),@applicationGrantSCD)
			,@applicationGrantName  = isnull(context.node.value('@ApplicationGrantName','nvarchar(150)'),@applicationGrantName)
			...
			,@applicationGrantXID   = isnull(context.node.value('@ApplicationGrantXID' ,'varchar(150)' ),@applicationGrantXID)
