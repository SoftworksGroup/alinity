...
GO
CREATE PROCEDURE [sf].[pAuditAction#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.AuditAction
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.AuditAction for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.AuditAction table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fAuditAction#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pAuditAction#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.AuditAction'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.AuditAction'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.AuditAction'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.AuditAction'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.AuditAction'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_AuditAction'
		)
		begin
			exec sp_executesql N'alter table sf.AuditAction drop constraint ck_AuditAction'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.AuditAction with CHECK add constraint ck_AuditAction
			check
				(
				sf.fAuditAction#Check
					(
					 AuditActionSID
					,AuditActionSCD
					,AuditActionName
			...
			select @RowCount = count(1) from sf.AuditAction											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.AuditAction with NOCHECK add constraint ck_AuditAction
			check
				(
				sf.fAuditAction#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.AuditAction'
		and
			br.BusinessRuleStatus <> 'x'
