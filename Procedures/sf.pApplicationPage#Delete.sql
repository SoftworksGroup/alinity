...
GO
CREATE PROCEDURE [sf].[pApplicationPage#Delete]
	 @ApplicationPageSID       int               = null -- required! id of row to delete - must be set in custom logic if not passed
...
as
/*********************************************************************************************************************************
Procedure : sf.pApplicationPage#Delete
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : deletes 1 row in the sf.ApplicationPage table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.ApplicationPage table. The procedure requires a primary key value to locate the record
to delete.
...
and in the ext.pApplicationPage procedure for client-specific deletion rules.
...
begin
	...
	begin try
		...
		if @RowStamp is null select @RowStamp = x.RowStamp from sf.ApplicationPage x where x.ApplicationPageSID = @ApplicationPageSID
    ...
    --! <PreDelete>
		...
		-- Ji Zhou | Feb 2013
    -- Ensure the row can be deleted. This error should not appear if the UI is enabling    -- the deletion option based on the value returned from the IsDeleteEnabled function
		-- in the #Ext view.
    set @IsDeleteEnabled = sf.fApplicationPage#IsDeleteEnabled(@ApplicationPageSID)
    ...
    begin
      ...
      exec sf.pMessage#Get
        ...
        ,@MessageText   = @errorText output
        ,@DefaultText   = N'Deletion of this %1 record is not allowed because one or more %2 uses it or you do not have sufficient grant.'
        ,@Arg1          = N'application page'
   ...
   raiserror(@errorText, 16, 1)
    ...
    --! </PreDelete>
		...
		update																																-- set audit details on sf.ApplicationPageReport rows that will delete through CASCADE
