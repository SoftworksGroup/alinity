...
GO
CREATE PROCEDURE [dbo].[pPersonGroupFolder#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PersonGroupFolder
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PersonGroupFolder for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PersonGroupFolder table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPersonGroupFolder#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPersonGroupFolder#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PersonGroupFolder'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PersonGroupFolder'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PersonGroupFolder'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PersonGroupFolder'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PersonGroupFolder'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PersonGroupFolder'
		)
		begin
			exec sp_executesql N'alter table dbo.PersonGroupFolder drop constraint ck_PersonGroupFolder'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PersonGroupFolder with CHECK add constraint ck_PersonGroupFolder
			check
				(
				dbo.fPersonGroupFolder#Check
					(
			...
			select @RowCount = count(1) from dbo.PersonGroupFolder							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PersonGroupFolder'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.PersonGroupFolderSID
				,x.RowGUID
			from
				dbo.PersonGroupFolder x
			order by
				x.PersonGroupFolderSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPersonGroupFolder#Check
							(
							 x.PersonGroupFolderSID
							,x.ParentPersonGroupFolderSID
							,x.PersonGroupSID
							,x.FolderName
							,x.IsRoot
							,x.PersonGroupFolderXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.PersonGroupFolder  x
					where
						x.PersonGroupFolderSID = @nextSID
			...
			alter table dbo.PersonGroupFolder with NOCHECK add constraint ck_PersonGroupFolder
			check
				(
				dbo.fPersonGroupFolder#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PersonGroupFolder'
		and
			br.BusinessRuleStatus <> 'x'
