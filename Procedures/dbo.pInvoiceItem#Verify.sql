...
GO
CREATE PROCEDURE [dbo].[pInvoiceItem#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.InvoiceItem
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.InvoiceItem for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.InvoiceItem table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fInvoiceItem#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pInvoiceItem#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.InvoiceItem'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.InvoiceItem'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.InvoiceItem'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.InvoiceItem'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.InvoiceItem'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_InvoiceItem'
		)
		begin
			exec sp_executesql N'alter table dbo.InvoiceItem drop constraint ck_InvoiceItem'							-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.InvoiceItem with CHECK add constraint ck_InvoiceItem
			check
				(
				dbo.fInvoiceItem#Check
					(
					 InvoiceItemSID
					,InvoiceSID
			...
			select @RowCount = count(1) from dbo.InvoiceItem										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fInvoiceItem#Check
							(
							 x.InvoiceItemSID
							,x.InvoiceSID
			...
			alter table dbo.InvoiceItem with NOCHECK add constraint ck_InvoiceItem
			check
				(
				dbo.fInvoiceItem#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.InvoiceItem'
		and
			br.BusinessRuleStatus <> 'x'
