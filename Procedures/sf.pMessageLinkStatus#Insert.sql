...
GO
CREATE PROCEDURE [sf].[pMessageLinkStatus#Insert]
	 @MessageLinkStatusSID   int               = null output								-- identity value assigned to the new record
	,@MessageLinkStatusSCD   varchar(10)       = null												-- required! if not passed value must be set in custom logic prior to insert
	,@MessageLinkStatusLabel nvarchar(35)      = null												-- required! if not passed value must be set in custom logic prior to insert
	,@UsageNotes             nvarchar(max)     = null												-- required! if not passed value must be set in custom logic prior to insert
	,@IsResendEnabled        bit               = null												-- default: (0)
...
as
/*********************************************************************************************************************************
Procedure : sf.pMessageLinkStatus#Insert
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : inserts 1 row into the sf.MessageLinkStatus table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.MessageLinkStatus table with parameter values represent columns of the table. Additional
parameters are provided for all columns in the vMessageLinkStatus entity view, however, the base logic of the procedure inserts the
...
begin
	...
	set @MessageLinkStatusSID = null																				-- initialize output parameter
	...
	begin try
		...
		set @IsResendEnabled = isnull(@IsResendEnabled,(0))
		...
		begin
			...
			select @recordSID = x.MessageLinkStatusSID from sf.MessageLinkStatus x where x.IsDefault = @ON
		...
		select
			...
			,@MessageLinkStatusSID = scope_identity()														-- capture key value of new row
