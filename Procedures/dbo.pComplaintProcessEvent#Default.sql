...
GO
CREATE PROCEDURE [dbo].[pComplaintProcessEvent#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pComplaintProcessEvent#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.ComplaintProcessEvent records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.ComplaintProcessEvent table. When a new record is to be added from the UI, this procedure
...
columns of the vComplaintProcessEvent entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pComplaintProcessEvent
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pComplaintProcessEvent of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@complaintProcessEventSID    int               = -1									-- specific default required by EF - do not override
		,@complaintProcessSID         int               = null								-- no default provided from DB constraint - OK to override
		...
		,@sequence                    decimal(5,1)      = (0.0)								-- default provided from DB constraint - OK to override
		,@targetDurationDays          smallint          = (0)									-- default provided from DB constraint - OK to override
		,@userDefinedColumns          xml               = null								-- no default provided from DB constraint - OK to override
		,@complaintProcessEventXID    varchar(150)      = null								-- no default provided from DB constraint - OK to override
		...
		,@complaintProcessLabel       nvarchar(35)      = N'?'								-- not a base table column (default ignored)
		,@complaintProcessIsDefault   bit               = 0										-- not a base table column (default ignored)
		,@parentComplaintProcessSID   int																			-- not a base table column (default ignored)
		,@complaintProcessRowGUID     uniqueidentifier  = newid()							-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @complaintProcessSID       = isnull(context.node.value('@ComplaintProcessSID'     ,'int'          ),@complaintProcessSID)
			...
			,@sequence                  = isnull(context.node.value('@Sequence'                ,'decimal(5,1)' ),@sequence)
			,@targetDurationDays        = isnull(context.node.value('@TargetDurationDays'      ,'smallint'     ),@targetDurationDays)
			,@complaintProcessEventXID  = isnull(context.node.value('@ComplaintProcessEventXID','varchar(150)' ),@complaintProcessEventXID)
		...
		if isnull(@complaintProcessSID  ,0) = 0 select @complaintProcessSID   = x.ComplaintProcessSID   from dbo.ComplaintProcess   x where x.IsDefault = @ON
