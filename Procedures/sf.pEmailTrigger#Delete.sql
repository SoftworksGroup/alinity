...
GO
CREATE PROCEDURE [sf].[pEmailTrigger#Delete]
	 @EmailTriggerSID                  int               = null -- required! id of row to delete - must be set in custom logic if not passed
	...
	,@EmailTemplateLabel               nvarchar(35)      = null
	...
	,@EmailTemplateRowGUID             uniqueidentifier  = null
...
as
/*********************************************************************************************************************************
Procedure : sf.pEmailTrigger#Delete
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : deletes 1 row in the sf.EmailTrigger table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.EmailTrigger table. The procedure requires a primary key value to locate the record
to delete.
...
and in the ext.pEmailTrigger procedure for client-specific deletion rules.
...
begin
	...
	begin try
		...
		if @RowStamp is null select @RowStamp = x.RowStamp from sf.EmailTrigger x where x.EmailTriggerSID = @EmailTriggerSID
