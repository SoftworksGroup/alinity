...
GO
CREATE PROCEDURE [sf].[pCardType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.CardType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.CardType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.CardType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fCardType#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pCardType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.CardType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.CardType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.CardType'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.CardType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.CardType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_CardType'
		)
		begin
			exec sp_executesql N'alter table sf.CardType drop constraint ck_CardType'											-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.CardType with CHECK add constraint ck_CardType
			check
				(
				sf.fCardType#Check
					(
			...
			select @RowCount = count(1) from sf.CardType												-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.CardType with NOCHECK add constraint ck_CardType
			check
				(
				sf.fCardType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.CardType'
		and
			br.BusinessRuleStatus <> 'x'
