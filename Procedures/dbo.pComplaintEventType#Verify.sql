...
GO
CREATE PROCEDURE [dbo].[pComplaintEventType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ComplaintEventType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ComplaintEventType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ComplaintEventType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fComplaintEventType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pComplaintEventType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ComplaintEventType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ComplaintEventType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ComplaintEventType'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ComplaintEventType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ComplaintEventType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ComplaintEventType'
		)
		begin
			exec sp_executesql N'alter table dbo.ComplaintEventType drop constraint ck_ComplaintEventType'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ComplaintEventType with CHECK add constraint ck_ComplaintEventType
			check
				(
				dbo.fComplaintEventType#Check
					(
			...
			select @RowCount = count(1) from dbo.ComplaintEventType							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ComplaintEventType'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fComplaintEventType#Check
							(
							 x.ComplaintEventTypeSID
							,x.ComplaintEventTypeSCD
							,x.ComplaintEventTypeLabel
							,x.IsDefault
							,x.ComplaintEventTypeXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.ComplaintEventType  x
					where
						x.ComplaintEventTypeSID = @nextSID
			...
			alter table dbo.ComplaintEventType with NOCHECK add constraint ck_ComplaintEventType
			check
				(
				dbo.fComplaintEventType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ComplaintEventType'
		and
			br.BusinessRuleStatus <> 'x'
