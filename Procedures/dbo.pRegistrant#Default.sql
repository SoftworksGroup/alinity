...
GO
CREATE PROCEDURE [dbo].[pRegistrant#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pRegistrant#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.Registrant records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.Registrant table. When a new record is to be added from the UI, this procedure
...
columns of the vRegistrant entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pRegistrant
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pRegistrant of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@registrantSID                  int               = -1								-- specific default required by EF - do not override
		...
		,@yearOfInitialEmployment        smallint          = null							-- no default provided from DB constraint - OK to override
		,@isOnPublicRegistry             bit               = CONVERT(bit,(1))	-- default provided from DB constraint - OK to override
		,@cityNameOfBirth                nvarchar(30)      = null							-- no default provided from DB constraint - OK to override
		...
		,@lateFeeExclusionYear           smallint          = null							-- no default provided from DB constraint - OK to override
		,@isRenewalAutoApprovalBlocked   bit               = CONVERT(bit,(0))	-- default provided from DB constraint - OK to override
		,@renewalExtensionExpiryTime     datetime          = null							-- no default provided from DB constraint - OK to override
		,@publicDirectoryComment         nvarchar(max)     = null							-- no default provided from DB constraint - OK to override
		...
		,@registrantXID                  varchar(150)      = null							-- no default provided from DB constraint - OK to override
		...
		,@countryName                    nvarchar(50)													-- not a base table column (default ignored)
		...
		,@registrationSID                int																	-- not a base table column (default ignored)
		,@registrationNo                 nvarchar(50)													-- not a base table column (default ignored)
		,@practiceRegisterSID            int																	-- not a base table column (default ignored)
		,@practiceRegisterSectionSID     int																	-- not a base table column (default ignored)
		...
		,@isActivePractice               bit																	-- not a base table column (default ignored)
		,@practiceRegisterSectionLabel   nvarchar(35)													-- not a base table column (default ignored)
		,@isSectionDisplayedOnLicense    bit																	-- not a base table column (default ignored)
		,@licenseRegistrationYear        smallint															-- not a base table column (default ignored)
		,@renewalRegistrationYear        smallint															-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@yearOfInitialEmployment         = isnull(context.node.value('@YearOfInitialEmployment'       ,'smallint'         ),@yearOfInitialEmployment)
			,@isOnPublicRegistry              = isnull(context.node.value('@IsOnPublicRegistry'            ,'bit'              ),@isOnPublicRegistry)
			,@cityNameOfBirth                 = isnull(context.node.value('@CityNameOfBirth'               ,'nvarchar(30)'     ),@cityNameOfBirth)
			...
			,@lateFeeExclusionYear            = isnull(context.node.value('@LateFeeExclusionYear'          ,'smallint'         ),@lateFeeExclusionYear)
			,@isRenewalAutoApprovalBlocked    = isnull(context.node.value('@IsRenewalAutoApprovalBlocked'  ,'bit'              ),@isRenewalAutoApprovalBlocked)
			,@renewalExtensionExpiryTime      = isnull(context.node.value('@RenewalExtensionExpiryTime'    ,'datetime'         ),@renewalExtensionExpiryTime)
			,@publicDirectoryComment          = isnull(context.node.value('@PublicDirectoryComment'        ,'nvarchar(max)'    ),@publicDirectoryComment)
			,@archivedTime                    = isnull(context.node.value('@ArchivedTime'                  ,'datetimeoffset(7)'),@archivedTime)
			,@registrantXID                   = isnull(context.node.value('@RegistrantXID'                 ,'varchar(150)'     ),@registrantXID)
