...
GO
CREATE PROCEDURE [dbo].[pRegistrantIdentifier#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pRegistrantIdentifier#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.RegistrantIdentifier records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.RegistrantIdentifier table. When a new record is to be added from the UI, this procedure
...
columns of the vRegistrantIdentifier entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pRegistrantIdentifier
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pRegistrantIdentifier of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@registrantIdentifierSID        int               = -1								-- specific default required by EF - do not override
		,@registrantSID                  int               = null							-- no default provided from DB constraint - OK to override
		,@identifierValue                varchar(50)       = null							-- no default provided from DB constraint - OK to override
		,@identifierTypeSID              int               = null							-- no default provided from DB constraint - OK to override
		,@effectiveDate                  date              = null							-- no default provided from DB constraint - OK to override
		...
		,@registrantIdentifierXID        varchar(150)      = null							-- no default provided from DB constraint - OK to override
		...
		,@identifierTypeLabel            nvarchar(35)      = N'?'							-- not a base table column (default ignored)
		,@identifierTypeCategory         nvarchar(65)													-- not a base table column (default ignored)
		,@isOtherRegistration            bit               = 0								-- not a base table column (default ignored)
		,@displayRank                    tinyint           = 0								-- not a base table column (default ignored)
		,@editMask                       varchar(50)       = '?'							-- not a base table column (default ignored)
		,@identifierCode                 varchar(15)       = '?'							-- not a base table column (default ignored)
		,@identifierTypeIsDefault        bit               = 0								-- not a base table column (default ignored)
		,@identifierTypeRowGUID          uniqueidentifier  = newid()					-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@identifierValue          = isnull(context.node.value('@IdentifierValue'        ,'varchar(50)' ),@identifierValue)
			,@identifierTypeSID        = isnull(context.node.value('@IdentifierTypeSID'      ,'int'         ),@identifierTypeSID)
			,@effectiveDate            = isnull(context.node.value('@EffectiveDate'          ,'date'        ),@effectiveDate)
			,@expiryDate               = isnull(context.node.value('@ExpiryDate'             ,'date'        ),@expiryDate)
			,@registrantIdentifierXID  = isnull(context.node.value('@RegistrantIdentifierXID','varchar(150)'),@registrantIdentifierXID)
		...
		if isnull(@identifierTypeSID,0) = 0 select @identifierTypeSID = x.IdentifierTypeSID from dbo.IdentifierType x where x.IsDefault = @ON
