...
GO
CREATE PROCEDURE [dbo].[pRegistrantAuditReviewResponse#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pRegistrantAuditReviewResponse#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.RegistrantAuditReviewResponse records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.RegistrantAuditReviewResponse table. When a new record is to be added from the UI, this procedure
...
columns of the vRegistrantAuditReviewResponse entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pRegistrantAuditReviewResponse
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pRegistrantAuditReviewResponse of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@registrantAuditReviewResponseSID int               = -1							-- specific default required by EF - do not override
		,@registrantAuditReviewSID         int               = null						-- no default provided from DB constraint - OK to override
		...
		,@registrantAuditReviewResponseXID varchar(150)      = null						-- no default provided from DB constraint - OK to override
		...
		,@registrantAuditSID               int               = 0							-- not a base table column (default ignored)
		...
		,@recommendationSID                int																-- not a base table column (default ignored)
		...
		,@registrantAuditReviewRowGUID     uniqueidentifier  = newid()				-- not a base table column (default ignored)
	...
	begin try
		...
		if @SetFKDefaults = @ON
		begin
			set @registrantAuditReviewSID = -1
			set @formOwnerSID = -1
		end
		...
		select
			 @registrantAuditReviewSID          = isnull(context.node.value('@RegistrantAuditReviewSID'        ,'int'         ),@registrantAuditReviewSID)
			,@formOwnerSID                      = isnull(context.node.value('@FormOwnerSID'                    ,'int'         ),@formOwnerSID)
			,@registrantAuditReviewResponseXID  = isnull(context.node.value('@RegistrantAuditReviewResponseXID','varchar(150)'),@registrantAuditReviewResponseXID)
