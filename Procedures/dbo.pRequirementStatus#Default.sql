...
GO
CREATE PROCEDURE [dbo].[pRequirementStatus#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pRequirementStatus#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.RequirementStatus records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.RequirementStatus table. When a new record is to be added from the UI, this procedure
...
columns of the vRequirementStatus entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pRequirementStatus
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pRequirementStatus of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@requirementStatusSID      int               = -1										-- specific default required by EF - do not override
		,@requirementStatusSCD      varchar(10)       = null									-- no default provided from DB constraint - OK to override
		,@requirementStatusLabel    nvarchar(35)      = null									-- no default provided from DB constraint - OK to override
		,@isFinal                   bit               = CONVERT(bit,(0))			-- default provided from DB constraint - OK to override
		,@description               nvarchar(max)     = null									-- no default provided from DB constraint - OK to override
		,@requirementStatusSequence int               = (0)										-- default provided from DB constraint - OK to override
		...
		,@requirementStatusXID      varchar(150)      = null									-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @requirementStatusSCD       = isnull(context.node.value('@RequirementStatusSCD'     ,'varchar(10)'  ),@requirementStatusSCD)
			,@requirementStatusLabel     = isnull(context.node.value('@RequirementStatusLabel'   ,'nvarchar(35)' ),@requirementStatusLabel)
			,@isFinal                    = isnull(context.node.value('@IsFinal'                  ,'bit'          ),@isFinal)
			,@description                = isnull(context.node.value('@Description'              ,'nvarchar(max)'),@description)
			,@requirementStatusSequence  = isnull(context.node.value('@RequirementStatusSequence','int'          ),@requirementStatusSequence)
			,@isDefault                  = isnull(context.node.value('@IsDefault'                ,'bit'          ),@isDefault)
			,@requirementStatusXID       = isnull(context.node.value('@RequirementStatusXID'     ,'varchar(150)' ),@requirementStatusXID)
