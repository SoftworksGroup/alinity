...
GO
CREATE PROCEDURE [dbo].[pRequirementStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RequirementStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RequirementStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RequirementStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRequirementStatus#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRequirementStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RequirementStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RequirementStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RequirementStatus'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RequirementStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RequirementStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RequirementStatus'
		)
		begin
			exec sp_executesql N'alter table dbo.RequirementStatus drop constraint ck_RequirementStatus'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RequirementStatus with CHECK add constraint ck_RequirementStatus
			check
				(
				dbo.fRequirementStatus#Check
					(
			...
			select @RowCount = count(1) from dbo.RequirementStatus							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RequirementStatus'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRequirementStatus#Check
							(
							 x.RequirementStatusSID
					...
					from
						dbo.RequirementStatus  x
					where
						x.RequirementStatusSID = @nextSID
			...
			alter table dbo.RequirementStatus with NOCHECK add constraint ck_RequirementStatus
			check
				(
				dbo.fRequirementStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RequirementStatus'
		and
			br.BusinessRuleStatus <> 'x'
