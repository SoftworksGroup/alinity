...
GO
CREATE PROCEDURE [dbo].[pRegistrantAppReviewResponse#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantAppReviewResponse
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantAppReviewResponse for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantAppReviewResponse table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantAppReviewResponse#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantAppReviewResponse#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantAppReviewResponse'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantAppReviewResponse'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAppReviewResponse'			-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAppReviewResponse'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantAppReviewResponse'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantAppReviewResponse'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantAppReviewResponse drop constraint ck_RegistrantAppReviewResponse'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantAppReviewResponse with CHECK add constraint ck_RegistrantAppReviewResponse
			check
				(
				dbo.fRegistrantAppReviewResponse#Check
					(
					 RegistrantAppReviewResponseSID
					,RegistrantAppReviewSID
					,FormOwnerSID
					,RegistrantAppReviewResponseXID
					,LegacyKey
					,IsDeleted
					,CreateUser
					,CreateTime
			...
			select @RowCount = count(1) from dbo.RegistrantAppReviewResponse		-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantAppReviewResponse'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrantAppReviewResponseSID
				,x.RowGUID
			from
				dbo.RegistrantAppReviewResponse x
			order by
				x.RegistrantAppReviewResponseSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantAppReviewResponse#Check
							(
							 x.RegistrantAppReviewResponseSID
							,x.RegistrantAppReviewSID
							,x.FormOwnerSID
							,x.RegistrantAppReviewResponseXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantAppReviewResponse  x
					where
						x.RegistrantAppReviewResponseSID = @nextSID
			...
			alter table dbo.RegistrantAppReviewResponse with NOCHECK add constraint ck_RegistrantAppReviewResponse
			check
				(
				dbo.fRegistrantAppReviewResponse#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantAppReviewResponse'
		and
			br.BusinessRuleStatus <> 'x'
