...
GO
CREATE PROCEDURE [sf].[pJobRun#Delete]
	 @JobRunSID                       int               = null -- required! id of row to delete - must be set in custom logic if not passed
	...
	,@JobRowGUID                      uniqueidentifier  = null
	,@IsDeleteEnabled                 bit               = null
	,@zContext                        xml               = null -- other values defining context for the delete (if any)
	,@JobStatusSCD                    varchar(35)       = null
	,@JobStatusLabel                  nvarchar(35)      = null
	,@RecordsPerMinute                int               = null
	,@RecordsRemaining                int               = null
	,@EstimatedMinutesRemaining       int               = null
	,@EstimatedEndTime                datetime          = null
	,@DurationMinutes                 int               = null
	,@StartTimeClientTZ               datetime          = null
	,@EndTimeClientTZ                 datetime          = null
	,@CancellationRequestTimeClientTZ datetime          = null
as
/*********************************************************************************************************************************
Procedure : sf.pJobRun#Delete
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : deletes 1 row in the sf.JobRun table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.JobRun table. The procedure requires a primary key value to locate the record
to delete.
...
and in the ext.pJobRun procedure for client-specific deletion rules.
...
begin
	...
	begin try
		...
		if @RowStamp is null select @RowStamp = x.RowStamp from sf.JobRun x where x.JobRunSID = @JobRunSID
		...
		update																																-- set audit details on sf.JobRunError rows that will delete through CASCADE
			sf.JobRunError
		set
