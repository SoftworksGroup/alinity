...
GO
CREATE PROCEDURE [dbo].[pTaxConfiguration#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pTaxConfiguration#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.TaxConfiguration records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.TaxConfiguration table. When a new record is to be added from the UI, this procedure
...
columns of the vTaxConfiguration entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pTaxConfiguration
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pTaxConfiguration of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@taxConfigurationSID       int               = -1										-- specific default required by EF - do not override
		,@taxSID                    int               = null									-- no default provided from DB constraint - OK to override
		,@taxRate                   decimal(4,4)      = null									-- no default provided from DB constraint - OK to override
		...
		,@taxConfigurationXID       varchar(150)      = null									-- no default provided from DB constraint - OK to override
		...
		,@taxLabel                  nvarchar(35)      = N'?'									-- not a base table column (default ignored)
		,@taxSequence               tinyint           = 0											-- not a base table column (default ignored)
		,@taxRowGUID                uniqueidentifier  = newid()								-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @taxSID               = isnull(context.node.value('@TaxSID'             ,'int'         ),@taxSID)
			,@taxRate              = isnull(context.node.value('@TaxRate'            ,'decimal(4,4)'),@taxRate)
			...
			,@taxConfigurationXID  = isnull(context.node.value('@TaxConfigurationXID','varchar(150)'),@taxConfigurationXID)
