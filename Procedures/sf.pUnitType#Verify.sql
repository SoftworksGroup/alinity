...
GO
CREATE PROCEDURE [sf].[pUnitType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.UnitType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.UnitType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.UnitType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fUnitType#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pUnitType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.UnitType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.UnitType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.UnitType'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.UnitType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.UnitType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_UnitType'
		)
		begin
			exec sp_executesql N'alter table sf.UnitType drop constraint ck_UnitType'											-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.UnitType with CHECK add constraint ck_UnitType
			check
				(
				sf.fUnitType#Check
					(
			...
			select @RowCount = count(1) from sf.UnitType												-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.UnitType with NOCHECK add constraint ck_UnitType
			check
				(
				sf.fUnitType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.UnitType'
		and
			br.BusinessRuleStatus <> 'x'
