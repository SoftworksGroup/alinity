...
GO
CREATE PROCEDURE [sf].[pGender#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.Gender
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.Gender for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.Gender table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fGender#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pGender#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.Gender'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.Gender'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.Gender'														-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.Gender'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.Gender'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_Gender'
		)
		begin
			exec sp_executesql N'alter table sf.Gender drop constraint ck_Gender'													-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.Gender with CHECK add constraint ck_Gender
			check
				(
				sf.fGender#Check
					(
			...
			select @RowCount = count(1) from sf.Gender													-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.Gender with NOCHECK add constraint ck_Gender
			check
				(
				sf.fGender#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.Gender'
		and
			br.BusinessRuleStatus <> 'x'
