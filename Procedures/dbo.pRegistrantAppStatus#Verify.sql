...
GO
CREATE PROCEDURE [dbo].[pRegistrantAppStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantAppStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantAppStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantAppStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantAppStatus#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantAppStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantAppStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantAppStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAppStatus'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAppStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantAppStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantAppStatus'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantAppStatus drop constraint ck_RegistrantAppStatus'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantAppStatus with CHECK add constraint ck_RegistrantAppStatus
			check
				(
				dbo.fRegistrantAppStatus#Check
					(
					 RegistrantAppStatusSID
					,RegistrantAppSID
					,FormStatusSID
					,RegistrantAppStatusXID
					,LegacyKey
					,IsDeleted
					,CreateUser
					,CreateTime
			...
			select @RowCount = count(1) from dbo.RegistrantAppStatus						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantAppStatus'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantAppStatus#Check
							(
							 x.RegistrantAppStatusSID
							,x.RegistrantAppSID
							,x.FormStatusSID
							,x.RegistrantAppStatusXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantAppStatus  x
					where
						x.RegistrantAppStatusSID = @nextSID
			...
			alter table dbo.RegistrantAppStatus with NOCHECK add constraint ck_RegistrantAppStatus
			check
				(
				dbo.fRegistrantAppStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantAppStatus'
		and
			br.BusinessRuleStatus <> 'x'
