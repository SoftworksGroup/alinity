...
GO
CREATE PROCEDURE [dbo].[pComplaintEvent#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ComplaintEvent
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ComplaintEvent for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ComplaintEvent table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fComplaintEvent#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pComplaintEvent#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ComplaintEvent'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ComplaintEvent'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ComplaintEvent'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ComplaintEvent'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ComplaintEvent'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ComplaintEvent'
		)
		begin
			exec sp_executesql N'alter table dbo.ComplaintEvent drop constraint ck_ComplaintEvent'				-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ComplaintEvent with CHECK add constraint ck_ComplaintEvent
			check
				(
				dbo.fComplaintEvent#Check
					(
					 ComplaintEventSID
					,ComplaintSID
			...
			select @RowCount = count(1) from dbo.ComplaintEvent									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ComplaintEvent'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fComplaintEvent#Check
							(
							 x.ComplaintEventSID
							,x.ComplaintSID
			...
			alter table dbo.ComplaintEvent with NOCHECK add constraint ck_ComplaintEvent
			check
				(
				dbo.fComplaintEvent#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ComplaintEvent'
		and
			br.BusinessRuleStatus <> 'x'
