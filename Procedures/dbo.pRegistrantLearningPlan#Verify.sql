...
GO
CREATE PROCEDURE [dbo].[pRegistrantLearningPlan#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantLearningPlan
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantLearningPlan for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantLearningPlan table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantLearningPlan#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantLearningPlan#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantLearningPlan'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantLearningPlan'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantLearningPlan'						-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantLearningPlan'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantLearningPlan'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantLearningPlan'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantLearningPlan drop constraint ck_RegistrantLearningPlan'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantLearningPlan with CHECK add constraint ck_RegistrantLearningPlan
			check
				(
				dbo.fRegistrantLearningPlan#Check
					(
			...
			select @RowCount = count(1) from dbo.RegistrantLearningPlan					-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantLearningPlan'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantLearningPlan#Check
							(
							 x.RegistrantLearningPlanSID
							,x.RegistrantSID
							,x.RegistrationYear
					...
					from
						dbo.RegistrantLearningPlan  x
					where
						x.RegistrantLearningPlanSID = @nextSID
			...
			alter table dbo.RegistrantLearningPlan with NOCHECK add constraint ck_RegistrantLearningPlan
			check
				(
				dbo.fRegistrantLearningPlan#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantLearningPlan'
		and
			br.BusinessRuleStatus <> 'x'
