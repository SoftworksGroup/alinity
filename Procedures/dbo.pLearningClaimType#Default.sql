...
GO
CREATE PROCEDURE [dbo].[pLearningClaimType#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pLearningClaimType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.LearningClaimType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.LearningClaimType table. When a new record is to be added from the UI, this procedure
...
columns of the vLearningClaimType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pLearningClaimType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pLearningClaimType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@learningClaimTypeSID      int               = -1										-- specific default required by EF - do not override
		,@learningClaimTypeLabel    nvarchar(35)      = null									-- no default provided from DB constraint - OK to override
		,@learningClaimTypeCategory nvarchar(65)      = null									-- no default provided from DB constraint - OK to override
		,@isValidForRenewal         bit               = CONVERT(bit,(1))			-- default provided from DB constraint - OK to override
		,@isComplete                bit               = (0)										-- default provided from DB constraint - OK to override
		,@isWithdrawn               bit               = CONVERT(bit,(0))			-- default provided from DB constraint - OK to override
		...
		,@learningClaimTypeXID      varchar(150)      = null									-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @learningClaimTypeLabel     = isnull(context.node.value('@LearningClaimTypeLabel'   ,'nvarchar(35)'),@learningClaimTypeLabel)
			,@learningClaimTypeCategory  = isnull(context.node.value('@LearningClaimTypeCategory','nvarchar(65)'),@learningClaimTypeCategory)
			,@isValidForRenewal          = isnull(context.node.value('@IsValidForRenewal'        ,'bit'         ),@isValidForRenewal)
			,@isComplete                 = isnull(context.node.value('@IsComplete'               ,'bit'         ),@isComplete)
			,@isWithdrawn                = isnull(context.node.value('@IsWithdrawn'              ,'bit'         ),@isWithdrawn)
			,@isDefault                  = isnull(context.node.value('@IsDefault'                ,'bit'         ),@isDefault)
			,@isActive                   = isnull(context.node.value('@IsActive'                 ,'bit'         ),@isActive)
			,@learningClaimTypeXID       = isnull(context.node.value('@LearningClaimTypeXID'     ,'varchar(150)'),@learningClaimTypeXID)
