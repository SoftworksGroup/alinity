...
GO
CREATE PROCEDURE [dbo].[pOrgType#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pOrgType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.OrgType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.OrgType table. When a new record is to be added from the UI, this procedure
...
columns of the vOrgType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pOrgType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pOrgType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@orgTypeSID         int               = -1														-- specific default required by EF - do not override
		,@orgTypeName        nvarchar(50)      = null													-- no default provided from DB constraint - OK to override
		,@orgTypeCode        varchar(20)       = null													-- no default provided from DB constraint - OK to override
		,@sectorCode         varchar(5)        = null													-- no default provided from DB constraint - OK to override
		,@orgTypeCategory    nvarchar(65)      = null													-- no default provided from DB constraint - OK to override
		...
		,@orgTypeXID         varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @orgTypeName      = isnull(context.node.value('@OrgTypeName'    ,'nvarchar(50)'),@orgTypeName)
			,@orgTypeCode      = isnull(context.node.value('@OrgTypeCode'    ,'varchar(20)' ),@orgTypeCode)
			,@sectorCode       = isnull(context.node.value('@SectorCode'     ,'varchar(5)'  ),@sectorCode)
			,@orgTypeCategory  = isnull(context.node.value('@OrgTypeCategory','nvarchar(65)'),@orgTypeCategory)
			,@isDefault        = isnull(context.node.value('@IsDefault'      ,'bit'         ),@isDefault)
			,@isActive         = isnull(context.node.value('@IsActive'       ,'bit'         ),@isActive)
			,@orgTypeXID       = isnull(context.node.value('@OrgTypeXID'     ,'varchar(150)'),@orgTypeXID)
