...
GO
CREATE PROCEDURE [dbo].[pRegistrationRequirementType#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pRegistrationRequirementType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.RegistrationRequirementType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.RegistrationRequirementType table. When a new record is to be added from the UI, this procedure
...
columns of the vRegistrationRequirementType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pRegistrationRequirementType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pRegistrationRequirementType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@registrationRequirementTypeSID      int               = -1					-- specific default required by EF - do not override
		,@registrationRequirementTypeLabel    nvarchar(35)      = null				-- no default provided from DB constraint - OK to override
		,@registrationRequirementTypeCode     varchar(20)       = null				-- no default provided from DB constraint - OK to override
		,@registrationRequirementTypeCategory nvarchar(65)      = null				-- no default provided from DB constraint - OK to override
		,@isAppliedToPeople                   bit               = CONVERT(bit,(1))											-- default provided from DB constraint - OK to override
		,@isAppliedToOrganizations            bit               = CONVERT(bit,(0))											-- default provided from DB constraint - OK to override
		...
		,@registrationRequirementTypeXID      varchar(150)      = null				-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @registrationRequirementTypeLabel     = isnull(context.node.value('@RegistrationRequirementTypeLabel'   ,'nvarchar(35)'),@registrationRequirementTypeLabel)
			,@registrationRequirementTypeCode      = isnull(context.node.value('@RegistrationRequirementTypeCode'    ,'varchar(20)' ),@registrationRequirementTypeCode)
			,@registrationRequirementTypeCategory  = isnull(context.node.value('@RegistrationRequirementTypeCategory','nvarchar(65)'),@registrationRequirementTypeCategory)
			,@isAppliedToPeople                    = isnull(context.node.value('@IsAppliedToPeople'                  ,'bit'         ),@isAppliedToPeople)
			,@isAppliedToOrganizations             = isnull(context.node.value('@IsAppliedToOrganizations'           ,'bit'         ),@isAppliedToOrganizations)
			,@isDefault                            = isnull(context.node.value('@IsDefault'                          ,'bit'         ),@isDefault)
			,@isActive                             = isnull(context.node.value('@IsActive'                           ,'bit'         ),@isActive)
			,@registrationRequirementTypeXID       = isnull(context.node.value('@RegistrationRequirementTypeXID'     ,'varchar(150)'),@registrationRequirementTypeXID)
