...
GO
CREATE PROCEDURE [sf].[pCrossWalk#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pCrossWalk#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.CrossWalk records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.CrossWalk table. When a new record is to be added from the UI, this procedure
...
columns of the vCrossWalk entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pCrossWalk
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pCrossWalk of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@crossWalkSID       int               = -1														-- specific default required by EF - do not override
		,@crossWalkLabel     nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		...
		,@crossWalkXID       varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @crossWalkLabel  = isnull(context.node.value('@CrossWalkLabel','nvarchar(35)'),@crossWalkLabel)
			,@isDefault       = isnull(context.node.value('@IsDefault'     ,'bit'         ),@isDefault)
			,@crossWalkXID    = isnull(context.node.value('@CrossWalkXID'  ,'varchar(150)'),@crossWalkXID)
