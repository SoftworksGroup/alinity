...
GO
CREATE PROCEDURE [dbo].[pPracticeRegister#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PracticeRegister
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PracticeRegister for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PracticeRegister table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPracticeRegister#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPracticeRegister#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PracticeRegister'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PracticeRegister'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRegister'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRegister'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PracticeRegister'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PracticeRegister'
		)
		begin
			exec sp_executesql N'alter table dbo.PracticeRegister drop constraint ck_PracticeRegister'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PracticeRegister with CHECK add constraint ck_PracticeRegister
			check
				(
			...
			select @RowCount = count(1) from dbo.PracticeRegister								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PracticeRegister'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPracticeRegister#Check
							(
							 x.PracticeRegisterSID
							...
							,x.IsActivePractice
							,x.IsPublicRegistryEnabled
							,x.IsRenewalEnabled
							,x.IsLearningPlanEnabled
							,x.IsNextCEFormAutoAdded
							,x.IsEligibleSupervisor
							,x.IsSupervisionRequired
							,x.IsEmploymentTerminated
							,x.IsGroupMembershipTerminated
							,x.TermPermitDays
							,x.RegisterRank
							,x.LearningModelSID
							,x.ReasonGroupSID
							,x.IsDefault
							,x.IsDefaultInactivePractice
							,x.IsActive
							,x.PracticeRegisterXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
			...
			alter table dbo.PracticeRegister with NOCHECK add constraint ck_PracticeRegister
			check
				(
				dbo.fPracticeRegister#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PracticeRegister'
		and
			br.BusinessRuleStatus <> 'x'
