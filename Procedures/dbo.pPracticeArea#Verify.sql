...
GO
CREATE PROCEDURE [dbo].[pPracticeArea#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PracticeArea
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PracticeArea for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PracticeArea table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPracticeArea#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPracticeArea#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PracticeArea'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PracticeArea'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeArea'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeArea'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PracticeArea'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PracticeArea'
		)
		begin
			exec sp_executesql N'alter table dbo.PracticeArea drop constraint ck_PracticeArea'						-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PracticeArea with CHECK add constraint ck_PracticeArea
			check
				(
				dbo.fPracticeArea#Check
					(
					 PracticeAreaSID
					,PracticeAreaName
					,PracticeAreaCode
			...
			select @RowCount = count(1) from dbo.PracticeArea										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPracticeArea#Check
							(
							 x.PracticeAreaSID
							,x.PracticeAreaName
							,x.PracticeAreaCode
							,x.PracticeAreaCategory
							,x.IsPracticeScopeRequired
							,x.IsDefault
							,x.IsActive
							,x.PracticeAreaXID
							,x.LegacyKey
			...
			alter table dbo.PracticeArea with NOCHECK add constraint ck_PracticeArea
			check
				(
				dbo.fPracticeArea#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PracticeArea'
		and
			br.BusinessRuleStatus <> 'x'
