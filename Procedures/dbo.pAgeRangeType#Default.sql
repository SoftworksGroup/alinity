...
GO
CREATE PROCEDURE [dbo].[pAgeRangeType#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pAgeRangeType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.AgeRangeType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.AgeRangeType table. When a new record is to be added from the UI, this procedure
...
columns of the vAgeRangeType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pAgeRangeType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pAgeRangeType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@ageRangeTypeSID    int               = -1														-- specific default required by EF - do not override
		,@ageRangeTypeLabel  nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		,@ageRangeTypeCode   varchar(15)       = null													-- no default provided from DB constraint - OK to override
		...
		,@ageRangeTypeXID    varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @ageRangeTypeLabel  = isnull(context.node.value('@AgeRangeTypeLabel','nvarchar(35)' ),@ageRangeTypeLabel)
			,@ageRangeTypeCode   = isnull(context.node.value('@AgeRangeTypeCode' ,'varchar(15)'  ),@ageRangeTypeCode)
			...
			,@ageRangeTypeXID    = isnull(context.node.value('@AgeRangeTypeXID'  ,'varchar(150)' ),@ageRangeTypeXID)
