...
GO
CREATE PROCEDURE [sf].[pTaskQueueSubscriber#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.TaskQueueSubscriber
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.TaskQueueSubscriber for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.TaskQueueSubscriber table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fTaskQueueSubscriber#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pTaskQueueSubscriber#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.TaskQueueSubscriber'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.TaskQueueSubscriber'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.TaskQueueSubscriber'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.TaskQueueSubscriber'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.TaskQueueSubscriber'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_TaskQueueSubscriber'
		)
		begin
			exec sp_executesql N'alter table sf.TaskQueueSubscriber drop constraint ck_TaskQueueSubscriber'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.TaskQueueSubscriber with CHECK add constraint ck_TaskQueueSubscriber
			check
				(
			...
			select @RowCount = count(1) from sf.TaskQueueSubscriber							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'TaskQueueSubscriber'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.TaskQueueSubscriberSID
				,x.RowGUID
			from
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fTaskQueueSubscriber#Check
							(
							 x.TaskQueueSubscriberSID
							,x.TaskQueueSID
							...
							,x.ExpiryTime
							,x.IsNewTaskEmailed
							,x.IsDailySummaryEmailed
							,x.TaskQueueSubscriberXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						sf.TaskQueueSubscriber  x
					where
						x.TaskQueueSubscriberSID = @nextSID
			...
			alter table sf.TaskQueueSubscriber with NOCHECK add constraint ck_TaskQueueSubscriber
			check
				(
				sf.fTaskQueueSubscriber#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.TaskQueueSubscriber'
		and
			br.BusinessRuleStatus <> 'x'
