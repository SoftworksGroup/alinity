...
GO
CREATE PROCEDURE [dbo].[pRegistrationProfile#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrationProfile
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrationProfile for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrationProfile table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrationProfile#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrationProfile#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrationProfile'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrationProfile'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationProfile'							-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationProfile'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrationProfile'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrationProfile'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrationProfile drop constraint ck_RegistrationProfile'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrationProfile with CHECK add constraint ck_RegistrationProfile
			check
				(
			...
			select @RowCount = count(1) from dbo.RegistrationProfile						-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrationProfile'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrationProfile#Check
							(
							 x.RegistrationProfileSID
							,x.RegistrationSnapshotSID
							,x.JursidictionStateProvinceISONumber
							,x.RegistrantSID
							,x.RegistrantNo
							,x.GenderSCD
							,x.BirthDate
							,x.PersonMailingAddressSID
							,x.ResidenceStateProvinceISONumber
							,x.ResidencePostalCode
							,x.ResidenceCountryISONumber
							,x.ResidenceIsDefaultCountry
							,x.RegistrationSID
							,x.IsActivePractice
							,x.Education1RegistrantCredentialSID
							,x.Education1CredentialCode
							,x.Education1GraduationYear
							,x.Education1StateProvinceISONumber
							,x.Education1CountryISONumber
							,x.Education1IsDefaultCountry
							,x.Education2RegistrantCredentialSID
							,x.Education2CredentialCode
							,x.Education2GraduationYear
							,x.Education2StateProvinceISONumber
							,x.Education2CountryISONumber
							,x.Education2IsDefaultCountry
							,x.Education3RegistrantCredentialSID
							,x.Education3CredentialCode
							,x.Education3GraduationYear
							,x.Education3StateProvinceISONumber
							,x.Education3CountryISONumber
							,x.Education3IsDefaultCountry
							,x.RegistrantPracticeSID
							,x.EmploymentStatusCode
							,x.EmploymentCount
							,x.PracticeHours
							,x.Employment1RegistrantEmploymentSID
							,x.Employment1TypeCode
							,x.Employment1StateProvinceISONumber
							,x.Employment1CountryISONumber
							,x.Employment1IsDefaultCountry
							,x.Employment1PostalCode
							,x.Employment1OrgTypeCode
							,x.Employment1PracticeAreaCode
							,x.Employment1PracticeScopeCode
							,x.Employment1RoleCode
							,x.Employment2RegistrantEmploymentSID
							,x.Employment2TypeCode
							,x.Employment2StateProvinceISONumber
							,x.Employment2IsDefaultCountry
							,x.Employment2CountryISONumber
							,x.Employment2PostalCode
							,x.Employment2OrgTypeCode
							,x.Employment2PracticeAreaCode
							,x.Employment2PracticeScopeCode
							,x.Employment2RoleCode
							,x.Employment3RegistrantEmploymentSID
							,x.Employment3TypeCode
							,x.Employment3StateProvinceISONumber
							,x.Employment3CountryISONumber
							,x.Employment3IsDefaultCountry
							,x.Employment3PostalCode
							,x.Employment3OrgTypeCode
							,x.Employment3PracticeAreaCode
							,x.Employment3PracticeScopeCode
							,x.Employment3RoleCode
							,x.IsInvalid
							,x.MessageText
							,x.CheckSumOnLastExport
							,x.RegistrationProfileXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrationProfile  x
					where
						x.RegistrationProfileSID = @nextSID
			...
			alter table dbo.RegistrationProfile with NOCHECK add constraint ck_RegistrationProfile
			check
				(
				dbo.fRegistrationProfile#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrationProfile'
		and
			br.BusinessRuleStatus <> 'x'
