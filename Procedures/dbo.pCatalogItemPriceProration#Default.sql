...
GO
CREATE PROCEDURE [dbo].[pCatalogItemPriceProration#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pCatalogItemPriceProration#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.CatalogItemPriceProration records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.CatalogItemPriceProration table. When a new record is to be added from the UI, this procedure
...
columns of the vCatalogItemPriceProration entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pCatalogItemPriceProration
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pCatalogItemPriceProration of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@catalogItemPriceProrationSID int               = -1									-- specific default required by EF - do not override
		,@catalogItemPriceSID          int               = null								-- no default provided from DB constraint - OK to override
		,@startMonthDay                char(4)           = null								-- no default provided from DB constraint - OK to override
		,@price                        decimal(11,2)     = (0.0)							-- default provided from DB constraint - OK to override
		,@percentageOfCurrentPrice     decimal(6,3)      = (0.0)							-- default provided from DB constraint - OK to override
		,@userDefinedColumns           xml               = null								-- no default provided from DB constraint - OK to override
		,@catalogItemPriceProrationXID varchar(150)      = null								-- no default provided from DB constraint - OK to override
		...
		,@catalogItemSID               int               = 0									-- not a base table column (default ignored)
		,@catalogItemPricePrice        decimal(11,2)     = convert(decimal(11,2), 0.0)									-- not a base table column (default ignored)
		,@effectiveTime                datetime          = sf.fNow()					-- not a base table column (default ignored)
		,@catalogItemPriceRowGUID      uniqueidentifier  = newid()						-- not a base table column (default ignored)
		...
		,@fiscalStartMonthDay          smallint																-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @catalogItemPriceSID           = isnull(context.node.value('@CatalogItemPriceSID'         ,'int'          ),@catalogItemPriceSID)
			...
			,@percentageOfCurrentPrice      = isnull(context.node.value('@PercentageOfCurrentPrice'    ,'decimal(6,3)' ),@percentageOfCurrentPrice)
			,@catalogItemPriceProrationXID  = isnull(context.node.value('@CatalogItemPriceProrationXID','varchar(150)' ),@catalogItemPriceProrationXID)
