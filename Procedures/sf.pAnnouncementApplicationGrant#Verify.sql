...
GO
CREATE PROCEDURE [sf].[pAnnouncementApplicationGrant#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.AnnouncementApplicationGrant
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.AnnouncementApplicationGrant for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.AnnouncementApplicationGrant table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fAnnouncementApplicationGrant#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pAnnouncementApplicationGrant#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.AnnouncementApplicationGrant'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.AnnouncementApplicationGrant'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.AnnouncementApplicationGrant'			-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.AnnouncementApplicationGrant'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.AnnouncementApplicationGrant'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_AnnouncementApplicationGrant'
		)
		begin
			exec sp_executesql N'alter table sf.AnnouncementApplicationGrant drop constraint ck_AnnouncementApplicationGrant'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.AnnouncementApplicationGrant with CHECK add constraint ck_AnnouncementApplicationGrant
			check
				(
				sf.fAnnouncementApplicationGrant#Check
					(
			...
			select @RowCount = count(1) from sf.AnnouncementApplicationGrant		-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'AnnouncementApplicationGrant'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.AnnouncementApplicationGrantSID
				,x.RowGUID
			from
				sf.AnnouncementApplicationGrant x
			order by
				x.AnnouncementApplicationGrantSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fAnnouncementApplicationGrant#Check
							(
							 x.AnnouncementApplicationGrantSID
							,x.AnnouncementSID
							,x.ApplicationGrantSID
							,x.AnnouncementApplicationGrantXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						sf.AnnouncementApplicationGrant  x
					where
						x.AnnouncementApplicationGrantSID = @nextSID
			...
			alter table sf.AnnouncementApplicationGrant with NOCHECK add constraint ck_AnnouncementApplicationGrant
			check
				(
				sf.fAnnouncementApplicationGrant#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.AnnouncementApplicationGrant'
		and
			br.BusinessRuleStatus <> 'x'
