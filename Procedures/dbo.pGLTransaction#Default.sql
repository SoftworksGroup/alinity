...
GO
CREATE PROCEDURE [dbo].[pGLTransaction#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pGLTransaction#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.GLTransaction records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.GLTransaction table. When a new record is to be added from the UI, this procedure
...
columns of the vGLTransaction entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pGLTransaction
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pGLTransaction of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@gLTransactionSID            int               = -1									-- specific default required by EF - do not override
		,@paymentSID                  int               = null								-- no default provided from DB constraint - OK to override
		,@invoicePaymentSID           int               = null								-- no default provided from DB constraint - OK to override
		,@debitGLAccountCode          varchar(50)       = null								-- no default provided from DB constraint - OK to override
		,@creditGLAccountCode         varchar(50)       = null								-- no default provided from DB constraint - OK to override
		,@amount                      decimal(11,2)     = null								-- no default provided from DB constraint - OK to override
		,@gLPostingDate               date              = null								-- no default provided from DB constraint - OK to override
		,@paymentCheckSum             int               = null								-- no default provided from DB constraint - OK to override
		,@invoicePaymentCheckSum      int               = null								-- no default provided from DB constraint - OK to override
		,@reversedGLTransactionSID    int               = null								-- no default provided from DB constraint - OK to override
		,@isExcluded                  bit               = CONVERT(bit,(0))		-- default provided from DB constraint - OK to override
		,@userDefinedColumns          xml               = null								-- no default provided from DB constraint - OK to override
		,@gLTransactionXID            varchar(150)      = null								-- no default provided from DB constraint - OK to override
		...
		,@invoicePaymentPaymentSID    int																			-- not a base table column (default ignored)
		,@amountApplied               decimal(11,2)														-- not a base table column (default ignored)
		,@appliedDate                 date																		-- not a base table column (default ignored)
		,@invoicePaymentGLPostingDate date																		-- not a base table column (default ignored)
		,@invoicePaymentCancelledTime datetimeoffset(7)												-- not a base table column (default ignored)
		,@invoicePaymentReasonSID     int																			-- not a base table column (default ignored)
		,@invoicePaymentRowGUID       uniqueidentifier												-- not a base table column (default ignored)
		...
		,@fullDebitAccountLabel       nvarchar(86)														-- not a base table column (default ignored)
		,@fullCreditAccountLabel      nvarchar(86)														-- not a base table column (default ignored)
		,@isReversing                 bit																			-- not a base table column (default ignored)
		,@isReversed                  bit																			-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@invoicePaymentSID         = isnull(context.node.value('@InvoicePaymentSID'       ,'int'          ),@invoicePaymentSID)
			,@debitGLAccountCode        = isnull(context.node.value('@DebitGLAccountCode'      ,'varchar(50)'  ),@debitGLAccountCode)
			,@creditGLAccountCode       = isnull(context.node.value('@CreditGLAccountCode'     ,'varchar(50)'  ),@creditGLAccountCode)
			,@amount                    = isnull(context.node.value('@Amount'                  ,'decimal(11,2)'),@amount)
			,@gLPostingDate             = isnull(context.node.value('@GLPostingDate'           ,'date'         ),@gLPostingDate)
			,@paymentCheckSum           = isnull(context.node.value('@PaymentCheckSum'         ,'int'          ),@paymentCheckSum)
			,@invoicePaymentCheckSum    = isnull(context.node.value('@InvoicePaymentCheckSum'  ,'int'          ),@invoicePaymentCheckSum)
			,@reversedGLTransactionSID  = isnull(context.node.value('@ReversedGLTransactionSID','int'          ),@reversedGLTransactionSID)
			,@isExcluded                = isnull(context.node.value('@IsExcluded'              ,'bit'          ),@isExcluded)
			,@gLTransactionXID          = isnull(context.node.value('@GLTransactionXID'        ,'varchar(150)' ),@gLTransactionXID)
