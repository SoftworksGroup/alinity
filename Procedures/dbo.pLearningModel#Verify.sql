...
GO
CREATE PROCEDURE [dbo].[pLearningModel#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.LearningModel
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.LearningModel for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.LearningModel table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fLearningModel#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pLearningModel#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.LearningModel'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.LearningModel'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.LearningModel'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.LearningModel'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.LearningModel'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_LearningModel'
		)
		begin
			exec sp_executesql N'alter table dbo.LearningModel drop constraint ck_LearningModel'					-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.LearningModel with CHECK add constraint ck_LearningModel
			check
				(
			...
			select @RowCount = count(1) from dbo.LearningModel									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'LearningModel'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fLearningModel#Check
							(
							 x.LearningModelSID
							,x.LearningModelSCD
			...
			alter table dbo.LearningModel with NOCHECK add constraint ck_LearningModel
			check
				(
				dbo.fLearningModel#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.LearningModel'
		and
			br.BusinessRuleStatus <> 'x'
