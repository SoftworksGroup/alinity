...
GO
CREATE PROCEDURE [sf].[pMailingPreference#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pMailingPreference#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.MailingPreference records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.MailingPreference table. When a new record is to be added from the UI, this procedure
...
columns of the vMailingPreference entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pMailingPreference
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pMailingPreference of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@mailingPreferenceSID   int               = -1												-- specific default required by EF - do not override
		,@mailingPreferenceLabel nvarchar(35)      = null											-- no default provided from DB constraint - OK to override
		,@usageNotes             nvarchar(max)     = null											-- no default provided from DB constraint - OK to override
		,@isAutoOptIn            bit               = (1)											-- default provided from DB constraint - OK to override
		...
		,@mailingPreferenceXID   varchar(150)      = null											-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @mailingPreferenceLabel  = isnull(context.node.value('@MailingPreferenceLabel','nvarchar(35)' ),@mailingPreferenceLabel)
			,@usageNotes              = isnull(context.node.value('@UsageNotes'            ,'nvarchar(max)'),@usageNotes)
			,@isAutoOptIn             = isnull(context.node.value('@IsAutoOptIn'           ,'bit'          ),@isAutoOptIn)
			,@isActive                = isnull(context.node.value('@IsActive'              ,'bit'          ),@isActive)
			,@mailingPreferenceXID    = isnull(context.node.value('@MailingPreferenceXID'  ,'varchar(150)' ),@mailingPreferenceXID)
