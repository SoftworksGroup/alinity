...
GO
CREATE PROCEDURE [sf].[pTask#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.Task
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.Task for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.Task table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fTask#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pTask#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.Task'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.Task'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.Task'															-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.Task'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.Task'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_Task'
		)
		begin
			exec sp_executesql N'alter table sf.Task drop constraint ck_Task'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.Task with CHECK add constraint ck_Task
			check
				(
				sf.fTask#Check
					(
			...
			select @RowCount = count(1) from sf.Task														-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.Task with NOCHECK add constraint ck_Task
			check
				(
				sf.fTask#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.Task'
		and
			br.BusinessRuleStatus <> 'x'
