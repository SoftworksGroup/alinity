...
GO
CREATE PROCEDURE [sf].[pApplicationPage#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ApplicationPage
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ApplicationPage for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ApplicationPage table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fApplicationPage#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pApplicationPage#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ApplicationPage'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ApplicationPage'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationPage'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationPage'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ApplicationPage'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ApplicationPage'
		)
		begin
			exec sp_executesql N'alter table sf.ApplicationPage drop constraint ck_ApplicationPage'				-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ApplicationPage with CHECK add constraint ck_ApplicationPage
			check
				(
				sf.fApplicationPage#Check
					(
					 ApplicationPageSID
			...
			select @RowCount = count(1) from sf.ApplicationPage									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ApplicationPage'
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fApplicationPage#Check
							(
							 x.ApplicationPageSID
			...
			alter table sf.ApplicationPage with NOCHECK add constraint ck_ApplicationPage
			check
				(
				sf.fApplicationPage#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ApplicationPage'
		and
			br.BusinessRuleStatus <> 'x'
