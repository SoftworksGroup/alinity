...
GO
CREATE PROCEDURE [sf].[pFileFormat#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pFileFormat#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.FileFormat records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.FileFormat table. When a new record is to be added from the UI, this procedure
...
columns of the vFileFormat entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pFileFormat
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pFileFormat of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@fileFormatSID      int               = -1														-- specific default required by EF - do not override
		,@fileFormatSCD      varchar(10)       = null													-- no default provided from DB constraint - OK to override
		,@fileFormatLabel    nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		...
		,@fileFormatXID      varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @fileFormatSCD    = isnull(context.node.value('@FileFormatSCD'  ,'varchar(10)' ),@fileFormatSCD)
			,@fileFormatLabel  = isnull(context.node.value('@FileFormatLabel','nvarchar(35)'),@fileFormatLabel)
			,@isDefault        = isnull(context.node.value('@IsDefault'      ,'bit'         ),@isDefault)
			,@fileFormatXID    = isnull(context.node.value('@FileFormatXID'  ,'varchar(150)'),@fileFormatXID)
