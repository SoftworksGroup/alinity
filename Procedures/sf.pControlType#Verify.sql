...
GO
CREATE PROCEDURE [sf].[pControlType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ControlType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ControlType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ControlType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fControlType#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pControlType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ControlType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ControlType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ControlType'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ControlType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ControlType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ControlType'
		)
		begin
			exec sp_executesql N'alter table sf.ControlType drop constraint ck_ControlType'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ControlType with CHECK add constraint ck_ControlType
			check
				(
				sf.fControlType#Check
					(
					 ControlTypeSID
					,ControlTypeSCD
					,ControlTypeLabel
			...
			select @RowCount = count(1) from sf.ControlType											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.ControlType with NOCHECK add constraint ck_ControlType
			check
				(
				sf.fControlType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ControlType'
		and
			br.BusinessRuleStatus <> 'x'
