...
GO
CREATE PROCEDURE [dbo].[pRegionMapping#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegionMapping
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegionMapping for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegionMapping table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegionMapping#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegionMapping#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegionMapping'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegionMapping'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegionMapping'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegionMapping'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegionMapping'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegionMapping'
		)
		begin
			exec sp_executesql N'alter table dbo.RegionMapping drop constraint ck_RegionMapping'					-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegionMapping with CHECK add constraint ck_RegionMapping
			check
				(
				dbo.fRegionMapping#Check
					(
					 RegionMappingSID
					,PostalCodeMask
					,RegionSID
			...
			select @RowCount = count(1) from dbo.RegionMapping									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegionMapping'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegionMapping#Check
							(
							 x.RegionMappingSID
							,x.PostalCodeMask
							,x.RegionSID
			...
			alter table dbo.RegionMapping with NOCHECK add constraint ck_RegionMapping
			check
				(
				dbo.fRegionMapping#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegionMapping'
		and
			br.BusinessRuleStatus <> 'x'
