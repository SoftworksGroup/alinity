...
GO
CREATE PROCEDURE [dbo].[pComplaintType#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pComplaintType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.ComplaintType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.ComplaintType table. When a new record is to be added from the UI, this procedure
...
columns of the vComplaintType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pComplaintType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pComplaintType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@complaintTypeSID      int               = -1												-- specific default required by EF - do not override
		,@complaintTypeLabel    nvarchar(35)      = null											-- no default provided from DB constraint - OK to override
		,@complaintTypeCategory nvarchar(65)      = null											-- no default provided from DB constraint - OK to override
		...
		,@complaintTypeXID      varchar(150)      = null											-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @complaintTypeLabel     = isnull(context.node.value('@ComplaintTypeLabel'   ,'nvarchar(35)' ),@complaintTypeLabel)
			,@complaintTypeCategory  = isnull(context.node.value('@ComplaintTypeCategory','nvarchar(65)' ),@complaintTypeCategory)
			...
			,@complaintTypeXID       = isnull(context.node.value('@ComplaintTypeXID'     ,'varchar(150)' ),@complaintTypeXID)
