...
GO
CREATE PROCEDURE [dbo].[pEmploymentStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.EmploymentStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.EmploymentStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.EmploymentStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fEmploymentStatus#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pEmploymentStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.EmploymentStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.EmploymentStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.EmploymentStatus'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.EmploymentStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.EmploymentStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_EmploymentStatus'
		)
		begin
			exec sp_executesql N'alter table dbo.EmploymentStatus drop constraint ck_EmploymentStatus'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.EmploymentStatus with CHECK add constraint ck_EmploymentStatus
			check
				(
				dbo.fEmploymentStatus#Check
					(
					 EmploymentStatusSID
			...
			select @RowCount = count(1) from dbo.EmploymentStatus								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'EmploymentStatus'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fEmploymentStatus#Check
							(
							 x.EmploymentStatusSID
							,x.EmploymentStatusName
							,x.EmploymentStatusCode
							,x.IsDefault
							,x.IsEmploymentExpected
							,x.IsActive
							,x.EmploymentStatusXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
			...
			alter table dbo.EmploymentStatus with NOCHECK add constraint ck_EmploymentStatus
			check
				(
				dbo.fEmploymentStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.EmploymentStatus'
		and
			br.BusinessRuleStatus <> 'x'
