...
GO
CREATE PROCEDURE [dbo].[pRegistrationSnapshotType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrationSnapshotType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrationSnapshotType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrationSnapshotType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrationSnapshotType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrationSnapshotType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrationSnapshotType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrationSnapshotType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationSnapshotType'					-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrationSnapshotType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrationSnapshotType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrationSnapshotType'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrationSnapshotType drop constraint ck_RegistrationSnapshotType'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrationSnapshotType with CHECK add constraint ck_RegistrationSnapshotType
			check
				(
				dbo.fRegistrationSnapshotType#Check
					(
			...
			select @RowCount = count(1) from dbo.RegistrationSnapshotType				-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrationSnapshotType'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrationSnapshotType#Check
							(
							 x.RegistrationSnapshotTypeSID
							,x.RegistrationSnapshotTypeLabel
							,x.RegistrationSnapshotTypeSCD
							,x.RegistrationSnapshotLabelTemplate
							,x.IsDefault
							,x.IsActive
							,x.RegistrationSnapshotTypeXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrationSnapshotType  x
					where
						x.RegistrationSnapshotTypeSID = @nextSID
			...
			alter table dbo.RegistrationSnapshotType with NOCHECK add constraint ck_RegistrationSnapshotType
			check
				(
				dbo.fRegistrationSnapshotType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrationSnapshotType'
		and
			br.BusinessRuleStatus <> 'x'
