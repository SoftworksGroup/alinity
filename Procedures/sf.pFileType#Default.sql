...
GO
CREATE PROCEDURE [sf].[pFileType#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pFileType#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.FileType records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.FileType table. When a new record is to be added from the UI, this procedure
...
columns of the vFileType entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pFileType
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pFileType of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@fileTypeSID        int               = -1														-- specific default required by EF - do not override
		,@fileTypeSCD        varchar(8)        = null													-- no default provided from DB constraint - OK to override
		,@fileTypeLabel      nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		,@mimeType           varchar(255)      = null													-- no default provided from DB constraint - OK to override
		,@isInline           bit               = (0)													-- default provided from DB constraint - OK to override
		...
		,@fileTypeXID        varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			...
			,@fileTypeLabel  = isnull(context.node.value('@FileTypeLabel','nvarchar(35)'),@fileTypeLabel)
			,@mimeType       = isnull(context.node.value('@MimeType'     ,'varchar(255)'),@mimeType)
			,@isInline       = isnull(context.node.value('@IsInline'     ,'bit'         ),@isInline)
			,@isActive       = isnull(context.node.value('@IsActive'     ,'bit'         ),@isActive)
			,@fileTypeXID    = isnull(context.node.value('@FileTypeXID'  ,'varchar(150)'),@fileTypeXID)
