...
GO
CREATE PROCEDURE [dbo].[pPersonDoc#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PersonDoc
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PersonDoc for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PersonDoc table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPersonDoc#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPersonDoc#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PersonDoc'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PersonDoc'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PersonDoc'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PersonDoc'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PersonDoc'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PersonDoc'
		)
		begin
			exec sp_executesql N'alter table dbo.PersonDoc drop constraint ck_PersonDoc'									-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PersonDoc with CHECK add constraint ck_PersonDoc
			check
				(
			...
			select @RowCount = count(1) from dbo.PersonDoc											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.PersonDoc with NOCHECK add constraint ck_PersonDoc
			check
				(
				dbo.fPersonDoc#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PersonDoc'
		and
			br.BusinessRuleStatus <> 'x'
