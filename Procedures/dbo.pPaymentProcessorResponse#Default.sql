...
GO
CREATE PROCEDURE [dbo].[pPaymentProcessorResponse#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPaymentProcessorResponse#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.PaymentProcessorResponse records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PaymentProcessorResponse table. When a new record is to be added from the UI, this procedure
...
columns of the vPaymentProcessorResponse entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pPaymentProcessorResponse
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pPaymentProcessorResponse of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@paymentProcessorResponseSID int               = -1									-- specific default required by EF - do not override
		,@paymentSID                  int               = null								-- no default provided from DB constraint - OK to override
		,@responseTime                datetime          = null								-- no default provided from DB constraint - OK to override
		,@responseSource              varchar(15)       = null								-- no default provided from DB constraint - OK to override
		,@responseDetails             xml               = CONVERT(xml,N'<Events />')										-- default provided from DB constraint - OK to override
		,@transactionID               varchar(50)       = null								-- no default provided from DB constraint - OK to override
		,@isPaid                      bit               = CONVERT(bit,(0))		-- default provided from DB constraint - OK to override
		,@userDefinedColumns          xml               = null								-- no default provided from DB constraint - OK to override
		,@paymentProcessorResponseXID varchar(150)      = null								-- no default provided from DB constraint - OK to override
		...
		,@paymentTransactionID        varchar(50)															-- not a base table column (default ignored)
		...
		,@isManual                    bit																			-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@responseTime                 = isnull(context.node.value('@ResponseTime'               ,'datetime'    ),@responseTime)
			,@responseSource               = isnull(context.node.value('@ResponseSource'             ,'varchar(15)' ),@responseSource)
			,@transactionID                = isnull(context.node.value('@TransactionID'              ,'varchar(50)' ),@transactionID)
			,@isPaid                       = isnull(context.node.value('@IsPaid'                     ,'bit'         ),@isPaid)
			,@paymentProcessorResponseXID  = isnull(context.node.value('@PaymentProcessorResponseXID','varchar(150)'),@paymentProcessorResponseXID)
