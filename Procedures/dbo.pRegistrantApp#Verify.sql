...
GO
CREATE PROCEDURE [dbo].[pRegistrantApp#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantApp
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantApp for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantApp table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantApp#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantApp#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantApp'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantApp'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantApp'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantApp'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantApp'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantApp'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantApp drop constraint ck_RegistrantApp'					-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantApp with CHECK add constraint ck_RegistrantApp
			check
				(
			...
			select @RowCount = count(1) from dbo.RegistrantApp									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantApp'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantApp#Check
							(
							 x.RegistrantAppSID
							,x.RegistrationSID
			...
			alter table dbo.RegistrantApp with NOCHECK add constraint ck_RegistrantApp
			check
				(
				dbo.fRegistrantApp#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantApp'
		and
			br.BusinessRuleStatus <> 'x'
