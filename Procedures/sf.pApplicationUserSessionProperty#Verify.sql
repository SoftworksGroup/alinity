...
GO
CREATE PROCEDURE [sf].[pApplicationUserSessionProperty#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ApplicationUserSessionProperty
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ApplicationUserSessionProperty for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ApplicationUserSessionProperty table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fApplicationUserSessionProperty#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pApplicationUserSessionProperty#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ApplicationUserSessionProperty'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ApplicationUserSessionProperty'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationUserSessionProperty'		-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationUserSessionProperty'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ApplicationUserSessionProperty'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ApplicationUserSessionProperty'
		)
		begin
			exec sp_executesql N'alter table sf.ApplicationUserSessionProperty drop constraint ck_ApplicationUserSessionProperty'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ApplicationUserSessionProperty with CHECK add constraint ck_ApplicationUserSessionProperty
			check
				(
				sf.fApplicationUserSessionProperty#Check
					(
			...
			select @RowCount = count(1) from sf.ApplicationUserSessionProperty	-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ApplicationUserSessionProperty'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.ApplicationUserSessionPropertySID
				,x.RowGUID
			from
				sf.ApplicationUserSessionProperty x
			order by
				x.ApplicationUserSessionPropertySID
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fApplicationUserSessionProperty#Check
							(
							 x.ApplicationUserSessionPropertySID
							,x.ApplicationUserSessionSID
							,x.PropertyName
							,x.ApplicationUserSessionPropertyXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						sf.ApplicationUserSessionProperty  x
					where
						x.ApplicationUserSessionPropertySID = @nextSID
			...
			alter table sf.ApplicationUserSessionProperty with NOCHECK add constraint ck_ApplicationUserSessionProperty
			check
				(
				sf.fApplicationUserSessionProperty#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ApplicationUserSessionProperty'
		and
			br.BusinessRuleStatus <> 'x'
