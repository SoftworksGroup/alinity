...
GO
CREATE PROCEDURE [dbo].[pRegistrantAppResponse#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantAppResponse
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantAppResponse for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantAppResponse table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantAppResponse#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantAppResponse#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantAppResponse'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantAppResponse'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAppResponse'						-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAppResponse'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantAppResponse'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantAppResponse'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantAppResponse drop constraint ck_RegistrantAppResponse'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantAppResponse with CHECK add constraint ck_RegistrantAppResponse
			check
				(
				dbo.fRegistrantAppResponse#Check
					(
					 RegistrantAppResponseSID
					,RegistrantAppSID
					,FormOwnerSID
					,RegistrantAppResponseXID
					,LegacyKey
					,IsDeleted
					,CreateUser
					,CreateTime
			...
			select @RowCount = count(1) from dbo.RegistrantAppResponse					-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantAppResponse'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.RegistrantAppResponseSID
				,x.RowGUID
			from
				dbo.RegistrantAppResponse x
			order by
				x.RegistrantAppResponseSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantAppResponse#Check
							(
							 x.RegistrantAppResponseSID
							,x.RegistrantAppSID
							,x.FormOwnerSID
							,x.RegistrantAppResponseXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantAppResponse  x
					where
						x.RegistrantAppResponseSID = @nextSID
			...
			alter table dbo.RegistrantAppResponse with NOCHECK add constraint ck_RegistrantAppResponse
			check
				(
				dbo.fRegistrantAppResponse#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantAppResponse'
		and
			br.BusinessRuleStatus <> 'x'
