...
GO
CREATE PROCEDURE [dbo].[pPracticeRegisterRestriction#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PracticeRegisterRestriction
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PracticeRegisterRestriction for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PracticeRegisterRestriction table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPracticeRegisterRestriction#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPracticeRegisterRestriction#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PracticeRegisterRestriction'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PracticeRegisterRestriction'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRegisterRestriction'			-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeRegisterRestriction'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PracticeRegisterRestriction'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PracticeRegisterRestriction'
		)
		begin
			exec sp_executesql N'alter table dbo.PracticeRegisterRestriction drop constraint ck_PracticeRegisterRestriction'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PracticeRegisterRestriction with CHECK add constraint ck_PracticeRegisterRestriction
			check
				(
			...
			select @RowCount = count(1) from dbo.PracticeRegisterRestriction		-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PracticeRegisterRestriction'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.PracticeRegisterRestrictionSID
				,x.RowGUID
			from
				dbo.PracticeRegisterRestriction x
			order by
				x.PracticeRegisterRestrictionSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPracticeRegisterRestriction#Check
							(
							 x.PracticeRegisterRestrictionSID
							...
							,x.EffectiveTime
							,x.ExpiryTime
							,x.PracticeRegisterRestrictionXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.PracticeRegisterRestriction  x
					where
						x.PracticeRegisterRestrictionSID = @nextSID
			...
			alter table dbo.PracticeRegisterRestriction with NOCHECK add constraint ck_PracticeRegisterRestriction
			check
				(
				dbo.fPracticeRegisterRestriction#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PracticeRegisterRestriction'
		and
			br.BusinessRuleStatus <> 'x'
