...
GO
CREATE PROCEDURE [dbo].[pPracticeScope#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PracticeScope
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PracticeScope for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PracticeScope table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPracticeScope#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPracticeScope#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PracticeScope'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PracticeScope'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeScope'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PracticeScope'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PracticeScope'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PracticeScope'
		)
		begin
			exec sp_executesql N'alter table dbo.PracticeScope drop constraint ck_PracticeScope'					-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PracticeScope with CHECK add constraint ck_PracticeScope
			check
				(
				dbo.fPracticeScope#Check
					(
					 PracticeScopeSID
					,PracticeScopeName
					,PracticeScopeCode
			...
			select @RowCount = count(1) from dbo.PracticeScope									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PracticeScope'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPracticeScope#Check
							(
							 x.PracticeScopeSID
							,x.PracticeScopeName
							,x.PracticeScopeCode
			...
			alter table dbo.PracticeScope with NOCHECK add constraint ck_PracticeScope
			check
				(
				dbo.fPracticeScope#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PracticeScope'
		and
			br.BusinessRuleStatus <> 'x'
