...
GO
CREATE PROCEDURE [dbo].[pComplaintProcessEvent#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ComplaintProcessEvent
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ComplaintProcessEvent for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ComplaintProcessEvent table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fComplaintProcessEvent#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pComplaintProcessEvent#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ComplaintProcessEvent'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ComplaintProcessEvent'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ComplaintProcessEvent'						-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ComplaintProcessEvent'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ComplaintProcessEvent'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ComplaintProcessEvent'
		)
		begin
			exec sp_executesql N'alter table dbo.ComplaintProcessEvent drop constraint ck_ComplaintProcessEvent'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ComplaintProcessEvent with CHECK add constraint ck_ComplaintProcessEvent
			check
				(
				dbo.fComplaintProcessEvent#Check
					(
			...
			select @RowCount = count(1) from dbo.ComplaintProcessEvent					-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ComplaintProcessEvent'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fComplaintProcessEvent#Check
							(
							 x.ComplaintProcessEventSID
							,x.ComplaintProcessSID
							,x.ComplaintEventTypeSID
							,x.Description
							,x.Sequence
							,x.TargetDurationDays
							,x.ComplaintProcessEventXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.ComplaintProcessEvent  x
					where
						x.ComplaintProcessEventSID = @nextSID
			...
			alter table dbo.ComplaintProcessEvent with NOCHECK add constraint ck_ComplaintProcessEvent
			check
				(
				dbo.fComplaintProcessEvent#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ComplaintProcessEvent'
		and
			br.BusinessRuleStatus <> 'x'
