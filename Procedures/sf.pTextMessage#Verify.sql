...
GO
CREATE PROCEDURE [sf].[pTextMessage#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.TextMessage
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.TextMessage for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.TextMessage table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fTextMessage#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pTextMessage#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.TextMessage'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.TextMessage'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.TextMessage'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.TextMessage'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.TextMessage'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_TextMessage'
		)
		begin
			exec sp_executesql N'alter table sf.TextMessage drop constraint ck_TextMessage'								-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.TextMessage with CHECK add constraint ck_TextMessage
			check
				(
			...
			select @RowCount = count(1) from sf.TextMessage											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.TextMessage with NOCHECK add constraint ck_TextMessage
			check
				(
				sf.fTextMessage#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.TextMessage'
		and
			br.BusinessRuleStatus <> 'x'
