...
GO
CREATE PROCEDURE [dbo].[pAgeRange#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pAgeRange#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.AgeRange records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.AgeRange table. When a new record is to be added from the UI, this procedure
...
columns of the vAgeRange entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pAgeRange
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pAgeRange of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@ageRangeSID           int               = -1												-- specific default required by EF - do not override
		,@ageRangeTypeSID       int               = null											-- no default provided from DB constraint - OK to override
		,@ageRangeLabel         nvarchar(35)      = null											-- no default provided from DB constraint - OK to override
		,@startAge              smallint          = null											-- no default provided from DB constraint - OK to override
		,@endAge                smallint          = null											-- no default provided from DB constraint - OK to override
		...
		,@ageRangeXID           varchar(150)      = null											-- no default provided from DB constraint - OK to override
		...
		,@ageRangeTypeLabel     nvarchar(35)      = N'?'											-- not a base table column (default ignored)
		,@ageRangeTypeCode      varchar(15)       = '?'												-- not a base table column (default ignored)
		,@ageRangeTypeIsDefault bit               = 0													-- not a base table column (default ignored)
		,@ageRangeTypeRowGUID   uniqueidentifier  = newid()										-- not a base table column (default ignored)
		...
		,@displayOrder          bigint																				-- not a base table column (default ignored)
	...
	begin try
		...
		select
			 @ageRangeTypeSID  = isnull(context.node.value('@AgeRangeTypeSID','int'         ),@ageRangeTypeSID)
			,@ageRangeLabel    = isnull(context.node.value('@AgeRangeLabel'  ,'nvarchar(35)'),@ageRangeLabel)
			,@startAge         = isnull(context.node.value('@StartAge'       ,'smallint'    ),@startAge)
			,@endAge           = isnull(context.node.value('@EndAge'         ,'smallint'    ),@endAge)
			,@isDefault        = isnull(context.node.value('@IsDefault'      ,'bit'         ),@isDefault)
			,@ageRangeXID      = isnull(context.node.value('@AgeRangeXID'    ,'varchar(150)'),@ageRangeXID)
		...
		if isnull(@ageRangeTypeSID,0) = 0 select @ageRangeTypeSID = x.AgeRangeTypeSID from dbo.AgeRangeType x where x.IsDefault = @ON
