...
GO
CREATE PROCEDURE [sf].[pApplicationUser#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.ApplicationUser
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.ApplicationUser for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.ApplicationUser table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fApplicationUser#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pApplicationUser#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.ApplicationUser'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.ApplicationUser'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationUser'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.ApplicationUser'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.ApplicationUser'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_ApplicationUser'
		)
		begin
			exec sp_executesql N'alter table sf.ApplicationUser drop constraint ck_ApplicationUser'				-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.ApplicationUser with CHECK add constraint ck_ApplicationUser
			check
				(
			...
			select @RowCount = count(1) from sf.ApplicationUser									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ApplicationUser'
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fApplicationUser#Check
							(
							 x.ApplicationUserSID
							,x.PersonSID
							,x.CultureSID
							...
							,x.LastReviewTime
							,x.LastReviewUser
							,x.IsPotentialDuplicate
							,x.IsTemplate
							,x.GlassBreakPassword
							,x.LastGlassBreakPasswordChangeTime
							,x.IsActive
							,x.AuthenticationSystemID
							,x.ApplicationUserXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
			...
			alter table sf.ApplicationUser with NOCHECK add constraint ck_ApplicationUser
			check
				(
				sf.fApplicationUser#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.ApplicationUser'
		and
			br.BusinessRuleStatus <> 'x'
