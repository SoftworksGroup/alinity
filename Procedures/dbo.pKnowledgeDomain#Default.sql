...
GO
CREATE PROCEDURE [dbo].[pKnowledgeDomain#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pKnowledgeDomain#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.KnowledgeDomain records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.KnowledgeDomain table. When a new record is to be added from the UI, this procedure
...
columns of the vKnowledgeDomain entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pKnowledgeDomain
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pKnowledgeDomain of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@knowledgeDomainSID   int               = -1													-- specific default required by EF - do not override
		,@knowledgeDomainLabel nvarchar(35)      = null												-- no default provided from DB constraint - OK to override
		...
		,@knowledgeDomainXID   varchar(150)      = null												-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @knowledgeDomainLabel  = isnull(context.node.value('@KnowledgeDomainLabel','nvarchar(35)'),@knowledgeDomainLabel)
			,@isDefault             = isnull(context.node.value('@IsDefault'           ,'bit'         ),@isDefault)
			,@isActive              = isnull(context.node.value('@IsActive'            ,'bit'         ),@isActive)
			,@knowledgeDomainXID    = isnull(context.node.value('@KnowledgeDomainXID'  ,'varchar(150)'),@knowledgeDomainXID)
