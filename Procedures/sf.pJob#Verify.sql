...
GO
CREATE PROCEDURE [sf].[pJob#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.Job
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.Job for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.Job table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fJob#Check(), on each row. If an error is
...
select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.Job'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.Job'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.Job'																-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.Job'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.Job'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_Job'
		)
		begin
			exec sp_executesql N'alter table sf.Job drop constraint ck_Job'			-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.Job with CHECK add constraint ck_Job
			check
				(
				sf.fJob#Check
					(
			...
			select @RowCount = count(1) from sf.Job															-- rules validated - calculate row count to return
		...
		begin
				...
				begin try
					...
					select
						@isValid = sf.fJob#Check
							...
							,x.IsCancelEnabled
							,x.IsParallelEnabled
							,x.IsFullTraceEnabled
							,x.IsAlertOnSuccessEnabled
							,x.JobScheduleSID
							,x.JobScheduleSequence
							,x.IsRunAfterPredecessorsOnly
							,x.MaxErrorRate
							,x.MaxRetriesOnFailure
							,x.IsActive
							,x.JobXID
							,x.LegacyKey
							,x.IsDeleted
			...
			alter table sf.Job with NOCHECK add constraint ck_Job
			check
				(
				sf.fJob#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.Job'
		and
			br.BusinessRuleStatus <> 'x'
