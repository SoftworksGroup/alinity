...
GO
CREATE PROCEDURE [sf].[pEmailMessage#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.EmailMessage
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.EmailMessage for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.EmailMessage table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fEmailMessage#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pEmailMessage#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.EmailMessage'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.EmailMessage'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.EmailMessage'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.EmailMessage'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.EmailMessage'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_EmailMessage'
		)
		begin
			exec sp_executesql N'alter table sf.EmailMessage drop constraint ck_EmailMessage'							-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.EmailMessage with CHECK add constraint ck_EmailMessage
			check
				(
			...
			select @RowCount = count(1) from sf.EmailMessage										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fEmailMessage#Check
							(
							 x.EmailMessageSID
			...
			alter table sf.EmailMessage with NOCHECK add constraint ck_EmailMessage
			check
				(
				sf.fEmailMessage#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.EmailMessage'
		and
			br.BusinessRuleStatus <> 'x'
