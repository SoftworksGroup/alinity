...
GO
CREATE PROCEDURE [dbo].[pOrgOtherName#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pOrgOtherName#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.OrgOtherName records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.OrgOtherName table. When a new record is to be added from the UI, this procedure
...
columns of the vOrgOtherName entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pOrgOtherName
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pOrgOtherName of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@orgOtherNameSID                int               = -1								-- specific default required by EF - do not override
		...
		,@orgOtherNameXID                varchar(150)      = null							-- no default provided from DB constraint - OK to override
		...
		,@effectiveDate                  date																	-- not a base table column (default ignored)
	...
	begin try
		...
		select
			...
			,@orgOtherNameXID  = isnull(context.node.value('@OrgOtherNameXID','varchar(150)' ),@orgOtherNameXID)
