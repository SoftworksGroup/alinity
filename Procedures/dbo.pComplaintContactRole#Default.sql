...
GO
CREATE PROCEDURE [dbo].[pComplaintContactRole#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pComplaintContactRole#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.ComplaintContactRole records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.ComplaintContactRole table. When a new record is to be added from the UI, this procedure
...
columns of the vComplaintContactRole entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pComplaintContactRole
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pComplaintContactRole of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@complaintContactRoleSID  int               = -1											-- specific default required by EF - do not override
		,@complaintContactRoleSCD  varchar(20)       = null										-- no default provided from DB constraint - OK to override
		,@complaintContactRoleName nvarchar(50)      = null										-- no default provided from DB constraint - OK to override
		...
		,@complaintContactRoleXID  varchar(150)      = null										-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @complaintContactRoleSCD   = isnull(context.node.value('@ComplaintContactRoleSCD' ,'varchar(20)' ),@complaintContactRoleSCD)
			,@complaintContactRoleName  = isnull(context.node.value('@ComplaintContactRoleName','nvarchar(50)'),@complaintContactRoleName)
			,@isDefault                 = isnull(context.node.value('@IsDefault'               ,'bit'         ),@isDefault)
			,@complaintContactRoleXID   = isnull(context.node.value('@ComplaintContactRoleXID' ,'varchar(150)'),@complaintContactRoleXID)
