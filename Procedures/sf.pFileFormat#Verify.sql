...
GO
CREATE PROCEDURE [sf].[pFileFormat#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.FileFormat
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.FileFormat for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.FileFormat table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fFileFormat#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pFileFormat#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.FileFormat'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.FileFormat'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.FileFormat'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.FileFormat'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.FileFormat'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_FileFormat'
		)
		begin
			exec sp_executesql N'alter table sf.FileFormat drop constraint ck_FileFormat'									-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.FileFormat with CHECK add constraint ck_FileFormat
			check
				(
				sf.fFileFormat#Check
					(
					 FileFormatSID
					,FileFormatSCD
					,FileFormatLabel
			...
			select @RowCount = count(1) from sf.FileFormat											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.FileFormat with NOCHECK add constraint ck_FileFormat
			check
				(
				sf.fFileFormat#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.FileFormat'
		and
			br.BusinessRuleStatus <> 'x'
