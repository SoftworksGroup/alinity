...
GO
CREATE PROCEDURE [dbo].[pComplaintContact#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ComplaintContact
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ComplaintContact for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ComplaintContact table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fComplaintContact#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pComplaintContact#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ComplaintContact'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ComplaintContact'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ComplaintContact'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ComplaintContact'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ComplaintContact'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ComplaintContact'
		)
		begin
			exec sp_executesql N'alter table dbo.ComplaintContact drop constraint ck_ComplaintContact'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ComplaintContact with CHECK add constraint ck_ComplaintContact
			check
				(
				dbo.fComplaintContact#Check
					(
					 ComplaintContactSID
					,ComplaintSID
					,PersonSID
			...
			select @RowCount = count(1) from dbo.ComplaintContact								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ComplaintContact'

			insert
				@work																															-- load a work table with pk values to process
			select
				 x.ComplaintContactSID
				,x.RowGUID
			from
				dbo.ComplaintContact x
			order by
				x.ComplaintContactSID
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fComplaintContact#Check
							(
							 x.ComplaintContactSID
							,x.ComplaintSID
							,x.PersonSID
			...
			alter table dbo.ComplaintContact with NOCHECK add constraint ck_ComplaintContact
			check
				(
				dbo.fComplaintContact#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ComplaintContact'
		and
			br.BusinessRuleStatus <> 'x'
