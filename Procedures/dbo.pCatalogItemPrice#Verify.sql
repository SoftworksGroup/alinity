...
GO
CREATE PROCEDURE [dbo].[pCatalogItemPrice#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.CatalogItemPrice
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.CatalogItemPrice for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.CatalogItemPrice table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fCatalogItemPrice#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pCatalogItemPrice#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.CatalogItemPrice'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.CatalogItemPrice'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.CatalogItemPrice'									-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.CatalogItemPrice'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.CatalogItemPrice'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_CatalogItemPrice'
		)
		begin
			exec sp_executesql N'alter table dbo.CatalogItemPrice drop constraint ck_CatalogItemPrice'		-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.CatalogItemPrice with CHECK add constraint ck_CatalogItemPrice
			check
				(
				dbo.fCatalogItemPrice#Check
					(
					 CatalogItemPriceSID
					,CatalogItemSID
					,Price
			...
			select @RowCount = count(1) from dbo.CatalogItemPrice								-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'CatalogItemPrice'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fCatalogItemPrice#Check
							(
							 x.CatalogItemPriceSID
							,x.CatalogItemSID
							,x.Price
							,x.EffectiveTime
							,x.CatalogItemPriceXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
			...
			alter table dbo.CatalogItemPrice with NOCHECK add constraint ck_CatalogItemPrice
			check
				(
				dbo.fCatalogItemPrice#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.CatalogItemPrice'
		and
			br.BusinessRuleStatus <> 'x'
