...
GO
CREATE PROCEDURE [dbo].[pRecommendationGroup#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pRecommendationGroup#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.RecommendationGroup records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.RecommendationGroup table. When a new record is to be added from the UI, this procedure
...
columns of the vRecommendationGroup entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pRecommendationGroup
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pRecommendationGroup of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@recommendationGroupSID   int               = -1											-- specific default required by EF - do not override
		,@recommendationGroupSCD   varchar(15)       = null										-- no default provided from DB constraint - OK to override
		,@recommendationGroupLabel nvarchar(35)      = null										-- no default provided from DB constraint - OK to override
		,@userDefinedColumns       xml               = null										-- no default provided from DB constraint - OK to override
		,@recommendationGroupXID   varchar(150)      = null										-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @recommendationGroupSCD    = isnull(context.node.value('@RecommendationGroupSCD'  ,'varchar(15)' ),@recommendationGroupSCD)
			,@recommendationGroupLabel  = isnull(context.node.value('@RecommendationGroupLabel','nvarchar(35)'),@recommendationGroupLabel)
			,@recommendationGroupXID    = isnull(context.node.value('@RecommendationGroupXID'  ,'varchar(150)'),@recommendationGroupXID)
