...
GO
CREATE PROCEDURE [dbo].[pAuditType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.AuditType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.AuditType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.AuditType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fAuditType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pAuditType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.AuditType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.AuditType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.AuditType'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.AuditType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.AuditType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_AuditType'
		)
		begin
			exec sp_executesql N'alter table dbo.AuditType drop constraint ck_AuditType'									-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.AuditType with CHECK add constraint ck_AuditType
			check
				(
				dbo.fAuditType#Check
					(
					 AuditTypeSID
					,AuditTypeLabel
					,AuditTypeCategory
			...
			select @RowCount = count(1) from dbo.AuditType											-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.AuditType with NOCHECK add constraint ck_AuditType
			check
				(
				dbo.fAuditType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.AuditType'
		and
			br.BusinessRuleStatus <> 'x'
