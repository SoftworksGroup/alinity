...
GO
CREATE PROCEDURE [dbo].[pPaymentStatus#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pPaymentStatus#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.PaymentStatus records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.PaymentStatus table. When a new record is to be added from the UI, this procedure
...
columns of the vPaymentStatus entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pPaymentStatus
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pPaymentStatus of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@paymentStatusSID      int               = -1												-- specific default required by EF - do not override
		,@paymentStatusSCD      varchar(25)       = null											-- no default provided from DB constraint - OK to override
		,@paymentStatusLabel    nvarchar(35)      = null											-- no default provided from DB constraint - OK to override
		...
		,@paymentStatusSequence int               = (0)												-- default provided from DB constraint - OK to override
		,@userDefinedColumns    xml               = null											-- no default provided from DB constraint - OK to override
		,@paymentStatusXID      varchar(150)      = null											-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @paymentStatusSCD       = isnull(context.node.value('@PaymentStatusSCD'     ,'varchar(25)'  ),@paymentStatusSCD)
			,@paymentStatusLabel     = isnull(context.node.value('@PaymentStatusLabel'   ,'nvarchar(35)' ),@paymentStatusLabel)
			...
			,@paymentStatusSequence  = isnull(context.node.value('@PaymentStatusSequence','int'          ),@paymentStatusSequence)
			,@paymentStatusXID       = isnull(context.node.value('@PaymentStatusXID'     ,'varchar(150)' ),@paymentStatusXID)
