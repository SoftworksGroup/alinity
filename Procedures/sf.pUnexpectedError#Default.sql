...
GO
CREATE PROCEDURE [sf].[pUnexpectedError#Default]
...
as
/*********************************************************************************************************************************
Procedure : sf.pUnexpectedError#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" sf.UnexpectedError records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the sf.UnexpectedError table. When a new record is to be added from the UI, this procedure
...
columns of the vUnexpectedError entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pUnexpectedError
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pUnexpectedError of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@unexpectedErrorSID int               = -1														-- specific default required by EF - do not override
		,@messageSCD         varchar(128)      = null													-- no default provided from DB constraint - OK to override
		,@procName           nvarchar(128)     = null													-- no default provided from DB constraint - OK to override
		,@lineNumber         int               = null													-- no default provided from DB constraint - OK to override
		,@errorNumber        int               = null													-- no default provided from DB constraint - OK to override
		,@messageText        nvarchar(4000)    = null													-- no default provided from DB constraint - OK to override
		,@errorSeverity      int               = null													-- no default provided from DB constraint - OK to override
		,@errorState         int               = null													-- no default provided from DB constraint - OK to override
		,@sPIDNo             int               = null													-- no default provided from DB constraint - OK to override
		,@machineName        nvarchar(128)     = null													-- no default provided from DB constraint - OK to override
		,@dBUser             nvarchar(75)      = null													-- no default provided from DB constraint - OK to override
		,@callEvent          varchar(255)      = null													-- no default provided from DB constraint - OK to override
		,@callParameter      int               = null													-- no default provided from DB constraint - OK to override
		,@callSyntax         varchar(4000)     = null													-- no default provided from DB constraint - OK to override
		,@userDefinedColumns xml               = null													-- no default provided from DB constraint - OK to override
		,@unexpectedErrorXID varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @messageSCD          = isnull(context.node.value('@MessageSCD'        ,'varchar(128)'  ),@messageSCD)
			,@procName            = isnull(context.node.value('@ProcName'          ,'nvarchar(128)' ),@procName)
			,@lineNumber          = isnull(context.node.value('@LineNumber'        ,'int'           ),@lineNumber)
			,@errorNumber         = isnull(context.node.value('@ErrorNumber'       ,'int'           ),@errorNumber)
			,@messageText         = isnull(context.node.value('@MessageText'       ,'nvarchar(4000)'),@messageText)
			,@errorSeverity       = isnull(context.node.value('@ErrorSeverity'     ,'int'           ),@errorSeverity)
			,@errorState          = isnull(context.node.value('@ErrorState'        ,'int'           ),@errorState)
			,@sPIDNo              = isnull(context.node.value('@SPIDNo'            ,'int'           ),@sPIDNo)
			,@machineName         = isnull(context.node.value('@MachineName'       ,'nvarchar(128)' ),@machineName)
			,@dBUser              = isnull(context.node.value('@DBUser'            ,'nvarchar(75)'  ),@dBUser)
			,@callEvent           = isnull(context.node.value('@CallEvent'         ,'varchar(255)'  ),@callEvent)
			,@callParameter       = isnull(context.node.value('@CallParameter'     ,'int'           ),@callParameter)
			,@callSyntax          = isnull(context.node.value('@CallSyntax'        ,'varchar(4000)' ),@callSyntax)
			,@unexpectedErrorXID  = isnull(context.node.value('@UnexpectedErrorXID','varchar(150)'  ),@unexpectedErrorXID)
