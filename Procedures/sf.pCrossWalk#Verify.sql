...
GO
CREATE PROCEDURE [sf].[pCrossWalk#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.CrossWalk
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.CrossWalk for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.CrossWalk table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fCrossWalk#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pCrossWalk#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.CrossWalk'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.CrossWalk'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.CrossWalk'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.CrossWalk'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.CrossWalk'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_CrossWalk'
		)
		begin
			exec sp_executesql N'alter table sf.CrossWalk drop constraint ck_CrossWalk'										-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.CrossWalk with CHECK add constraint ck_CrossWalk
			check
				(
				sf.fCrossWalk#Check
					(
			...
			select @RowCount = count(1) from sf.CrossWalk												-- rules validated - calculate row count to return
		...
		begin
			...
			alter table sf.CrossWalk with NOCHECK add constraint ck_CrossWalk
			check
				(
				sf.fCrossWalk#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.CrossWalk'
		and
			br.BusinessRuleStatus <> 'x'
