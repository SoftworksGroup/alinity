...
GO
CREATE PROCEDURE [dbo].[pPAPTransaction#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PAPTransaction
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PAPTransaction for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PAPTransaction table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPAPTransaction#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPAPTransaction#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PAPTransaction'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PAPTransaction'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PAPTransaction'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PAPTransaction'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PAPTransaction'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PAPTransaction'
		)
		begin
			exec sp_executesql N'alter table dbo.PAPTransaction drop constraint ck_PAPTransaction'				-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PAPTransaction with CHECK add constraint ck_PAPTransaction
			check
				(
				dbo.fPAPTransaction#Check
					(
					 PAPTransactionSID
					,PAPBatchSID
					,PAPSubscriptionSID
			...
			select @RowCount = count(1) from dbo.PAPTransaction									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PAPTransaction'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPAPTransaction#Check
							(
							 x.PAPTransactionSID
							,x.PAPBatchSID
			...
			alter table dbo.PAPTransaction with NOCHECK add constraint ck_PAPTransaction
			check
				(
				dbo.fPAPTransaction#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PAPTransaction'
		and
			br.BusinessRuleStatus <> 'x'
