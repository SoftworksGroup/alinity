...
GO
CREATE PROCEDURE [dbo].[pComplaintSeverity#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ComplaintSeverity
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ComplaintSeverity for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ComplaintSeverity table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fComplaintSeverity#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pComplaintSeverity#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ComplaintSeverity'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ComplaintSeverity'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ComplaintSeverity'								-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ComplaintSeverity'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ComplaintSeverity'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ComplaintSeverity'
		)
		begin
			exec sp_executesql N'alter table dbo.ComplaintSeverity drop constraint ck_ComplaintSeverity'	-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ComplaintSeverity with CHECK add constraint ck_ComplaintSeverity
			check
				(
				dbo.fComplaintSeverity#Check
					(
			...
			select @RowCount = count(1) from dbo.ComplaintSeverity							-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'ComplaintSeverity'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fComplaintSeverity#Check
							(
							 x.ComplaintSeveritySID
							,x.ComplaintSeverityLabel
							,x.ComplaintSeverityCategory
							,x.IsDefault
							,x.IsActive
							,x.ComplaintSeverityXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.ComplaintSeverity  x
					where
						x.ComplaintSeveritySID = @nextSID
			...
			alter table dbo.ComplaintSeverity with NOCHECK add constraint ck_ComplaintSeverity
			check
				(
				dbo.fComplaintSeverity#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ComplaintSeverity'
		and
			br.BusinessRuleStatus <> 'x'
