...
GO
CREATE PROCEDURE [dbo].[pExamQuestion#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.ExamQuestion
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.ExamQuestion for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.ExamQuestion table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fExamQuestion#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pExamQuestion#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.ExamQuestion'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.ExamQuestion'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.ExamQuestion'											-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.ExamQuestion'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.ExamQuestion'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_ExamQuestion'
		)
		begin
			exec sp_executesql N'alter table dbo.ExamQuestion drop constraint ck_ExamQuestion'						-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.ExamQuestion with CHECK add constraint ck_ExamQuestion
			check
				(
				dbo.fExamQuestion#Check
					(
					 ExamQuestionSID
					,ExamSectionSID
					,Sequence
			...
			select @RowCount = count(1) from dbo.ExamQuestion										-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fExamQuestion#Check
							(
							 x.ExamQuestionSID
							,x.ExamSectionSID
							,x.Sequence
			...
			alter table dbo.ExamQuestion with NOCHECK add constraint ck_ExamQuestion
			check
				(
				dbo.fExamQuestion#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.ExamQuestion'
		and
			br.BusinessRuleStatus <> 'x'
