...
GO
CREATE PROCEDURE [dbo].[pReinstatement#Delete]
	 @ReinstatementSID                       int               = null -- required! id of row to delete - must be set in custom logic if not passed
	...
	,@ReinstatementLabel                     nvarchar(80)      = null
	,@IsRegisterChange                       bit               = null
	,@HasOpenAudit                           bit               = null
	,@IsReinstatementOpen                    bit               = null
	,@NewFormStatusSCD                       varchar(25)       = null
	,@ReasonSIDOnApprove                     int               = null
as
/*********************************************************************************************************************************
Procedure : dbo.pReinstatement#Delete
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : deletes 1 row in the dbo.Reinstatement table
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.Reinstatement table. The procedure requires a primary key value to locate the record
to delete.
...
and in the ext.pReinstatement procedure for client-specific deletion rules.
...
begin
	...
	begin try
		...
		if @RowStamp is null select @RowStamp = x.RowStamp from dbo.Reinstatement x where x.ReinstatementSID = @ReinstatementSID
		...
		update																																-- set audit details on dbo.ReinstatementResponse rows that will delete through CASCADE
		...
		update																																-- set audit details on dbo.ReinstatementStatus rows that will delete through CASCADE
