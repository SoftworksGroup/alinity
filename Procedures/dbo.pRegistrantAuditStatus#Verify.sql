...
GO
CREATE PROCEDURE [dbo].[pRegistrantAuditStatus#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.RegistrantAuditStatus
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.RegistrantAuditStatus for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.RegistrantAuditStatus table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fRegistrantAuditStatus#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pRegistrantAuditStatus#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.RegistrantAuditStatus'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.RegistrantAuditStatus'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAuditStatus'						-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.RegistrantAuditStatus'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.RegistrantAuditStatus'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_RegistrantAuditStatus'
		)
		begin
			exec sp_executesql N'alter table dbo.RegistrantAuditStatus drop constraint ck_RegistrantAuditStatus'-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.RegistrantAuditStatus with CHECK add constraint ck_RegistrantAuditStatus
			check
				(
				dbo.fRegistrantAuditStatus#Check
					(
			...
			select @RowCount = count(1) from dbo.RegistrantAuditStatus					-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'RegistrantAuditStatus'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fRegistrantAuditStatus#Check
							(
							 x.RegistrantAuditStatusSID
							,x.RegistrantAuditSID
							,x.FormStatusSID
							,x.RegistrantAuditStatusXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
							,x.CreateTime
					...
					from
						dbo.RegistrantAuditStatus  x
					where
						x.RegistrantAuditStatusSID = @nextSID
			...
			alter table dbo.RegistrantAuditStatus with NOCHECK add constraint ck_RegistrantAuditStatus
			check
				(
				dbo.fRegistrantAuditStatus#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.RegistrantAuditStatus'
		and
			br.BusinessRuleStatus <> 'x'
