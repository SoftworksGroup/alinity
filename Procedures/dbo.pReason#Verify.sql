...
GO
CREATE PROCEDURE [dbo].[pReason#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.Reason
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.Reason for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.Reason table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fReason#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pReason#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.Reason'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.Reason'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.Reason'														-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.Reason'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.Reason'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_Reason'
		)
		begin
			exec sp_executesql N'alter table dbo.Reason drop constraint ck_Reason'												-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.Reason with CHECK add constraint ck_Reason
			check
				(
				dbo.fReason#Check
					(
			...
			select @RowCount = count(1) from dbo.Reason													-- rules validated - calculate row count to return
		...
		begin
			...
			alter table dbo.Reason with NOCHECK add constraint ck_Reason
			check
				(
				dbo.fReason#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.Reason'
		and
			br.BusinessRuleStatus <> 'x'
