...
GO
CREATE PROCEDURE [dbo].[pGLAccount#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.GLAccount
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.GLAccount for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.GLAccount table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fGLAccount#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pGLAccount#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.GLAccount'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.GLAccount'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.GLAccount'												-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.GLAccount'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.GLAccount'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_GLAccount'
		)
		begin
			exec sp_executesql N'alter table dbo.GLAccount drop constraint ck_GLAccount'									-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.GLAccount with CHECK add constraint ck_GLAccount
			check
				(
			...
			select @RowCount = count(1) from dbo.GLAccount											-- rules validated - calculate row count to return
		...
		begin
				...
				begin try
					...
					select
						@isValid = dbo.fGLAccount#Check
							...
							,x.IsTaxAccount
							,x.IsPAPAccount
							,x.IsUnappliedPaymentAccount
							,x.DeferredGLAccountCode
							,x.IsActive
							,x.GLAccountXID
							,x.LegacyKey
							,x.IsDeleted
			...
			alter table dbo.GLAccount with NOCHECK add constraint ck_GLAccount
			check
				(
				dbo.fGLAccount#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.GLAccount'
		and
			br.BusinessRuleStatus <> 'x'
