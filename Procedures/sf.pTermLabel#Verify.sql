...
GO
CREATE PROCEDURE [sf].[pTermLabel#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for sf.TermLabel
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks sf.TermLabel for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the sf.TermLabel table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, sf.fTermLabel#Check(), on each row. If an error is
...
Example:
	...
	exec sf.pTermLabel#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'sf.TermLabel'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'sf.TermLabel'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'sf.TermLabel'													-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'sf.TermLabel'
			)
		...
		where
			brx.ApplicationEntitySCD = 'sf.TermLabel'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'sf'
		and
			cc.ConstraintName = 'ck_TermLabel'
		)
		begin
			exec sp_executesql N'alter table sf.TermLabel drop constraint ck_TermLabel'										-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table sf.TermLabel with CHECK add constraint ck_TermLabel
			check
				(
				sf.fTermLabel#Check
					(
			...
			select @RowCount = count(1) from sf.TermLabel												-- rules validated - calculate row count to return
		...
		begin
			...
			begin
				...
				begin try
					...
					select
						@isValid = sf.fTermLabel#Check
							(
							 x.TermLabelSID
							,x.TermLabelSCD
							,x.TermLabel
							,x.TermLabelUpdateTime
							,x.DefaultLabel
							,x.DefaultLabelUpdateTime
							,x.TermLabelXID
							,x.LegacyKey
							,x.IsDeleted
							,x.CreateUser
			...
			alter table sf.TermLabel with NOCHECK add constraint ck_TermLabel
			check
				(
				sf.fTermLabel#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'sf.TermLabel'
		and
			br.BusinessRuleStatus <> 'x'
