...
GO
CREATE PROCEDURE [dbo].[pPersonNoteType#Verify]
...
as
/*********************************************************************************************************************************
Sproc    : Business Rule Verification for dbo.PersonNoteType
Notice   : Copyright © 2012 Softworks Group Inc.
Summary  : checks dbo.PersonNoteType for current business rules and logs errors encountered into sf.BusinessRuleError
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used to verify that data in the dbo.PersonNoteType table meets the business rules currently in effect for it.
This is a batch operation that calls the verification function, dbo.fPersonNoteType#Check(), on each row. If an error is
...
Example:
	...
	exec dbo.pPersonNoteType#Verify
		@ReturnSelect = 1

select * from sf.vBusinessRule      where ApplicationEntitySCD = 'dbo.PersonNoteType'
select * from sf.vBusinessRuleError where ApplicationEntitySCD = 'dbo.PersonNoteType'
...
begin
	...
	begin try
		...
		in
			...
			where
				bre.ApplicationEntitySCD = 'dbo.PersonNoteType'										-- the application entity SCD is the schema + table name
			)
		...
		in
			...
			from
				sf.vBusinessRuleError bre
			where
				bre.ApplicationEntitySCD = 'dbo.PersonNoteType'
			)
		...
		where
			brx.ApplicationEntitySCD = 'dbo.PersonNoteType'
		and
			brx.IsMandatory = 1
		and
		...
		where
			cc.SchemaName = 'dbo'
		and
			cc.ConstraintName = 'ck_PersonNoteType'
		)
		begin
			exec sp_executesql N'alter table dbo.PersonNoteType drop constraint ck_PersonNoteType'				-- drop via dynamic SQL to avoid reference errors in VS project
		end
		...
		begin try
			...
			alter table dbo.PersonNoteType with CHECK add constraint ck_PersonNoteType
			check
				(
				dbo.fPersonNoteType#Check
					(
					 PersonNoteTypeSID
					,PersonNoteTypeLabel
			...
			select @RowCount = count(1) from dbo.PersonNoteType									-- rules validated - calculate row count to return
		...
		begin
			...
			and
				ae.BaseTableName       = 'PersonNoteType'
			...
			begin
				...
				begin try
					...
					select
						@isValid = dbo.fPersonNoteType#Check
							(
							 x.PersonNoteTypeSID
							,x.PersonNoteTypeLabel
							,x.PersonNoteTypeCategory
							,x.IsDefault
							,x.IsActive
							,x.PersonNoteTypeXID
							,x.LegacyKey
			...
			alter table dbo.PersonNoteType with NOCHECK add constraint ck_PersonNoteType
			check
				(
				dbo.fPersonNoteType#Check
					(
		...
		from
			sf.vBusinessRule br
		where
			br.ApplicationEntitySCD = 'dbo.PersonNoteType'
		and
			br.BusinessRuleStatus <> 'x'
