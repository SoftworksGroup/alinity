...
GO
CREATE PROCEDURE [dbo].[pReasonGroup#Default]
...
as
/*********************************************************************************************************************************
Procedure : dbo.pReasonGroup#Default
Notice    : Copyright © 2019 Softworks Group Inc.
Summary   : provides a blank row with default values for presentation in the UI for "new" dbo.ReasonGroup records
-----------------------------------------------------------------------------------------------------------------------------------
...
Comments
--------
This procedure is used in maintenance of the dbo.ReasonGroup table. When a new record is to be added from the UI, this procedure
...
columns of the vReasonGroup entity view, however, only some values (as noted above) are eligible for default setting.  The other
...
product and applies to all client configurations. Client-specific customizations must be implemented in the ext.pReasonGroup
procedure. The extended procedure is only called where it exists in database. The parameter "@Mode" is set to "default.pre" to
advise ext.pReasonGroup of the context of the call. All other parameters are also passed, however, only those parameters eligible
...
begin
	...
	declare
		...
		,@reasonGroupSID     int               = -1														-- specific default required by EF - do not override
		,@reasonGroupSCD     varchar(20)       = null													-- no default provided from DB constraint - OK to override
		,@reasonGroupLabel   nvarchar(35)      = null													-- no default provided from DB constraint - OK to override
		,@isLockedGroup      bit               = CONVERT(bit,(0))							-- default provided from DB constraint - OK to override
		,@userDefinedColumns xml               = null													-- no default provided from DB constraint - OK to override
		,@reasonGroupXID     varchar(150)      = null													-- no default provided from DB constraint - OK to override
	...
	begin try
		...
		select
			 @reasonGroupSCD    = isnull(context.node.value('@ReasonGroupSCD'  ,'varchar(20)' ),@reasonGroupSCD)
			,@reasonGroupLabel  = isnull(context.node.value('@ReasonGroupLabel','nvarchar(35)'),@reasonGroupLabel)
			,@isLockedGroup     = isnull(context.node.value('@IsLockedGroup'   ,'bit'         ),@isLockedGroup)
			,@reasonGroupXID    = isnull(context.node.value('@ReasonGroupXID'  ,'varchar(150)'),@reasonGroupXID)
