...
Comments
--------
The dbo.PersonDoc table includes both the FileTypeSID foreign key value and the FileTypeSCD. The code value is required to support 
...
so the FileTypeSID column is included; although redundant.
...
sprocs (dbo.pPersonDoc#Insert and dbo.pPersonDoc#Update) set the SID value correctly so the trigger should
...
begin
	...
	begin try
		...
		from
			inserted			i
		join
			sf.FileType	dt				on i.FileTypeSCD = dt.FileTypeSCD						-- TODO: Tim Edlund | Apr 2015 - table is moving to SF!
		join
			dbo.PersonDoc	doc		on i.PersonDocSID = doc.PersonDocSID
		where
